import type {
	...
	ConvertedMetadata,
	DivAnnotationExpression,
	DivByAnnotationExpression,
...
/**
 * Returns either the value of the object or the object itself if it is not a primitive object.
 */
...
export type ComparisonOperator = "===" | "!==" | ">=" | ">" | "<=" | "<" | "Has" | "In";
...
export type ArithmeticOperator = "+" | "-" | "*" | "/" | "%" | "Neg" | "Floor";
export type ArithmeticExpression = BaseExpression<number> & {
	_type: "Arithmetic";
...
function isActionParameter(serviceObject: unknown): serviceObject is ActionParameter {
	return (serviceObject as ActionParameter)?._type === "ActionParameter";
}
...
export function _checkExpressionsAreEqual<T>(a: BindingToolkitExpression<T>, b: BindingToolkitExpression<T>): boolean {
	...
	switch (a._type) {
		...
		case "Arithmetic":
			return (
				a.operator === (b as ArithmeticExpression).operator &&
				_checkExpressionsAreEqual(a.operand1, (b as ArithmeticExpression).operand1) &&
				_checkExpressionsAreEqual(a.operand2, (b as ArithmeticExpression).operand2)
			);
		case "Not":
...
export function add<T extends PrimitiveType>(
	leftOperand: ExpressionOrPrimitive<T>,
	rightOperand: ExpressionOrPrimitive<T>
): BindingToolkitExpression<number> {
	return arithmetic("+", leftOperand, rightOperand);
}

export function subtract<T extends PrimitiveType>(
	leftOperand: ExpressionOrPrimitive<T>,
	rightOperand: ExpressionOrPrimitive<T>
): BindingToolkitExpression<number> {
	return arithmetic("-", leftOperand, rightOperand);
...
export function multiply<T extends PrimitiveType>(
	leftOperand: ExpressionOrPrimitive<T>,
	rightOperand: ExpressionOrPrimitive<T>
): BindingToolkitExpression<number> {
	return arithmetic("*", leftOperand, rightOperand);
...
export function divide<T extends PrimitiveType>(
	leftOperand: ExpressionOrPrimitive<T>,
	rightOperand: ExpressionOrPrimitive<T>
): BindingToolkitExpression<number> {
	return arithmetic("/", leftOperand, rightOperand);
...
export function modulo<T extends PrimitiveType>(
	leftOperand: ExpressionOrPrimitive<T>,
	rightOperand: ExpressionOrPrimitive<T>
): BindingToolkitExpression<number> {
	return arithmetic("%", leftOperand, rightOperand);
...
export function negate<T extends PrimitiveType>(operand: ExpressionOrPrimitive<T>): BindingToolkitExpression<number> {
	return arithmetic("Neg", operand, null);
}

export function floor<T extends PrimitiveType>(operand: ExpressionOrPrimitive<T>): BindingToolkitExpression<number> {
	return arithmetic("Floor", operand, null);
}
 ...
 * @param leftOperand
 * @param rightOperand
 * @returns Expression evaluating to number
 */
export function arithmetic<T extends PrimitiveType>(
	operator: ArithmeticOperator,
	leftOperand: ExpressionOrPrimitive<T>,
	rightOperand: ExpressionOrPrimitive<T>
): BindingToolkitExpression<number> {
	...
	if (isConstant(leftExpression) && isConstant(rightExpression)) {
		switch (operator) {
			case "+":
				if (leftExpression.value === null || leftExpression.value === undefined) {
					return constant(rightExpression.value as number);
				} else if (rightExpression.value === null || rightExpression.value === undefined) {
					return constant(leftExpression.value as number);
				}
				const sum = (leftExpression.value as number) + (rightExpression.value as number);
				return constant(sum);
			case "-":
				if (leftExpression.value === null || leftExpression.value === undefined) {
					return constant(rightExpression.value as number);
				...
				const diff = (leftExpression.value as number) - (rightExpression.value as number);
			...
			case "/":
				...
				const div = (leftExpression.value as number) / (rightExpression.value as number);
				return constant(div);
			case "Neg":
				if (leftExpression.value === null || leftExpression.value === undefined) {
					return constant(0);
				}
				return constant(-leftExpression.value);
			case "%":
				if (
					leftExpression.value === null ||
					leftExpression.value === undefined ||
				...
				const mod = (leftExpression.value as number) % (rightExpression.value as number);
				return constant(mod);
			case "Floor":
				if (leftExpression.value === null || leftExpression.value === undefined) {
					return constant(0);
				}
				return constant(Math.floor(leftExpression.value as number));
			case "*":
				if (
					leftExpression.value === null ||
					leftExpression.value === undefined ||
				...
				const mult = (leftExpression.value as number) * (rightExpression.value as number);
...
let CONVERTER_ROOT_SYMBOL: symbol | undefined;
export function setConverterRootSymbol(converterRootSymbol: symbol): void {
	CONVERTER_ROOT_SYMBOL = converterRootSymbol;
}
...
export function getExpressionFromAnnotation<T extends PrimitiveType>(
	...
	let converterRoot: ConvertedMetadata | undefined;
	if (CONVERTER_ROOT_SYMBOL) {
		converterRoot = (annotationValue?.[CONVERTER_ROOT_SYMBOL as never] as unknown as Function)?.() as ConvertedMetadata | undefined;
	}
	...
	switch (annotationValue.type) {
		...
		case "Not":
			return not(
				parseAnnotationCondition(annotationValue.$Not, visitedNavigationPaths, pathVisitor, converterRoot)
			) as BindingToolkitExpression<PrimitiveTypeCast<T>>;
		case "Eq":
			return equal(
				parseAnnotationCondition(annotationValue.$Eq[0], visitedNavigationPaths, pathVisitor, converterRoot),
				parseAnnotationCondition(annotationValue.$Eq[1], visitedNavigationPaths, pathVisitor, converterRoot)
			) as BindingToolkitExpression<PrimitiveTypeCast<T>>;
		case "Ne":
			return notEqual(
				parseAnnotationCondition(annotationValue.$Ne[0], visitedNavigationPaths, pathVisitor, converterRoot),
				parseAnnotationCondition(annotationValue.$Ne[1], visitedNavigationPaths, pathVisitor, converterRoot)
			) as BindingToolkitExpression<PrimitiveTypeCast<T>>;
		case "Gt":
			return greaterThan(
				parseAnnotationCondition(annotationValue.$Gt[0], visitedNavigationPaths, pathVisitor, converterRoot),
				parseAnnotationCondition(annotationValue.$Gt[1], visitedNavigationPaths, pathVisitor, converterRoot)
			) as BindingToolkitExpression<PrimitiveTypeCast<T>>;
		case "Ge":
			return greaterOrEqual(
				parseAnnotationCondition(annotationValue.$Ge[0], visitedNavigationPaths, pathVisitor, converterRoot),
				parseAnnotationCondition(annotationValue.$Ge[1], visitedNavigationPaths, pathVisitor, converterRoot)
			) as BindingToolkitExpression<PrimitiveTypeCast<T>>;
		case "Lt":
			return lessThan(
				parseAnnotationCondition(annotationValue.$Lt[0], visitedNavigationPaths, pathVisitor, converterRoot),
				parseAnnotationCondition(annotationValue.$Lt[1], visitedNavigationPaths, pathVisitor, converterRoot)
			) as BindingToolkitExpression<PrimitiveTypeCast<T>>;
		case "Le":
			return lessOrEqual(
				parseAnnotationCondition(annotationValue.$Le[0], visitedNavigationPaths, pathVisitor, converterRoot),
				parseAnnotationCondition(annotationValue.$Le[1], visitedNavigationPaths, pathVisitor, converterRoot)
			) as BindingToolkitExpression<PrimitiveTypeCast<T>>;
		case "Or":
			return or(
				...annotationValue.$Or.map(function (orCondition) {
					return parseAnnotationCondition<boolean>(orCondition, visitedNavigationPaths, pathVisitor, converterRoot);
				})
			) as BindingToolkitExpression<PrimitiveTypeCast<T>>;
		case "And":
			return and(
				...annotationValue.$And.map(function (andCondition) {
					return parseAnnotationCondition<boolean>(andCondition, visitedNavigationPaths, pathVisitor, converterRoot);
				})
			) as BindingToolkitExpression<PrimitiveTypeCast<T>>;
		case "In":
			return comparison(
				"In",
				parseAnnotationCondition(annotationValue.$In[0], visitedNavigationPaths, pathVisitor, converterRoot),
				parseAnnotationCondition(annotationValue.$In[1], visitedNavigationPaths, pathVisitor, converterRoot)
			) as BindingToolkitExpression<PrimitiveTypeCast<T>>;
		case "Has":
			return comparison(
				"Has",
				parseAnnotationCondition(annotationValue.$Has[0], visitedNavigationPaths, pathVisitor, converterRoot),
				parseAnnotationCondition(annotationValue.$Has[1], visitedNavigationPaths, pathVisitor, converterRoot)
			) as BindingToolkitExpression<PrimitiveTypeCast<T>>;
		case "Add":
			return add(
				parseAnnotationCondition(annotationValue.$Add[0], visitedNavigationPaths, pathVisitor, converterRoot),
				parseAnnotationCondition(annotationValue.$Add[1], visitedNavigationPaths, pathVisitor, converterRoot)
			) as BindingToolkitExpression<PrimitiveTypeCast<T>>;
		case "Sub":
			return subtract(
				parseAnnotationCondition(annotationValue.$Sub[0], visitedNavigationPaths, pathVisitor, converterRoot),
				parseAnnotationCondition(annotationValue.$Sub[1], visitedNavigationPaths, pathVisitor, converterRoot)
			) as BindingToolkitExpression<PrimitiveTypeCast<T>>;
		case "Mul":
			return multiply(
				parseAnnotationCondition(annotationValue.$Mul[0], visitedNavigationPaths, pathVisitor, converterRoot),
				parseAnnotationCondition(annotationValue.$Mul[1], visitedNavigationPaths, pathVisitor, converterRoot)
			) as BindingToolkitExpression<PrimitiveTypeCast<T>>;
		case "Div":
			return floor(
				divide(
					parseAnnotationCondition(annotationValue.$Div[0], visitedNavigationPaths, pathVisitor, converterRoot),
					parseAnnotationCondition(annotationValue.$Div[1], visitedNavigationPaths, pathVisitor, converterRoot)
				)
			) as BindingToolkitExpression<PrimitiveTypeCast<T>>;
		case "DivBy":
			return divide(
				parseAnnotationCondition(annotationValue.$DivBy[0], visitedNavigationPaths, pathVisitor, converterRoot),
				parseAnnotationCondition(annotationValue.$DivBy[1], visitedNavigationPaths, pathVisitor, converterRoot)
			) as BindingToolkitExpression<PrimitiveTypeCast<T>>;
		case "Mod":
			return modulo(
				parseAnnotationCondition(annotationValue.$Mod[0], visitedNavigationPaths, pathVisitor, converterRoot),
				parseAnnotationCondition(annotationValue.$Mod[1], visitedNavigationPaths, pathVisitor, converterRoot)
			) as BindingToolkitExpression<PrimitiveTypeCast<T>>;
		case "Neg":
			return negate(
				parseAnnotationCondition(annotationValue.$Neg[0], visitedNavigationPaths, pathVisitor, converterRoot)
			) as BindingToolkitExpression<PrimitiveTypeCast<T>>;
		case "Apply":
 ...
 * @param visitedNavigationPaths The path from the root entity set
 * @param pathVisitor A function to modify the resulting path
 * @param converterRoot The root converted metadata
...
function parseAnnotationCondition<T extends PrimitiveType>(
	...
	visitedNavigationPaths: string[] = [],
	pathVisitor?: Function,
	converterRoot?: ConvertedMetadata
	...
	if (annotationValue === null || typeof annotationValue !== "object") {
		...
		return or(
			...((annotationValue as OrConditionalExpression).$Or.map(function (orCondition) {
				return parseAnnotationCondition(orCondition, visitedNavigationPaths, pathVisitor, converterRoot);
		...
		return and(
			...((annotationValue as AndConditionalExpression).$And.map(function (andCondition) {
				return parseAnnotationCondition(andCondition, visitedNavigationPaths, pathVisitor, converterRoot);
			}) as unknown as BindingToolkitExpression<boolean>[])
		) as BindingToolkitExpression<T>;
	} else if (annotationValue.hasOwnProperty("$Not")) {
		return not(
			parseAnnotationCondition((annotationValue as NotConditionalExpression).$Not, visitedNavigationPaths, pathVisitor, converterRoot)
		) as BindingToolkitExpression<T>;
	} else if (annotationValue.hasOwnProperty("$Eq")) {
		return equal(
			parseAnnotationCondition(
				(annotationValue as EqConditionalExpression).$Eq[0],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			),
			parseAnnotationCondition(
				(annotationValue as EqConditionalExpression).$Eq[1],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			)
		...
		return notEqual(
			parseAnnotationCondition(
				(annotationValue as NeConditionalExpression).$Ne[0],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			),
			parseAnnotationCondition(
				(annotationValue as NeConditionalExpression).$Ne[1],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			)
		...
		return greaterThan(
			parseAnnotationCondition(
				(annotationValue as GtConditionalExpression).$Gt[0],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			),
			parseAnnotationCondition(
				(annotationValue as GtConditionalExpression).$Gt[1],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			)
		...
		return greaterOrEqual(
			parseAnnotationCondition(
				(annotationValue as GeConditionalExpression).$Ge[0],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			),
			parseAnnotationCondition(
				(annotationValue as GeConditionalExpression).$Ge[1],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			)
		...
		return lessThan(
			parseAnnotationCondition(
				(annotationValue as LtConditionalExpression).$Lt[0],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			),
			parseAnnotationCondition(
				(annotationValue as LtConditionalExpression).$Lt[1],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			)
		...
		return lessOrEqual(
			parseAnnotationCondition(
				(annotationValue as LeConditionalExpression).$Le[0],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			),
			parseAnnotationCondition(
				(annotationValue as LeConditionalExpression).$Le[1],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			)
		...
		return constant(resolveEnumValue((annotationValue as { $EnumMember?: string }).$EnumMember, converterRoot) as T);
	} else if (annotationValue.hasOwnProperty("$In")) {
		return comparison(
			"In",
			parseAnnotationCondition(
				(annotationValue as InConditionalExpression).$In[0],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			),
			parseAnnotationCondition(
				(annotationValue as InConditionalExpression).$In[1],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			)
		) as BindingToolkitExpression<T>;
	} else if (annotationValue.hasOwnProperty("$Has")) {
		return comparison(
			"Has",
			parseAnnotationCondition(
				(annotationValue as HasConditionalExpression).$Has[0],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			),
			parseAnnotationCondition(
				(annotationValue as HasConditionalExpression).$Has[1],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			)
		) as BindingToolkitExpression<T>;
	} else if (annotationValue.hasOwnProperty("$Add")) {
		return add(
			parseAnnotationCondition(
				(annotationValue as AddConditionalExpression).$Add[0],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			),
			parseAnnotationCondition(
				(annotationValue as AddConditionalExpression).$Add[1],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			)
		) as BindingToolkitExpression<T>;
	} else if (annotationValue.hasOwnProperty("$Sub")) {
		return subtract(
			parseAnnotationCondition(
				(annotationValue as SubConditionalExpression).$Sub[0],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			),
			parseAnnotationCondition(
				(annotationValue as SubConditionalExpression).$Sub[1],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			)
		) as BindingToolkitExpression<T>;
	} else if (annotationValue.hasOwnProperty("$Mul")) {
		return multiply(
			parseAnnotationCondition(
				(annotationValue as MulConditionalExpression).$Mul[0],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			),
			parseAnnotationCondition(
				(annotationValue as MulConditionalExpression).$Mul[1],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			)
		) as BindingToolkitExpression<T>;
	} else if (annotationValue.hasOwnProperty("$Div")) {
		return floor(
			divide(
				parseAnnotationCondition(
					(annotationValue as DivConditionalExpression).$Div[0],
					visitedNavigationPaths,
					pathVisitor,
					converterRoot
				),
				parseAnnotationCondition(
					(annotationValue as DivConditionalExpression).$Div[1],
					visitedNavigationPaths,
					pathVisitor,
					converterRoot
				)
	...
	} else if (annotationValue.hasOwnProperty("$DivBy")) {
		return divide(
			parseAnnotationCondition(
				(annotationValue as DivByConditionalExpression).$DivBy[0],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			),
			parseAnnotationCondition(
				(annotationValue as DivByConditionalExpression).$DivBy[1],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			)
		) as BindingToolkitExpression<T>;
	} else if (annotationValue.hasOwnProperty("$Mod")) {
		return modulo(
			parseAnnotationCondition(
				(annotationValue as ModConditionalExpression).$Mod[0],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			),
			parseAnnotationCondition(
				(annotationValue as ModConditionalExpression).$Mod[1],
				visitedNavigationPaths,
				pathVisitor,
				converterRoot
			)
		) as BindingToolkitExpression<T>;
	} else if (annotationValue.hasOwnProperty("$Neg")) {
		return negate(
			parseAnnotationCondition((annotationValue as NegConditionalExpression).$Neg, visitedNavigationPaths, pathVisitor, converterRoot)
		) as BindingToolkitExpression<T>;
	}
	return constant(annotationValue as T);
}
...
export function annotationIfExpression<T extends PrimitiveType>(
	...
	return ifElse(
		parseAnnotationCondition(annotationValue[0], visitedNavigationPaths, pathVisitor, converterRoot),
		parseAnnotationCondition(annotationValue[1] as ConditionalCheckOrValue, visitedNavigationPaths, pathVisitor, converterRoot),
		parseAnnotationCondition(annotationValue[2] as ConditionalCheckOrValue, visitedNavigationPaths, pathVisitor, converterRoot)
	);
}
	...
	if (isConstant(leftExpression) && isConstant(rightExpression)) {
		switch (operator) {
			...
			case "Has": {
				...
				const leftValueInt = leftExpression.value as number;
				const rightValueInt = rightExpression.value as number;
				return constant(!!(leftValueInt & rightValueInt));
			}
			case "In": {
				if (
					leftExpression.value === null ||
				...
				// Check all the children of the right expression are constants
				const allChildrenConstants = (rightExpression.value as T[]).every((item) => isConstant(item));
				if (allChildrenConstants) {
					const rightValues = (rightExpression.value as ConstantExpression<T>[]).map((item) => item.value);
					return constant(rightValues.includes(leftExpression.value));
				} else {
					return {
						_type: "Comparison",
						operator: operator,
...
/**
 * Comparison: "Include" (In).
 * @template T The target type
 * @param leftOperand The operand on the left side
 * @param rightOperand The operand on the right side of the comparison
 * @returns An expression representing the comparison
 */
export function includes<T extends DefinedPrimitiveType>(
	leftOperand: ExpressionOrPrimitive<T>,
	rightOperand: ExpressionOrPrimitive<T[]>
): BindingToolkitExpression<boolean> {
	return comparison("In", leftOperand, rightOperand as ExpressionOrPrimitive<T>);
}
...
export function formatWithTypeInformation<T>(
	...
	if (outExpression.type === "sap.ui.model.odata.type.String" && (isProperty(property) || isActionParameter(property))) {
...
export function addTypeInformation<T, U extends Fn<T>>(
	...
	if (type === "sap.ui.model.odata.type.Unit") {
	...
	} else if (type === "sap.ui.model.odata.type.Currency" || type === "sap.fe.core.type.DynamicCurrency") {
		...
		if (oConstraintOptions?.skipDecimalsValidation === true) {
			// This doesn't do anything because it gets overwritten by the `decimals` from the currency code list
			oFormatOptions.maxFractionDigits = 99; // setting to 99 to avoid rounding in currency conversion
		}
 ...
 */
function _getComplexTypeFormatOptionsFromFirstParam(
	param: PathInModelExpression<PrimitiveType>,
...
/**
 * Shortcut function to determine if a binding can be statically resolvable.
 * @param exp The expression to evaluate
 * @returns A Boolean evaluating if there is no relative path dependent on the current context
 */
export function isExpressionStaticallyResolvable<T extends PrimitiveType>(exp: BindingToolkitExpression<T>): boolean {
	const referencedPaths: string[] = [];
	transformRecursively(
		exp,
		"PathInModel",
		(pathExp) => {
			if (pathExp.modelName === undefined && typeof pathExp.path === "string") {
				referencedPaths.push(pathExp.path);
			}
			return pathExp;
		},
		true
	...
	return referencedPaths.every((p) => p.startsWith("/"));
}

export function getStaticallyResolvableExpression<T extends PrimitiveType>(
	exp: BindingToolkitExpression<T>,
	defaultValue: BindingToolkitExpression<T>
): BindingToolkitExpression<T> {
	if (exp._type === "Set") {
		const newOperands = exp.operands.map((operand: BindingToolkitExpression<boolean>) => {
			if (isExpressionStaticallyResolvable(operand) || isConstant(operand)) {
				return operand;
			} else {
				return defaultValue;
			}
		...
		if (exp.operator === "||") {
			return or(...(newOperands as unknown as BindingToolkitExpression<boolean>[])) as unknown as BindingToolkitExpression<T>;
		} else {
			return and(...(newOperands as unknown as BindingToolkitExpression<boolean>[])) as unknown as BindingToolkitExpression<T>;
		}
	}
	return isExpressionStaticallyResolvable(exp) ? exp : defaultValue;
}

/**
 * Function used to concatenate multiple expressions.
 * @param inExpressions The expressions to concatenate
 * @returns The concatenated expression
 */
...
/**
 * Concatenate multiple expressions with a separator.
 * @param separator The separator to insert between expressions
 * @param inExpressions Expressions to concatenate
 * @returns The concatenated expression
 */
export function concatWithSeparator(
	separator: string,
	...inExpressions: ExpressionOrPrimitive<string>[]
): BindingToolkitExpression<string> {
	const expressions = inExpressions.map(wrapPrimitive);
	if (hasUnresolvableExpression(...expressions)) {
		return unresolvableExpression;
	...
	const separatorConstant = constant(separator);

	if (expressions.every(isConstant)) {
		return constant(
			expressions.reduce((concatenated: string, value, index) => {
				const constantValue = value;
				if (constantValue.value !== undefined && constantValue.value !== null) {
					const prefix = index > 0 ? separator : "";
					return concatenated + prefix + constantValue.value.toString();
				}
				return concatenated;
			}, "")
		);
	...
	// Interleave separator constants between expressions
	const expressionsWithSeparators: BindingToolkitExpression<string>[] = [];
	expressions.forEach((expr, index) => {
		if (index > 0) {
			expressionsWithSeparators.push(separatorConstant);
		}
		expressionsWithSeparators.push(expr);
	});

	if (expressionsWithSeparators.some(isComplexTypeExpression)) {
		let pathIdx = 0;
		const myConcatExpression = formatResult(expressionsWithSeparators, "._formatters.StandardFormatter#concat", undefined, true);
		const allParts = [];
		transformRecursively(myConcatExpression, "PathInModel", (constantPath: PathInModelExpression<unknown>) => {
			allParts.push(constantPath);
			return pathInModel(`$${pathIdx++}`, "$");
	...
	return {
		_type: "Concat",
		expressions: expressionsWithSeparators
	};
}
...
export function transformRecursively<T extends PrimitiveType | unknown, ET extends ExpressionType>(
	...
	switch (expression._type) {
		...
		case "Arithmetic":
			...
			expression = arithmetic(expression.operator, operand1, operand2) as BindingToolkitExpression<T>;
			break;
		case "Constant":
...
/**
 * Compiled Binding Toolkit Expression
 * This is a type representing the compiled form of a Binding Toolkit expression as a binding string.
 */
...
export function compileExpression<T extends PrimitiveType>(
	...
	switch (expr._type) {
		...
		case "Arithmetic":
			const arithmeticExpression = compileArithmeticExpression(expr);
			return wrapBindingExpression(arithmeticExpression, embeddedInBinding);
...
function compileArithmeticExpression(expression: ArithmeticExpression): string {
	...
	switch (expression.operator) {
		case "Neg":
			return `-(${compileOperand(expression.operand1)})`;
		case "Floor":
			return `Math.floor(${compileOperand(expression.operand1)})`;
		case "%":
		case "/":
		case "*":
		case "+":
...
function compileComparisonExpression(expression: ComparisonExpression): string {
	...
	switch (expression.operator) {
		case "In": {
			const path = compileOperand(expression.operand1);
			const collection = compileOperand(expression.operand2) ?? [];
			return `${collection}.includes(${path})`;
		}
		case "Has": {
			const path = compileOperand(expression.operand1);
			const flags = compileOperand(expression.operand2) ?? [];
			return `(${path} & ${flags}) > 0`;
		}
		case "===":
		case "!==":
