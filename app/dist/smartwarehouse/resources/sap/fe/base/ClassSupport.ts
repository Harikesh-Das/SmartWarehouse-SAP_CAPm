...
type EventNamesOf<T> = {
	[K in keyof T]: T[K] extends (event: infer E) => void ? (E extends UI5Event ? K : never) : never;
}[keyof T];
export type EventsOf<T> = Partial<Pick<T, EventNamesOf<T>>>;
...
/**
 * Type for a generic state object
 */
export type GenericState = Record<string, never>;
...
export function defineUI5Class(target: string, metadataDefinition?: Partial<UI5ControlMetadataDefinition>): ClassDecorator {
	return function (constructor: UI5ControlAndMore) {
		...
		return registerUI5Metadata(constructor, target, constructor.prototype);
	} as ClassDecorator;
}
...
/**
 * The name identifier used to reference the component state model in the state management system.
 * This constant defines the key under which component state data is stored and retrieved.
 */
export const STATE_MODEL_NAME = "$componentState";
...
function createStateProxy(
	...
	targetControl.setModel(new JSONModel(initialData), STATE_MODEL_NAME);
	const stateModel = targetControl.getModel(STATE_MODEL_NAME) as JSONModel;
	...
	if (obj.metadata?.stateProperty || targetControl[boundProperties]) {
		...
		if (obj.metadata?.stateProperty) {
			...
			targetControl.applySettings = function (
				...
				if (mSettings?.models?.$componentState) {
					// We cannot remove the model directly from mSettings.models as this object is shared with the original object when cloning
					mSettings.models = { ...mSettings.models };
		...
		if (targetControl[boundProperties].length > 0) {
			targetControl.setProperty = function (sPropertyName: string, oValue: unknown, bSuppressInvalidate): typeof targetControl {
				if (targetControl[boundProperties].includes(sPropertyName) && !this._ignoreStatePropertyUpdate) {
					...
					(this.getModel(STATE_MODEL_NAME) as JSONModel).setProperty(`/${sPropertyName}`, oValue);
