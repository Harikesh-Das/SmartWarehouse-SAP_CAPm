...
export default class ShortcutExplanationProvider extends ManagedObject {
	...
	// Keep reference to the created user action so we can clean it up on iframe reload to avoid duplicate
	static _userAction: Item | undefined;

	static _cleanupAttached = false;
	...
	static async getInstance(): Promise<ShortcutExplanationProvider | undefined> {
		...
		try {
			if (shellContainer && !this.instance) {
				...
				const appLifeCycle = (await shellContainer.getServiceAsync("AppLifeCycle")) as AppLifeCycle | undefined;
	...
	static async registerUserAction(shellExtensionService: Extension): Promise<ShortcutExplanationProvider | undefined> {
		try {
			...
			// Destroy previous user action before creating a new one to prevent duplicates in keep-alive navigation
			if (this._userAction) {
			...
			// Store reference for cleanup when the iframe is reloaded/unloaded
