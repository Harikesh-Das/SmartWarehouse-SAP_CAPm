...
function convertDateToString(value: Date, tokenType: TokenType, isDateTimeOffset: boolean): string {
	...
	switch (tokenType) {
		...
		case "Calendar":
			formattedDateTime = DateFormat.getDateInstance({ UTC: true }).format(value);
			break;
		case "Time":
			formattedDateTime = DateFormat.getTimeInstance({ UTC: true }).format(value);
			break;
		default:
			break;
	}
 ...
 */
export function resolvePropertyName(path: string): string {
