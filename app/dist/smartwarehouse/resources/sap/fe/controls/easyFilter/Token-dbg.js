...
sap.ui.define(["sap/base/Log", "sap/fe/base/ClassSupport", "sap/fe/base/jsx-runtime/jsx", "sap/m/Token", "sap/m/delegate/ValueStateMessage", "sap/ui/core/Lib", "sap/ui/core/library", "sap/ui/model/FilterOperator", "./innerControls/CalendarFactory", "./innerControls/MenuWithCheckBoxFactory", "./innerControls/MenuWithSingleSelectFactory", "./innerControls/TimeFactory", "./innerControls/ValueHelpPreviewFactory", "./utils", "sap/fe/base/jsx-runtime/jsxs", "sap/fe/base/jsx-runtime/jsx"], function (Log, ClassSupport, jsx, Token, ValueStateMessage, Lib, library, FilterOperator, CalenderFactory, MenuWithCheckBoxFactory, MenuWithSingleSelectFactory, TimeFactory, ValueHelpPreviewFactory, EasyFilterUtils, _jsxs, _jsx) {
  ...
  let EasyFilterToken = (_dec = defineUI5Class("sap.fe.controls.easyFilter.Token"), _dec2 = property({
    ...
    _proto.getAriaText = function getAriaText() {
      let ariaText = this.label ?? "";
      const isEmpty = this.value === this.resourceBundle.getText("M_EASY_FILTER_MANDATORY_TOKEN_SELECT_VALUE");
      if (!isEmpty) {
        ariaText += ` ${this.value ?? ""}`;
      }
      ariaText += ` ${this.resourceBundle.getText("M_EASY_FILTER_TOKEN_NAME")}`;
      if (this.mandatory ?? false) {
        ariaText += ",";
        ariaText += isEmpty ? this.resourceBundle.getText("M_EASY_FILTER_MANDATORY_TOKEN_ERROR").slice(0, -1) : this.resourceBundle.getText("M_EASY_FILTER_TOKEN_MANDATORY");
      }
      ariaText += `,${this.resourceBundle.getText("M_EASY_FILTER_TOKEN_OPEN_POPOVER")}`;
      if (this.valueState === ValueState.Error) {
        ariaText += `${this.resourceBundle.getText("M_EASY_FILTER_TOKEN_ERROR")}`;
      }
      return ariaText;
    };
    EasyFilterToken.render = function render(rm, control) {
      return jsx.renderUsingRenderManager(rm, control, () => {
        ...
        const classesForValue = ["sapMTokenText"];
        ...
        if (control.items?.length !== 0) {
          classesForValue.push("sapFeControlsTokenPropertyValues");
        }
        return _jsxs("div", {
          ref: control,
          class: classes.join(" "),
          ...
          role: "option",
          "aria-label": control.getAriaText(),
          children: [_jsxs("span", {
