...
export default class CollaborationDraft extends BuildingBlock {
		...
		return (
			<>
				<CollaborationDraft.GridTableColumnControl width={CommonHelper.isDesktop() ? "10%" : "3em"}>
					{{
						label: <Text text={this.getTranslatedText("C_COLLABORATIONDRAFT_INVITATION_TABLE_USER_AVATAR_COLUMN")} />,
						template: (
				...
				<CollaborationDraft.GridTableColumnControl width={CommonHelper.isDesktop() ? "8%" : "3em"}>
					{{
						label: <Text text={this.getTranslatedText("C_COLLABORATIONDRAFT_INVITATION_TABLE_REMOVE_COLUMN")} />,
						template: (
							<HBox>
								<Button
	...
	getManageDialogUserTable(): Table | undefined {
		const tableTitle = (
			<InvisibleText text={this.getTranslatedText("C_COLLABORATIONDRAFT_INVITATION_TABLE_TITLE")} />
		) as InvisibleText;
