...
export function getDesigntimeSettings(): DesigntimeSetting[] {
	...
	const avatarImageFitType: DesigntimeSetting = {
		...
		value: "Cover",
		enums: [
			{ id: "Cover", name: "RTA_CONFIGURATION_OP_AIFT_ENUM_COVER" },
			{ id: "Contain", name: "RTA_CONFIGURATION_OP_AIFT_ENUM_CONTAIN" }
		]
	};
	...
	designtimeSettings.forEach(function (setting) {
		switch (setting.id) {
			case "avatarImageFitType":
				propertyValues.avatarImageFitType =
					viewDataSettings.content?.header?.avatar?.imageFitType !== undefined
						? viewDataSettings.content.header.avatar.imageFitType
						: propertyValues.avatarImageFitType;
				break;
