 ...
 */
export const buildCriticalityVisibleFromIdentificationExpression = function (
 ...
 */
export const getFooterVisibilityExpressionInternal = function (
	context: IContext,
		...
		if (actions.length) {
			return actions.map(
				(action) =>
					...
					(resolveBindingString(action.visible ?? true, "boolean") as BindingToolkitExpression<boolean>)
			);
		}
		return [constant(false)];
	};
