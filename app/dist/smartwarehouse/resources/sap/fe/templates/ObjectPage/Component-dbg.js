...
sap.ui.define(["sap/base/Log", "sap/fe/base/ClassSupport", "sap/fe/core/CommonUtils", "sap/fe/core/TemplateComponent", "sap/fe/core/helpers/ModelHelper", "sap/fe/core/library", "sap/fe/templates/ObjectPage/ExtendPageDefinition", "sap/fe/templates/library"], function (Log, ClassSupport, CommonUtils, TemplateComponent, ModelHelper, CoreLibrary, ExtendPageDefinition, templateLib) {
  ...
  let ObjectPageComponent = (_dec = defineUI5Class("sap.fe.templates.ObjectPage.Component", {
    ...
    function ObjectPageComponent() {
      ...
      _initializerDefineProperty(_this, "disableObjectPageRequestOptimization", _descriptor11, _this);
    ...
    ;
    _proto.createDeferredContext = function createDeferredContext(sPath, parameters, bActionCreate) {
      if (!this.DeferredContextCreated) {
        this.DeferredContextCreated = true;
        if (!parameters.listBinding || parameters.listBinding.getUpdateGroupId() === CommonUtils.INLINEEDIT_UPDATEGROUPID) {
          const oParameters = {
            $$groupId: "$auto.Heroes",
            $$updateGroupId: "$auto"
          };
          ...
          parameters.listBinding = this.getModel().bindList(sPath.replace("(...)", ""), undefined, undefined, undefined, oParameters);
        }
        const oStartUpParams = this.oAppComponent?.getComponentData()?.startupParameters;
        let dataForCreate;
        if (oStartUpParams?.preferredMode?.[0].includes("create") === true) {
          const oInboundParameters = this.getViewData().inboundParameters;
          const metaModel = this.getModel()?.getMetaModel();
          const contextPath = this.contextPath || `/${this.entitySet}`;
          dataForCreate = Object.assign({}, parameters.data, CommonUtils.getAdditionalParamsForCreate(oStartUpParams, oInboundParameters, metaModel, contextPath));
        } else {
          dataForCreate = parameters.data;
        }

        // for now wait until the view and the controller is created
        this.getRootControl().getController().editFlow.createDocument(parameters.listBinding, {
          creationMode: CreationMode.Sync,
          createAction: bActionCreate,
          data: dataForCreate,
          bFromDeferred: true,
          selectedContexts: parameters.parentContext ? [parameters.parentContext] : undefined,
    ...
    writable: true,
    initializer: null
  }), _descriptor11 = _applyDecoratedDescriptor(_class2.prototype, "disableObjectPageRequestOptimization", [_dec12], {
    configurable: true,
    enumerable: true,
    writable: true,
    initializer: null
