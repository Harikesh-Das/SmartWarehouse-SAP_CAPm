...
sap.ui.define(["sap/fe/base/BindingToolkit", "sap/fe/core/converters/ManifestSettings", "sap/fe/core/converters/MetaModelConverter", "sap/fe/core/helpers/BindingHelper", "sap/fe/core/helpers/ModelHelper", "sap/fe/core/helpers/StableIdHelper", "sap/fe/core/helpers/TitleHelper", "sap/fe/macros/CommonHelper", "sap/fe/macros/field/FieldTemplating", "sap/m/library", "sap/ui/base/BindingInfo", "sap/ui/core/Lib", "sap/ui/model/odata/v4/AnnotationHelper"], function (BindingToolkit, ManifestSettings, MetaModelConverter, BindingHelper, ModelHelper, StableIdHelper, TitleHelper, CommonHelper, FieldTemplating, mLibrary, BindingInfo, Library, ODataModelAnnotationHelper) {
  ...
  /**
   * Returns a compiled expression that determines the emphasized button type based on criticality across all actions.
   * If a critical action is rendered, it is treated as the primary action, and the template's default primary action is suppressed.
   * @param dataContextPath The DataModelObjectPath related to the context
   * @returns A compiled expression that evaluates to the button type to use
   */
  _exports.isManifestAction = isManifestAction;
  const buildCriticalityVisibleFromIdentificationExpression = function (dataContextPath) {
    ...
    if (!dataFieldsWithCriticality.length) {
      return constant(false);
    }
    return or(...dataFieldsWithCriticality.map(dataField =>
    // Criticality suppresses emphasis if the action itself is visible (Hidden is false)
    not(equal(getExpressionFromAnnotation(dataField.annotations?.UI?.Hidden), true))));
  };
  _exports.buildCriticalityVisibleFromIdentificationExpression = buildCriticalityVisibleFromIdentificationExpression;
  const buildEmphasizedButtonExpression = function (dataContextPath) {
    const criticalityVisibleExpr = buildCriticalityVisibleFromIdentificationExpression(dataContextPath);
    // If there is at least one visible action with criticality, the template's default primary action is set back to Ghost
    // else it is emphasized
    return compileExpression(ifElse(criticalityVisibleExpr, ButtonType.Ghost, ButtonType.Emphasized));
  };
  ...
  const createBindingForAlternateAndSecondaryKeys = function (dataModelObjectPath) {
    if (!dataModelObjectPath?.navigationProperties?.length) {
      return undefined;
    }
    let binding;
  ...
  const getPressExpressionForEdit = function (oDataField, sEntitySetName, oHeaderAction) {
    ...
    const oParams = {
      ...
      disableStrictHandling: oHeaderAction?.disableStrictHandling
    };
  ...
  const getFooterVisibilityExpressionInternal = function (context, footerActions) {
    const metaModel = context.getInterface(1).getModel();
    const _generateBindingsForActions = actions => {
      if (actions.length) {
        return actions.map(action => action.visibleExpression ?? resolveBindingString(action.visible ?? true, "boolean"));
      }
      return [constant(false)];
    };
   ...
   * @param context The context for the formatter.
   * @param footerActions The list of footer actions.
   * @returns A compiled binding expression
   */
  _exports.getFooterVisibilityExpressionInternal = getFooterVisibilityExpressionInternal;
  const getFooterVisibilityExpression = function (context, footerActions) {
    return compileExpression(getFooterVisibilityExpressionInternal(context, footerActions));
  };
