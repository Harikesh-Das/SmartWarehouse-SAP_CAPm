...
class ObjectPageComponent extends TemplateComponent {
	...
	enableLazyLoading: boolean | undefined;

	@property({
		type: "boolean",
		defaultValue: undefined
	})
	disableObjectPageRequestOptimization: boolean | undefined;
	...
	createDeferredContext(
		sPath: string,
		parameters: { listBinding?: ODataListBinding; parentContext?: Context; data?: object; requestAtLeast?: string[] },
		...
		if (!this.DeferredContextCreated) {
			...
			if (oStartUpParams?.preferredMode?.[0].includes("create") === true) {
				const oInboundParameters = this.getViewData().inboundParameters as Record<string, InboundParameter> | undefined;
				const metaModel = this.getModel()?.getMetaModel();
				const contextPath = this.contextPath || `/${this.entitySet}`;
				dataForCreate = Object.assign(
					{},
					parameters.data,
					CommonUtils.getAdditionalParamsForCreate(oStartUpParams, oInboundParameters, metaModel, contextPath)
				);
			} else {
			...
			this.getRootControl()
				.getController()
				.editFlow.createDocument(parameters.listBinding, {
