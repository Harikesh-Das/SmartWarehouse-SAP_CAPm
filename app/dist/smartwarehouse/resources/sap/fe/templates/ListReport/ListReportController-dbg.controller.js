 ...
 */
sap.ui.define(["sap/base/Log", "sap/base/util/ObjectPath", "sap/fe/base/ClassSupport", "sap/fe/core/CommonUtils", "sap/fe/core/PageController", "sap/fe/core/controllerextensions/IntentBasedNavigation", "sap/fe/core/controllerextensions/InternalIntentBasedNavigation", "sap/fe/core/controllerextensions/InternalRouting", "sap/fe/core/controllerextensions/KPIManagement", "sap/fe/core/controllerextensions/MessageHandler", "sap/fe/core/controllerextensions/Placeholder", "sap/fe/core/controllerextensions/Share", "sap/fe/core/controllerextensions/SideEffects", "sap/fe/core/controllerextensions/ViewState", "sap/fe/core/helpers/EditState", "sap/fe/core/helpers/MessageStrip", "sap/fe/core/helpers/ResourceModelHelper", "sap/fe/core/helpers/StableIdHelper", "sap/fe/core/library", "sap/fe/macros/chart/ChartUtils", "sap/fe/macros/filter/FilterUtils", "sap/fe/navigation/SelectionVariant", "sap/fe/templates/ListReport/ExtensionAPI", "sap/fe/templates/ListReport/overrides/SideEffects", "sap/fe/templates/TableScroller", "sap/ui/Device", "sap/ui/core/InvisibleMessage", "sap/ui/core/library", "sap/ui/core/mvc/OverrideExecution", "sap/ui/mdc/p13n/StateUtil", "sap/ui/thirdparty/hasher", "./ListReportTemplating", "./overrides/IntentBasedNavigation", "./overrides/MessageHandler", "./overrides/Share", "./overrides/ViewState"], function (Log, ObjectPath, ClassSupport, CommonUtils, PageController, IntentBasedNavigation, InternalIntentBasedNavigation, InternalRouting, KPIManagement, MessageHandler, Placeholder, Share, SideEffects, ViewState, EditState, MessageStrip, ResourceModelHelper, StableIdHelper, CoreLibrary, ChartUtils, FilterUtils, SelectionVariant, ExtensionAPI, SideEffectsOverride, TableScroller, Device, InvisibleMessage, library, OverrideExecution, StateUtil, hasher, ListReportTemplating, IntentBasedNavigationOverride, MessageHandlerOverride, ShareOverrides, ViewStateOverrides) {
  "use strict";
  ...
  let ListReportController = (_dec = defineUI5Class("sap.fe.templates.ListReport.ListReportController"), _dec2 = usingExtension(InternalRouting.override({
    ...
    function ListReportController() {
      ...
      _initializerDefineProperty(_this, "sideEffects", _descriptor3, _this);
      ...
      _initializerDefineProperty(_this, "messageHandler", _descriptor9, _this);
      _this.formatters = {
      ...
      _this.handlers = {
        onFilterLayoutSwitch(oEvent) {
          const currentKey = oEvent.getSource().selectedKey;
          const isAiLayout = currentKey === "ai";
          const controlVariantControls = this._getControlVariantManagementControls();
          if (controlVariantControls.length > 0) {
            // Add more controls here as the AI layout gains support for them
            const filterBarVM = this._getFilterBarVariantControl();
            const tableVM = this._getTable()?.getVariant();
            const supportedOnAiLayout = new Set([filterBarVM, tableVM].filter(vm => !!vm));
            controlVariantControls.forEach(variantControl => {
              const isVisible = !isAiLayout || supportedOnAiLayout.has(variantControl);
        ...
        onSegmentedButtonPressed(oEvent) {
          ...
          this.getExtensionAPI().updateAppState();
          // setTimeout cause the variant needs to be applied before judging the auto search or updating the app state
          this.focusHandlingForSegmentedButton();
        },
    ...
    _proto.onInit = function onInit() {
      ...
      const uiModelContext = this.getOwnerComponent().getBindingContext("ui");
      const filterBar = this._getFilterBarControl();
      if (filterBar) {
    ...
    _proto._onAfterBinding = async function _onAfterBinding() {
      ...
      await this.getAppComponent().getRootViewController().waitForEditCleanup();
     ...
     */;
    _proto._focusWhenHeaderExpanded = function _focusWhenHeaderExpanded(filterBar) {
     ...
     */;
    _proto.setFocusOnEmphasizedHeaderAction = function setFocusOnEmphasizedHeaderAction() {
      const dynamicPage = this._getDynamicListReportControl();
      const title = dynamicPage.getTitle();
      const actions = title.getActions();
      const emphasizedButton = actions.find(control => {
        return control && control.isA?.("sap.m.Button") && control.getType() === "Emphasized" && control.getVisible() && control.getEnabled() !== false;
      });
      if (emphasizedButton) {
     ...
     */;
    _proto._setInitialFocus = async function _setInitialFocus() {
      ...
      if (filterBar && !this._isFilterBarHiddenUsed()) {
        ...
        if (isHeaderExpanded) {
          ...
          await this._getTable()?.focusRow(0).catch(function (error) {
            if (error) {
              // if there is no error it can just be that there are no lines to focus
              Log.info("Error while setting initial focus on the table ", error);
            }
          });
        }
      } else {
     ...
     */;
    _proto._getBrowserTitle = async function _getBrowserTitle(_pageTitleInformation) {
    ...
    _proto._applyAppState = async function _applyAppState() {
     ...
     */;
    _proto._getControlVariantManagementControls = function _getControlVariantManagementControls() {
      ...
      variantIdArray.forEach(variantId => {
        const variantControl = this.getView().byId(variantId.trim());
        if (variantControl) {
    ...
    _proto._getSegmentedButton = function _getSegmentedButton() {
      const contentSwitcher = this.getView().byId("fe::ContentSwitcher");
    ...
    /**
     * Handles focus for the segmented button.
     */
    _proto.focusHandlingForSegmentedButton = function focusHandlingForSegmentedButton() {
      const segmentedButton = this._getSegmentedButton();
      if (segmentedButton) {
        const eventDelegate = {
          onAfterRendering: () => {
    ...
    /**
     * Handles the visibility of the message button in the footer in case there is inline edit.
     * @param event
     */;
    _proto.handleMessageButtonVisibility = function handleMessageButtonVisibility(event) {
      if (event.getParameter("visible")) {
    ...
    writable: true,
    initializer: null
  }), _descriptor9 = _applyDecoratedDescriptor(_class2.prototype, "messageHandler", [_dec10], {
    configurable: true,
    enumerable: true,
    writable: true,
    initializer: null
