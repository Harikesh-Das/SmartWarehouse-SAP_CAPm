...
class ListReportController extends PageController {
	...
	_getAdaptationFilterBarControl(): Control | undefined {
	...
	_getFilterBarVariantControl(): VariantManagement | undefined {
		return this._getControlFromPageModelProperty("/variantManagement/id") as VariantManagement | undefined;
	}
	...
	handlers = {
		onFilterLayoutSwitch(this: ListReportController, oEvent: UI5Event<{}, ContentSwitcher>): void {
			...
			if (controlVariantControls.length > 0) {
				...
				const supportedOnAiLayout = new Set<VariantManagement>([filterBarVM, tableVM].filter((vm) => !!vm));
	 ...
	 */
	focusHandlingForSegmentedButton(): void {
	 ...
	 * @param event
	 */
	handleMessageButtonVisibility(event: UI5Event<{ visible: boolean }, MessageButton>): void {
