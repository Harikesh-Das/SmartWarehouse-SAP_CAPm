...
import type { default as ODataContext } from "sap/ui/model/odata/v4/Context";
...
export default Object.assign({}, MdcMultiValueFieldDelegate, {
	...
	updateItemsFromConditions: async function (control: MultiValueField, conditions: ConditionObject[]) {
	...
	_updateItemsInODataListBinding: async function (
		...
		const contextsToDelete: ODataContext[] = [];
		...
		// As we want to perform multiple delete and create we wait for the previous editflow task here
		// and then use doNotWaitForSyncTask in the delete and create calls
		await controller.editFlow.syncTask();

		for (let i = 0; i < contexts.length; i++) {
			const deleteContext = contexts[i];
			if (!newKeys.includes(deleteContext.getProperty(keyPath))) {
				contextsToDelete.push(deleteContext);
			} else {
				newKeys.splice(newKeys.indexOf(deleteContext.getProperty(keyPath)), 1);
				// remove the key so that it doesn't get recognize twice
			}
		}
		if (contextsToDelete.length > 0) {
			modificationPromises.push(
				controller.editFlow.deleteMultipleDocuments(contextsToDelete, {
					controlId: control.getId(),
					noDialog: true,
					hideBusyIndicator: true,
					requestSideEffects: true,
					doNotWaitForSyncTask: true,
					throwOnError: true
				})
			);
		}
		...
		for (const condition of conditions) {
			if (!oldKeys.includes(condition.values[0])) {
				...
				const index = oldKeys.indexOf(condition.values[0]);
		...
		if (dataToAdd.length) {
			modificationPromises.push(
				controller.editFlow.createMultipleDocuments(listBinding, dataToAdd, {
					createAtEnd: true,
					hideBusyIndicator: true,
					requestSideEffects: true,
					doNotWaitForSyncTask: true
