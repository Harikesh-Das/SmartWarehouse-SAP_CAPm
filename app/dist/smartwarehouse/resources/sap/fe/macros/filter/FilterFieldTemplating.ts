...
import type { ComputedAnnotationInterface, DisplayMode } from "sap/fe/core/templating/UIFormatters";
import type Context from "sap/ui/model/odata/v4/Context";
import type ODataMetaModel from "sap/ui/model/odata/v4/ODataMetaModel";
import type { ValueListInfo } from "sap/ui/model/odata/v4/ODataMetaModel";

export const getDisplayProperty = function (propertyObjectPath: DataModelObjectPath<Property>, propertyConverted: Property): DisplayMode {
...
export async function _getDisplayModeFromValueHelp(
	...
	return metaModel.requestValueListInfo(context.getPath(), true, context).then(function (valueListInfo) {
		...
		const valueListMetaModel = firstValueListInfo.$model.getMetaModel?.() ?? metaModel;
		const propertyConverted = MetaModelConverter.getInvolvedDataModelObjects<Property>(
			valueListMetaModel.createBindingContext("/" + firstValueListInfo.CollectionPath + "/" + valueListProperty)!
		);
