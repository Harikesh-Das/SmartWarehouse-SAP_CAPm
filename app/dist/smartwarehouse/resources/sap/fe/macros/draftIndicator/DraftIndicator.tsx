...
export default class DraftIndicator extends BuildingBlock<Control> {
		...
		if (hasDraftEntity) {
			...
			if (!userDescription) {
				...
				return draftInProcessByUser
					? this.getTranslatedText("M_FIELD_RUNTIME_DRAFT_POPOVER_LOCKED_BY_KNOWN", [userDescription])
					: this.getTranslatedText("M_FIELD_RUNTIME_DRAFT_POPOVER_UNSAVED_CHANGES_BY_KNOWN", [userDescription]);
			}
		} else {
	...
	getDraftOwnerTextBindingExpression(): {
		...
		return { parts, formatter: this.formatDraftOwnerTextInPopover.bind(this) };
	}
