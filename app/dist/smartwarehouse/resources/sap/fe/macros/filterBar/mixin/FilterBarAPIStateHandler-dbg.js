    ...
    _proto.applyNavigationParameters = async function applyNavigationParameters(navigationParameter) {
      return new Promise(async resolve => {
        try {
          ...
          if (filterBar) {
            if (navigationParameter.navigationType !== NavType.initial && reqStdVariant && selectionVariantWasApplied || !variantManagement && view.getViewData().initialLoad === InitialLoadMode.Enabled || controller._shouldAutoTriggerSearch(variantManagement)) {
            ...
            if (!bPreventInitialSearch && searchTriggeredByInitialLoad) {
              ...
              await filterBar.validate(searchTriggeredByInitialLoad);
            }
          } else {
