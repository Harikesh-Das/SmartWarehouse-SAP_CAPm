...
import type TableCreationOptions from "../TableCreationOptions";
...
export default class ContextMenuHandler implements IInterfaceWithMixin {
	 ...
	 */
	_alignTextAndVisiblityInContextMenu(actionItem: Button | MenuItem, contextMenuItem: MenuItem, tableAction: ActionToolbarAction): void {
		...
		// For MenuButton-type actions, also align the text of sub-items in the context menu.
		// Sub-items may have i18n bindings that are not propagated to the context menu popup layer.
		if (actionItem.isA<MenuButton>("sap.m.MenuButton")) {
			const toolbarSubItems = (actionItem as unknown as MenuButton).getMenu().getItems();
			const contextSubItems = contextMenuItem.getItems();
			for (const toolbarSubItem of toolbarSubItems) {
				const toolbarActionId = toolbarSubItem.data("actionId") as string;
				if (!toolbarActionId) {
					continue;
				}
				const contextSubItem = contextSubItems.find((item) => {
					const contextActionId = item.data("actionId") as string;
					return (
						contextActionId === toolbarActionId.replace("::ActionMenu", "::ContextMenu") ||
						contextActionId === toolbarActionId + "::ContextMenu"
					);
				});
				if (contextSubItem) {
					this._alignTextAndVisiblityInContextMenu(toolbarSubItem, contextSubItem, tableAction);
				}
			}
		}
	}
	...
	_onContextMenuPress(this: ITableBlock & ContextMenuHandler, oEvent: Table$BeforeOpenContextMenuEvent): void {
		...
		const isInlineCreationMode =
			(tableAPI.getAggregation("creationMode") as TableCreationOptions | undefined)?.getProperty("name") ===
		...
		const readOnlyDraftEnabled = tableAPI.readOnly && draft !== "undefined";
		const updatableContexts: Context[] = [];
		for (const selectedContext of activeSelectedContexts) {
			if (
				TableRuntime.isUpdatableContext(tableDefinition.annotation.updatablePropertyPathInfo, selectedContext, readOnlyDraftEnabled)
			) {
