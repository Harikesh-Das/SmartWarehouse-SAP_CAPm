...
sap.ui.define(["sap/fe/core/ActionRuntime", "sap/fe/core/CommonUtils", "sap/fe/core/converters/ManifestSettings", "sap/fe/core/helpers/DeleteHelper", "sap/fe/core/helpers/ModelHelper", "sap/fe/core/helpers/StableIdHelper", "sap/m/MenuItem", "sap/ui/fl/util/ManagedObjectModel", "sap/ui/performance/trace/FESRHelper", "../TableRuntime"], function (ActionRuntime, CommonUtils, ManifestSettings, deleteHelper, ModelHelper, StableIdHelper, MenuItem, ManagedObjectModel, FESRHelper, TableRuntime) {
  ...
  let ContextMenuHandler = /*#__PURE__*/function () {
    ...
    _proto._alignTextAndVisiblityInContextMenu = function _alignTextAndVisiblityInContextMenu(actionItem, contextMenuItem, tableAction) {
      ...
      if (actionItem.isA("sap.m.MenuButton")) {
        const toolbarSubItems = actionItem.getMenu().getItems();
        const contextSubItems = contextMenuItem.getItems();
        for (const toolbarSubItem of toolbarSubItems) {
          const toolbarActionId = toolbarSubItem.data("actionId");
          if (!toolbarActionId) {
            continue;
          }
          const contextSubItem = contextSubItems.find(item => {
            const contextActionId = item.data("actionId");
            return contextActionId === toolbarActionId.replace("::ActionMenu", "::ContextMenu") || contextActionId === toolbarActionId + "::ContextMenu";
          });
          if (contextSubItem) {
    ...
    _proto._onContextMenuPress = function _onContextMenuPress(oEvent) {
      ...
      const isInlineCreationMode = tableAPI.getAggregation("creationMode")?.getProperty("name") === CreationMode.InlineCreationRows;
      ...
      for (const selectedContext of activeSelectedContexts) {
        if (TableRuntime.isUpdatableContext(tableDefinition.annotation.updatablePropertyPathInfo, selectedContext, readOnlyDraftEnabled)) {
      ...
      ActionRuntime.setActionEnablement(internalModelContext, JSON.parse(tableDefinition.operationAvailableMap), activeSelectedContexts, "table", true, true);
    };
