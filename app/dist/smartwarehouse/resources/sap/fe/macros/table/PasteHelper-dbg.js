 ...
 */
sap.ui.define(["sap/base/Log", "sap/fe/core/converters/ManifestSettings", "sap/fe/core/converters/MetaModelConverter", "sap/fe/core/templating/UIFormatters", "sap/fe/macros/ai/EasyFilterDataFetcher", "sap/fe/macros/internal/valuehelp/ValueListHelper", "sap/m/MessageBox", "sap/ui/core/Lib", "sap/ui/core/Messaging", "sap/ui/core/util/PasteHelper", "sap/ui/model/FilterOperator", "sap/ui/model/ValidateException"], function (Log, ManifestSettings, MetaModelConverter, UIFormatters, EasyFilterDataFetcher, ValueListHelper, MessageBox, Library, Messaging, CorePasteHelper, FilterOperator, ValidateException) {
  "use strict";
  ...
  const PasteHelper = {
    ...
    async _getValueList(propertyPath, metaModel, metaContext) {
      const valueLists = await ValueListHelper.getValueListInfo({
        getBindingContext() {
          return metaContext;
        }
      }, propertyPath, undefined, metaModel);
      return valueLists[0];
    },
    /**
    ...
    getColumnInfo: function (table, columns) {
      ...
      function getColumnFromPropertyInfos(dataProperty, hasValueHelp, property) {
        ...
        if (dataProperty.exportSettings === null) {
          if (dataProperty.path) {
            const infoForEntityProperty = PasteHelper.getInfoForEntityProperty(dataProperty.path, rowBindingPath, metaContext, metaModel, tableDefinition.control.enablePastingOfComputedProperties, false);
            ...
            const isComputedProperty = !!entityTypeProperties.find(p => p.name === dataProperty.path)?.annotations.Core?.Computed?.valueOf();
      ...
      (columns ?? table.getColumns()).forEach(column => {
        ...
        if (infoProperty.propertyInfos) {
          ...
          const mainPropInfo = {
            ...propertyInfoDict[column.getPropertyKey()]
          };
          mainPropInfo.path = column.getPropertyKey();
          const mainProp = entityTypeProperties.by_name(mainPropInfo.name);
          if (mainProp?.annotations.Common?.Text && mainProp.annotations.Common.Text.annotations?.UI?.TextArrangement?.valueOf() === "UI.TextArrangementType/TextOnly") {
            ...
            if (columnToPush) {
              ...
              columnToPush.customParseFunction = async data => {
                ...
                if (valueList && ValueListHelper.isValueListSearchable(metaContext.getPath(mainProp.name), valueList)) {
                  const values = await EasyFilterDataFetcher.resolveTokenValue(valueList, {
                  ...
                  if (!values[0].noMatch) {
                    if (values[0].selectedValues.length > 1) {
                      const potentialValues = values[0].selectedValues.map(val => val.value + "(" + val.description + ")").join(",");
                      throw new ValidateException(resourceBundle.getText("C_PASTE_HELPER_SAPFE_PASTE_ERROR_AMBIGUOUS_VALUES", [potentialValues]));
                    }
                    return values[0].selectedValues[0].value;
                  } else {
      ...
      for (const row of data) {
        ...
        if (rowContext) {
          for (const dataProperty in row) {
            const editMode = table.getParent().getPropertyEditMode(dataProperty, rowContext);
    ...
    getPasteInformationForGridTable: function (rawData, cellSelection, posFirstSelectedRow, table) {
      ...
      const grandTotalRowsCount = isAnalyticalTable ? table.getRowBinding().getCurrentContexts().filter(c => !c.isTransient() && c.getPath().endsWith("()")).length : 0;
      ...
      const inactiveRowsCount = isResponsiveTableInGetPasteInfo ? 0 : table.getRowBinding().getAllCurrentContexts().filter(c => c.isInactive?.()).length;
    ...
    pasteRangeData: async function (rawData, cellSelection, table) {
      ...
      if (parsedData.length > 0) {
        ...
        if (pasteInfo.numberOfNewCreationRows > 0) {
          ...
          await table.getParent().createEmptyRows(table.getRowBinding(), table, false, pasteInfo.numberOfNewCreationRows, !isResponsiveTable);
        }
        if (isResponsiveTable && pasteInfo.numberOfNewCreationRows > 0) {
          ...
          const existingRows = await table.getRowBinding().requestContexts(posFirstSelectedRow + pasteInfo.numberOfNewCreationRows, existingRowCount);
        ...
        if (!isResponsiveTable && pasteInfo.numberOfNewCreationRows === 0) {
          await table.getParent().createEmptyRows(table.getRowBinding(), table, false, undefined, true);
        }
        ...
        const inactiveRowContext = table.getRowBinding().getAllCurrentContexts().find(c => c.isInactive?.());
        const posFirstInlineCreationRow = inactiveRowContext?.getIndex() ?? pasteInfo.rows.find(context => context.isTransient())?.getIndex();
    ...
    pasteData: async function (rawData, table, controller) {
      ...
      return this.parsePastedData(rawData, table).then(async aParsedData => {
        ...
        const newContexts = aValidatedData.length > 0 ? await editFlow.createMultipleDocuments(table.getRowBinding(), aValidatedData, {
