...
import type { CreateBehaviorExternal, UpdatablePropertyPathInfo } from "sap/fe/core/converters/controls/Common/Table";
import { type StandardAction, StandardActionKeys } from "sap/fe/core/converters/controls/Common/table/StandardActions";
...
import type { EventType } from "types/extension_types";
...
const TableRuntime = {
	...
	setContextsAsync: async function (table: Table): Promise<void[] | undefined> {
		...
		return ActionRuntime.setActionEnablement(
			oInternalModelContext,
	...
	isUpdatableContext: function (
		...
		const anyObject = new Any({ anyBoolean: updatablePropertyPathInfo.bindingExpression });
		anyObject.setModel(selectedContext.getModel());
		anyObject.setBindingContext(selectedContext);
		const isUpdatable = (anyObject.getBinding("anyBoolean") as PropertyBinding).getExternalValue() as boolean;
		anyObject.destroy();
		return !!isUpdatable;
	},
	...
	updateCutCopyPasteEnablement: function (modelObject: { [key: string]: unknown }, table: Table, forContextMenu = false): void {
		...
		}) as StandardAction | undefined;
		const cutEnabled = cutAction !== undefined && cutAction.isTemplated !== "false";
		const cutableContexts = cutEnabled
			? (dataModelsrc.updatableContexts as Context[]).filter(
					(context) =>
						TableRuntime.checkIfNodeIsMovable(context, table) &&
						(!checkDraftInstance || context.getProperty("HasDraftEntity") !== true)
			  )
			: [];
	...
	updateMoveUpDownEnablement: function (modelObject: { [key: string]: unknown }, table: Table, forContextMenu = false): void {
		...
		if (
			...
			if (
				moveUpDownActions.length !== 2 ||
				!moveUpDownActions.every((action) => {
	...
	onFieldChangeInCreationRow: function (oEvent: EventType<typeof Field.prototype.change>, withCustomValidationFunction: boolean): void {
		// CREATION ROW CASE
