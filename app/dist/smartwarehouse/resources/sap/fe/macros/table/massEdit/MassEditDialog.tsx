...
export default class MassEditDialog {
	...
	private readonly contextsOnError: Set<string> = new Set();

	private readonly updatedProperties: Set<string> = new Set();

	private readonly updatedContexts: Set<string> = new Set();
	...
	async manageMessage(): Promise<void> {
		...
		if (isErrorAndSuccess) {
			...
			MessageBox.success(
				this.resourceModel.getText("C_MASS_EDIT_CHANGES_WITH_ERROR", [this.updatedContexts.size, this.contexts.length]),
				{
	 ...
	 */
	async updateProperty(selectedContext: ODataV4Context, propertyPath: string, value: unknown, groupId: string): Promise<void> {
