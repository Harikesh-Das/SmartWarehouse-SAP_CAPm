 ...
 */
sap.ui.define(["sap/base/Log", "sap/fe/base/HookSupport", "sap/fe/core/CommonUtils", "sap/fe/core/controllerextensions/editFlow/draft", "sap/fe/core/helpers/ModelHelper", "sap/fe/core/helpers/ResourceModelHelper", "sap/fe/macros/CollectionBindingInfo", "sap/fe/macros/insights/CommonInsightsHelper", "sap/fe/macros/insights/InsightsService", "sap/fe/macros/insights/TableInsightsHelper", "sap/fe/macros/table/Utils", "sap/m/MessageBox", "sap/ui/core/Element", "sap/ui/core/Lib", "sap/ui/core/routing/HashChanger", "sap/ui/model/Filter", "../../filterBar/SemanticDateOperators"], function (Log, HookSupport, CommonUtils, draft, ModelHelper, ResourceModelHelper, CollectionBindingInfoAPI, CommonInsightsHelper, InsightsService, TableInsightsHelper, TableUtils, MessageBox, UI5Element, Library, HashChanger, Filter, SemanticDateOperators) {
  "use strict";
  ...
  let TableSharing = (_dec = controllerExtensionHandler("collaborationManager", "collectAvailableCards"), _class = /*#__PURE__*/function () {
    ...
    _proto.getDownloadUrlWithFilters = async function getDownloadUrlWithFilters() {
      ...
      // Fire beforeRebindTable event to allow applications to add custom filters (e.g., via onBeforeRebind extension)
      // This ensures the tile count URL includes the same filters as the actual table binding
      const bindingInfo = {
        path: binding.getPath(),
        filters: allRelevantFilters.length > 0 ? new Filter({
      ...
      this.fireEvent("beforeRebindTable", {
        collectionBindingInfo: collectionBindingInfoAPI,
        quickFilterKey: quickFilter?.getSelectedKey()
      });

      // Get the final filters after the event (may include filters added by onBeforeRebind)
      const finalFilters = bindingInfo.filters ?? new Filter({
        filters: [],
        and: true
      });
      const parameters = {
      ...
      // create hidden binding with all filters e.g. static filters, filters with semantic operators, and filters added via onBeforeRebind
      const tempTableBinding = model.bindList(binding.getPath(), undefined, undefined, finalFilters, parameters);
