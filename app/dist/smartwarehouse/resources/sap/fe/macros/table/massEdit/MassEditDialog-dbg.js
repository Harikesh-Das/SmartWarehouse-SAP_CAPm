...
sap.ui.define(["sap/base/Log", "sap/fe/core/CommonUtils", "sap/fe/core/controllerextensions/BusyLocker", "sap/fe/core/controllerextensions/messageHandler/messageHandling", "sap/fe/core/converters/ManifestSettings", "sap/fe/core/helpers/PromiseKeeper", "sap/fe/core/helpers/ResourceModelHelper", "sap/fe/core/helpers/StableIdHelper", "sap/fe/core/library", "sap/m/Button", "sap/m/Dialog", "sap/m/Label", "sap/m/MessageBox", "sap/m/MessageStrip", "sap/m/MessageToast", "sap/ui/core/Fragment", "sap/ui/core/Lib", "sap/ui/core/util/XMLPreprocessor", "sap/ui/layout/form/Form", "sap/ui/layout/form/FormContainer", "sap/ui/layout/form/FormElement", "sap/ui/layout/form/ResponsiveGridLayout", "sap/ui/model/json/JSONModel", "./MassEditField", "./MassEditSideEffects", "sap/fe/base/jsx-runtime/jsx", "sap/fe/base/jsx-runtime/Fragment", "sap/fe/base/jsx-runtime/jsxs"], function (Log, CommonUtils, BusyLocker, messageHandling, ManifestSettings, PromiseKeeper, ResourceModelHelper, ID, FELibrary, Button, Dialog, Label, MessageBox, MessageStrip, MessageToast, Fragment, Library, XMLPreprocessor, Form, FormContainer, FormElement, ResponsiveGridLayout, JSONModel, MassEditField, MassEditSideEffects, _jsx, _Fragment, _jsxs) {
  ...
  let MassEditDialog = /*#__PURE__*/function () {
    ...
    function MassEditDialog(props) {
      ...
      this.contextsOnError = new Set();
      this.updatedProperties = new Set();
      this.updatedContexts = new Set();
    ...
    _proto.manageMessage = async function manageMessage() {
      const isErrorAndSuccess = this.contextsOnError.size > 0 && this.updatedContexts.size > 0;
      const coreResourceBundle = Library.getResourceBundleFor("sap.fe.core");
      ...
      await controller.messageHandler.showMessages({
        onBeforeShowMessage: (messages, showMessageParameters) => {
          ...
          if (!this.contextsOnError.size) {
            // only successes
            if (this.updatedContexts.size) {
          ...
          } else if (isErrorAndSuccess) {
            // errors and successes
            controller.editFlow.setDraftStatus(DraftStatus.Saved);
          } else {
            controller.editFlow.setDraftStatus(DraftStatus.Clear); //only errors
          }
      ...
      if (isErrorAndSuccess) {
        const confirmButtonTxt = coreResourceBundle.getText("C_COMMON_DIALOG_OK");
        MessageBox.success(this.resourceModel.getText("C_MASS_EDIT_CHANGES_WITH_ERROR", [this.updatedContexts.size, this.contexts.length]), {
          ...
          if (!control.isReadOnlyOnContext(selectedContext)) {
            ...
            for (const propertyPath in values) {
              if (selectedContext.getProperty(propertyPath) !== values[propertyPath]) {
                ...
                fieldPromises.push(this.updateProperty(selectedContext, propertyPath, values[propertyPath], groupId));
    ...
    /**
     * Updates a property in the selected context.
     * @param selectedContext The context to update.
     * @param propertyPath The path of the property to update.
     * @param value The new value for the property.
     * @param groupId The ID of the group for the update.
     */;
    _proto.updateProperty = async function updateProperty(selectedContext, propertyPath, value, groupId) {
      const contextPath = selectedContext.getPath();
      try {
        await selectedContext.setProperty(propertyPath, value, groupId);
        this.updatedContexts.add(contextPath);
        this.updatedProperties.add(propertyPath);
      } catch (error) {
        this.contextsOnError.add(contextPath);
    ...
    _proto.onApply = async function onApply() {
      ...
      if (!this.isAdaptation) {
        const globalUIModel = this.view.getModel("ui");
        this.dialog?.close();
        BusyLocker.lock(globalUIModel);
        ...
        BusyLocker.unlock(globalUIModel);
      }
      this.onClose();
    }
