...
sap.ui.define(["sap/fe/core/CommonUtils", "sap/fe/core/designtime/AnnotationBasedChanges", "sap/fe/macros/designtime/Designtime.helper", "sap/fe/macros/table/designtime/Table.designtime.helper"], function (CommonUtils, AnnotationBasedChanges, Designtime_helper, Table_designtime_helper) {
  ...
  var _exports = {};
  var getTableType = Table_designtime_helper.getTableType;
  var getSettingsApplicableForTableType = Table_designtime_helper.getSettingsApplicableForTableType;
   ...
   */
  function getSettings(table, propertyValues) {
    const items = [];
    ...
    getDesigntimeSettings(tableAPI).forEach(function (manifestSetting) {
      let enums = manifestSetting.enums;
      if (manifestSetting.keyUser === true && enums?.length && tableType) {
        enums = manifestSetting.enums?.filter(enumValue => enumValue.applicableForTableTypes?.includes(tableType));
      }
      ...
      settings: {
        fe: {
          ...
          handler: async function (control) {
            ...
            return openAdaptionDialog(configContext, propertyValues, "{sap.fe.i18n>RTA_CONFIGURATION_TITLE_TABLE}", resourceModel, configPath, "{sap.fe.i18n>RTA_CONFIGURATION_INFO_MESSAGE}", {
              width: "650px",
              height: "800px"
