 ...
 */
sap.ui.define(["sap/base/Log", "sap/base/util/deepEqual", "sap/base/util/deepExtend", "sap/fe/base/BindingToolkit", "sap/fe/base/jsx-runtime/jsx", "sap/fe/core/ActionRuntime", "sap/fe/core/CommonUtils", "sap/fe/core/controllerextensions/cards/CollaborationManager", "sap/fe/core/converters/MetaModelConverter", "sap/fe/core/converters/controls/Common/filter/FilterRestrictions", "sap/fe/core/converters/controls/Common/table/Columns", "sap/fe/core/converters/helpers/DataFieldHelper", "sap/fe/core/formatters/ValueFormatter", "sap/fe/core/helpers/DeleteHelper", "sap/fe/core/helpers/ExcelFormatHelper", "sap/fe/core/helpers/PromiseKeeper", "sap/fe/core/helpers/ResourceModelHelper", "sap/fe/core/helpers/SizeHelper", "sap/fe/core/templating/DataModelPathHelper", "sap/fe/core/type/EDM", "sap/fe/macros/CollectionBindingInfo", "sap/fe/macros/CommonHelper", "sap/fe/macros/DelegateUtil", "sap/fe/macros/filterBar/FilterBarDelegate", "sap/fe/macros/table/MdcTableTemplate", "sap/fe/macros/table/TableHelper", "sap/fe/macros/table/TableRuntime", "sap/fe/macros/table/TableSizeHelper", "sap/fe/macros/table/Utils", "sap/m/IllustratedMessage", "sap/m/IllustratedMessageType", "sap/ui/core/Element", "sap/ui/mdc/odata/v4/TableDelegate", "sap/ui/mdc/odata/v4/TypeMap", "sap/ui/model/Filter", "sap/ui/model/Sorter", "sap/ui/model/json/JSONModel", "../TableEventHandlerProvider"], function (Log, deepEqual, deepExtend, BindingToolkit, jsx, ActionRuntime, CommonUtils, CollaborationManager, MetaModelConverter, FilterRestrictions, Columns, DataFieldHelper, ValueFormatter, DeleteHelper, ExcelFormat, PromiseKeeper, ResourceModelHelper, SizeHelper, DataModelPathHelper, EDM, CollectionBindingInfoAPI, CommonHelper, DelegateUtil, FilterBarDelegate, MdcTableTemplate, TableHelper, TableRuntime, TableSizeHelper, TableUtils, IllustratedMessage, IllustratedMessageType, UI5Element, TableDelegateBase, TypeMap, Filter, Sorter, JSONModel, TableEventHandlerProvider) {
  "use strict";
  ...
  var getConnectedFieldsSeparator = DataFieldHelper.getConnectedFieldsSeparator;
  ...
  return Object.assign({}, TableDelegateBase, {
     ...
     * @param metaModel ODataMetaModel instance
     * @private
     * @alias sap.fe.macros.TableDelegate
     */
    _computeVisualSettingsForFieldGroup: function (table, propertyInfo, propertyInfos, metaModel) {
        ...
        oFieldGroup.Data.forEach(function (oData) {
          ...
          switch (oData.$Type) {
            ...
            case "com.sap.vocabularies.UI.v1.DataField":
              oDataFieldWidth = TableSizeHelper.getWidthForDataField(oData, propertyInfos, convertedMetaData, false, involvedDataModelObjects, showDataFieldsLabel);
              break;
     ...
     * @alias sap.fe.macros.TableDelegate
     */
    _computeVisualSettingsForConnectedField: function (table, propertyInfo, propertyInfos, metaModel) {
      if (propertyInfo.key.indexOf("DataFieldForAnnotation::ConnectedFields::") === 0) {
        const involvedDataModelObjects = getInvolvedDataModelObjects(metaModel.getContext(propertyInfo.annotationPath));
        const convertedMetaData = involvedDataModelObjects.convertedTypes;
        const dataField = involvedDataModelObjects.targetObject;
        const connectedFields = dataField.Target.$target;
        const connectedFieldsSeparator = getConnectedFieldsSeparator(connectedFields);
        const fieldWidths = [];
        getConnectedFieldsData(connectedFields).forEach(function (data) {
          const dataFieldWidth = TableSizeHelper.getWidthForDataField(data, propertyInfos, convertedMetaData, false, involvedDataModelObjects);
          if (dataFieldWidth) {
        ...
        const columnWidth = fieldWidths.reduce(function (acc, value) {
          return acc + value;
        }, 0);
        propertyInfo.visualSettings = deepExtend(propertyInfo.visualSettings, {
          widthCalculation: {
    ...
    /**
     * Adds a gap to the column width when the property has a Criticality annotation.
     * A field with Criticality renders as ObjectStatus, which prepends a status icon to the value.
     * The extra gap accounts for the icon width to prevent the value from being truncated.
     * @param propertyInfo The current propertyInfo object.
     * @param dataField The raw data field object from the metadata.
     */
    _computeVisualSettingsForPropertyCriticality: function (propertyInfo, dataField) {
      if (dataField?.Criticality) {
        const widthCalculation = propertyInfo.visualSettings?.widthCalculation;
        const currentGap = widthCalculation?.gap ?? 0;
        propertyInfo.visualSettings = deepExtend(propertyInfo.visualSettings, {
          widthCalculation: {
            gap: currentGap + 1
          }
    ...
    _computeVisualSettingsForPropertyWithValueHelp: function (table, property, metaModel) {
    ...
    _updatePropertyInfo: function (table, properties, metaModel) {
      const labelMap = {};
      ...
      properties.forEach(property => {
        this._computeVisualSettingsForFieldGroup(table, property, properties, metaModel);
        this._computeVisualSettingsForConnectedField(table, property, properties, metaModel);
        this._computeVisualSettingsForPropertyWithValueHelp(table, property, metaModel);
      ...
      // For analytical tables, restriction properties (e.g. __EntityControl/Deletable,
      // __EntityControl/Updatable) are not display columns and therefore have no property info.
      // Without an entry, MDC excludes them from the $apply group-by, causing context.getProperty()
      // to return undefined for row-level checks. This mirrors the $editState pattern above.
      //
      // UpdateRestrictions paths can point to navigation properties (e.g. __ENTITY_CONTROL/Updatable),
      // which ABAP backends reject when injected into $apply groupby. These are tagged with
      // "updateRestriction" origin and excluded when they contain "/".
      // DeleteRestrictions paths (struct paths like __EntityControl/Deletable) use "restriction"
      // origin and are always included with technicallyGroupable: true.

      const tableAPI = this._getTableAPI(table);
      if (tableAPI.getTableDefinition().enableAnalytics) {
        const requestAtLeast = tableAPI.getTableDefinition().requestAtLeast;
        const existingKeys = new Set(properties.map(p => p.key));
        Object.entries(requestAtLeast).filter(_ref => {
          let [path, origins] = _ref;
          if (existingKeys.has(path)) {
            return false;
          }
          if (origins.includes("restriction")) {
            return true;
          }
          if (origins.includes("updateRestriction")) {
    ...
    _fetchPropertyInfo: function (metaModel, columnInfo, table, appComponent) {
      const sAbsoluteNavigationPath = columnInfo.annotationPath,
        ...
        bIsAnalyticalTable = this._getTableAPI(table).getTableDefinition().enableAnalytics === true,
      ...
      this._computeVisualSettingsForPropertyCriticality(propertyInfo, oDataField);
      return propertyInfo;
    },
    /**
    ...
    _fetchCustomPropertyInfo: function (columnInfo, table, appComponent) {
      const resolveBinding = function (value) {
        if (value?.binding) {
          return value.binding.getExternalValue();
        } else if (typeof value === "object") {
          return CommonUtils.getTranslatedTextFromExpBindingString(value, table);
        } else {
          return getLocalizedText(value, appComponent);
        }
      };
      const label = columnInfo.header ? resolveBinding(columnInfo.header) : "";
      const tooltip = columnInfo.tooltip ? resolveBinding(columnInfo.tooltip) : "";
      const propertyInfo = {
        key: columnInfo.name,
    ...
    _fetchPropertiesForEntity: async function (table, entityTypePath, metaModel, appComponent) {
      ...
      const nonFilterableProperties = FilterRestrictions.getFilterRestrictionsByDataModel(dataModel).NonFilterableProperties;
      return Promise.resolve(this.getColumnsFor(table)).then(columns => {
        ...
        fetchedProperties = this._updatePropertyInfo(table, fetchedProperties, appComponent.getModel().getMetaModel());
        const tableAPI = this._getTableAPI(table);
    ...
    _getCachedOrFetchPropertiesForEntity: async function (table, entityTypePath, metaModel, appComponent) {
      ...
      return this._fetchPropertiesForEntity(table, entityTypePath, metaModel, appComponent).then(async function (subFetchedProperties) {
        tableAPI.setCachedPropertyInfos(subFetchedProperties);
        await tableAPI.preloadModulesPromise; // This is the first time we load the properties, so this is table initialization --> a good time to wait for the preloaded modules for manifest events
    ...
    setNoDataInformation: function (table, illustratedMessageInformation) {
      ...
      if (typeof noDataAggregation != "string" && noDataAggregation?.isA("sap.m.IllustratedMessage")) {
        ...
        currentIllustratedMessage.setAriaTitleLevel(illustratedMessageInformation.ariaTitleLevel);
      } else {
    ...
    setTableNoDataIllustratedMessage: function (table, bindingInfo) {
      const tableAPI = this._getTableAPI(table);
      if (tableAPI.isCustomNoDataSet()) {
        // If a custom noData is set, we don't override it
        return;
      }
      ...
      illustratedInformation.ariaTitleLevel = tableAPI.getIllustratedMessageAriaTitleLevel(tableAPI.headerStyle ?? tableAPI.headerLevel);
    ...
    _handleSortersOnCurrenciesOrUoM: function (table, bindingInfo) {
      ...
      if (sorters?.length) {
        ...
        // PV-defined sort condition keys: only for these should a currency/UoM sorter be auto-injected.
        // User-initiated (interactive) sorts must not have a currency/UoM sorter prepended, as it would
        // make a multi-currency dataset appear unsorted from the user's perspective (CS20260012416597).
        const pvSortKeys = new Set((tableAPI.getTableDefinition().annotation?.sortConditions?.sorters ?? []).map(s => s.key));
        for (const sorter of sorters) {
          const tableProperty = tableProperties?.find(property => property.path === sorter.getPath());
          if (tableProperty?.unit && pvSortKeys.has(tableProperty.key)) {
    ...
    _handleFiltersForExternalID: function (table, bindingInfo) {
      ...
      let entityTypePath = bindingInfo.path + "/";
      if (!entityTypePath.startsWith("/")) {
        // The binding info is relative, we need to prepend the metaContext of the table
        entityTypePath = `${metaModel.getMetaPath(table.getBindingContext()?.getPath() ?? "")}/${entityTypePath}`;
      }
    ...
    getInResultPropertyKeys: function (table) {
      ...
      if (tableAPI.tableDefinition.control.analyticalConfiguration?.aggregationOnLeafLevel) {
        ...
        .filter(fieldName => !allKkeysRequested ? tableAPI.tableDefinition.requestAtLeast[fieldName].some(origin => origin !== "restriction" && origin !== "updateRestriction" && origin !== "semanticKey") : true);
      }
      return Object.keys(tableAPI.tableDefinition.requestAtLeast ?? {});
    },
    /**
    ...
    updateBinding: function (table, bindingInfo, binding, parameters) {
      const tableAPI = this._getTableAPI(table);
      if (parameters?.forceRefresh && tableAPI.getNeedsRefreshOnSearch()) {
        tableAPI.setNeedsRefreshOnSearch(false);
        binding = undefined;
      }
      const bIsSuspended = tableAPI?.getProperty("bindingSuspended");
      if (!bIsSuspended) {
        ...
        tableAPI?.setupAnalyticalOutdatedWatcher();
      }
      tableAPI?.setProperty("outDatedBinding", bIsSuspended);
    },
    ...
    updateBindingInfoWithSearchQuery: function (bindingInfo, filterInfo, filter) {
      ...
      // We set $search to undefined instead of deleting it to trigger MDC's change detection
      // when search changes from a value to empty. The model ignores undefined parameters, so it won't be included
      // in the request
      bindingInfo.parameters.$search = CommonUtils.normalizeSearchTerm(filterInfo.search ?? undefined);
    },
    /**
    ...
    removeItem: async function (oTable, oPropertyInfoName, mPropertyBag) {
      ...
      if (doRemoveItem) {
        const tableAPI = this._getTableAPI(oTable);
        tableAPI.setNeedsRefreshOnSearch(true);
      }
      return Promise.resolve(doRemoveItem);
    },
    ...
    addItem: async function (table, propertyInfoName, mPropertyBag) {
      ...
      if (columnInfo.type === "Slot") {
        return getSlotColumn(sTableId, columnInfo, false, tableAPI);
      }
      ...
      const fnTemplateFragment = async (oInPropertyInfo, oView) => {
        ...
        return Promise.all([DelegateUtil.getCustomDataWithModifier(table, "tableType", oModifier), DelegateUtil.getCustomDataWithModifier(table, "creationMode", oModifier)]).then(async aCustomData => {
          bDisplayMode = tableAPI.readOnly === true;
          sTableTypeCustomData = aCustomData[0];
          sCreationModeCustomData = aCustomData[1];
