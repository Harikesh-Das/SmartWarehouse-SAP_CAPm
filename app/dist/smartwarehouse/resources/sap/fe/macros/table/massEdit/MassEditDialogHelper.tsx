...
export default class MassEditDialogHelper {
					...
					const fieldData = {
						...
						textBinding: {
							expression: getTextBinding(propertyDataModel, {
								displayMode: getDisplayMode(targetProperty, propertyDataModel.targetEntityType)
							}) as CompiledBindingToolkitExpression,
							type:
	...
	private async getMissingData(bindingsToFetch: BindingToFetch[]): Promise<undefined> {
		...
		for (const binding of bindingsToFetch.filter((binding) => binding.expression?.startsWith("{"))) {
			...
			for (const context of analyzedContexts) {
				...
				controlBinding?.setBindingMode(BindingMode.OneTime); // set the binding mode to OneTime to avoid the binding to be updated when the context changes, indeed even if the value doesn't change it's trigger a PATCH request
				...
				if (controlBinding) {
					if (controlBinding.isA<CompositeBinding>("sap.ui.model.CompositeBinding")) {
						promises.push(
							Promise.all(
								(controlBinding as CompositeBinding).getBindings().map((nestedBinding) => nestedBinding.requestValue?.())
							)
						);
					} else {
						promises.push(controlBinding.requestValue?.() ?? Promise.resolve());
					}
				}
			}
		}
	...
	private async getRuntimeSelection(fieldData: MassFieldProperties): Promise<SelectInfo[]> {
		...
		const textBinding = anyObject.getBinding("anyText") as ODataPropertyBinding;
	...
	/**
	 * Checks if the edit mode is editable.
	 * @param editMode
	 * @private
	 */
	private isEditableEditMode(editMode: string | FieldEditMode): boolean {
		return ([FieldEditMode.Editable] as string[]).includes(editMode);
	}
	...
	private getFieldEditable(expBinding: CompiledBindingToolkitExpression): boolean {
		if (this.isEditableEditMode(expBinding as string)) {
			return true;
			...
			const visible = this.analyzedContexts.some((context) => {
				...
				return this.isEditableEditMode((anyControl.getBinding("any") as PropertyBinding).getExternalValue());
			});
			anyControl.destroy();
			return visible;
		} else {
	...
	private isFieldVisible(fieldProperties: MassFieldProperties): boolean {
		if (this.isAdaptation || (this.manifestSettings.visibleFields.length && this.degradedMode)) {
			...
			const isEditable =
				!isStaticEditMode || (isStaticEditMode && this.isEditableEditMode(fieldProperties.visibilityBindings.editMode as string));
