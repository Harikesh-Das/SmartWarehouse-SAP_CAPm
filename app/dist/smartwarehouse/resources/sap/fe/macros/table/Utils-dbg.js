 ...
 */
sap.ui.define(["sap/fe/core/CommonUtils", "sap/fe/macros/DelegateUtil", "sap/fe/macros/filter/FilterUtils", "sap/ui/core/Element", "sap/ui/core/format/NumberFormat", "sap/ui/mdc/odata/v4/TableDelegate", "sap/ui/model/Filter", "../CollectionBindingInfo"], function (CommonUtils, DelegateUtil, FilterUtils, UI5Element, NumberFormat, TableDelegateBase, Filter, CollectionBindingInfoAPI) {
  "use strict";
   ...
   * @param params.itemKey The quick filter item key
   * @returns The count
   */
  async function requestCountForTable(table, params) {
    const pageBindingContext = table.getBindingContext();
    const oDataModel = table.getModel();
    const tableAPI = table.getParent();
    const filterInfo = tableUtils.getFilterInfo(table);
    let filters = (Array.isArray(params.additionalFilters) ? params.additionalFilters : []).concat(filterInfo.filters).concat(tableUtils.getP13nFilters(table));
    if (table.getDataStateIndicator()?.isFiltering()) {
      // Include filters on messageStrip
      const bindingInfoBase = {};
      TableDelegateBase.updateBindingInfo.apply(table.getControlDelegate(), [table, bindingInfoBase]);
      filters = filters.concat(bindingInfoBase.filters || []);
    }
    const bindingInfo = {
    ...
    const listBinding = oDataModel.bindList(listBindingPath, pageBindingContext ?? undefined, [], bindingInfo.filters);
  ...
  function getFilterInfo(oTable, ignoreProperties) {
    ...
    const filterBarID = oTable.getFilter();
    const filterBar = filterBarID ? UI5Element.getElementById(filterBarID) : undefined;
    let aIgnoreProperties = ignoreProperties || [];
    if (!filterBar && oTableDefinition.annotation.searchable === false) {
      aIgnoreProperties = aIgnoreProperties.concat(["search"]);
    }
    if (oTableDefinition.enableAnalytics || oTableDefinition.control.type === "TreeTable") {
  ...
  const tableUtils = {
    ...
    requestCountForTable: requestCountForTable,
