  ...
  let MassEditSideEffects = /*#__PURE__*/function () {
    ...
    _proto.generateSideEffectsDefinition = function generateSideEffectsDefinition() {
      ...
      for (const key in sideEffectsMap) {
        ...
        for (const sideEffectsName in keySideEffects) {
          ...
          const sideEffectsContext = view.getController().sideEffects.getContextForSideEffects(this.referenceRowContext, sideEffectsEntityType);
    ...
    _proto.getSideEffectsInformation = function getSideEffectsInformation(oDataSideEffect, sideEffectsContext, entitySetContext, sideEffectsService) {
      ...
      immediateTargets = targetEntities.reduce((entities, targetEntity) => {
        ...
        if (isEntityType(target) && target === entitySetDataModelPath.targetEntityType && targetEntity.$NavigationPropertyPath === "") {
          //The refresh is requested on the row entity
          entities.push(targetEntity);
        } else if (isNavigationProperty(target)) {
    ...
    _proto.getSideEffectsMap = function getSideEffectsMap() {
      ...
      for (const property of properties) {
        ...
        baseSideEffectsMapArray[property] = this.view.getController().sideEffects.getSideEffectsMapForFieldGroups(fieldGroupIds);
      }
    ...
    _proto.executeImmediateSideEffects = async function executeImmediateSideEffects(rowContext, field, groupId) {
      ...
      for (const sideEffectsProperties of (this.sideEffectsDefinition[field] ?? []).filter(sideEffects => sideEffects.immediate.targets.length || sideEffects.immediate.triggerAction)) {
        const context = sideEffectsProperties.onRowContext ? rowContext : controller.sideEffects.getContextForSideEffects(rowContext, sideEffectsProperties.sideEffects.fullyQualifiedName.split("@")[0]);
        if (context) {
          const action = sideEffectsProperties.immediate.triggerAction;
          const contextPath = context.getPath();
          sideEffectsPromises.push(controller.sideEffects.requestSideEffects(sideEffectsProperties.sideEffects, context, groupId, () => {
          ...
          controller.sideEffects.unregisterFailedSideEffects(sideEffectsProperties.sideEffects.fullyQualifiedName, context);
        }
      }

      //Execute the previous failed SideEffects requests
      const allFailedSideEffects = controller.sideEffects.getRegisteredFailedRequests();
      for (const context of [rowContext, this.view.getBindingContext()]) {
        if (context) {
          const contextPath = context.getPath();
          const failedSideEffects = allFailedSideEffects[contextPath] ?? [];
          controller.sideEffects.unregisterFailedSideEffectsForAContext(contextPath);
          for (const failedSideEffect of failedSideEffects) {
            sideEffectsPromises.push(controller.sideEffects.requestSideEffects(failedSideEffect, context));
          }
        }
      }
    ...
    _proto.executeDeferredSideEffects = function executeDeferredSideEffects(updatedFields) {
      ...
      for (const sourceProperty of Object.keys(this.sideEffectsDefinition).filter(propertyName => updatedFields.has(propertyName))) {
        for (const sideEffectsProperties of this.sideEffectsDefinition[sourceProperty].filter(sideEffects => sideEffects.deferred.targets.length || sideEffects.deferred.triggerAction)) {
          const context = sideEffectsProperties.onRowContext ? this.referenceRowContext : controller.sideEffects.getContextForSideEffects(this.referenceRowContext, sideEffectsProperties.sideEffects.fullyQualifiedName.split("@")[0]);
          if (context && !sideEffectsExecuted.has(sideEffectsProperties.sideEffects.fullyQualifiedName)) {
            ...
            controller.sideEffects.requestSideEffects(sideEffectsProperties.sideEffects, context, "$auto.massEditDeferred", () => {
              return {
      ...
      if (genericSideEffects && genericTargetEntity) {
        const context = controller.sideEffects.getContextForSideEffects(this.referenceRowContext, genericSideEffects.sideEffects.fullyQualifiedName.split("@")[0]);
        if (context) {
          controller.sideEffects.requestSideEffects(genericSideEffects.sideEffects, context, "$auto.massEditDeferred", () => {
            return {
