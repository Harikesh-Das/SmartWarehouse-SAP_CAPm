 ...
 */
sap.ui.define(["sap/base/Log", "sap/fe/core/ActionRuntime", "sap/fe/core/CommonUtils", "sap/fe/core/controllerextensions/BusyLocker", "sap/fe/core/controls/Any", "sap/fe/core/converters/MetaModelConverter", "sap/fe/core/converters/controls/Common/table/StandardActions", "sap/fe/core/helpers/DeleteHelper", "sap/fe/core/helpers/FPMHelper", "sap/fe/core/helpers/ResourceModelHelper", "sap/fe/core/library", "sap/fe/macros/table/Utils", "sap/m/MessageToast", "sap/ui/core/Messaging", "../field/FieldRuntimeHelper"], function (Log, ActionRuntime, CommonUtils, BusyLocker, Any, MetaModelConverter, StandardActions, deleteHelper, FPMHelper, ResourceModelHelper, FELibrary, TableUtils, MessageToast, Messaging, FieldRuntimeHelper) {
  "use strict";

  var StandardActionKeys = StandardActions.StandardActionKeys;
  const CreationMode = FELibrary.CreationMode;
  /**
   * Static class used by the Table building block during runtime
   * @private
   */
  const TableRuntime = {
    ...
    setContextsAsync: async function (table) {
      ...
      if (!tableAPI || !metaModel || tableAPI.getIgnoreSelectionChangeEvent()) {
        return;
      }
      ...
      const bReadOnlyDraftEnabled = tableAPI.readOnly && !!collectionEntity?.annotations.Common?.DraftRoot;
      ...
      const oModelObject = Object.assign(oInternalModelContext.getObject() || {}, {
        ...
        hasActiveGrouping: oInternalModelContext.getProperty("hasActiveGrouping") ?? false,
      ...
      for (const oSelectedContext of activeSelectedContexts) {
        ...
        if (this.isUpdatableContext(tableDefinition.annotation.updatablePropertyPathInfo, oSelectedContext, bReadOnlyDraftEnabled)) {
          aUpdatableContexts.push(oSelectedContext);
        }
      }
      if (tableDefinition.enableAnalytics !== true) {
      ...
      return ActionRuntime.setActionEnablement(oInternalModelContext, oActionOperationAvailableMap, activeSelectedContexts, "table", false, true);
    },
    /**
     * Checks if the selected context is updatable for Mass Edit.
     * @param updatablePropertyPathInfo The updatablePropertyPathInfo from the table annotation
    ...
    isUpdatableContext: function (updatablePropertyPathInfo, selectedContext, readOnlyDraftEnabled) {
      /* The updatable contexts with mass edit depend on the following:
       	1. The update is dependent on current entity property (updatablePaths).
       	2. The table is read only and draft enabled (such as on the list report page). Only active contexts can be used with mass edit (not draft contexts).
      ...
      if (!notDraftInReadOnlyMode) {
        return false;
      }
      if (!updatablePropertyPathInfo || updatablePropertyPathInfo.paths.length === 0) {
        return true;
      }
      if (updatablePropertyPathInfo.isPathAnnotationExpression) {
        return !!selectedContext.getProperty(updatablePropertyPathInfo.paths[0]);
      }
      const anyObject = new Any({
        anyBoolean: updatablePropertyPathInfo.bindingExpression
      });
      anyObject.setModel(selectedContext.getModel());
      anyObject.setBindingContext(selectedContext);
      const isUpdatable = anyObject.getBinding("anyBoolean").getExternalValue();
      anyObject.destroy();
      return !!isUpdatable;
    },
    /**
    ...
    updateCutCopyPasteEnablement: function (modelObject, table) {
      ...
      const cutAction = tableDefinition.actions.find(action => {
        return action.key === StandardActionKeys.Cut;
      });
      const cutEnabled = cutAction !== undefined && cutAction.isTemplated !== "false";
      const cutableContexts = cutEnabled ? dataModelsrc.updatableContexts.filter(context => TableRuntime.checkIfNodeIsMovable(context, table) && (!checkDraftInstance || context.getProperty("HasDraftEntity") !== true)) : [];
    ...
    updateMoveUpDownEnablement: function (modelObject, table) {
      ...
      if (selectedContexts.length !== 1 || tableDefinition.control.type !== "TreeTable" || table.getBindingContext("internal")?.getProperty("isSorted") === true || selectedContexts[0].getProperty("@$ui5.node.level") === 1 && !changeSiblingForRootsSupported // Disable move up/down for roots
        ...
        const moveUpDownActions = tableDefinition.actions.filter(action => {
          return action.key === StandardActionKeys.MoveDown || action.key === StandardActionKeys.MoveUp;
        });
        if (moveUpDownActions.length !== 2 || !moveUpDownActions.every(action => {
          return action.isTemplated !== "false";
        })) {
          // Move up/down are disabled
          modelObjectPath.singleContextMovableUp = false;
          modelObjectPath.singleContextMovableDown = false;
          return;
        }
    ...
    onCreateMenuItemPress: async function (event, valueIndex, selectedContexts) {
      ...
      const creationParameters = {
        ...
        requestAtLeast: Object.keys(tableAPI.tableDefinition.requestAtLeast ?? {})
      };
      ...
      view.getController().editFlow.createDocument(tableAPI.getRowBinding(), creationParameters).catch(error => {
        if (error !== "cancel") {
          // Cancelling the creation is not an error and shouldn't be logged
