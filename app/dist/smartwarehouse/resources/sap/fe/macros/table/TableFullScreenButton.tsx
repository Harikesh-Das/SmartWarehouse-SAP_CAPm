...
export default class TableFullScreenButton extends BuildingBlock<Button> {
	...
	protected getTableAPI(): TableAPI {
		if (!this.tableAPI) {
			...
			} while (!currentControl.isA("sap.fe.macros.Table"));
	 ...
	 * Additionally, if the table originally lived inside a Dialog, force a
	 * re-render of the MDC table so that the inner table
	 * correctly displays its rows in fullscreen.
	 */
	private afterDialogOpen(): void {
		this.content?.focus();

		if (this.originalTableParent && this.originalTableParent instanceof Dialog) {
			try {
				const tableAPI = this.getTableAPI();
				const mdcTable = tableAPI.getContent() as Control;

				// Defer the invalidation until the next rendering tick to ensure
				// the fullscreen Dialog has finished its layout and sizing.
				setTimeout(() => {
					// Force a re-render of the MDC table, which will also refresh
					// the inner table DOM.
					mdcTable.invalidate();
				}, 0);
			} catch {
				//does not break the dialog opening if any error occurs
			}
		}
	}
	...
	private afterDialogClose(): void {
		...
		// Re-render the MDC table to ensure correct display after exiting fullscreen
		this.getTableAPI()?.getContent()?.invalidate();
	}
