...
import ActionLayoutData from "sap/ui/mdc/table/ActionLayoutData";
...
import { type EventHandlerType } from "./TableEventHandlerProvider";
...
function getMenuItemForAction(
	...
	const pressHandler =
		menuItemAction.command && !forContextMenu
			? undefined
			: parameters.handlerProvider.getDataFieldForActionButtonPressHandler(dataField, menuItemAction, forContextMenu);
...
function getDataFieldButtonForAction(
	...
	const isBound = dataField.ActionTarget?.isBound;
	const pressHandler =
		action.command && !forContextMenu
			? undefined
			: parameters.handlerProvider.getDataFieldForActionButtonPressHandler(dataField, action, forContextMenu);
...
function getDataFieldButtonForIntentBasedNavigation(
	...
	const pressHandler =
		action.command && !forContextMenu ? undefined : handlerProvider.getDataFieldForIBNPressHandler(action, forContextMenu);
	const pressCommand = action.command && !forContextMenu ? `cmd:${action.command}|press` : undefined;
	const enabled =
		action.enabled !== undefined
			? action.enabled
...
function getTextButton(action: BaseAction | AnnotationAction | CustomAction): unknown {
...
function getMenuButton(
	...
	if (!forContextMenu) {
			...
			return (
				...
				>
					<MenuButton
						...
						useDefaultActionOnly={!!(action as CustomAction).defaultAction}
						icon={icon}
						buttonMode={(action as CustomAction).defaultAction ? MenuButtonMode.Split : MenuButtonMode.Regular}
		...
		action.menu?.forEach((menuItemAction) => {
			if (
				typeof menuItemAction !== "string" &&
				TableHelper.isActionShownInContextMenu(menuItemAction, table.contextObjectPath) &&
				menuItemAction.key !== defaultAction?.key
			) {
...
function getDefaultButton(
	...
	const pressHandler =
		action.command && !forContextMenu ? undefined : handlerProvider.getManifestActionPressHandler(action, forContextMenu);
	const pressCommand = action.command && !forContextMenu ? `cmd:${action.command}|press` : undefined;
 ...
 */
export function getFullScreen(table: TableBlockProperties): ActionToolbarAction | undefined {
		...
		return (
			...
			>
				{{
					...
					layoutData: <ActionLayoutData position={TableActionPosition.ViewActions} />
				}}
			</ActionToolbarAction>
		);
	}
 ...
 */
export function getAnalyticalRefreshButton(
	table: TableBlockProperties,
	handlerProvider: TableEventHandlerProvider
): ActionToolbarAction | undefined {
				...
				{{
					action: (
						<Button
							...
							enabled={compileExpression(UI.AnalyticalRefreshNeeded) as unknown as boolean}
							visible={compileExpression(UI.IsEditable) as unknown as boolean}
							type="Transparent"
						/>
					),
					layoutData: <ActionLayoutData position={TableActionPosition.ModificationActionsEnd} />
				}}
			</ActionToolbarAction>
		);
	}
...
type CutCopyPasteParams = {
	idKey: "Cut" | "Copy" | "Paste";
	text: string;
 ...
 * @returns The button
 */
function getCutCopyPasteButton(
	action: StandardAction,
	table: TableBlockProperties,
	forContextMenu: boolean,
	params: CutCopyPasteParams
): ActionToolbarAction | MenuItem {
	if (!forContextMenu) {
		// for table toolbar
		const button = (
		...
		sap.ui.require(["sap/ui/core/ShortcutHintsMixin"], (ShortcutHintsMixin: { addConfig: Function }) => {
			ShortcutHintsMixin.addConfig(button, { shortcut: params.shortcut }, undefined);
		});
		return (
			<ActionToolbarAction id={generate([table.contentId, params.idKey, "ActionToolbarAction"])} dt:designtime="not-adaptable-tree">
				{{
					action: button
				}}
			</ActionToolbarAction>
