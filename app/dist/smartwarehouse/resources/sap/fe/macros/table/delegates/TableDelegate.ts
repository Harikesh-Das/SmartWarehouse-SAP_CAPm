...
import { type EnhancedFEPropertyInfo, type default as TableAPI } from "sap/fe/macros/Table";
...
import type { BaseAggregationBindingInfo, PropertyBindingInfo } from "sap/ui/base/ManagedObject";
...
export default Object.assign({}, TableDelegateBase, {
	 ...
	 */
	_computeVisualSettingsForPropertyCriticality: function (propertyInfo: PropertyInfo, dataField: Record<string, unknown>) {
		if (dataField?.Criticality) {
			const widthCalculation = (propertyInfo.visualSettings as { widthCalculation?: { gap?: number } } | undefined)?.widthCalculation;
	...
	_computeVisualSettingsForPropertyWithValueHelp: function (table: Table, property: EnhancedFEPropertyInfo, metaModel: ODataMetaModel) {
	...
	_updatePropertyInfo: function (table: Table, properties: EnhancedFEPropertyInfo[], metaModel: ODataMetaModel) {
	...
	_fetchCustomPropertyInfo: function (columnInfo: CustomBasedTableColumn, table: Table, appComponent: AppComponent) {
		const resolveBinding = function (value: string | PropertyBindingInfo): string {
			if (value?.binding) {
	...
	_fetchPropertiesForEntity: async function (
		...
		return Promise.resolve(this.getColumnsFor(table))
			...
			.then(() => {
				fetchedProperties = this._updatePropertyInfo(
					table,
					fetchedProperties as EnhancedFEPropertyInfo[],
					appComponent.getModel().getMetaModel()
				);
				const tableAPI = this._getTableAPI(table);
	...
	_getCachedOrFetchPropertiesForEntity: async function (
		...
		return this._fetchPropertiesForEntity(table, entityTypePath, metaModel, appComponent).then(async function (
	...
	_handleSortersOnCurrenciesOrUoM: function (table: Table, bindingInfo: SerializedCollectionBindingInfo) {
		...
		if (sorters?.length) {
			...
			const pvSortKeys = new Set(
				(tableAPI.getTableDefinition().annotation?.sortConditions?.sorters ?? []).map((s: { key: string }) => s.key)
			);
					...
					.filter((fieldName: string) =>
						!allKkeysRequested
							? tableAPI.tableDefinition.requestAtLeast[fieldName].some(
									(origin) => origin !== "restriction" && origin !== "updateRestriction" && origin !== "semanticKey"
							  )
							: true
					)
			);
	...
	addItem: async function (
		...
		if (columnInfo.type === "Slot") {
			return getSlotColumn(sTableId, columnInfo as CustomElement<CustomBasedTableColumn>, false, tableAPI) as unknown as Control;
		}
		...
		const fnTemplateFragment = async (
			...
			return Promise.all([
				...
				sTableTypeCustomData = aCustomData[0] as string | undefined;
				sCreationModeCustomData = aCustomData[1] as string | undefined;
