...
import type { TableDesigntimeSetting, TableSettingsData } from "./Table.designtime";
 ...
 */
export function getDesigntimeProperties(): TableDesigntimeSetting[] {
	const tableType: TableDesigntimeSetting = {
		id: "type",
	...
	const showTitle: TableDesigntimeSetting = {
		id: "headerVisible",
	...
	const titleText: TableDesigntimeSetting = {
		id: "header",
	...
	const showFullScreen: TableDesigntimeSetting = {
		id: "enableFullScreen",
	...
	const enableExport: TableDesigntimeSetting = {
		id: "enableExport",
		...
		value: true,
		keyUser: true,
		applicableForTableTypes: ["ResponsiveTable", "GridTable", "TreeTable", "AnalyticalTable"]
	};

	/*
	 * Creation Mode Settings
	 * - Inline: Supported for ResponsiveTable, GridTable, TreeTable
	 * - NewPage: Supported for ResponsiveTable, GridTable, TreeTable
	 * - CreationRow: Not supported (only for SD)
	 * - External: Not supported yet, requires more config settings
	 * - InlineCreationRows: Supported for ResponsiveTable, GridTable
	 */
	const creationModeName: TableDesigntimeSetting = {
		id: "creationModeName",
		...
		enums: [
			{
				...
				applicableForTableTypes: ["ResponsiveTable", "GridTable", "TreeTable"]
			},
			{
				id: "Inline",
			...
			{ id: "CreationRow", name: "RTA_CONFIGURATION_TABLE_TYPE_ENUM_CREAT_ROW", applicableForTableTypes: [] },
			{ id: "External", name: "RTA_CONFIGURATION_TABLE_TYPE_ENUM_EXTERNAL", applicableForTableTypes: [] },
			{
				id: "InlineCreationRows",
				name: "RTA_CONFIGURATION_TABLE_TYPE_ENUM_INLINE_ROW",
				applicableForTableTypes: ["ResponsiveTable", "GridTable"]
			}
		],
		keyUser: true,
		applicableForTableTypes: ["ResponsiveTable", "GridTable", "TreeTable", "AnalyticalTable"]
	...
	const creationModeCreateAtEnd: TableDesigntimeSetting = {
	...
	const frozenColumnCount: TableDesigntimeSetting = {
		...
		value: 0,
		keyUser: true,
		applicableForTableTypes: ["GridTable", "TreeTable", "AnalyticalTable"]
	};

	const showCounts: TableDesigntimeSetting = {
		id: "showCounts",
	...
	const personalization: TableDesigntimeSetting = {
		id: "personalization",
	...
	const personalizationSort: TableDesigntimeSetting = {
	...
	const personalizationColumn: TableDesigntimeSetting = {
	...
	const personalizationFilter: TableDesigntimeSetting = {
	...
	const personalizationGroup: TableDesigntimeSetting = {
		...
		skipChange: true,
		keyUser: true,
		applicableForTableTypes: ["ResponsiveTable", "AnalyticalTable"]
	};

	const personalizationAggregate: TableDesigntimeSetting = {
	...
	const rowCount: TableDesigntimeSetting = {
		id: "rowCount",
	...
	const rowCountMode: TableDesigntimeSetting = {
		id: "rowCountMode",
	...
	const condensedTableLayout: TableDesigntimeSetting = {
	...
	const widthIncludingColumnHeader: TableDesigntimeSetting = {
	...
	const selectionMode: TableDesigntimeSetting = {
		id: "selectionMode",
	...
	const selectAll: TableDesigntimeSetting = {
		id: "selectAll",
	...
	const selectionLimit: TableDesigntimeSetting = {
		id: "selectionLimit",
	...
	const threshold: TableDesigntimeSetting = {
		id: "threshold",
	...
	const scrollThreshold: TableDesigntimeSetting = {
		id: "scrollThreshold",
	...
	const hierarchyQualifier: TableDesigntimeSetting = {
	...
	const enableAddCardToInsights: TableDesigntimeSetting = {
		...
		restrictedTo: [TemplateType.ListReport],
		type: "boolean",
		value: false,
		keyUser: true,
		applicableForTableTypes: ["ResponsiveTable"]
	};

	const beforeRebindTable: TableDesigntimeSetting = {
	...
	const selectionChange: TableDesigntimeSetting = {
		id: "selectionChange",
 ...
 */
export function getDesigntimeSettings(table: TableAPI): TableDesigntimeSetting[] {
	const instanceSpecificDesigntimeSettings: TableDesigntimeSetting[] = [];
	...
	getDesigntimeProperties().forEach((setting) => {
		if (
			!setting.restrictedTo ||
			(isOnListReport && setting.restrictedTo.includes(TemplateType.ListReport)) ||
			(!isOnListReport && setting.restrictedTo.includes(TemplateType.ObjectPage))
		) {
...
/**
 * Get the table type.
 * @param control MDCTable control
 * @returns The table type.
 */
export function getTableType(control: MDCTable): TableType | undefined {
	let tableType = control.data("tableType") as TableType | undefined;
	const enableAnalytics = control.data("enableAnalytics");
	if (enableAnalytics && enableAnalytics !== "false") {
		tableType = "AnalyticalTable" as TableType;
	}
	return tableType;
}

/**
 * Filter settings by the current table type, forcing AnalyticalTable when analytics is enabled.
 * @param control MDCTable control instance used to determine table type.
 * @param items Array of settings enriched with applicability info.
 * @returns Settings applicable for resolved table type.
 */
export function getSettingsApplicableForTableType(control: MDCTable, items: TableSettingsData[]): TableSettingsData[] {
	const tableType = getTableType(control);
	if (!tableType) {
		return items;
	}
	return items.filter((item) => item.applicableForTableTypes?.includes(tableType));
}
