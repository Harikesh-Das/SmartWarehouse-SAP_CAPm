import AiNoticeHelper from "sap/fe/controls/AiNoticeHelper";
...
const generateHeaderAIPopover = async (params: {
	...
	if (!content) {
		...
		if (params.contentFragmentName && view) {
			...
			return AiNoticeHelper.generatePopover({ parent: params.parent, placementType: params.placementType, content });
			...
			content = new Text({ text: resourceBundle?.getText("T_TABLE_COLUMN_AINOTICE_CONTENT") });
		}
	}
	return AiNoticeHelper.generatePopover({ parent: params.parent, placementType: params.placementType, content });
};
