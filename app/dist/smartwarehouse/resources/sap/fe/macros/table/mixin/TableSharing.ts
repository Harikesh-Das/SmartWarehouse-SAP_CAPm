...
import CollectionBindingInfoAPI, { type SerializedCollectionBindingInfo } from "sap/fe/macros/CollectionBindingInfo";
...
export default class TableSharing implements IInterfaceWithMixin {
	...
	async getDownloadUrlWithFilters(this: ITableBlock & TableSharing): Promise<string> {
		...
		const bindingInfo: SerializedCollectionBindingInfo = {
			path: binding.getPath(),
			filters: allRelevantFilters.length > 0 ? new Filter({ filters: allRelevantFilters, and: true }) : undefined
		};
		...
		const finalFilters = bindingInfo.filters ?? new Filter({ filters: [], and: true });
