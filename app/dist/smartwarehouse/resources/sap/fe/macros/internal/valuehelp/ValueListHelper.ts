...
import { getViewDataForTemplate, putDefaultQualifierFirst } from "sap/fe/macros/internal/valuehelp/ValueHelpUtils";
import ValueHelpFilterBarCreator from "sap/fe/macros/valuehelp/ValueHelpFilterBarCreator";
...
export type AnnotationsForProperty = {
...
function _getDefaultSortPropertyName(valueListInfo: AnnotationValueListType): string | undefined {
	...
	const foundElement = valueListInfo.Parameters.find(function (element) {
		return (
			...
			!element.ValueListProperty.includes("/") &&
			!(
...
const ValueListHelper = {
	...
	entityIsSearchable: function (
		...
		const searchSupported =
				propertyAnnotations.ValueList?.SearchSupported ??
				propertyAnnotations["@com.sap.vocabularies.Common.v1.ValueList"]?.SearchSupported,
			searchable =
				collectionAnnotations.SearchRestrictions?.Searchable ??
				collectionAnnotations["@Org.OData.Capabilities.V1.SearchRestrictions"]?.Searchable;
	 ...
	 * @private
	 */
	_getDialogTitle: function (valueHelp: ValueHelp, valuehelpLabel: string | undefined, isUnitValueHelp = false): string {
	 ...
	 * @private
	 */
	resolveFilterBarValuePath: function (
		localDataPropertyPath: string,
		contextMetaPath: MetaPath<unknown>,
		newPath: MetaPath<Property>
	): string | undefined {
	...
	_getVhParameter: function (
		...
		if (conditionModel && conditionModel.length > 0) {
			if (
				...
				valueHelp?.getParent()?.getParent()?.getParent()?.isA<FilterBarAPI>("sap.fe.macros.FilterBar") === true &&
				bindingContext &&
				ValueListHelper._parameterIsA(parameter, [
					CommonAnnotationTypes.ValueListParameterIn,
					CommonAnnotationTypes.ValueListParameterInOut
	...
	_enrichPath: function (
		...
		isPropertyNotNullable: boolean,
		isAdaptationFilterBar: boolean,
		collectionPath: string,
		...
		if (
			!path.key &&
			ValueListHelper._parameterIsA(parameter, ParameterTypes as CommonAnnotationTypes[]) &&
	 ...
	 */
	_isNavigationProperty: function (
		metaModel: ODataMetaModel,
		entitySet: string,
		propertyName: string,
		payloadPropertyPath: string,
		...
		const targetType = (resolvedPath?.target as { targetType?: EntityType })?.targetType;
	...
	_processParameters: function (
		...
		let vhKeys: string[] = [];
		let isAdaptationFilterBar: boolean;
		...
		for (const parameter of annotationValueListType.Parameters) {
			...
			const propertyPath = `/${annotationValueListType.CollectionPath}/${parameter.ValueListProperty}`,
				...
				propertyAnnotations = payload?.externalIdPath
					? ((metaModelVH.getObject(`/${annotationValueListType.CollectionPath}/${payload?.externalIdPath.split("/").pop()}@`) ||
			...
			const isPropertyNotNullable = property?.$Nullable === false,
				field = (valueHelp as ValueHelp)?.getControl?.() as FieldBase;
			...
			if (property) {
				...
				ValueListHelper._enrichPath(
					...
					isPropertyNotNullable,
					isAdaptationFilterBar,
					`${annotationValueListType.CollectionPath}`,
					metaModel,
			...
			if (parameter.$Type === AnnotationValueListParameterConstant) {
				const constantParameter: InOutParameter = {
					parmeterType: parameter.$Type,
	 ...
	 * @private
	 */
	_filterValueHelpQualifiers: function (
		valueHelp: ValueHelp | undefined,
		propertyPath: string,
		valueHelpQualifiers: string[],
		valueListByQualifier: Record<string, AnnotationValueListType>
	): string[] {
		if (!valueHelp) {
			return valueHelpQualifiers;
		}

		const control: Control | undefined = valueHelp.getControl?.();
		const isFilterField = control?.isA<FilterField>("sap.ui.mdc.FilterField");

		// Check if this FilterField contains a visual filter
		if (isFilterField === true) {
			const visualFilterQualifier = control?.getCustomData().find((cdv: CustomData) => cdv.getKey() === "valueListQualifier");
			let visualFilterQualifierValue: string;
		...
		// SECOND: For non-FilterFields, filter out qualifiers with Chart annotations
			...
			return valueHelpQualifiers.filter((qualifier) => {
				...
				const hasChartAnnotation = presentationVariant?.Visualizations?.some((visualization) => {
					const annotationPath = (visualization as { $AnnotationPath?: string }).$AnnotationPath;
	...
	getValueListInfo: async function (
		...
		try {
			...
			valueHelpQualifiers = ValueListHelper._filterValueHelpQualifiers(
				valueHelp,
				propertyPath,
				valueHelpQualifiers,
				valueListByQualifier
			);
	...
	_templateFragment: async function <T extends Table | MdcInnerTable | MdcFilterBar | ValueHelp>(
		...
		const fragmentDefinition = await XMLPreprocessor.process(
			await XMLTemplateProcessor.loadTemplatePromise(fragmentName, "fragment"),
			{ name: fragmentName },
			{
				bindingContexts: {
	 ...
	 */
	_createValueHelpFilterBar: function (
		valueListInfo: ValueListInfo,
		sourceModelDialog: JSONModel,
		valueHelp: ValueHelp
		...
		return templateComponent.runAsOwner(() => {
			return new ValueHelpFilterBarCreator({
				...
				parentVHControl: valueHelp as unknown as ValueHelpFilterBarCreator["parentVHControl"],
			...
			if (isTypeahead) {
				...
				if (field) {
					const title = CommonUtils.getTranslatedTextFromExpBindingString(
						ValueListHelper._getDialogTitle(valueHelp, selectedInfo.valueListInfo?.Label, payload.isUnitValueHelp),
						field
					);
	...
	isMultiValueField: function (
		...
		const collectionPath = (oValueList as unknown as AnnotationValueListType).CollectionPath;
		const metaModel = (oValueList as unknown as AnnotationValueListType).$model?.getMetaModel();
		if (!metaModel) {
			return false;
		}
