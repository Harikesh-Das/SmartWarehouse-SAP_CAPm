...
sap.ui.define(["sap/fe/base/BindingToolkit", "sap/fe/core/controllerextensions/collaboration/CollaborationCommon", "sap/fe/core/converters/MetaModelConverter", "sap/fe/core/converters/annotations/DataField", "sap/fe/core/formatters/CollaborationFormatter", "sap/fe/core/formatters/ValueFormatter", "sap/fe/core/helpers/BindingHelper", "sap/fe/core/helpers/MetaModelFunction", "sap/fe/core/helpers/ModelHelper", "sap/fe/core/helpers/StableIdHelper", "sap/fe/core/helpers/TitleHelper", "sap/fe/core/helpers/TypeGuards", "sap/fe/core/templating/DataModelPathHelper", "sap/fe/core/templating/PropertyHelper", "sap/fe/core/templating/SemanticObjectHelper", "sap/fe/core/templating/UIFormatters", "sap/fe/macros/field/FieldTemplating", "sap/fe/macros/internal/valuehelp/AdditionalValueFormatter", "sap/fe/macros/situations/SituationsIndicator", "sap/ui/mdc/enums/FieldEditMode", "../../field/FieldHelper"], function (BindingToolkit, CollaborationCommon, MetaModelConverter, DataField, CollaborationFormatters, valueFormatters, BindingHelper, MetaModelFunction, ModelHelper, StableIdHelper, TitleHelper, TypeGuards, DataModelPathHelper, PropertyHelper, SemanticObjectHelper, UIFormatters, FieldTemplating, additionalValueFormatter, SituationsIndicator, FieldEditMode, FieldHelper) {
  ...
  function setUpField(field, controlConfiguration, viewDataModel, internalModel, appComponent, isReadOnlyInitial, metaPath, contextPath) {
    ...
    if (resultField.editMode !== "Display") {
    ...
    if (resultField.displayStyle && !aDisplayStylesWithoutPropText.includes(resultField.displayStyle) && resultField.dataModelPath.targetObject) {
      resultField.text = resultField.text ?? FieldTemplating.getTextBinding(resultField.dataModelPathExternalID || resultField.dataModelPath, resultField.formatOptions, true, undefined, resultField.description, resultField.value);
    } else {
      resultField.text = "";
    }
  ...
  function computeCollaborationProperties(field) {
    const computedEditableExpression = UIFormatters.getEditableExpressionAsObject(field.propertyForFieldControl, field.convertedMetaPath, field.dataModelPath, undefined, undefined, field.formatOptions.isAnalytics);
    if (ModelHelper.isCollaborationDraftSupported(field.odataMetaModel) && field.editMode !== FieldEditMode.Display) {
      ...
      const editMode = compileExpression(ifElse(equal(pathInModel("/hasCollaborationAuthorization", "ui"), false), "Display", ifElse(collaborationExpression, constant("ReadOnly"), ifElse(and(UI.IsInactive, !!field.hasPropertyInsertRestrictions), "Display", field.editModeAsObject))));
      field.collaborationEnabled = true;
      field.editableExpression = editableExpression;
      field.computedEditMode = editMode;
    } else {
      field.editableExpression = field.editMode === FieldEditMode.Display && field.formatOptions.fieldMode === "nowrapper" ? compileExpression(false) : compileExpression(computedEditableExpression);
    }
  }
  ...
  function computeEditableExpressions(field) {
    ...
    field.requiredExpression = field.editMode === FieldEditMode.Display && field.formatOptions.fieldMode === "nowrapper" ? undefined : requiredExpression;
  }
  ...
  function computeEditMode(field) {
    if (field.editMode !== undefined && field.editMode !== null) {
      ...
      field.editModeAsObject = UIFormatters.getEditMode(field.propertyForFieldControl, field.dataModelPath, measureReadOnly, true, field.convertedMetaPath, UI.IsEditable, field.formatOptions.isAnalytics);
  ...
  function getFormatOptions(field) {
    return {
      ...
      imageFitType: field.formatOptions.imageFitType,
      enableEnlargeImage: field.formatOptions.enableEnlargeImage
    };
  }
  ...
  function getDecimalPadding(appComponent, property) {
    ...
    if (property?.annotations?.Measures?.ISOCurrency) {
      // Return the manifest override when set; otherwise return undefined so that
      // DynamicCurrency computes the correct padding from the codelist at runtime.
      return manifest?.macros?.table?.currency?.decimalPadding;
    }
    return manifest?.macros?.table?.unitOfMeasure?.decimalPadding ?? 3;
  }
  ...
  function setUpDisplayStyle(field, dataField, dataModelPath, internalModel, appComponent) {
    ...
    if (property.annotations?.UI?.IsImageURL && isImageURL(property)) {
    ...
    switch (dataField.$Type) {
      ...
      case "com.sap.vocabularies.UI.v1.DataFieldWithIntentBasedNavigation":
        {
          const ibnField = dataField;
          const ibnEnabledPath = pathInModel(`ibn/${ibnField.SemanticObject}-${ibnField.Action}/bEnabled`, "internal");
          resultField.IBNEnabled = compileExpression(ibnEnabledPath);
  ...
  function setUpObjectIdentifierTitleAndText(field, propertyDataModelObjectPath) {
    ...
    // Apply fallback logic for LabelSemanticKey when using composite display modes
    if (semanticStyle === "Label" && (displayMode === "DescriptionValue" || displayMode === "ValueDescription")) {
      const titleWithFallback = buildSemanticKeyTitleWithFallback(propertyDataModelObjectPath, displayMode);
      if (titleWithFallback) {
        field.identifierTitle = titleWithFallback;
      }
    }
  }

  /**
   * Builds an enhanced title binding for LabelSemanticKey that shows "Unnamed Object (KEY)"
   * when the description is empty but the key has a value (non-create mode).
   *
   * This prevents the field from appearing completely empty when only the label/description
   * is missing but the key value exists.
   *
   * Note: Intentionally does NOT mirror ObjectIdentifier create-mode semantics to preserve
   * 'New Object' display behavior during creation.
   * @param propertyPath DataModelObjectPath for the semantic key property
   * @param displayMode Active display mode determining description/value order
   * @returns Compiled binding expression with fallback logic, or undefined if enhancement not needed
   * @internal
   */
  _exports.setUpObjectIdentifierTitleAndText = setUpObjectIdentifierTitleAndText;
  function buildSemanticKeyTitleWithFallback(propertyPath, displayMode) {
    // Only enhance composite display modes where description may be empty
    if (displayMode !== "DescriptionValue" && displayMode !== "ValueDescription") {
      return undefined;
    }
    const property = propertyPath.targetObject;
    const textAnnotation = property?.annotations?.Common?.Text;

    // No text annotation means key is already displayed - no enhancement needed
    if (!textAnnotation) {
      return undefined;
    }

    // Build binding expression parts
    const relativeLocation = getRelativePaths(propertyPath);
    const descriptionExpression = getExpressionFromAnnotation(textAnnotation, relativeLocation);
    const keyPathExpression = pathInModel(getContextRelativeTargetObjectPath(propertyPath));
    const keyExpression = formatWithTypeInformation(property, keyPathExpression);

    // Arrange parts based on display mode and format accordingly
    const standardTitleExpression = displayMode === "DescriptionValue" ? formatResult([descriptionExpression, keyExpression], valueFormatters.formatTitle) : formatResult([keyExpression, descriptionExpression], valueFormatters.formatTitle);

    // Build fallback: "Unnamed Object (KEY)"
    const unnamedObjectLabel = pathInModel("T_ANNOTATION_HELPER_DEFAULT_HEADER_TITLE_NO_HEADER_INFO", "sap.fe.i18n");
    const fallbackTitleExpression = formatResult([unnamedObjectLabel, keyExpression], valueFormatters.formatTitle);

    // Apply fallback when: description is empty AND key has value AND not in create mode
    const shouldUseFallback = and(isEmpty(descriptionExpression), not(isEmpty(keyExpression)), not(UI.IsCreateMode));
    return compileExpression(ifElse(shouldUseFallback, fallbackTitleExpression, standardTitleExpression));
  }
  _exports.buildSemanticKeyTitleWithFallback = buildSemanticKeyTitleWithFallback;
  ...
  const FieldStructureHelper = {
    ...
    setUpValueState,
    buildSemanticKeyTitleWithFallback
  };
