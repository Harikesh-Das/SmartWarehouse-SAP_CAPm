 ...
 */
sap.ui.define(["sap/fe/core/CommonUtils", "sap/fe/core/formatters/Scope", "sap/fe/core/formatters/ValueFormatter", "sap/fe/core/helpers/ModelHelper", "sap/fe/core/helpers/ResourceModelHelper", "sap/fe/macros/internal/valuehelp/AdditionalValueHelper", "sap/ui/core/library"], function (CommonUtils, Scope, valueFormatters, ModelHelper, ResourceModelHelper, AdditionalValueHelper, library) {
  "use strict";
  ...
  getGrowing.__functionName = "._formatters.AdditionalValueFormatter.bind($control)#getGrowing";
  ...
  function formatValueState(recommendationValue, recommendationDescription, all_recommendations, isEditable, sourcepath, fieldValue, fieldTextValue) {
    ...
    const associatedControl = this.getParent();
    const fieldBindingContext = associatedControl?.getBindingContext();
    ...
    const presentValueState = associatedControl?.getValueState();
    ...
    const metaPath = ModelHelper.getMetaPathForContext(fieldBindingContext);
    const entitySetName = metaPath?.startsWith("/") ? metaPath.substring(1) : metaPath;
    const recommendationsText = ResourceModelHelper.getResourceModel(associatedControl).getText("RECOMMENDATIONS_DATA_INFO", undefined, entitySetName);
    ...
    if (!fieldValue && !fieldTextValue && recommendationsStateApplicable) {
      if (!fieldBindingContext.getObject() && fieldBindingContext.getBinding()?.isA?.("sap.ui.model.odata.v4.ODataListBinding")) {
        ...
        _setValueStateForFieldWithNoDataContext.call(associatedControl, recommendationValue, recommendationDescription, fieldBindingContext, all_recommendations, sourcepath, recommendationsText);
      } else {
        const {
          valueState,
          valueStateText
        } = _getValueStateForRecommendationField(recommendationValue, recommendationDescription, all_recommendations, fieldBindingContext, sourcepath, recommendationsText, associatedControl);
  ...
  formatPlaceholder.__functionName = "._formatters.AdditionalValueFormatter.bind($control)#formatPlaceholder";
  ...
  // @ts-ignore
  Scope._formatters.AdditionalValueFormatter = additionalValueFormatter;
