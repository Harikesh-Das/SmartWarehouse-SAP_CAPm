...
export type FieldBlockProperties = {
	...
	change: undefined | EventHandler<CheckBox$SelectEvent | InputBase$ChangeEvent | Field$ChangeEvent | RatingIndicator$ChangeEvent>;
	metaPath: Context;
	contextPath: Context;
	isPublicField: boolean;
	...
	label?: CompiledBindingToolkitExpression;
	...
	mdcFieldDataType?: string;
	showTimezone?: boolean;
	timezone?: CompiledBindingToolkitExpression;
	...
	IBNEnabled?: CompiledBindingToolkitExpression;
	_apiId?: string;
	entityType?: Context;
	...
	eventHandlers: {
		change: EventHandler<CheckBox$SelectEvent | InputBase$ChangeEvent | Field$ChangeEvent | RatingIndicator$ChangeEvent>;
		liveChange: EventHandler;
...
export function computeCollaborationProperties(field: FieldBlockProperties): void {
	const computedEditableExpression = UIFormatters.getEditableExpressionAsObject(
		...
		undefined,
		undefined,
		field.formatOptions.isAnalytics
	);
	if (ModelHelper.isCollaborationDraftSupported(field.odataMetaModel) && field.editMode !== FieldEditMode.Display) {
		...
		const editMode = compileExpression(
			ifElse(
				equal(pathInModel("/hasCollaborationAuthorization", "ui"), false),
				"Display",
				ifElse(
		...
		field.editableExpression =
			field.editMode === FieldEditMode.Display && field.formatOptions.fieldMode === "nowrapper"
				? compileExpression(false)
				: compileExpression(computedEditableExpression);
	}
}
...
export function computeEditableExpressions(field: FieldBlockProperties): void {
	...
	field.requiredExpression =
		field.editMode === FieldEditMode.Display && field.formatOptions.fieldMode === "nowrapper" ? undefined : requiredExpression;
}
...
export function setUpDisplayStyle(
	...
	switch (dataField.$Type as string) {
		...
		case UIAnnotationTypes.DataFieldWithIntentBasedNavigation: {
			const ibnField = dataField as DataFieldWithIntentBasedNavigation;
...
export function setUpObjectIdentifierTitleAndText(
	...
	if (semanticStyle === "Label" && (displayMode === "DescriptionValue" || displayMode === "ValueDescription")) {
		const titleWithFallback = buildSemanticKeyTitleWithFallback(propertyDataModelObjectPath, displayMode as DisplayMode);
 ...
 * @internal
 */
export function buildSemanticKeyTitleWithFallback(
	...
	const property = propertyPath.targetObject as Property;
	...
	const standardTitleExpression =
		displayMode === "DescriptionValue"
			? formatResult([descriptionExpression, keyExpression], valueFormatters.formatTitle)
			: formatResult([keyExpression, descriptionExpression], valueFormatters.formatTitle);
