...
import { getAssociatedTextPropertyPath, hasValueHelpWithFixedValues } from "sap/fe/core/templating/PropertyHelper";
...
import { getMultiCurrencyLink, getMultipleLinesForDataField, getTextAlignment } from "sap/fe/macros/field/FieldTemplating";
...
import { createCustomDatas } from "../../TSXUtils";
...
const EditStyle = {
	...
	getInputTemplate(field: FieldBlockProperties): string {
		...
		if (field.formatOptions.isAnalytics ?? false) {
			return (
				<ConditionalWrapper condition={UIFormatter.isTotalOutdated() as unknown as boolean}>
					{{
						contentTrue: <Text emptyIndicatorMode="On" text="" />,
	...
	getInputWithValueHelpTemplate(field: FieldBlockProperties): string {
		...
		if (this.showAsRadioButton(field) !== true) {
			const customData = createCustomDatas([
				{ key: "sourcePath", value: field.dataSourcePath },
				{ key: "valueState", value: field.valueState }
			]);
			const fieldControl = (
				<Field
					core:require="{Field: 'sap/fe/macros/Field'}"
				...
				return (
					<ConditionalWrapper condition={UIFormatter.isTotalOutdated() as unknown as boolean}>
						{{
							contentTrue: <Text emptyIndicatorMode="On" text="" />,
							contentFalse: (
								<ConditionalWrapper condition={field.hasValidAnalyticalCurrencyOrUnit as unknown as boolean}>
									{{
