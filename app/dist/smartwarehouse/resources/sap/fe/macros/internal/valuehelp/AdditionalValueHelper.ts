...
const additionalValueHelper = {
	...
	addDraftFilter: function (
		...
		if (valueHelpBindingInfo.path && metaModel) {
			// Check if IsActiveEntity is already part of the ValueList qualifiers
			const qualifierKey = payload?.valueHelpQualifier ?? "";
			const qualifierInfo = payload?.qualifiers[qualifierKey];
			const hasIsActiveEntityInQualifiers = qualifierInfo?.vhProperties?.includes("IsActiveEntity") ?? false;

			// Skip adding the filter if IsActiveEntity is already in qualifiers
			if (hasIsActiveEntityInQualifiers) {
				return;
			}
	...
	updateFinished: async function name(event: UI5Event): Promise<void> {
		...
		// More data should only be fetched when :
		// 1. total has reached all the currently available contexts
		// 2. the actual no of items is the same as the total.
		const fetchMoreData = allListBinding && allListBinding.getAllCurrentContexts().length === total && actual === total;
		// as user scrolls down and once we reach end of the list, we fetch the next set of data from the backend
		if (fetchMoreData && reason === "Growing") {
			...
			additionalData.forEach(async (contextData: FieldDataType) => {
				...
				try {
					...
					if (error) {
						// Since we are creating transient contexts hence the errors should not be logged as errors but rather info.
						Log.info(
							"context.created() failed - since we are creating transient contexts which implies that the create will be cancelled",
							(error as ContextErrorType).name
						);
					}
				}
			});
	...
	addActiveRecommendation: function (context: V4Context | Context, propertyPath: string): void {
		...
		// Check if getLocalAnnotationModel method exists on the model
		if (typeof model.getLocalAnnotationModel !== "function") {
			return;
		}
...
export { AdditionalValueGroupKey, additionalValueHelper, RecommendationValueType };
