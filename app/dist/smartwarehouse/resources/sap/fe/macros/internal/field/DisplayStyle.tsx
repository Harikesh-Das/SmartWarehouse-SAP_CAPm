...
import { getTooltipValue, hasValidAnalyticalCurrencyOrUnit } from "sap/fe/core/templating/UIFormatters";
...
const DisplayStyle = {
	...
	getTextTemplateForSuppressedLink(field: FieldBlockProperties): string | undefined {
	...
	getLinkTemplate(field: FieldBlockProperties): string {
		...
		switch (field.convertedMetaPath.$Type as string) {
						...
						{{
							customData: (
								<CustomData
									key="IBNData"
									value={{
										semanticObject: (field.convertedMetaPath as DataFieldWithIntentBasedNavigation).SemanticObject,
										action: (field.convertedMetaPath as DataFieldWithIntentBasedNavigation).Action
									}}
								/>
							)
						}}
			...
			case UIAnnotationTypes.DataFieldWithUrl: {
				const html5LinkTarget =
					field.convertedMetaPath.annotations?.HTML5?.LinkTarget ?? field.property.annotations.HTML5?.LinkTarget;
	...
	getObjectStatus(field: FieldBlockProperties): string {
		...
		if (field.formatOptions.isAnalytics && field.hasUnitOrCurrency) {
			const content = FieldTemplating.getMultiCurrencyLink(field, <Text text={field.text} textAlign="End"></Text>);
			objectStatus = (
