...
export default class MessageButton extends BuildingBlock<Button> {
	 ...
	 */
	@event()
	visibilityChange!: EventHandler<UI5Event<{ visible: boolean }, MessageButton>>;
	...
	async processMessages(): Promise<void> {
		const messages: Message[] = messageHandling.getMessages(true);

		const messageDetails = messages.reduce((reducedMessages: MessageDetail[], message): MessageDetail[] => {
			const technicalDetails = message.getTechnicalDetails() as undefined | { [key: string]: unknown };
	...
	handleActiveTitlePress(messageItem?: MessageItem): void {
		...
		if (messageDetail.activeTitleHandler) {
			...
			for (const controlId of controlIds) {
				const control = UI5Element.getElementById(controlId) as Control | undefined;
	 ...
	 */
	private focusControl(control: Control): void {
	...
	createContent(): Button {
		const button = new Button({ visible: false, press: this.toggleMessagePopover.bind(this) });
