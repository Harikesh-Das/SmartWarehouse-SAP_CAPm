...
import { computeSelectionVariantAndURLParameters, getDynamicPathFromSemanticObject } from "sap/fe/core/templating/SemanticObjectHelper";
...
const SimpleLinkDelegate = Object.assign({}, LinkDelegate) as LinkDelegate & {
	...
	_findLabelControlInQuickViewPanel: (oPanel: UI5Element) => { labelledByControl: Title | Text | undefined; linkControl: Link | undefined };
		...
		try {
			...
			if (mLineContext) {
				...
				mLineContextData = oController._intentBasedNavigation.prepareContextForExternalNavigation(
					mLineContextData,
					mLineContext,
					this.payload.entityType ? [this.payload.entityType] : []
				);
			}
...
/**
 * Finds the appropriate label control within the QuickView panel for accessibility.
 * Performs a single recursive scan returning the first Title/Text as aria-labelledby candidate
 * and the first Link as a fallback. Links are excluded from labelledByControl — see fetchPopoverTitle.
 * @private
 * @param panel The panel control to search
 * @returns The first Title or Text control and the first Link if present
 */
SimpleLinkDelegate._findLabelControlInQuickViewPanel = function (
	panel: UI5Element
): { labelledByControl: Title | Text | undefined; linkControl: Link | undefined } {
	const allControls = panel.findAggregatedObjects(true) as unknown as UI5Element[];
	let labelledByControl: Title | Text | undefined;
	let linkControl: Link | undefined;

	for (const control of allControls) {
		if (!labelledByControl && (control.isA("sap.m.Title") || (control as UI5Element).isA("sap.m.Text"))) {
			labelledByControl = control as Title | Text;
		} else if (!linkControl && control.isA("sap.m.Link")) {
			linkControl = control as Link;
		}
		if (labelledByControl && linkControl) {
			break;
		}
	}

	return { labelledByControl, linkControl };
};

/**
 * Delegate method to fetch popover title and labelledBy control.
 * When the title is a non-interactive Title or Text control, it is returned as oLabelledByControl
 * so MDC sets aria-labelledby on the popover. When the title is an interactive Link (primary semantic
 * object action present), oLabelledByControl is undefined so MDC falls back to aria-label — this
 * prevents screen readers from announcing the popover name twice (once as dialog label via
 * aria-labelledby, once when focus lands on the Link as the first interactive element).
 * @param link
 * @param panel
 * @returns A title and a labelledBy control.
 */
// eslint-disable-next-line @typescript-eslint/require-await -- MDC LinkDelegate interface mandates a Promise return; implementation is synchronous
SimpleLinkDelegate.fetchPopoverTitle = async function (
	link: MdcLink | undefined,
	panel: Control
	...
	const { labelledByControl, linkControl } = SimpleLinkDelegate._findLabelControlInQuickViewPanel(
		panel as unknown as UI5Element
	);

	let title = "";
	if (labelledByControl) {
		title = labelledByControl.getText() || "";
	} else if (linkControl) {
		// Title is rendered as a Link — extract text for aria-label but do not set oLabelledByControl
		title = linkControl.getText() || "";
	}

	return { sTitle: title, oLabelledByControl: labelledByControl };
};
