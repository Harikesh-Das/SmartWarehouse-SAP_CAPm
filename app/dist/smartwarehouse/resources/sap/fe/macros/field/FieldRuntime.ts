...
const FieldRuntime = {
	 ...
	 */
	createValueListFilters: function (
		valueListInfo: AnnotationValueListType,
		propertyId: string,
		propertyValue: string,
		bindingContext: BaseContext | null | undefined
	): Filter[] {
		...
		if (bindingContext && valueListInfo.Parameters) {
			const relevantParameters = valueListInfo.Parameters.filter((parameter) => {
				const isInOrInOut =
					parameter.$Type === "com.sap.vocabularies.Common.v1.ValueListParameterIn" ||
					parameter.$Type === "com.sap.vocabularies.Common.v1.ValueListParameterInOut";
				return (
					isInOrInOut &&
					parameter.LocalDataProperty?.$PropertyPath &&
					parameter.ValueListProperty &&
					parameter.ValueListProperty !== propertyId
				);
			});
			...
			return oMetaModel
				...
				.then(function (mValueListInfo) {
					...
					if (oTextAnnotation && oTextAnnotation.$Path) {
						...
						const filters = FieldRuntime.createValueListFilters(
							oValueListInfo,
							oParamWithKey.ValueListProperty,
							sPropertyValue,
							bindingContext
						);
	...
	displayAggregateDetails: async function (event: Event, aggregatedPropertyPath?: string): Promise<void> {
		...
		const popover: ResponsivePopover = await Util.createOrUpdateMultiUnitPopover(popoverId, {
		...
		popoverUIContext?.setProperty("", { isEditable: false });
