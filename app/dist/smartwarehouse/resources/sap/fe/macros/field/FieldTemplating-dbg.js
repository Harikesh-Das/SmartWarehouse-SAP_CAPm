 ...
 */
sap.ui.define(["sap/fe/base/BindingToolkit", "sap/fe/core/converters/annotations/DataField", "sap/fe/core/formatters/ValueFormatter", "sap/fe/core/helpers/TypeGuards", "sap/fe/core/templating/CommonFormatters", "sap/fe/core/templating/DataFieldFormatters", "sap/fe/core/templating/DataModelPathHelper", "sap/fe/core/templating/PropertyHelper", "sap/fe/core/templating/SemanticObjectHelper", "sap/fe/core/templating/UIFormatters", "sap/m/AvatarShape", "sap/m/Link", "sap/ui/model/json/JSONModel", "./FieldHelper", "sap/fe/base/jsx-runtime/jsx"], function (BindingToolkit, DataField, valueFormatters, TypeGuards, CommonFormatters, DataFieldFormatters, DataModelPathHelper, PropertyHelper, SemanticObjectHelper, UIFormatters, AvatarShape, Link, JSONModel, FieldHelper, _jsx) {
  "use strict";
  ...
  var isActionParameter = TypeGuards.isActionParameter;
  ...
  const getTextBinding = function (inputDataModelPath, fieldFormatOptions) {
    ...
    let descriptionBinding = arguments.length > 4 ? arguments[4] : undefined;
    let valueBinding = arguments.length > 5 ? arguments[5] : undefined;
    if (valueBinding && typeof valueBinding === "string") {
      return valueBinding;
    }
    ...
    } else if (valueBinding && typeof valueBinding === "object") {
      oTargetBinding = CommonFormatters.getBindingWithTextArrangement(propertyDataModelObjectPath, oPropertyBindingExpression, fieldFormatOptions, customFormatter, descriptionBinding, valueBinding);
    } else {
  ...
  const getValueBinding = function (oPropertyDataModelObjectPath, fieldFormatOptions) {
    ...
    if (isProperty(targetObject) || isActionParameter(targetObject)) {
      ...
      if (isPathInModelExpression(oBindingExpression)) {
        if (targetObject.annotations?.Communication?.IsEmailAddress) {
        ...
        } else if (isProperty(targetObject) && targetObject.annotations?.Common?.Masked?.valueOf()) {
          oBindingExpression = formatWithTypeInformation(targetObject, oBindingExpression);
          oBindingExpression.formatOptions = {
            ...oBindingExpression.formatOptions,
            style: "password"
          ...
          if (String(fieldFormatOptions.isCurrencyOrUnitAligned) === "true") {
            if (decimalPadding !== undefined) {
              targetFormatOptions["decimalPadding"] = decimalPadding;
            } else if (targetObject.annotations?.Measures?.ISOCurrency) {
              // No static override: signal DynamicCurrency to compute padding from the codelist.
              // computeDecimalPadding is consumed by getBindingWithUnitOrCurrency to select the
              // type and is not passed to addTypeInformation, so it never appears in the XML.
              targetFormatOptions["computeDecimalPadding"] = true;
            }
          }
  ...
  const setEditStyleProperties = function (oProps, oDataField, oDataModelPath, onlyEditStyle) {
    ...
    if (_hasValueHelpToShow(oProperty, oProps.formatOptions?.measureDisplayMode) || oProps.formatOptions?.measureDisplayMode !== "Hidden" && (oProperty.annotations?.Measures?.ISOCurrency || oProperty.annotations?.Measures?.Unit && !hasStaticPercentUnit(oProperty))) {
      if (!onlyEditStyle) {
        ...
        const isJSONModelStringBinding = typeof oProps.value === "string" && /\{\w+>.+\}/.test(oProps.value);
        const isJSONModelUsedForValue = !!oProps.value && (isJSONModelStringBinding || typeof oProps.value === "object" && oProps.value !== null && "model" in oProps.value);
        if (isJSONModelUsedForValue) {
          if (oProps.description) {
            ...
            if (isJSONModelStringBinding) {
              // For string bindings without an explicit description, show only the value
              oProps.formatOptions.displayMode = "Value";
            }
          }
          if (PropertyHelper.isGuid(oProperty)) {
            oProps.mdcFieldDataType = "sap.ui.model.odata.type.Guid";
          }
        } else {
          // Only set textBindingExpression when TextArrangement is NOT TextSeparate
          const textAnnotation = oProperty.annotations?.Common?.Text;
          const textArrangementAnnotation = textAnnotation?.annotations?.UI?.TextArrangement?.valueOf();
          if (textArrangementAnnotation !== "UI.TextArrangementType/TextSeparate") {
    ...
    switch (oProperty.type) {
      ...
      case "Edm.DateTimeOffset":
        ...
        if (!oProperty.annotations?.Common?.Timezone) {
          ...
          const timezonePath = PropertyHelper.getAssociatedTimezonePropertyPath(oProperty);
          if (timezonePath && !timezonePath.includes("/")) {
            // The UI5 DateTimePicker placeholder does not update when timezone is bound via OData alone (DINC0895466).
            // Explicitly providing the timezone binding fixes the placeholder rendering.
            // Navigation paths (containing "/") are relative to the annotated property's entity type and cannot be
            // bound directly against the DateTimePicker's OData binding context — doing so generates requests
            // against the wrong entity and causes HTTP 404 errors (DINC0934996).
            oProps.timezone = compileExpression(pathInModel(timezonePath));
          }
        }
        return;
      case "Edm.Boolean":
  ...
  /**
   * Generates a Link control that displays aggregation details for analytical tables with multiple currencies/units.
   * @param field Reference to the current field instance
   * @param currencyOrUnit The content control to be cloned into the Link's dependents
   * @param visibleExpression Optional visibility expression for the Link
   * @returns A Link control with "Show Details" text
   */
  _exports.getAvatarShape = getAvatarShape;
  const getMultiCurrencyLink = function (field, currencyOrUnit, visibleExpression) {
    return _jsx(Link, {
  ...
  _exports.getMultiCurrencyLink = getMultiCurrencyLink;
  return _exports;
}, false);
