...
import type { CheckBox$SelectEventParameters } from "sap/m/CheckBox";
...
import type { InputBase$ChangeEventParameters } from "sap/m/InputBase";
...
const FieldRuntimeHelper = {
	...
	fetchRecommendations: function (field: Control, controller: PageController | ExtensionAPI): void {
		...
		// Recommendations require an OData V4 context. Skip if the field is bound to a non-OData model (e.g. JSON).
		if (!fieldBindingContext || !(fieldBindingContext as unknown as { isA: Function }).isA("sap.ui.model.odata.v4.Context")) {
			return;
		}
		...
		const odataContext = fieldBindingContext?.isA?.("sap.ui.model.odata.v4.Context") ? (fieldBindingContext as Context) : undefined;
		const metaPath = odataContext ? ModelHelper.getMetaPathForContext(odataContext) : "";
	 ...
	 */
	getFieldStateOnChange: function (
		oEvent: UI5Event<
			| { value: string | number | boolean | undefined; isValid: boolean }
			...
			CheckBox | InputBase | mdcField | RatingIndicator | Field
		>
	): {
		field: Control;
		state: {
		...
		if (oSourceField?.isA<mdcField>("sap.ui.mdc.Field")) {
			let bIsValid: boolean =
				(oEvent as Field$ChangeEvent).getParameter("valid") ||
				(oEvent as UI5Event<{ value: string | number | boolean | undefined; isValid: boolean }, mdcField>).getParameter("isValid");
	...
	_callSideEffectsForStream: function (
		...
		oFEController.sideEffects.handleFieldChange(oEvent, FieldRuntimeHelper.getFieldStateOnChange(oEvent).state["validity"]);
	},
	...
	/**
	 * Updates the enabled state for Intent-Based Navigation links by calling getLinks API.
	 * @param source The link control
	 * @param semanticObject The semantic object
	 * @param action The action
	 */
	updateIBNLinkEnabled: async function (source: Control, semanticObject: string, action: string): Promise<void> {
		...
		if (!internalModelContext || !semanticObject || !action) {
			return;
		}

		const getIbnParams = async (): Promise<Record<string, { value: unknown }>> => {
			if (!bindingContext) {
				return {};
			}

			try {
				const oData = await (bindingContext as Context).requestObject();
				if (!oData) {
					return {};
				}

				return Object.keys(oData).reduce(
					(params, key) => {
						if (key.indexOf("_") !== 0 && !key.includes("odata.context")) {
							params[key] = { value: oData[key] };
						}
						return params;
					},
				...
				Log.error("Cannot retrieve context data for IBN link authorization check", error as string);
				return {};
			}
		};

		// Optimistically enable the link before the async shell check to avoid a disabled state
		// while waiting for the getLinks response.
		(internalModelContext as InternalModelContext).setProperty(`ibn/${semanticObject}-${action}/bEnabled`, true);

		// Call getLinks API to check authorization and update to the definitive state
		try {
			const aLink = await appComponent.getShellServices().getLinks([
				{
					semanticObject: semanticObject,
					action: action,
					params: await getIbnParams()
				}
			]);

			const bEnabled = !!(aLink && aLink.length === 1 && aLink[0] && aLink[0].length === 1);

			// Update internal model with definitive enabled state from shell
			(internalModelContext as InternalModelContext).setProperty(`ibn/${semanticObject}-${action}/bEnabled`, bEnabled);
		} catch (error) {
			Log.error("Cannot retrieve the links from the shell service for IBN link", error as string);
		}
	},
