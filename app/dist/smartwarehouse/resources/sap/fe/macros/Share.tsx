...
export default class Share extends BuildingBlock<ShareAPI> {
	...
	/**
	 * Add the Export as Excel option to the menu button.
	 * @param controller Page controller
	 * @param view The view this building block is used in
	 */
	_addExportExcelOptions(controller: PageController, view: View): void {
		const viewData = view.getViewData() as ViewData;
		if (
			viewData?.share?.hasOwnProperty("exportToSpreadsheet") === true &&
			viewData?.share?.exportToSpreadsheet?.hasOwnProperty("press") === true
		) {
			this?.menu?.current?.addItem(
				<MenuItem
	...
	/**
	 * Add the Export as CSV option to the menu button.
	 * @param controller Page controller
	 * @param view The view this building block is used in
	 */
	_addExportCSVOptions(controller: PageController, view: View): void {
		const viewData = view.getViewData() as ViewData;
		if (viewData?.share?.hasOwnProperty("exportToCSV") === true && viewData?.share?.exportToCSV?.hasOwnProperty("press") === true) {
			this?.menu?.current?.addItem(
				<MenuItem
	...
	/**
	 * Add the Export as PDF option to the menu button.
	 * @param controller Page controller
	 * @param view The view this building block is used in
	 */
	_addExportPDFOptions(controller: PageController, view: View): void {
		const viewData = view.getViewData() as ViewData;
		if (viewData?.share?.hasOwnProperty("exportToPDF") === true && viewData?.share?.exportToPDF?.hasOwnProperty("press") === true) {
			this?.menu?.current?.addItem(
				<MenuItem
	...
	setShareOptionsVisibility(): void {
		...
		// Check if there are custom menu items (print, export, etc.) that were added
		const hasCustomMenuItems = !!(
			this.printMenuItem?.current ||
			this.exportToSpreadsheetMenuItem?.current ||
			this.exportToCSVMenuItem?.current ||
			this.exportToPDFMenuItem?.current
		);

		// If there are custom menu items, the share block should be visible even if standard options are hidden
		const shareBlockVisibleExp = hasCustomMenuItems ? constant(true) : or(...visibleBindingExps);
	...
	_getShareOptionVisibilityExpression(option?: ShareOptionKey): BindingToolkitExpression<boolean> {
		...
		switch (option) {
			...
			case enumShareOption.SHARE_JAM: {
				const showShareInWorkzone = resolveBindingString<boolean>(this.shareOptions?.showShareInWorkzone ?? true, "boolean");
				const internalOption = resolveBindingString(
					this._blockInternalConfig.showShareInWorkzone ?? true,
					"boolean"
				) as unknown as BindingToolkitExpression<boolean>;
				if (isConstant(internalOption)) {
					exp = internalOption.value
						? ((this.shareOptions?.showShareInWorkzone ?? true) as unknown as EmbeddedUI5BindingExpression<boolean>)
						: constant(false);
				} else {
					exp = and(showShareInWorkzone, internalOption) as unknown as EmbeddedUI5BindingExpression<boolean>;
				}
				break;
			}
			case enumShareOption.SAVE_AS_TILE: {
				const showSaveAsTile = resolveBindingString<boolean>(this.shareOptions?.showSaveAsTile ?? true, "boolean");
				const internalOption = resolveBindingString(
					this._blockInternalConfig.showSaveAsTile ?? true,
					"boolean"
				) as unknown as BindingToolkitExpression<boolean>;
				if (isConstant(internalOption)) {
					exp = internalOption.value
						? ((this.shareOptions?.showSaveAsTile ?? true) as unknown as EmbeddedUI5BindingExpression<boolean>)
						: constant(false);
				} else {
					exp = and(showSaveAsTile, internalOption) as unknown as EmbeddedUI5BindingExpression<boolean>;
				}
				break;
			}
			case enumShareOption.MS_TEAMS_GROUP: {
	...
	_getShareBlockConfig(view: View): ConfigOptions {
		...
		const showSaveAsTile = this._getOptionBlockConfig(feAppShareConfigs?.showSaveAsTile, viewShareConfigs?.showSaveAsTile);
		const showShareInWorkzone = this._getOptionBlockConfig(
			feAppShareConfigs?.showShareInWorkzone,
			viewShareConfigs?.showShareInWorkzone
		);
		...
		return { showSendEmail, showMsTeamsOptions, enableCard, showCollaborationManager, showSaveAsTile, showShareInWorkzone };
	}
