 ...
 */
sap.ui.define(["sap/base/Log", "sap/base/util/merge", "sap/chart/TimeUnitType", "sap/fe/core/CommonUtils", "sap/fe/core/converters/MetaModelConverter", "sap/fe/core/converters/controls/Common/filter/FilterRestrictions", "sap/fe/core/formatters/ValueFormatter", "sap/fe/core/helpers/MetaModelFunction", "sap/fe/core/helpers/ModelHelper", "sap/fe/core/helpers/ResourceModelHelper", "sap/fe/macros/CommonHelper", "sap/fe/macros/DelegateUtil", "sap/fe/macros/chart/ChartHelper", "sap/fe/macros/chart/ChartUtils", "sap/fe/macros/filter/FilterUtils", "sap/m/IllustratedMessageSize", "sap/m/IllustratedMessageType", "sap/ui/core/Element", "sap/ui/core/Lib", "sap/ui/mdc/enums/ChartItemRoleType", "sap/ui/mdc/odata/v4/util/DelegateUtil", "sap/ui/mdc/odata/v4/vizChart/ChartDelegate", "sap/ui/model/Filter", "sap/ui/model/FilterOperator", "../filterBar/FilterBarDelegate"], function (Log, merge, TimeUnitType, CommonUtils, MetaModelConverter, FetchFilterRestrictions, valueFormatters, MetaModelFunction, ModelHelper, ResourceModelHelper, CommonHelper, MacrosDelegateUtil, ChartHelper, ChartUtils, FilterUtils, IllustratedMessageSize, IllustratedMessageType, Element, Library, ChartItemRoleType, DelegateUtil, BaseChartDelegate, Filter, FilterOperator, FilterBarDelegate) {
  "use strict";
  ...
  ChartDelegate._handleProperty = function (oContext, oMDCChart, mEntitySetAnnotations, mKnownAggregatableProps, mCustomAggregates, aProperties, sCriticality, unitOfMeasures) {
    ...
    const filterRestrictions = FetchFilterRestrictions.getFilterRestrictionsByDataModel(dataModel);
    ...
    if (oObj && oObj.$kind === "Property") {
      ...
      if (bAggregatable) {
        ...
        aAggregateProperties.forEach(function (oAggregateProperty) {
          ...
          if (oAggregateProperty.label.startsWith("{metaModel>")) {
            oAggregateProperty.label = ModelHelper.fetchTextFromMetaModel(oAggregateProperty.label, undefined, oMDCChart.getModel().getMetaModel());
          } else {
            oAggregateProperty.label = getLocalizedText(oAggregateProperty.label, oMDCChart);
          }
        });
  ...
  function addPropertyToChart(aProperties, sKey, oPropertyAnnotations, isFilterable, oFilterRestrictionsInfo, isSortable, oMDCChart, sCriticality, oObj, bIsGroupable, bIsAggregatable, bIsNavigationText, bIsHidden) {
    ...
    if (oProperty.label.startsWith("{metaModel>")) {
      oProperty.label = ModelHelper.fetchTextFromMetaModel(oProperty.label, undefined, oMDCChart.getModel().getMetaModel());
    } else {
      oProperty.label = getLocalizedText(oProperty.label, oMDCChart);
    }
  ...
  ChartDelegate.updateBindingInfo = function (oChart, oBindingInfo) {
    ...
    if (oFilter) {
      ...
      // in the request
      if (oApplySupported && oApplySupported.enableSearch) {
        oBindingInfo.parameters.$search = CommonUtils.normalizeSearchTerm(oInfo.search ?? undefined);
      } else {
  ...
  ChartDelegate._removeSiblingEntityForDraftChart = function (items) {
    if (!items?.length) {
      return [];
    }
    return items.map(item => {
      ...
      const childFilters = item.getFilters();
      if (childFilters && childFilters.length > 0) {
        const filteredChildren = ChartDelegate._removeSiblingEntityForDraftChart(childFilters);
        if (filteredChildren.length) {
          return new Filter({
            filters: filteredChildren,
            and: item.isAnd()
          });
        }
        return null;
