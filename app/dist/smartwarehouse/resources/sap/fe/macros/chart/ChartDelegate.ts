...
import ResourceModelHelper, { getLocalizedText } from "sap/fe/core/helpers/ResourceModelHelper";
		...
		if (bAggregatable) {
			...
			aAggregateProperties.forEach(function (oAggregateProperty: ChartProperty) {
				...
				if (oAggregateProperty.label.startsWith("{metaModel>")) {
					oAggregateProperty.label = ModelHelper.fetchTextFromMetaModel(
						oAggregateProperty.label,
						undefined,
						(oMDCChart.getModel() as ODataModel).getMetaModel()
					);
				} else {
...
function addPropertyToChart(
	...
	if (oProperty.label.startsWith("{metaModel>")) {
		oProperty.label = ModelHelper.fetchTextFromMetaModel(
			oProperty.label,
			undefined,
			(oMDCChart.getModel() as ODataModel).getMetaModel()
		);
...
ChartDelegate._removeSiblingEntityForDraftChart = function (items: Filter[]): Filter[] {
	...
	return items
		.map((item: Filter) => {
			...
			if (childFilters && childFilters.length > 0) {
				...
				if (filteredChildren.length) {
					return new Filter({ filters: filteredChildren, and: item.isAnd() });
				}
				return null;
			}
			if (item.getPath()) {
		...
		.filter((item: Filter | null): item is Filter => item !== null);
};
