 ...
 */
sap.ui.define(["sap/base/Log", "sap/base/util/deepClone", "sap/base/util/merge", "sap/base/util/uid", "sap/fe/core/CommonUtils", "sap/fe/core/buildingBlocks/templating/BuildingBlockTemplateProcessor", "sap/fe/core/controls/CommandExecution", "sap/fe/core/converters/ConverterContext", "sap/fe/core/converters/ManifestSettings", "sap/fe/core/converters/ManifestWrapper", "sap/fe/core/converters/MetaModelConverter", "sap/fe/core/converters/annotations/DataField", "sap/fe/core/converters/controls/Common/Action", "sap/fe/core/converters/controls/Common/DataVisualization", "sap/fe/core/helpers/FPMHelper", "sap/fe/core/helpers/ModelHelper", "sap/fe/core/helpers/StableIdHelper", "sap/fe/core/helpers/TypeGuards", "sap/fe/core/templating/DataModelPathHelper", "sap/fe/macros/CommonHelper", "sap/m/Button", "sap/m/FlexItemData", "sap/m/IllustratedMessage", "sap/m/IllustratedMessageSize", "sap/m/IllustratedMessageType", "sap/m/Menu", "sap/m/MenuButton", "sap/m/MenuItem", "sap/m/OverflowToolbarLayoutData", "sap/m/ToolbarSeparator", "sap/m/library", "sap/ui/Device", "sap/ui/core/CustomData", "sap/ui/core/Element", "sap/ui/core/InvisibleText", "sap/ui/core/Popup", "sap/ui/fl/variants/VariantManagement", "sap/ui/mdc/Chart", "sap/ui/mdc/actiontoolbar/ActionToolbarAction", "sap/ui/mdc/chart/Item", "sap/ui/mdc/p13n/PersistenceProvider", "sap/ui/model/json/JSONModel", "../internal/helpers/ActionHelper", "./ChartHelper", "sap/fe/base/jsx-runtime/jsx"], function (Log, deepClone, merge, uid, CommonUtils, BuildingBlockTemplateProcessor, CommandExecution, ConverterContext, ManifestSettings, ManifestWrapper, MetaModelConverter, DataField, Action, DataVisualization, FPMHelper, ModelHelper, StableIdHelper, TypeGuards, DataModelPathHelper, CommonHelper, Button, FlexItemData, IllustratedMessage, IllustratedMessageSize, IllustratedMessageType, Menu, MenuButton, MenuItem, OverflowToolbarLayoutData, ToolbarSeparator, library, Device, CustomData, Element, InvisibleText, Popup, VariantManagement, MdcChart, ActionToolbarAction, MdcChartItem, PersistenceProvider, JSONModel, ActionHelper, ChartHelper, _jsx) {
  "use strict";
    ...
    if (chartBlock.actions) {
      Object.values(chartBlock.actions)?.forEach(action => {
        if (action.isA("sap.fe.macros.chart.ActionGroup")) {
          ...
          }, action.actions.map((_, index) => action.key + "_Menu_" + index), action.group, action.overflowGroup);
          ...
          customActions[action.key] = getActionProperties(action.key, action.text, action.press, !!action.requiresSelection, action.enabled === null || action.enabled == undefined ? true : action.enabled, {
            placement: action.placement,
            anchor: action.anchor
          }, undefined, action.group, action.overflowGroup, action.priority);
        }
      });
    }
    if (chartBlock.actionOverflowGroups) {
      Object.values(chartBlock.actionOverflowGroups)?.forEach(overflowGroup => {
        actionOverflowGroups[overflowGroup.overflowGroup.toString()] = {
          showSeparator: overflowGroup.showSeparator ?? false
        };
      });
    }
  ...
  function getActionProperties(key, text, press, requiresSelection, enabled, position, menu, group, overflowGroup, priority) {
    ...
    if (priority !== undefined) {
      actionProperties.priority = priority;
    }
  ...
  function getDynamicMeasures(measures, chartDynamicMeasure, entitySetPath, chart, chartBlock) {
    ...
    if (key.includes("/")) {
      ...
      dynamicMeasure.label = aggregatedProperty.annotations?.Common?.Label?.toString() ?? aggregatedProperty.AggregatableProperty.$target?.annotations?.Common?.Label?.toString() ?? "";
  ...
  function getCustomActions(action, chartContext, chartBlock) {
    ...
    if (action.type === "Default") {
      ...
      return getActionToolbarMenuAction(action, chartContext, chartBlock);
    }
  }
  ...
  function getActionToolbarMenuAction(action, chartContext, chartBlock) {
    const aMenuItems = action.menu.map(menuItem => {
      return getMenuItemFromMenu(menuItem, chartContext, chartBlock);
    });
    const isAIOperation = isMenuAIOperation(action.menu);

    // Compute derived values inline
    const useDefaultActionOnly = !!action.defaultAction;
    const buttonMode = action.defaultAction ? MenuButtonMode.Split : MenuButtonMode.Regular;
    return _jsx(ActionToolbarAction, {
      children: _jsx(MenuButton, {
        ...
        useDefaultActionOnly: useDefaultActionOnly,
        buttonMode: buttonMode,
        menuPosition: Dock.BeginBottom,
        defaultAction: action.defaultAction,
        icon: isAIOperation ? aiIcon : undefined,
        ariaDescribedBy: isAIOperation ? [`${chartBlock.getId()}--fe::AiAction::AriaText`] : undefined,
        children: {
          menu: _jsx(Menu, {
            children: aMenuItems
          }),
    ...
    if (action.type === "ForNavigation") {
      ...
      return _jsx(ActionToolbarAction, {
        children: _jsx(MenuButton, {
          ...
          ariaDescribedBy: action?.isAIOperation === true ? [`${chartBlock.getId()}--fe::AiAction::AriaText`] : undefined,
          children: {
            menu: _jsx(Menu, {
              children: menuItems
            }),
  ...
  function getSeparator(action) {
    return _jsx(ActionToolbarAction, {
      children: _jsx(ToolbarSeparator, {
        children: {
          layoutData: _jsx(OverflowToolbarLayoutData, {
            group: action.group
          })
  ...
  function getActionToolbarAction(action, actionControlProperties, isMenuItem, chartBlock, dataField, chartOperationAvailableMap) {
    if (isMenuItem) {
      ...
      return _jsx(MenuItem, {
        text: actionControlProperties.label,
        ...pressEvent,
        enabled: actionControlProperties.enabled,
        visible: actionControlProperties.visible
      });
    } else {
      return buildAction(action, actionControlProperties, chartBlock, dataField, chartOperationAvailableMap);
    }
  }
  function buildAction(action, actionControlProperties, chartBlock, dataField, chartOperationAvailableMap) {
    return _jsx(ActionToolbarAction, {
      children: _jsx(Button, {
        id: actionControlProperties.id,
        text: actionControlProperties.label,
        ariaHasPopup: actionControlProperties.ariaHasPopup,
  ...
  function getSegmentedButton(chartBlock) {
    return _jsx(ActionToolbarAction, {
      id: generate([chartBlock._contentId, "ContentSwitcher"]),
      layoutInformation: {
        aggregationName: "end",
        alignment: "End"
      }
  ...
  function initialise(chartBlock, viewData, diagnostics, contextObjectPath) {
    ...
    if (chartBlock._chart) {
      ...
      const navProps = contextObjectPath.navigationProperties;
      const lastNavProp = navProps[navProps.length - 1];
      const isContainment = lastNavProp?.containsTarget && contextObjectPath.targetEntitySet === undefined;
      if ((chartBlock.chartDefinition.isParameterizedEntitySet ?? false) || (isContainment ?? false)) {
  ...
  function getFilterBar(chartBlock) {
    ...
    if (chartBlock.filterBar && filterBar?.isA("sap.fe.macros.FilterBar")) {
  ...
  function getMdcChartTemplate(chartBlock, viewData, diagnostics, contextObjectPath) {
    ...
    return _jsx(MdcChart, {
      ...
      children: {
        ...
        dependents: [getDependents(chartBlock, chartBlock._chartContext), getPersistenceProvider(chartBlock), getAccAIActionInvisibleText(chartBlock)],
  ...
  /**
   * This function returns the InvisibleText for the AI Action buttons.
   * @param chartBlock ChartBlock instance
   * @returns InvisibleText control
   */
  _exports.getMdcChartTemplate = getMdcChartTemplate;
  function getAccAIActionInvisibleText(chartBlock) {
    const invisibleTextAccAIAction = new InvisibleText({
      id: `${chartBlock.getId()}--fe::AiAction::AriaText`,
      text: "{sap.fe.i18n>C_ACCESSIBILITY_AI_ACTION_ARIA_TEXT}"
    });
    invisibleTextAccAIAction.toStatic();
    return invisibleTextAccAIAction;
  }
  return _exports;
}, false);
