...
import { isDisabledExpression, isReadOnlyExpression, isRequiredExpression } from "sap/fe/core/templating/FieldControlHelper";
...
export default class ActionParameterDialog {
	...
	private async createFormElement(parameter: ActionParameter): Promise<FormElementType> {
		...
		if (parameter.isCollection) {
			...
			fileRequired = field
				.getCustomData()
				.find((customData: CustomData) => customData.getKey() === "isRequired")
				?.getValue();
		} else {
		...
		return (
			<>
				<Label id={generate(["APD_", parameter.name, streamParameter?.name, "Label"])} required={fileRequired} text={label} />
				{field}
			</>
		);
	}
	...
	private createFileUploader(parameter: ActionParameter): FileUploader {
		...
		return (
			<FileUploader
				...
				customData={<CustomData key="isRequired" value={required} />}
			/>
		);
	}
	...
	private async handleFieldChange(event: MultiValueField$ChangeEvent | Field$ChangeEvent, parameter: ActionParameter): Promise<void> {
		...
		try {
			...
			if (Array.isArray(parameterInfo.value)) {
				...
				parameterInfo.value = parameterInfo.value[0] as unknown as string;
			}
	 ...
	 */
	private lockDialogContent(): void {
		if (!this.dialog) {
			return;
		}
	 ...
	 */
	private unlockDialogContent(): void {
		if (!this.dialog) {
			return;
		}
	...
	private async getAdditionalValue(oInterface: ComputedAnnotationInterface): Promise<string | undefined> {
		...
		if (valueListParameter && valueListParameter.ValueListProperty) {
			...
			if (textContext) {
				const formattedBinding = await AnnotationHelper.format(textContext.getObject(), { context: textContext });
