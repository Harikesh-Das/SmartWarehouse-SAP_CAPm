  ...
  let ActionParameterDialog = /*#__PURE__*/function () {
    ...
    _proto.createFormElement = async function createFormElement(parameter) {
      ...
      if (parameter.isCollection) {
        ...
        fileRequired = field.getCustomData().find(customData => customData.getKey() === "isRequired")?.getValue();
      } else {
    ...
    _proto.createMultiField = async function createMultiField(parameter, parameterContext) {
      ...
      return _jsx(MultiValueField, {
        ...
        required: compileExpression(isRequiredExpression(parameter)),
    ...
    _proto.createFileUploader = function createFileUploader(parameter) {
      ...
      const required = compileExpression(isRequiredExpression(parameter));
    ...
    _proto.handleFieldChange = async function handleFieldChange(event, parameter) {
      ...
      try {
        ...
        if (Array.isArray(parameterInfo.value)) {
          // Fix for DINC0757590, we should return a single value instead of an array
          parameterInfo.value = parameterInfo.value[0];
        }
    ...
    _proto.handleFileUploaderChange = async function handleFileUploaderChange(event, parameter) {
      ...
      this.removeMessagesForParameter(parameter);
      // Clear missing file error state
      parameterInfo.field.setValueState("None");
      parameterInfo.field.setValueStateText("");
      parameterInfo.value = {};
    ...
    _proto.createDialog = async function createDialog() {
      ...
      this.dialog = dialog;
      // Set initialFocus to the dialog itself so that screen readers announce the dialog title (header region)
      // when the dialog opens. Focus is then moved programmatically to the first field in afterOpen() with
      // silent: true to suppress VH typeahead. Setting initialFocus to the endButton (footer region) caused
      // JAWS to announce "footer region" instead of the dialog context. (ACC-264.1 / DINC0822752)
      dialog.setInitialFocus(dialog);
      return dialog;
    }
    ...
    /**
     * Locks the dialog content to show busy indicator.
     * Applies busy state to the dialog content instead of the dialog itself
     * to comply with SAP Fiori Design Guidelines and avoid focus border issues.
     */;
    _proto.lockDialogContent = function lockDialogContent() {
      if (!this.dialog) {
        return;
      }
      // Apply busy state to content area instead of dialog control
      const dialogContent = this.dialog.getContent()[0];
      BusyLocker.lock(dialogContent ? dialogContent : this.dialog);
    }

    /**
     * Removes the busy state of the dialog content.
     */;
    _proto.unlockDialogContent = function unlockDialogContent() {
      if (!this.dialog) {
        return;
      }
      // Check content first, then fallback to dialog
      const dialogContent = this.dialog.getContent()[0];
      if (dialogContent && BusyLocker.isLocked(dialogContent)) {
        BusyLocker.unlock(dialogContent);
      } else if (BusyLocker.isLocked(this.dialog)) {
     ...
     * @deprecated Use unlockDialogContent() instead
     */;
    ...
    _proto.closeDialog = function closeDialog() {
      if (this.dialog) {
        // Ensure busy lock is released to prevent block-layer from intercepting pointer events
        this.unlockDialogContent();
    ...
    _proto.getAdditionalValue = async function getAdditionalValue(oInterface) {
      ...
      if (valueListParameter && valueListParameter.ValueListProperty) {
        ...
        if (textContext) {
          const formattedBinding = await AnnotationHelper.format(textContext.getObject(), {
            context: textContext
          });
          // Replace the actual path with a dummy path to prevent binding collisions when multiple parameters
          // share the same value help collection path. This preserves type information for formatting
          // while ensuring the mdcField falls back to getting the displayed value from the valueHelp table.
          // The dummy path uses the parameter name to ensure uniqueness across parameters.
          return formattedBinding.replace(/path\s*:\s*'[^']+'/g, `path:'fe_dummy_${localDataProperty}'`);
        }
      }
    }

    /**
     * Gets the value binding for an action parameter. We add special handling for currency and unit beause they are not
     * part of the annotation helper format function.
     * @param parameter The parameter
     * @param parameterContext The parameter context
     * @param oInterface The parameter
     * @returns The binding for the value of the action parameter.
     */;
    _proto.getAPDialogBindingValue = async function getAPDialogBindingValue(parameter, parameterContext) {
      const actionMetaPath = this.action.isBound ? `${this.metaModel.getMetaPath(this.parameters.contexts[0].getPath())}/${convertTypes(this.metaModel).namespace}.${this.action.name}` : `/${this.action.name}`;
      const metaContext = this.metaModel.createBindingContext(`${actionMetaPath}/${parameter.name}`);
      try {
        const propertyObjectPath = getInvolvedDataModelObjects(metaContext);
        return getValueBinding(propertyObjectPath, {});
      } catch (error) {
        // Fallback to old approach until we can reach unbound actions
        return AnnotationHelper.format(parameterContext.getObject(), {
          context: parameterContext
        });
      }
    };
