...
class FeMessagePopover extends MessagePopover {
	...
	getLinkForErrorExplanation(messagePopoverInstance: MessagePopover): Link | null {
		...
		if (environmentService.environmentCapabilities.ErrorExplanation) {
			const ariaLabel = Library.getResourceBundleFor("sap.fe.macros")!.getText("C_GENERATE_EXPLANATION_ARIA_LABEL");
			const link = new Link({
			...
			link.addEventDelegate(
				{
					onAfterRendering: function (this: Link) {
						const domRef = this.getDomRef();
						if (domRef) {
							domRef.setAttribute("aria-label", ariaLabel);
							domRef.removeAttribute("aria-labelledby");
						}
					}
				},
				link
