...
class MessageButton extends Button {
	...
	_checkMergeLogicForMessageGrouping(
		...
		if (singleSubsectionInPageLayout) {
			...
			if (content?.isA("sap.fe.macros.Table")) {
			...
			if (blocks.length === 1 && blocks[0]?.isA<SubSectionBlock>("sap.fe.templates.ObjectPage.controls.SubSectionBlock")) {
				...
				if (content?.isA("sap.fe.macros.Table")) {
					...
					if (content && content.length === 1 && content[0]?.isA("sap.fe.macros.Table")) {
						bIncludeTableGroupName = false;
					}
				}
			}
	...
	_checkControlIdInSections(messages: MessageItem[], enableBinding?: boolean): boolean | void {
		...
		if (aVisibleSections) {
			...
			for (i = 0; i < messages.length; i++) {
				...
				for (j = 0; j < aVisibleSections.length; j++) {
					let shouldBreak = false;
					// Loop over all visible sections from header section to last visible section
					section = aVisibleSections[j];
					aSubSections = section.getSubSections();
					for (k = 0; k < aSubSections.length; k++) {
			...
			if (sLongTextUrl) {
				jQuery.ajax({
					...
					success: function (data: string) {
						...
						const sCurrentDescription = config.item.getDescription() ?? "";
						// Only prepend the current description if it doesn't already contain the full diagnosis text
						// This prevents duplication when the handler is invoked multiple times for the same message
						// (e.g., on re-selection after navigation back, or in double-save race conditions)
						const bAlreadyHasDiagnosis = sCurrentDescription.includes(sDiagnosisText);
						const sNewDescription = bAlreadyHasDiagnosis ? sDiagnosisText : `${sCurrentDescription}${sDiagnosisText}`;
						config.item.setDescription(sNewDescription);
	...
	_setMessageData(): void {
		...
		this._setMessageDataTimeout = setTimeout(async () => {
			// Close messagePopover if objectPage showFooter is false
			if (this.oObjectPageLayout && !this.oObjectPageLayout.getShowFooter()) {
	...
	_getMessageRank(obj: Message & { sectionName: string; subSectionName: string }, aSections: ObjectPageSection[] | undefined): number {
		if (aSections) {
			...
			for (j = 0; j < aSections.length; j++) {
				// Loop over all sections from first to last
				section = aSections[j];
				aSubSections = section.getSubSections();
				for (k = 0; k < aSubSections.length; k++) {
					// Loop over all sub-sections from first to last
	...
	_getFilterToHideBackendMessageOnErrorValidation(): Filter {
		const getIdsOfFielsdWithErrorValidation = function (): string[] {
			return Messaging.getMessageModel()
				.getData()
				.filter((message: Message) => message.validation)
				.map((message: Message) => message.getControlId());
		};
	...
	async _applyFiltersAndSort(): Promise<void> {
		...
		// Initialize ObjectPageLayout if not already done
		this.oObjectPageLayout = this._getObjectPageLayout(this, this.oObjectPageLayout);
		...
		if (!oBindingContext) {
			...
			if (this.isHiddenDraftEnabled && viewContext) {
