...
sap.ui.define(["sap/base/Log", "sap/base/util/merge", "sap/fe/base/BindingToolkit", "sap/fe/base/ClassSupport", "sap/fe/core/buildingBlocks/BuildingBlock", "sap/fe/core/converters/ConverterContext", "sap/fe/core/converters/ManifestWrapper", "sap/fe/core/converters/controls/Common/DataVisualization", "sap/fe/core/converters/helpers/Aggregation", "sap/fe/core/helpers/ModelHelper", "sap/fe/core/helpers/PromiseKeeper", "sap/fe/core/helpers/StableIdHelper", "sap/m/Button", "sap/m/Label", "sap/m/OverflowToolbar", "sap/m/OverflowToolbarLayoutData", "sap/m/Title", "sap/m/ToolbarSpacer", "sap/m/VBox", "sap/m/library", "sap/ui/core/CustomData", "../controls/filterbar/VisualFilter", "./InParameter", "./InteractiveChartHelper", "./VisualFilterRuntime", "./fragments/InteractiveCharts", "sap/fe/base/jsx-runtime/jsx", "sap/fe/base/jsx-runtime/Fragment", "sap/fe/base/jsx-runtime/jsxs"], function (Log, merge, BindingToolkit, ClassSupport, BuildingBlock, ConverterContext, ManifestWrapper, DataVisualization, Aggregation, ModelHelper, PromiseKeeper, StableIdHelper, Button, Label, OverflowToolbar, OverflowToolbarLayoutData, Title, ToolbarSpacer, VBox, library, CustomData, VisualFilterControl, InParameter, InteractiveChartHelper, VisualFilterRuntime, InteractiveCharts, _jsx, _Fragment, _jsxs) {
  ...
  let VisualFilter = (_dec = defineUI5Class("sap.fe.macros.visualfilters.VisualFilter"), _dec2 = implementInterface("sap.ui.core.IFormContent"), _dec3 = property({
    ...
    type: "sap.fe.macros.visualfilters.InParameter",
    multiple: true,
    defaultClass: InParameter,
    defaultValue: []
  }), _dec14 = property({
    type: "boolean"
  }), _dec15 = property({
    ...
    _proto.initializeVisualFilterSettings = function initializeVisualFilterSettings() {
      ...
      const aggregatableProperties = this.getAggregateProperties(aggregations, measure);
      if (aggregatableProperties) {
        this.aggregateProperties = aggregatableProperties;
      }
      ...
      const aggregatablePropertyAnnotations = aggregatableProperties?.AggregatableProperty?.$target?.annotations;
    ...
    _proto.onMetadataAvailable = async function onMetadataAvailable() {
      // Return existing content if already created
      if (this.content) {
        return this.content;
      }

      // Return the promise if content creation is already in progress
      if (this.contentPromise) {
        return this.contentPromise.promise;
      }

      // Mark content creation as in progress
      this.contentPromise = new PromiseKeeper();

      // Initialize visual filter settings and properties.
      this.initializeVisualFilterSettings();

      // Create the content for the visual filter
      this.content = await this.createContent();
      this.contentPromise.resolve(this.content);
      return this.content;
    }
    ...
    _proto.getTitle = function getTitle() {
      ...
      // Attach event delegates after the controls are rendered and view is available
      this._attachTitleEventDelegates(titleControl);
      return titleControl;
    };
    ...
    _proto._handleInParamsChartBindingEnablement = function _handleInParamsChartBindingEnablement(changedFilterFieldPaths) {
      const inParameters = this.inParameters;
    ...
    /**
     * Attaches mouseover and mouseout event delegates to the measure dimension title.
     * This creates dynamic tooltips that combine measure titles with scale/UOM information.
     * @param titlecontrol The title control to which event delegates are attached.
     */;
    _proto._attachTitleEventDelegates = function _attachTitleEventDelegates(titlecontrol) {
      titlecontrol.addEventDelegate({
        onmouseover: _evt => {
          const tooltip = titlecontrol.getTooltip();
          titlecontrol.getDomRef()?.setAttribute("title", tooltip);
        }
      }, this);
    }
    ...
    _proto.createContent = async function createContent() {
      ...
      const visualFilterChart = await getVisualFilterChart(this);
