  ...
  const InteractiveChartHelper = {
    ...
    getAggregationBinding: function (chartAnnotations, collection, contextPath, textAssociation, dimensionType, sortOrder, selectionVariant, customAggregate, aggregation, UoMHasCustomAggregate, parameters, filterBarEntityType, draftSupported, chartMeasure, metaModel) {
      ...
      if (selectionVariantAnnotation) {
        const entityTypePath = entitySetPath.endsWith("/") ? entitySetPath : `${entitySetPath}/`;
        filterConditions = getFiltersConditionsFromSelectionVariant(entityTypePath, metaModel, selectionVariantAnnotation, VisualFilterUtils.getCustomConditions.bind(VisualFilterUtils));
    ...
    getInParameterFiltersBinding: function (inParameters) {
      if (inParameters.length > 0) {
        ...
        if (parts.length > 0) {
          paths = parts.join();
          return `{parts:[${paths}], formatter:'._formatters.VisualFilterRuntime.getFiltersFromConditions.bind($control)'}`;
        } else {
          return undefined;
        }
      } else {
    ...
    getfilterCountBinding: function (chartAnnotation) {
      ...
      return "{path:'$filters>/conditions/" + dimension + "', formatter:'._formatters.VisualFilterRuntime.getFilterCounts.bind($control)'}";
    },
