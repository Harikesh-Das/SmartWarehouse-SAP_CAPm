...
import { aggregation, defineUI5Class, event, implementInterface, property, type PropertiesOf } from "sap/fe/base/ClassSupport";
...
import InParameter from "./InParameter";
...
export default class VisualFilter extends BuildingBlock<VisualFilterControl> {
	...
	contentPromise?: PromiseKeeper<VisualFilterControl>;
	...
	constructor(props: $ControlSettings & PropertiesOf<VisualFilter>, others?: $ControlSettings) {
		super(props, others);
	}

	private initializeVisualFilterSettings(): void {
	...
	public async onMetadataAvailable(): Promise<VisualFilterControl> {
		...
		this.contentPromise = new PromiseKeeper<VisualFilterControl>();
	...
	_handleInParamsChartBindingEnablement(changedFilterFieldPaths: string[]): void {
		...
		const inParameterLocalDataProperties = inParameters.map((inParameter: InParameter) => inParameter.localDataProperty);
	 ...
	 */
	private _attachTitleEventDelegates(titlecontrol: Title): void {
		titlecontrol.addEventDelegate(
			{
				onmouseover: (_evt: MouseEvent) => {
					const tooltip = titlecontrol.getTooltip() as string;
	...
	async createContent(): Promise<VisualFilterControl> {
