...
import type { FormLayoutInformation, FormManifestConfiguration } from "sap/fe/core/converters/ManifestSettings";
...
@defineBuildingBlock({
	name: "Form",
	namespace: "sap.fe.macros.internal.OLD",
	publicNamespace: "sap.fe.macros.OLD",
...
export default class FormBlock extends BuildingBlockTemplatingBase {
	...
	constructor(props: PropertiesOf<FormBlock>, configuration: unknown, mSettings: TemplateProcessorSettings) {
		...
		if (this.metaPath && this.contextPath && (this.formContainers === undefined || this.formContainers === null)) {
			...
			const oFormDefinition = createFormDefinition(oFacetDefinition, compileExpression(this.visible), oConverterContext);
			if (hasFieldGroup) {
		...
		if (this.metaPath && this.contextPath) {
			const oContextObjectPath = getInvolvedDataModelObjects<CollectionFacet | ReferenceFacet>(this.metaPath, this.contextPath);
			const oConverterContext = this.getConverterContext(oContextObjectPath, this.contextPath.getPath(), mSettings, {});
			if (oContextObjectPath.targetObject) {
				const sections = oConverterContext.getManifestWrapper().getSections();
				let targetLayout;
				if (oContextObjectPath.targetObject.ID && sections[oContextObjectPath.targetObject.ID.toString()] !== undefined) {
					targetLayout = (sections[oContextObjectPath.targetObject.ID.toString()] as FormManifestConfiguration).layout;
				}
				if (!targetLayout) {
	...
	getDataFieldCollection(formContainer: FormContainer, facetContext: Context): string {
		...
		// Enhance: generate a stable form container id whenever we have a derived idPart even if this.id is undefined.
		// Previous logic only generated when (implicitly) guarding by one condition; now we allow fallback to idPart.
		// We still protect getFormContainerID from being called with an undefined idPart.
		const id = (this.id !== undefined || idPart) && idPart ? getFormContainerID(idPart) : undefined;
