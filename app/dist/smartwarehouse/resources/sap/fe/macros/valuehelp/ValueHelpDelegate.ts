...
export default Object.assign({}, ValueHelpDelegate, {
	 ...
	 */
	compareConditions(valueHelp: ValueHelp, oldCondition: ConditionObject, newCondition: ConditionObject): boolean {
		...
		if (vhKeys.length > 1) {
			const oldPayload = oldCondition.payload as ConditionPayloadMap | undefined;
			const newPayload = newCondition.payload as ConditionPayloadMap | undefined;
	...
	_onConditionPropagationUpdateProperty: function (
		...
		for (const outParameter of outParameters) {
			...
			this._updatePropertyViaOutParameter(
				...
				valueHelp.getPayload() as ValueHelpPayload
			);
	...
	_getSourcePropertyName(context: ODataV4Context, payload: ValueHelpPayload): string {
		const metaModel = context.getModel().getMetaModel();
	...
	_getVHPropertyNameMappedToSourceProperty(sourcePropertyName: string, payload: ValueHelpPayload): string | undefined {
		const vhParameters = payload.qualifiers[payload.valueHelpQualifier].vhParameters;
		return vhParameters?.find((vhParam) => {
			if (
				vhParam.parmeterType === "com.sap.vocabularies.Common.v1.ValueListParameterOut" ||
				vhParam.parmeterType === "com.sap.vocabularies.Common.v1.ValueListParameterInOut"
			) {
	...
	_updatePropertyViaOutParameter: function (
		...
		if (dynamicReadOnly && contextCanRequestSideEffects) {
			...
			const updateMainProperty =
				!!mappedVHProperty && outParameter.helpPath === mappedVHProperty && mappedVHProperty !== payload.valueHelpKeyPath;
...
*/
	...
	_buildPropertiesToRequest: function (propertiesToRequest: string[], rootPath: string, metaModel: ODataMetaModel, propertyPath: string) {
		...
		return propertiesToRequest.map((localDataPath) => {
			...
			return relativePathParts
				.slice(0, sliceEnd) // remove all matched parts (from the end)
				.map((part) => part.part)
	...
	_getInitialFilterConditionsFromFilterBar: async function (
		...
		for (const inParameter of inParameters) {
			...
			const conditions = this._getConditionsFromInParameter(
				sourceField as string,
				state,
				metaModel,
				filterBarProperty?.annotationPath
			);
			if (conditions) {
				// Use conditionPath if available, otherwise fallback to helpPath for backward compatibility
				const conditionKey = inParameter.conditionPath || inParameter.helpPath;
	 ...
	 * @since 1.114.0
	 */
	_getConditionsFromInParameter: function (
		sourceField: string,
		state: ExternalStateType,
		metaModel?: ODataMetaModel,
	 ...
	 */
	_extractActualValueFromConditionPayload: function (
		...
		const targetProperty = convertTypes(metaModel).resolvePath<Property>(annotationPath).target;
		...
		return conditions.map((condition: ConditionObject) => {
			const conditionPayloadMap = condition.payload as ConditionPayloadMap | undefined;
			if (!conditionPayloadMap) {
				return condition;
			}
	 ...
	 */
	checkIfContainerShouldBeDisplayed: async function (
		content: MTable,
		payload: ValueHelpPayload,
		metaModel: ODataMetaModel,
		valueHelp: ValueHelp,
	...
	checkIfSearchSupportedEnabled: async function (payload: ValueHelpPayload, metaModel: ODataMetaModel, valueHelp: ValueHelp) {
		...
		const propertyAnnotations = (convertTypes(metaModel).resolvePath<Property>(propertyPath)?.target?.annotations?.Common ||
			{}) as AnnotationsForProperty;
		const collectionMetaModel = valueListInfo.valueListInfo.$model.getMetaModel();
		const collectionAnnotations =
			convertTypes(collectionMetaModel).resolvePath<Property>(`/${valueListInfo.valueListInfo.CollectionPath}`)?.target?.annotations
				?.Capabilities || {};
	...
	typeAheadForFilterableContent: async function (valueHelp: ValueHelp, content: MDCPopover) {
		...
		const payload = valueHelp.getPayload() as ValueHelpPayload;
		const metaModel = (field.getModel() as ODataModel).getMetaModel();
	...
	conditionForRequestShowContainer: async function (valueHelp: ValueHelp, content: MDCPopover) {
		try {
			...
			if (
				...
				const payload = valueHelp.getPayload() as ValueHelpPayload;
				const metaModel = (valueHelpTable.getModel() as ODataModel).getMetaModel();
