 ...
 */
sap.ui.define(["sap/base/Log", "sap/base/util/deepEqual", "sap/base/util/ObjectPath", "sap/fe/core/CommonUtils", "sap/fe/core/controllerextensions/collaboration/CollaborationCommon", "sap/fe/core/converters/MetaModelConverter", "sap/fe/core/helpers/TypeGuards", "sap/fe/macros/filter/FilterUtils", "sap/fe/macros/internal/valuehelp/ValueListHelper", "sap/m/inputUtils/highlightDOMElements", "sap/ui/core/message/MessageType", "sap/ui/mdc/ValueHelpDelegate", "sap/ui/mdc/condition/Condition", "sap/ui/mdc/enums/ConditionValidated", "sap/ui/mdc/enums/RequestShowContainerReason", "sap/ui/mdc/odata/v4/TypeMap", "sap/ui/mdc/p13n/StateUtil", "sap/ui/mdc/valuehelp/RequestShowContainerDefault", "sap/ui/model/FilterType", "../internal/valuehelp/AdditionalValueHelper"], function (Log, deepEqual, ObjectPath, CommonUtils, CollaborationCommon, MetaModelConverter, TypeGuards, FilterUtils, ValueListHelper, highlightDOMElements, MessageType, ValueHelpDelegate, Condition, ConditionValidated, RequestShowContainerReason, TypeMap, StateUtil, RequestShowContainerDefault, FilterType, AdditionalValueHelper) {
  "use strict";
  ...
  return Object.assign({}, ValueHelpDelegate, {
     ...
     * @param _listBinding List binding (optional, unused)
     ...
     */
    checkConditionsEqual(oldCondition, newCondition) {
      // Compare operators
    ...
    /**
     * Compares two conditions including their payloads for composite-key value helps.
     * MDC's base implementation only checks operators and values, which isn't enough when
     * the same primary key appears with different secondary keys.
     * @param valueHelp The ValueHelp instance.
     * @param oldCondition First condition to compare.
     * @param newCondition Second condition to compare.
     * @returns True if conditions are equal.
     */
    compareConditions(valueHelp, oldCondition, newCondition) {
      // @ts-expect-error - compareConditions is available in MDC ValueHelpDelegate since 1.124.2
      const baseResult = ValueHelpDelegate.compareConditions(valueHelp, oldCondition, newCondition);
      if (!baseResult) {
        return false;
      }
      ...
      // For composite keys, also compare the payload
      // (assumes MDC only uses the first payload entry - if multi-select ever stores more, we'd need to iterate)
      if (vhKeys.length > 1) {
        const oldPayload = oldCondition.payload;
        const newPayload = newCondition.payload;
        ...
        const qualifier = payload.valueHelpQualifier;
        const oldConditionPayload = oldPayload[qualifier]?.[0];
        const newConditionPayload = newPayload[qualifier]?.[0];
        if (!oldConditionPayload && !newConditionPayload) {
          return true;
        }
        if (!oldConditionPayload || !newConditionPayload) {
          return false;
        }
        return vhKeys.every(vhKey => {
          // Use type-aware comparison (handles Date objects, nested structures)
          // consistent with MDC's Operator.compareConditions which uses deepEqual
          return deepEqual(oldConditionPayload[vhKey], newConditionPayload[vhKey]);
        });
      }
      return baseResult;
    },
     ...
     */
    _isEmptyDigitSequence: function (value, isDigitSequence) {
      let isEmptyDigitSequence = false;
      if (isDigitSequence) {
        isEmptyDigitSequence = typeof value === "string" ? value.split("").every(char => char === "0") ?? value === "" : value === null || value === undefined || value === 0;
      }
    ...
    updateBinding: async function (valueHelp, listBinding, bindingInfo, content) {
      ...
      if (content.isTypeahead()) {
        ...
        await this._updateBindingWithTransientContexts(payload, listBinding, bindingInfo, additionalValues, valueListProperty, field, sorters, table);
    ...
    _onConditionPropagationToFilterBar: async function (conditions, outParameters, filterBar, payload, listReportFilterBar) {
      try {
        ...
        for (const outParameter of outParameters) {
          ...
          if (filterBarProperty && stateItems?.find(item => item.name === filterBarProperty.name && (filterBarProperty.maxConditions === undefined || filterBarProperty.maxConditions === 1))) {
    ...
    _onConditionPropagationUpdateProperty: function (metaModel, outValues, outParameters, context, valueHelp) {
      ...
      for (const outParameter of outParameters) {
        ...
        this._updatePropertyViaOutParameter(convertedMetadata, rootPath, outValues, outParameter, context, contextCanRequestSideEffects, valueHelp.getPayload());
    ...
    _getSourcePropertyName(context, payload) {
      const metaModel = context.getModel().getMetaModel();
      const sourcePath = payload.propertyPath;
      const contextMetaPath = metaModel.getMetaPath(context.getPath());
      return sourcePath.replace(`${contextMetaPath}/`, "");
    },
    _getVHPropertyNameMappedToSourceProperty(sourcePropertyName, payload) {
      const vhParameters = payload.qualifiers[payload.valueHelpQualifier].vhParameters;
      return vhParameters?.find(vhParam => {
        if (vhParam.parmeterType === "com.sap.vocabularies.Common.v1.ValueListParameterOut" || vhParam.parmeterType === "com.sap.vocabularies.Common.v1.ValueListParameterInOut") {
          return vhParam.source === sourcePropertyName;
        }
        return false;
      })?.helpPath;
    },
    _updatePropertyViaOutParameter: function (convertedMetadata, rootPath, outValues, outParameter, context, contextCanRequestSideEffects, payload) {
      /* Updated property via out-parameter if value changed */
      ...
      if (dynamicReadOnly && contextCanRequestSideEffects) {
        ...
        // 'valueHelpKeyPath' value is propagated to the field(or vh source control) by default as 'main source property' value.
        // But if 'main value help property' is not same as 'valueHelpKeyPath', the 'main source property'(one mapped to 'main value help property') might need to be updated again in UI as it would have 'valueHelpKeyPath' value.
        // So in this case, irrespective of current and new value we update update context using setProperty.
        const mainVHPropertyName = this._getSourcePropertyName(context, payload);
        const mappedVHProperty = this._getVHPropertyNameMappedToSourceProperty(mainVHPropertyName, payload);
        const updateMainProperty = !!mappedVHProperty && outParameter.helpPath === mappedVHProperty && mappedVHProperty !== payload.valueHelpKeyPath;
        if (currentVal !== newVal || updateMainProperty) {
    ...
    _createInitialFilterCondition: function (value, initialValueIsSignificant) {
      ...
      } else if (value === "" && initialValueIsSignificant) {
        // Fix: Create EQ condition for empty string instead of Empty operator
        condition = Condition.createCondition("EQ", [""], null, null, ConditionValidated.Validated);
      } else if (value === null && initialValueIsSignificant) {
        // Keep Empty operator only for null
    ...
    _getInitialFilterConditionsFromBinding: async function (inConditions, control, inParameters, payload) {
      ...
      */
      ...
      if (!metaDataError) {
        ...
        errorMessagesInContext = (bindingContext.getMessages() ?? []).filter(message => {
    ...
    _buildPropertiesToRequest: function (propertiesToRequest, rootPath, metaModel, propertyPath) {
      ...
      .map(part => {
        ...
        const navPropDef = metaModelContext.getObject();
        return {
          part: part,
          partner: navPropDef.$Partner,
          isCollection: navPropDef.$isCollection === true
        };
      });
      // For 1:n navs, matchCount=0 means the source is on the root entity and must stay as-is;
      // for 1:1-only paths, matchCount=0 means the nav prefix must be prepended (DINC0669036).
      const hasCollectionNav = relativePathParts.some(p => p.isCollection);
      ...
      return propertiesToRequest.map(localDataPath => {
        ...
        if (matchCount) {
          sliceEnd = -matchCount;
        } else if (hasCollectionNav) {
          sliceEnd = 0;
        }
        return relativePathParts.slice(0, sliceEnd) // remove all matched parts (from the end)
    ...
    _getInitialFilterConditionsFromFilterBar: async function (inConditions, control, inParameters, payload) {
      const filterBar = FilterUtils.getFilterBarForAdaptationControl(control.getParent());
      if (!filterBar) {
        return inConditions;
      }
      const filterBarProperties = FilterUtils.getFilterPropertyInfo(filterBar);
      ...
      for (const inParameter of inParameters) {
        ...
        const conditions = this._getConditionsFromInParameter(sourceField, state, metaModel, filterBarProperty?.annotationPath);
        if (conditions) {
          // Use conditionPath if available, otherwise fallback to helpPath for backward compatibility
          const conditionKey = inParameter.conditionPath || inParameter.helpPath;
     ...
     * @param metaModel OData metamodel
     * @param annotationPath Annotation path
     * @returns The filter conditions
     * @since 1.114.0
     */
    _getConditionsFromInParameter: function (sourceField, state, metaModel, annotationPath) {
      const key = Object.keys(state.filter).find(key => key === sourceField); // condition path of the filter field
      const conditions = key ? state.filter[key] : undefined;
      // Extract actual ID from ExternalID conditions if applicable
      return this._extractActualValueFromConditionPayload(conditions, metaModel, annotationPath);
    },
    /**
     * Extracts the actual value value from conditions that have Common.ExternalID annotations.
     * When a field has an ExternalID annotation, the condition values contain the external ID (display value),
     * but the actual ID (key) is stored in the condition payload with a special marker.
     * This function extracts the actual ID from the payload and replaces the condition values.
     * @param conditions The conditions to process
     * @param metaModel The OData metamodel
     * @param annotationPath The annotation path of the property to check for ExternalID
     * @returns The modified conditions with actual values extracted from payload
     */
    _extractActualValueFromConditionPayload: function (conditions, metaModel, annotationPath) {
      if (!conditions || !annotationPath || !metaModel) {
        return conditions;
      }
      const targetProperty = convertTypes(metaModel).resolvePath(annotationPath).target;
      const externalIdAnnotation = targetProperty?.annotations?.Common?.ExternalID;
      if (!externalIdAnnotation?.path) {
        return conditions;
      }

      // Source field has ExternalID - extract its actual value from payload
      return conditions.map(condition => {
        ...
        // Find the payload entry with ExternalID marker
        for (const payloadKey of Object.keys(conditionPayloadMap)) {
          const conditionPayload = conditionPayloadMap[payloadKey]?.[0];
          if (conditionPayload) {
            const externalIdKey = Object.keys(conditionPayload).find(key => key.includes("@$ui5.fe.@Common/ExternalID"));
            if (externalIdKey) {
              // Replace the condition value with the actual ID from payload
              const actualIdValue = conditionPayload[externalIdKey];
              return {
                ...condition,
                values: [actualIdValue, ...(condition.values.slice(1) || [])]
              };
            }
          }
        }
    ...
    modifySelectionBehaviour: function (valueHelp, content, change) {
      const conditions = change.conditions;
      const oldConditions = content.getConditions();
      // Prevent de-select in dialog scenarios
      if (content?.getParent()?.isQuickSelectActive?.() && !conditions.length && oldConditions.length) {
    ...
    findConditionsForContext: function (valueHelp, content, context, conditions) {
      ...
      const findCondition = (conditionPayloadMap, value) => {
        ...
        if (vhKeys.length === 1 || payload?.useMultiValueField || !conditionPayloadMap) {
          ...
          return vhKeys.every(function (vhKey) {
            return !contextKeys.includes(vhKey) || !conditionSelectedRowKeys.includes(vhKey) || context?.getObject(vhKey) === conditionSelectedRow[vhKey];
          });
        }
      };
     ...
     */
    _updateBindingWithTransientContexts: async function (payload, listBinding, bindingInfo, additionalValues, valueListProperty, field, sorters, table) {
      try {
        ...
        } = await this._getRelevantAdditionalValueContextData(additionalValues, listBinding, bindingInfo, payload, valueListProperty, field, sorters, table);
     ...
     */
    _getRelevantAdditionalValueContextData: async function (additionalValues, listBinding, bindingInfo, payload, valueListProperty, field, sorters, table) {
      let {
        recommendationValuesContextData,
        othersValuesContextData
      } = await additionalValueHelper.requestForAdditionalValueContextData(additionalValues, listBinding, bindingInfo, payload, sorters, table);
    ...
    /**
     * Returns a boolean value wheather to display the popover container or not.
     * If searchSupported/searchable is enabled, the typeahead popover is always shown. Otherwise, it is shown only when recommendations or recent values exist.
     * @param content Filterable List Content
     * @param payload Payload for delegate
     * @param metaModel OData MetaModel for metadata access
     * @param valueHelp The ValueHelp instance
     * @param checkRecommendations Defines if we shall check for recommendations
     * @returns Boolean value
     */
    checkIfContainerShouldBeDisplayed: async function (content, payload, metaModel, valueHelp) {
      let checkRecommendations = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : true;
      if (await this.checkIfSearchSupportedEnabled(payload, metaModel, valueHelp)) {
        // If searchSupported is true, enable the typeahead popover
        return true;
      }
    ...
    checkIfSearchSupportedEnabled: async function (payload, metaModel, valueHelp) {
      ...
      const qualifierForTypeahead = valueHelp.data("valuelistForValidation") || "";
      ...
      const propertyAnnotations = convertTypes(metaModel).resolvePath(propertyPath)?.target?.annotations?.Common || {};
      const collectionMetaModel = valueListInfo.valueListInfo.$model.getMetaModel();
      const collectionAnnotations = convertTypes(collectionMetaModel).resolvePath(`/${valueListInfo.valueListInfo.CollectionPath}`)?.target?.annotations?.Capabilities || {};
     ...
     */
    typeAheadForFilterableContent: async function (valueHelp, content) {
      let showIt;
      const typeAheadContent = content?.getContent()[0];
      const field = typeAheadContent.getControl();
      const payload = valueHelp.getPayload();
      const metaModel = field.getModel().getMetaModel();
      if (field.isA("sap.ui.mdc.FilterField") || field.isA("sap.ui.mdc.MultiValueField")) {
        ...
        showIt = await this.checkIfContainerShouldBeDisplayed(typeAheadContent, payload, metaModel, valueHelp, false);
      } else {
        ...
        if (fieldValue || fieldTextValue) {
          ...
          const isDigitSequence = metaModel.getObject(`${payload.propertyPath}${AnnotationIsDigitSequence}`) ?? false;
          if (field.hasPendingUserInput() || this._isEmptyDigitSequence(fieldValue, isDigitSequence)) {
            showIt = await this.checkIfContainerShouldBeDisplayed(typeAheadContent, payload, metaModel, valueHelp);
          } else {
    ...
    conditionForRequestShowContainer: async function (valueHelp, content) {
      try {
        ...
        if (control && control.data("fe_InitialTypeAheadFocusDone") === null && content.getControl().getDomRef()?.getElementsByTagName("input")[0].id === control.getInitialFocus()) {
          ...
          const payload = valueHelp.getPayload();
          const metaModel = valueHelpTable.getModel().getMetaModel();
          ...
          return await this.checkIfContainerShouldBeDisplayed(valueHelpTable, payload, metaModel, valueHelp);
        }
      } catch (error) {
        Log.error(error);
      }
     ...
     * @returns Boolean value
     */
    requestShowContainer: async function (valueHelp, container, requestShowContainerReason) {
