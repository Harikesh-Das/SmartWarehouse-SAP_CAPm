...
import type { $ManagedObjectSettings, AggregationBindingInfo } from "sap/ui/base/ManagedObject";
...
export default class MicroChart extends BuildingBlock<MicroChartContainer> {
	 ...
	 * @public
	 */
	@property({ type: "string", allowedValues: ["XS", "S", "M", "L"] })
	size?: string;
	 ...
	 */
	@event()
	titlePress!: EventHandler<UI5Event<{}, MicroChart>>;
	 ...
	 */
	private _initializeManifestSettings(): void {
		const owner = this._getOwner();
		const odataMetaModel = owner?.getMetaModel();
		const viewData = owner?.getRootController()?.getView().getViewData() ?? (owner?.getViewData() as unknown as ViewData);

		const converterContext = MacroAPI.getConverterContext(
			this._microChartDataModelObjectPath as DataModelObjectPath<PresentationVariant>,
			this.contextPath ?? "",
			{
		...
		const visualizationPath = getContextRelativeTargetObjectPath(
			this._microChartDataModelObjectPath as DataModelObjectPath<PresentationVariant>
		);
		if (visualizationPath) {
			const manifestConfig: MicroChartManifestConfiguration = converterContext.getManifestControlConfiguration(visualizationPath);
	...
	createTitle(titleText: string | undefined): TitleLink | undefined {
		if (titleText) {
			...
			let titleLinkProperties: PropertiesOf<TitleLink>;
			...
			if (this.navigationType === null || this.navigationType === undefined) {
				...
				titleLinkProperties.linkPress = (pressEvent: Event): void => {
					// Get the binding context from the MicroChartContainer (parent of TitleLink)
					const source = pressEvent.getSource() as unknown as TitleLink;
					const microChartContainer = source.getParent() as MicroChartContainer;
					const context = microChartContainer?.getBindingContext();

					// Fire the titlePress event with the context
					this.fireEvent("titlePress", { context: context });
				};
			} else {
			...
			return <TitleLink {...titleLinkProperties} />;
		}
	}
