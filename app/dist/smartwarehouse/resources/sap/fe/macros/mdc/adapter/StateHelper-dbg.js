...
sap.ui.define(["sap/base/Log", "sap/base/util/deepClone", "sap/fe/core/templating/PropertyHelper", "sap/fe/macros/filterBar/DraftEditState", "sap/fe/macros/mdc/adapter/SelectionVariantToStateFilters", "sap/fe/macros/mdc/adapter/StateFilterToSelectionVariant", "sap/fe/navigation/SelectionVariant", "sap/ui/mdc/p13n/StateUtil"], function (Log, deepClone, PropertyHelper, DraftEditState, svToStateFilters, StateFiltersToSelectionVariant, SelectionVariant, StateUtil) {
  ...
  const StateHelper = {
    /**
     * Gets the selection variant from the MDC control.
     * @param control The control for which the Selection Variant is to be fetched
    ...
    async getSelectionVariant(control) {
      try {
        ...
        let parameters = [];
        if (control?.getParent()?.isA("sap.fe.macros.FilterBar") && control.getParent().getParameters !== undefined) {
    ...
    async _getSupportedFilterFields(control) {
      if (control.isA("sap.ui.mdc.Table")) {
        return control.getParent().getEnhancedFetchedPropertyInfos();
      } else if (control.isA("sap.ui.mdc.Filterbar")) {
     ...
     * @param fromNavigationParameters Optional. If true, the selection variant is assumed to come from navigation parameters. In this case, if the selection variant contains select options for properties that are not present in the filter bar, these select options will be ignored and will not lead to clearing the filter bar.
    ...
    async setSelectionVariantToMdcControl(controlInstance, selectionVariant) {
      ...
      let fromNavigationParameters = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
      try {
        ...
        if (fromNavigationParameters && selectionVariant.getSelectOptionsPropertyNames().length > 0 && Object.keys(conditions).length === 0) {
          // If the selection variant has select options but no conditions could be mapped to the filter bar, we return an empty array
          // This is to avoid clearing the filter bar in this case
          // This can happen if the selection variant has only parameters or select options for properties not present in the filter bar
          // In this case, we assume that the selection variant is not meant to clear the filter bar
          // and we do not apply it
          return undefined;
        }
