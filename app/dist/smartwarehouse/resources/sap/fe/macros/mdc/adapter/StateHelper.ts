...
const StateHelper = {
	...
	async getSelectionVariant(control: CoreControl | undefined): Promise<SelectionVariant> {
		try {
			...
			if (control?.getParent()?.isA("sap.fe.macros.FilterBar") && (control.getParent() as FilterBarAPI).getParameters !== undefined) {
	...
	async _getSupportedFilterFields(control: Control): Promise<ControlPropertyInfo[]> {
		if ((control as Table).isA("sap.ui.mdc.Table")) {
			return ((control as Table).getParent() as TableAPI).getEnhancedFetchedPropertyInfos() as unknown as ControlPropertyInfo[];
	...
	async setSelectionVariantToMdcControl(
		...
		try {
			...
			if (
				...
				selectionVariant.getSelectOptionsPropertyNames().length > 0 &&
