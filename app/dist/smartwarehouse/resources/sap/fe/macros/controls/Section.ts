		...
		if (contentRole === "provider") {
			...
			if (sectionOnlyContent.sendDataToConsumer) {
				...
				sectionOnlyContent.sendDataToConsumer({
					titleLevel: hasMultipleSubsections ? "H3" : "H2", // For section header level is H2 the for subsections header level should be H3,
					title: title,
				...
				if (!hasMultipleSubsections) {
					...
					const blocks = subSections[0].getBlocks();
					// if the subsection has only one table, we add aria-labelledby to the section for the table title
					const content =
						blocks.length === 1 ? ((blocks[0] as SubSectionBlock).getAggregation("content") as ManagedObject) : undefined;
					if (content?.isA<TableAPI>("sap.fe.macros.Table")) {
						const table = content;
						table.content
							.initialized()
							.then(() => {
								const tableTitle = table.content.findElements(true, (elem) => elem.isA("sap.m.Title"));
								if (tableTitle.length === 1) {
									this.$().addAriaLabelledBy(tableTitle[0].getId());
								}
								return;
							})
							.catch(() => {
								Log.error("Failed to set aria-labelledby on section");
							});
					}
				}
			}
