...
import type FilterBarBaseLayout from "sap/ui/mdc/filterbar/FilterBarBaseLayout";
import MdcFilterContainer from "sap/ui/mdc/filterbar/FilterContainer";
...
class FilterContainer extends MdcFilterContainer implements IFilterContainer {
	...
	getAllButtons(): Button[] {
		const innerLayout = this.getInner() as unknown as FilterBarBaseLayout;
		return innerLayout.getButtons();
	}

	removeButton(oControl: Button): void {
		const innerLayout = this.getInner() as unknown as FilterBarBaseLayout;
		innerLayout.removeButton(oControl);
	}
