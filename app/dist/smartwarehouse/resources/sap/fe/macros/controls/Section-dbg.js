      ...
      if (contentRole === "provider") {
        ...
        if (sectionOnlyContent.sendDataToConsumer) {
          ...
          sectionOnlyContent.sendDataToConsumer({
            titleLevel: hasMultipleSubsections ? "H3" : "H2",
            // For section header level is H2 the for subsections header level should be H3,
            title: title,
          ...
          if (!hasMultipleSubsections) {
            ...
            const content = blocks.length === 1 ? blocks[0].getAggregation("content") : undefined;
            if (content?.isA("sap.fe.macros.Table")) {
              const table = content;
              table.content.initialized().then(() => {
