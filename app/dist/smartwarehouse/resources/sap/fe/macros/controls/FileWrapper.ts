...
class FileWrapper extends FieldWrapper {
	...
	@aggregation({ type: "sap.fe.macros.controls.Avatar", multiple: false })
	...
	_getSideEffectController(): SideEffectsControllerExtension | undefined {
		...
		return controller ? controller.sideEffects : undefined;
	}
	...
	getUploadUrl(): string {
		...
		if (context && this.uploadUrl) {
			// If the backend did not explicitly set AddressViaNavigationPath, we need to use the canonical path for upload
			if (context.getModel().getMetaModel().getObject("/@com.sap.vocabularies.Common.v1.AddressViaNavigationPath") === true) {
				return this.uploadUrl;
			} else {
				return this.uploadUrl.replace(context.getPath(), context.getCanonicalPath());
			}
		}
		return "";
	}
	...
	static render(renderManager: RenderManager, fileWrapper: FileWrapper): void {
		...
		if (fileWrapper.avatar) {
			...
			// In display mode we can only occupy 90% otherwise the link would be cut off when in a table
			// In edit mode we have to reserve some space for the file upload and delete button (2rem each + 1rem margin)
			innerDivMaxWidth = CommonUtils.getIsEditable(fileWrapper as unknown as ManagedObject) ? "calc(100% - 6rem)" : "90%";
