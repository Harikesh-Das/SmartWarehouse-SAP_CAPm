 ...
 */
sap.ui.define(["sap/base/Log", "sap/fe/base/ClassSupport", "sap/fe/core/buildingBlocks/BuildingBlock", "sap/fe/core/controllerextensions/messageHandler/messageHandling", "sap/m/Button", "sap/m/MessageItem", "sap/m/MessagePopover", "sap/m/library", "sap/ui/core/Element", "sap/ui/core/Messaging", "sap/ui/core/message/MessageType", "sap/fe/base/jsx-runtime/jsx"], function (Log, ClassSupport, BuildingBlock, messageHandling, Button, MessageItem, MessagePopover, library, UI5Element, Messaging, MessageType, _jsx) {
  "use strict";
  ...
  let MessageButton = (_dec = defineUI5Class("sap.fe.macros.MessageButton"), _dec2 = event(), _dec(_class = (_class2 = /*#__PURE__*/function (_BuildingBlock) {
     ...
     * And calls the MessageHandler.beforeShowMessageButton controller extension to allow consumers to adapt the messages that are shown.
     *
    ...
    _proto.processMessages = async function processMessages() {
      const messages = messageHandling.getMessages(true);
      const messageDetails = messages.reduce((reducedMessages, message) => {
        const technicalDetails = message.getTechnicalDetails();
        const isCustomMessageFromExtensionAPI = technicalDetails?.isCustomMessageFromExtensionAPI === true;
        const isAPDMessage = message.getTargets()[0]?.includes("$Parameter");
        if (!isCustomMessageFromExtensionAPI && !isAPDMessage) {
     ...
     *
     * If an activeTitleHandler is provided by the consumer using the controller extension then it is called.
     * Otherwise, the message popover is closed and focus is set on the target control.
     * @param messageItem
     */;
    _proto.handleActiveTitlePress = function handleActiveTitlePress(messageItem) {
      ...
      if (messageDetail.activeTitleHandler) {
        ...
        const controlIds = messageDetail.message.getControlIds();
        ...
        Log.warning("MessageButton: No focusable control found for message");
      }
    }

    /**
     * Closes the message popover and focuses the given control after it closes, which replicates the MessageButton behavior of the object page.
     * @param control The control to focus.
     */;
    _proto.focusControl = function focusControl(control) {
      ...
      const fnOnClose = () => {
        control.focus();
        this._messagePopover.detachEvent("afterClose", fnOnClose);
      };
      this._messagePopover.attachEvent("afterClose", fnOnClose);
      this._messagePopover.close();
    };
    _proto.destroy = function destroy() {
      this._mmPropertyBinding.destroy();
      _BuildingBlock.prototype.destroy.call(this);
    }
    /**
    ...
    _proto.createContent = function createContent() {
      ...
      button.data("messageButtonId", this.getId(), true);
      return button;
    };
