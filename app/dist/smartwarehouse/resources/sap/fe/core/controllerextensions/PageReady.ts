...
class PageReadyControllerExtension extends ControllerExtension {
	...
	public registerAggregatedControls(mainBindingContext?: Context): Promise<void>[] {
		...
		controls.forEach((element: ManagedObject & { mBindingInfos?: Record<string, { binding?: Binding }> }): void => {
			...
			if (element.isA<TableAPI>("sap.fe.macros.Table") || element.isA<ChartBlock>("sap.fe.macros.Chart")) {
				const tableOrChart = element.isA<TableAPI>("sap.fe.macros.Table") ? element.getContent() : element.getChartControl();

				if (tableOrChart && tableOrChart.getBindingContext() !== null) {
