...
export type NavigationParameter = {
	...
	/**
	 * Defines whether the navigation selection variant has defaults only, which is relevant for deciding whether defaults from FLP and manifest/annotations need to be applied.
	 * @private
	 */
	navSelVarHasDefaultsOnly?: boolean;
};
...
class ViewState extends ControllerExtension {
	...
	async collectResults(fnCall: Function, ...args: unknown[]): Promise<Control[]> {
		const aResults: (Control | Promise<Control>)[] = [];
		args.push(aResults);
	...
	async applyViewState(
		...
		try {
			const beforeStatePromises: Promise<unknown>[] = [];
			this.base.viewState.onBeforeStateApplied.call(this, beforeStatePromises, oNavParameter.navigationType);
			await Promise.all(beforeStatePromises);

			//For transient AppState, we return without applying the state to controls in RootContainer's children views as there is no state to apply
	...
	_setInitialStatesForDeltaCompute = async (variantManagement?: VariantManagement): Promise<void> => {
		try {
			...
			await this.updateInitialState(variantControls);
	...
	_addEventListenersToVariantManagement(variantManagement: VariantManagement, variantControls: string[]): void {
		...
		const fnEvent = async (): Promise<void> => {
			await this._updateInitialStatesOnVariantChange(variantControls);
		};
		variantManagement.attachSave(oPayload, fnEvent, {});
		variantManagement.attachSelect(oPayload, fnEvent, {});
	}

	async _updateInitialStatesOnVariantChange(vmAssociatedControlsToReset: string[]): Promise<void> {
		const initialControlStatesMapper = this.initialControlStatesMapper;
		...
		await this.updateInitialState(vmAssociatedControlsToReset);
	}
