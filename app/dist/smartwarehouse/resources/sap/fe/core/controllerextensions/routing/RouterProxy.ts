...
type HistoryAction = BackHistoryAction | OtherHistoryAction;
...
class RouterProxy extends BaseObject {
	...
	_currentNavigationPromise?: Promise<void>;
	...
	initRaw(oRouter: Router): void {
		...
		this._oRouter.attachBeforeRouteMatched(() => {
			this._currentNavigationPromise = new Promise<void>((resolve) => {
	...
	async navToHash(
		...
		bForceFocus?: boolean,
		preserveShellBackNavigationHandler?: boolean,
		...
		if (this._oRouteMatchSynchronization) {
			return this._oRouteMatchSynchronization.waitFor().then(async () => {
				...
				return this._internalNavToHash(
					sHash,
					bPreserveHistory,
					bDisablePreservationCache,
					bForceFocus,
					skipHashGuardCheck,
					focusControl
				);
	...
	async navTo(
		sRouteName: string,
		oParameters?: { noPreservationCache?: boolean; bIsStickyMode?: boolean; isHiddenDraftEnabled?: boolean }
	): Promise<boolean> {
		const sHash = this._oRouter.getURL(sRouteName, oParameters);
		return this.navToHash(
			sHash!,
			false,
			oParameters?.noPreservationCache,
			false,
			(oParameters?.bIsStickyMode ?? false) || (oParameters?.isHiddenDraftEnabled ?? false)
		);
	}
	 ...
	 * @final
	 */
	async _rebuildBrowserHistory(oHistoryAction: HistoryAction, bRebuildOnly: boolean): Promise<boolean> {
	 ...
	 */
	private checkIsInFlpIframe(): boolean {
	 ...
	 */
	private tryFlpIframeRebuildShortcut(popCount: number, hasPoppedDraft: boolean): HistoryAction | undefined {
	 ...
	 */
	private propagateDraftToActiveFlip(hash: string, mapping: { oldPath: string; newPath: string }): string {
	 ...
	 *
	 */
	private storeFocusInfoForCurrentHash(focusRefControl: Element | undefined): void {
		const currentHash = removeAppStateInHash(this.getHash());
		const stateForHash = this._oManagedHistory.find((state) => {
			return removeAppStateInHash(state.hash) === currentHash;
		});
