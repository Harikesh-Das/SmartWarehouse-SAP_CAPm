...
import { isFulfilled } from "sap/fe/core/helpers/TypeGuards";
import type Field from "sap/fe/macros/Field";
import type { default as TableAPI, TableColumnProperties } from "sap/fe/macros/Table";
...
export type messageHandlingType = {
	...
	getRowIdentifierFromTargetPath: (message: MessageWithHeader, table: MDCTable) => string | undefined;
	...
	isMessageTargetContextPath: (oMessageObject: MessageWithHeader, oContextPath?: string | RegExp) => boolean;
};
...
function getTransitionMessagesForUIDecision(this: messageHandlingType, showBoundTransition: boolean, _context: ODataV4Context): Message[] {
	...
	if (showBoundTransition) {
		...
		// Always include ETag messages (412 with concurrent modification) even if showBoundTransition is false
		// These are critical messages that users must see to know about concurrent edit conflicts
		const eTagMessages = this.getMessages(true, true).filter((message) => {
			const technicalDetails = message.getTechnicalDetails() as MessageTechnicalDetails;
			return technicalDetails?.httpStatus === 412 && technicalDetails?.isConcurrentModification;
		});
		transitionMessages = transitionMessages.concat(eTagMessages);
	}
...
function addGenericSuccessMessage(
	...
	const operationResults = showMessageDialogParameter?.operationResults;
	const numberOfProcessedContexts = operationResults ? operationResults.filter(isFulfilled).length : 0;
	if (numberOfProcessedContexts > 1 && resourceModel) {
		...
		return new Message({
			message: resourceModel.getText("C_COMMON_SUCCESS_MESSAGE", [numberOfProcessedContexts], suffixResourceKey),
...
function showMessageBoxPerType(formattedText: string, close: { onClose: Function }, message: Message): void {
	const parameters = { ...close, closeOnNavigation: false };
	switch (message.getType()) {
		case MessageType.Error:
			MessageBox.error(formattedText, parameters);
			break;
		case MessageType.Warning:
			MessageBox.warning(formattedText, parameters);
			break;
		case MessageType.Success:
			MessageBox.success(formattedText, parameters);
			break;
		case MessageType.Information:
			MessageBox.information(formattedText, parameters);
			break;
		default:
			MessageBox.show(formattedText, parameters);
	}
}
...
function setGroupNameLRTable(oElem: Control | undefined, aModelData: MessageWithHeader, sActionName: string | undefined): void {
	...
	if (oRowBinding) {
		...
		if (aModelData.target?.indexOf(sElemeBindingPath) === 0) {
			// Determine the underlying table type to decide which method to use
			let childTableElement: UI5Element | undefined;
			(oElem as MDCTable).findElements(true).forEach((element) => {
				if (element.isA<UITable>("sap.ui.table.Table") || element.isA<Table>("sap.m.Table")) {
					childTableElement = element;
				}
			});
			// Use getContexts() for GridTable, getCurrentContexts() for other tables to get all contexts
			// without violating extended change detection protocol
			const allRowContexts = childTableElement?.isA<UITable>("sap.ui.table.Table")
			...
			allRowContexts.forEach((rowContext: Context) => {
				if (aModelData.target?.includes(rowContext.getPath())) {
					...
					const targetContextPath = messageHandling.isMessageTargetContextPath(aModelData, rowContext.getPath());

					// if target has some column name and column is visible in UI
					if ((columnPropertyName && sTableTargetColName) || targetContextPath) {
...
function updateAddtionalTextForMessageInOPTable(
	...
	if (identifierColumn) {
		...
		// For tree tables with collapsed nodes, row contexts are not loaded via getContexts().
		// Extract the row identifier directly from the message target path as a fallback.
		if (!rowIdentifierFound) {
			const rowIdentifierFromPath = messageHandling.getRowIdentifierFromTargetPath(message, mdcTable);
			if (rowIdentifierFromPath) {
				const columnNameSuffix = targetTableInfo.sTableTargetColName ? `, ${targetTableInfo.sTableTargetColName}` : "";
				message.setAdditionalText(`${rowIdentifierFromPath}${columnNameSuffix}`);
			}
		}
	...
	if (isCreationRow || tableRowContext?.isInactive()) {
		...
		if (tableRowBindingContexts.length > 0) {
			...
			if (targetedControl) {
				...
				// For tree tables with collapsed nodes, the row context is not available in getContexts().
				// Try to extract the row identifier directly from the message target path.
				rowSubtitleValue = messageHandling.getRowIdentifierFromTargetPath(message, table);
			}
...
/**
 * Checks if the message target exactly matches the provided context path.
 * This function is used specifically for message grouping in List Report (LR) tables
 * to determine if a message is targeted at the context path level rather than at
 * a specific property within the context.
 * @param messageObject The message object with header information
 * @param contextPath The context path to compare against (string or RegExp)
 * @returns True if the message target exactly equals the context path, false otherwise
 */
function isMessageTargetContextPath(messageObject: MessageWithHeader, contextPath?: string | RegExp): boolean {
	return messageObject.getTargets()[0] === contextPath;
}
...
/**
 * For tree tables with collapsed nodes, the row binding context is not available via getContexts()
 * because collapsed children are not loaded into the binding. This function extracts the row identifier
 * value directly from the message target path by parsing the entity key predicate.
 * @param message Message object with target path
 * @param table MDC table (must contain a sap.ui.table.TreeTable as inner control)
 * @returns The identifier value for the row, or undefined if not applicable
 */
function getRowIdentifierFromTargetPath(message: MessageWithHeader, table: MDCTable): string | undefined {
	// Only applies to tree tables
	const isTreeTable = table.findElements(true, (el) => el.isA("sap.ui.table.TreeTable")).length > 0;
	if (!isTreeTable) {
		return undefined;
	}
	const identifierColumn = (table.getParent() as TableAPI).getIdentifierColumn();
	if (!identifierColumn) {
		return undefined;
	}
	...
	// getIdentifierColumn can return a string or an array of strings.
	// Normalize to array to handle both cases uniformly.
	const identifierColumns = Array.isArray(identifierColumn) ? identifierColumn : [identifierColumn];
	// Extract the identifier value from the entity key predicate in the message target path.
	// The key predicate can appear at the start of the predicate list or after a comma, e.g.:
	// (IdentifierColumn='value',...) or (...,IdentifierColumn='value',...)
	// Also handles unquoted numeric keys: (...,IdentifierColumn=123,...)
	// The path may contain multiple key predicates (e.g. parent entity + child entity).
	// We use the LAST match to get the deepest (most specific) entity key — i.e., the row entity.
	for (const column of identifierColumns) {
		const keyPredicateRegex = new RegExp(`[,(]${column}=(?:'([^']*)'|(\\w+))`, "g");
		let lastMatch: RegExpExecArray | null = null;
		let currentMatch: RegExpExecArray | null;
		while ((currentMatch = keyPredicateRegex.exec(messageTarget)) !== null) {
...
const messageHandling: messageHandlingType = {
	...
	getRowIdentifierFromTargetPath: getRowIdentifierFromTargetPath,
	...
	isMessageTargetContextPath: isMessageTargetContextPath
};
