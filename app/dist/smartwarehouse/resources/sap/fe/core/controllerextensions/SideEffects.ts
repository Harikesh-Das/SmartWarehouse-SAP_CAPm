...
class SideEffectsControllerExtension extends ControllerExtension {
	...
	@publicExtension()
	@finalExtension()
	getContextForSideEffects(bindingContext: Context | null | undefined, sideEffectEntityType: string): Context | undefined {
		if (!bindingContext) {
			return undefined;
		}
	...
	prepareDeferredSideEffectsForField(
		event:
			| CheckBox$SelectEvent
			| InputBase$ChangeEvent
			| Field$ChangeEvent
			| RatingIndicator$ChangeEvent
			| MultiValueField$ChangeEvent
			| RadioButtonGroup$SelectEvent
	...
	async onEventDrivenSideEffectsReceived(_mParameters: {
		sideEffectEventName: string;
		sideEffectSource: string;
	}): Promise<{ interactionType?: "Notification" | "Confirmation" | "None" }> {
	...
	isDataPathRelevant(path: string, _eventName: string): boolean {
		const context = this.getView().getBindingContext() as Context | null | undefined;
		if (!context) {
			return false;
		}
	...
	_contextIsRelevant(path: string, context: Context): boolean {
		...
		if (
			!contextPath ||
			isAction(getInvolvedDataModelObjectsForTargetPath<unknown>(contextPath, this._view.getModel().getMetaModel())?.targetObject)
		) {
			return false;
		}
		...
		for (const binding of dependentBindings.filter(
			(binding) => !binding.isA<ODataContextBinding>("sap.ui.model.odata.v4.ODataContextBinding")
		)) {
