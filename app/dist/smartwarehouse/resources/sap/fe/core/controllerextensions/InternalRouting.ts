...
class InternalRouting extends ControllerExtension {
	...
	private _pendingTargetControlIdForEdit?: string;
	...
	beforeSetBindingContext(bindingContext: Context | null, mParameters?: unknown): void {
		...
		const requestOnBinding = (mParameters as { requestOnBinding?: string[] })?.requestOnBinding;
		if (bindingContext && requestOnBinding && requestOnBinding.length > 0) {
			bindingContext.requestProperty(requestOnBinding).catch((e: string | Error) => {
	...
	async navigateToContext(context: Context | ODataListBinding, parameters: RoutingNavigationParameters = {}): Promise<boolean> {
		if (context.isA<ODataListBinding>("sap.ui.model.odata.v4.ODataListBinding")) {
			...
			const overrideNav = await this.base
				.getView()
				.getController()
				.routing.onBeforeNavigation({ bindingContext: contextToNavigate, openInNewTab: parameters?.navMode === "openInNewTab" });
		...
		const hiddenDraftEnabled = (this._oAppComponent?.getEnvironmentCapabilities()?.getCapabilities()?.HiddenDraft as HiddenDraft)
			?.enabled;
		const isDocumentModified = this._oAppComponent?.getModel("ui")?.getProperty(UiModelConstants.DocumentModified) ?? true;
		if (
			this._oAppComponent.getRootViewController().isFclEnabled() &&
			this.getFCLLevel() === 0 &&
			hiddenDraftEnabled &&
			!isDocumentModified
		) {
	...
	async navigateToMessagePage(sErrorMessage: string, mParameters: NavigationToErrorPageResult): Promise<boolean> {
		...
		const navToCreateHash =
			mParameters.showMessagePageOnCreateFailure !== true &&
			(this._oRouterProxy.getHash().includes("i-action=create") || this._oRouterProxy.getHash().includes("i-action=autoCreate"));
		if (navToCreateHash) {
		...
		if (!navigationParameters.createOnNavigateParameters || navigationParameters.createOnNavigateParameters.mode === "Deferred") {
			...
			if (this._oPageComponent && this._oPageComponent.createDeferredContext) {
				this._oPageComponent.createDeferredContext(
					targetPath,
					navigationParameters.createOnNavigateParameters ?? {},
	 ...
	 */
	private async displayErrorPageForWrongSemanticURL(): Promise<void> {
		const resourceBundle = Library.getResourceBundleFor("sap.fe.core")!;
	...
	_createContext(sPath: string, oModel: ODataModel): Context {
		...
		if (oRootViewController.isFclEnabled()) {
			...
			if (!oContext) {
				...
				const propertiesToRequest = ["HasActiveEntity", "HasDraftEntity", "IsActiveEntity"]
					.concat(semanticKeys)
					.concat(alternateAndSecondaryKeys);
	 ...
	 */
	_setBindingContext(context: Context | null): void {
		let previousContext: Context, targetControl: View | EnhanceWithUI5<TemplateComponent>;
		if (this._oPageComponent) {
			previousContext = this._oPageComponent.getBindingContext() as Context;
		...
		if (previousContext?.isKeepAlive() && previousContext !== context) {
			...
			if (!rootViewController.getInstancedViews().some((pageView: View) => pageView.getBindingContext?.() === previousContext)) {
	...
	switchFullScreen(): void {
		...
		this.base._routing.navigateToContext(oContext, { layout: sNextLayout, persistOPScroll: true }).catch(function () {
