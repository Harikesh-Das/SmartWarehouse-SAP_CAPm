 ...
 */
sap.ui.define(["sap/base/Log", "sap/fe/base/ClassSupport", "sap/fe/core/helpers/Synchronization", "sap/ui/base/Object", "sap/ui/core/Element"], function (Log, ClassSupport, Synchronization, BaseObject, Element) {
  "use strict";
  ...
  let RouterProxy = (_dec = defineUI5Class("sap.fe.core.RouterProxy"), _dec(_class = /*#__PURE__*/function (_BaseObject) {
    ...
    _proto.initRaw = function initRaw(oRouter) {
      ...
      this._oRouter.attachBeforeRouteMatched(() => {
        this._currentNavigationPromise = new Promise(resolve => {
          this._oRouter.attachEventOnce("routeMatched", resolve);
        });
      }, this);
    }
     ...
     * @param preserveShellBackNavigationHandler If set to true, the back navigation callback is preserved. If undefined or false, the back navigation callback is reset.
     * @param skipHashGuardCheck If set to true, guard check is skipped
     * @param focusControl The focus control to use as reference for the current hash before navigating
     ...
     */;
    _proto.navToHash = async function navToHash(sHash, bPreserveHistory, bDisablePreservationCache, bForceFocus, preserveShellBackNavigationHandler, skipHashGuardCheck, focusControl) {
      sHash = sHash.startsWith("/") ? sHash.substring(1) : sHash;
      if (!preserveShellBackNavigationHandler) {
      ...
      if (this._oRouteMatchSynchronization) {
        return this._oRouteMatchSynchronization.waitFor().then(async () => {
          ...
          return this._internalNavToHash(sHash, bPreserveHistory, bDisablePreservationCache, bForceFocus, skipHashGuardCheck, focusControl);
        });
      } else {
    ...
    _proto._internalNavToHash = async function _internalNavToHash(sHash, bPreserveHistory, bDisablePreservationCache, bForceFocus, skipHashGuardCheck, focusControl) {
      ...
      if (!this.checkHashWithGuard(sHash) && !(skipHashGuardCheck ?? false)) {
        // confirmGuardCross is registered when entering edit mode (same time as guard is set)
        // It shows the appropriate data loss dialog and handles session discard
        const userConfirmed = await this._oAppComponent.getRootViewController().confirmGuardCross?.();
      ...
      this.storeFocusInfoForCurrentHash(focusControl);
    ...
    _proto.navBack = async function navBack() {
      ...
      if (sPreviousHash) {
        ...
        // in that case either :
        // - we go back to the FLP via the history back or,
        // - if the create app was opened in a new tab, close this tab
        const sapHistory = window.history.state?.sap?.history;
        const shouldClose = Array.isArray(sapHistory) && sapHistory.length === 1 && sapHistory[0]?.includes("(...)");
        if (shouldClose) {
          window.close();
        } else {
          window.history.back();
     ...
     * @param oParameters.isHiddenDraftEnabled
     * @returns Promise that is resolved when the navigation is finalized
     */;
    _proto.navTo = async function navTo(sRouteName, oParameters) {
      const sHash = this._oRouter.getURL(sRouteName, oParameters);
      return this.navToHash(sHash, false, oParameters?.noPreservationCache, false, (oParameters?.bIsStickyMode ?? false) || (oParameters?.isHiddenDraftEnabled ?? false));
    }
    ...
    /**
     * Waits for the current navigation to be finalized.
     * If no navigation is in progress, resolves immediately.
     * @returns Promise resolved when the navigation is finalized
     */;
    _proto.waitForNavigation = async function waitForNavigation() {
      return this._currentNavigationPromise ?? Promise.resolve();
    }
    ...
    _proto._pushNewState = function _pushNewState(oNewState, bRebuildOnly, bPreserveHistory, bDisableHistoryPreservation) {
      ...
      // Tracks whether any popped entry held a draft (IsActiveEntity=false). Used by the FLP-iframe
      // rebuild shortcut to decide between an in-place hash rewrite and walking the back-stack to
      // clear stale draft slots.
      let hasPoppedDraft = false;
      while (this._oManagedHistory.length > 0) {
        ...
        if ((bDisableHistoryPreservation || !oTopState.preserved) && this._compareCacheStates(oTopState, oNewState) !== enumState.ANCESTOR) {
          ...
          if (oLastRemovedItem.hash.includes("IsActiveEntity=false")) {
            hasPoppedDraft = true;
          }
          iPopCount++;
      ...
      if (bRebuildOnly && this.checkIsInFlpIframe()) {
        const flpIframeAction = this.tryFlpIframeRebuildShortcut(iPopCount, hasPoppedDraft);
        if (flpIframeAction) {
          return flpIframeAction;
        }
      }
      if (iPopCount === 0) {
        ...
        // More than 1 state was popped --> go back in history and replace hash if necessary
        return bHasSameHash ? {
          type: "back",
          steps: iPopCount - 1
        } : {
    ...
    _proto.setPathMapping = function setPathMapping(mappings) {
      ...
      const filteredNew = mappings.filter(mapping => mapping.oldPath !== mapping.newPath);
      // Accumulate: keep existing mappings whose oldPath is not being overridden by the new set.
      // This ensures that a Create-activation mapping (Create draft → active entity) is not lost
      // when a subsequent Edit-activation maps a different draft to the same active entity (FCL scenario).
      const newOldPaths = new Set(filteredNew.map(m => m.oldPath));
      this._pathMappings = [...this._pathMappings.filter(m => !newOldPaths.has(m.oldPath)), ...filteredNew];
    }

    /**
     * Detects whether the app is running inside the FLP UI5 app-runtime iframe (cFLP/portal).
     * In that context the shell parent and the iframe each own a history stack, so the timing
     * and effect of `history.go(-N)` differ from a direct flpSandbox.html execution.
     * @returns True when running inside the FLP UI5 app-runtime.
     */;
    _proto.checkIsInFlpIframe = function checkIsInFlpIframe() {
      const container = sap.ui.require("sap/ushell/Container");
      return container?.inAppRuntime() === true;
    }

    /**
     * Returns the history action to use for a shell-hierarchy rebuild inside the FLP UI5 app-runtime
     * iframe, or `undefined` when the standard back/back-replace path should still apply.
     *
     * Rewriting the top entry in place avoids the `router.stop()/initialize()` triggered by
     * `history.go(-N)`, which interrupts in-flight binding setup (e.g. Edit + Cancel after a
     * shell-back).
     *
     * The shortcut is skipped when popped entries contain a draft, because the back-stack must
     * actually be walked to clean stale draft slots — otherwise a subsequent shell-back lands
     * sideways into a draft.
     * @param popCount Number of states popped during the rebuild
     * @param hasPoppedDraft Whether any of the popped states held a draft (IsActiveEntity=false)
     * @returns A `replace` action when the shortcut applies, `undefined` otherwise.
     */;
    _proto.tryFlpIframeRebuildShortcut = function tryFlpIframeRebuildShortcut(popCount, hasPoppedDraft) {
      if (popCount > 1 && hasPoppedDraft) {
        return undefined;
      }
      return {
        type: "replace"
    ...
    /**
     * When a path mapping is a draft → active flip on a parent path, propagate the flip to deeper segments.
     * In OData V4 draft semantics, an active parent can only have active children, so a stale history entry
     * holding a sub-draft path would otherwise be rewritten to an invalid mixed-mode URL and trigger 404s.
     * @param hash The hash already rewritten with the parent mapping
     * @param mapping The mapping that was applied
     * @param mapping.oldPath The original path before rewriting
     * @param mapping.newPath The rewritten path
     * @returns The hash with deeper draft segments flipped to active
     */;
    _proto.propagateDraftToActiveFlip = function propagateDraftToActiveFlip(hash, mapping) {
      if (!mapping.oldPath.includes("IsActiveEntity=false") || !mapping.newPath.includes("IsActiveEntity=true")) {
        return hash;
      }
      const idxAfter = hash.indexOf(mapping.newPath) + mapping.newPath.length;
      return hash.slice(0, idxAfter) + hash.slice(idxAfter).replace(/IsActiveEntity=false/g, "IsActiveEntity=true");
    };
    _proto.hashGuard = function hashGuard() {
      ...
      if (sHash.includes("restoreHistory=true")) {
        ...
        if (mapping) {
          ...
          sHash = this.propagateDraftToActiveFlip(sHash, mapping);
     ...
     *
     */;
    _proto.storeFocusInfoForCurrentHash = function storeFocusInfoForCurrentHash(focusRefControl) {
      const currentHash = removeAppStateInHash(this.getHash());
      const stateForHash = this._oManagedHistory.find(state => {
        return removeAppStateInHash(state.hash) === currentHash;
      });
      if (stateForHash) {
        const focusControl = focusRefControl ?? Element.getActiveElement();
