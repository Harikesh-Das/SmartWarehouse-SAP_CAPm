	...
	if (sMessagesPath) {
		...
		if (removeTransitionMessages) {
			...
			if (data[sMessagesPath].length > 0) {
				...
				const messagesToRemove = (Messaging.getMessageModel().getData() as Message[]).filter(
					(message: Message) => message.getPersistent() && !messageHandling.isNonTechnicalMessage(message)
				);
		...
		const resolvedCanonicalPaths = canonicalPaths.map((canonicalPath, index) => {
			if (canonicalPath?.endsWith("/SiblingEntity")) {
				...
				if (activePath) {
					...
					const derivedPath = activePath.replace(
						/IsActiveEntity=(true|false)/g,
						(_, val) => `IsActiveEntity=${val === "true" ? "false" : "true"}`
					);
...
async function activateDocument(
	...
	if (!draft.hasPrepareAction(oContext)) {
		oActiveDocumentContext = await draft.executeDraftActivationAction(
			oContext,
			oAppComponent,
			messageHandler,
			sBatchGroup,
		...
		const ignoreEtag: boolean = oContext.getModel().hasPendingChanges(sBatchGroup) ? true : false;
...
async function deleteDraft(
	...
	if (bIsActiveEntity || (!bIsActiveEntity && !sDiscardAction)) {
		...
		if (oContext.hasPendingChanges() && !oContext.isInactive()) {
			...
			const useModelDelete =
				!oAppComponent._isFclEnabled() &&
				!oContext.getProperty("IsActiveEntity") &&
				ModelHelper.isDraftRoot(targetEntitySet) &&
 ...
 */
function terminateDraftSession(contextOrModel: Context | ODataModel): void {
	// Extract the model from context if a context was passed, otherwise use the model directly
	const model = contextOrModel?.isA<Context>("sap.ui.model.odata.v4.Context")
		? contextOrModel.getModel()
		: (contextOrModel as ODataModel);
	...
	if (model?.getHttpHeaders(true)?.["SAP-ContextId"]) {
		try {
			...
			(
				model as unknown as {
					oRequestor: { sendRequest: (method: string, url: string, headers: Record<string, string>) => void };
				}
			).oRequestor.sendRequest("HEAD", "", { "sap-terminate": "session" });
		} catch (error) {
			// Log error but don't block the operation
			Log.info("Failed to terminate ABAP session during draft cleanup", error as string);
		}
	}
}
