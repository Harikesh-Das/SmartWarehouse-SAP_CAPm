...
sap.ui.define(["sap/base/Log", "sap/base/util/isEmptyObject", "sap/fe/core/CommonUtils", "sap/fe/core/controllerextensions/editFlow/draftDataLossPopup", "sap/fe/core/controllerextensions/messageHandler/messageHandling", "sap/fe/core/converters/MetaModelConverter", "sap/fe/core/helpers/ResourceModelHelper", "sap/fe/core/helpers/TypeGuards", "sap/m/Button", "sap/m/Dialog", "sap/m/MessageBox", "sap/m/Text", "sap/ui/core/Lib", "sap/ui/core/Messaging", "../../helpers/ModelHelper", "./operations/Operation"], function (Log, isEmptyObject, CommonUtils, draftDataLossPopup, messageHandling, MetaModelConverter, ResourceModelHelper, TypeGuards, Button, Dialog, MessageBox, Text, Library, Messaging, ModelHelper, Operation) {
    ...
    if (convertedAction) {
      const operation = new Operation(CommonUtils.getAppComponent(oView), oContext.getModel(), convertedAction, {
        ...
        oDataProperties: {
          ...
          ignoreETag: isEmptyObject(oContext.getObject()),
      ...
      if (result[0].status === "rejected") {
        const modifiedError = new Error(result[0].reason.message);
   ...
   * @returns Resolve function returns the context of the operation
   */
  async function executeDraftActivationAction(oContext, oAppComponent, messageHandler, sGroupId, oPreparePromise, resourceModel, isStandardSave) {
    ...
    if (!oContext.getProperty("IsActiveEntity")) {
      ...
      let label;
      if (isStandardSave === false) {
        label = resourceModel?.getText("C_OP_OBJECT_PAGE_SAVE");
      } else {
        label = resourceModel?.getText("T_OP_OBJECT_PAGE_SAVE");
      }
      if (convertedAction) {
        try {
          ...
          const result = await operation.execute();
          if (result[0].status === "rejected") {
            // Re-throw the original rejection so the OData-model-parsed SAP_Messages
            // (already placed in the message model by the OData V4 _Requestor) are
            // available for messageHandler.showMessages() in the upstream catch block.
            throw result[0].reason instanceof Error ? result[0].reason : new Error(String(result[0].reason));
    ...
    if (sMessagesPath) {
      ...
      if (removeTransitionMessages) {
        ...
        if (data[sMessagesPath].length > 0) {
          // State messages from the PreparationAction are now authoritative for
          // "what's wrong with the entity". Remove transition messages that were
          // created by the failed activation request, EXCEPT user-relevant backend
          // errors (those for which `isNonTechnicalMessage` returns true — i.e.
          // messages with httpStatus, originalMessage, or code present). Those are
          // validation feedback the user MUST still see (DINC0864223).
          const messagesToRemove = Messaging.getMessageModel().getData().filter(message => message.getPersistent() && !messageHandling.isNonTechnicalMessage(message));
   ...
   */
  async function computeSiblingInformation(rootCurrentContext, rightmostCurrentContext, rootContextInfo, groupId, bindingParameters) {
    let createKeepAliveContext = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : false;
    ...
    try {
      ...
      // For containment navigation properties, requestCanonicalPath cannot be resolved via SiblingEntity
      // because there is no $NavigationPropertyBinding for the contained entity. In that case, the sibling
      // path can be derived by flipping IsActiveEntity (true↔false), since containment draft entities
      // since containment draft entities share the same key structure between active and draft instances.
      const resolvedCanonicalPaths = canonicalPaths.map((canonicalPath, index) => {
        if (canonicalPath?.endsWith("/SiblingEntity")) {
          // canonicalPaths[i] corresponds to oldPaths[paths.length - 1 - i] (reversed order due to unshift)
          const activePath = oldPaths[paths.length - 1 - index];
          if (activePath) {
            // Flip IsActiveEntity to derive the sibling path. Works in both directions
            // (active→draft: true→false, draft→active: false→true).
            // If IsActiveEntity is absent from the path, derivedPath === activePath and we fall through.
            const derivedPath = activePath.replace(/IsActiveEntity=(true|false)/g, (_, val) => `IsActiveEntity=${val === "true" ? "false" : "true"}`);
            if (derivedPath !== activePath) {
              return derivedPath;
            }
          }
        }
      ...
      if (resolvedCanonicalPaths.some(path => path && path.endsWith("/SiblingEntity"))) {
        // At least one sibling entity doesn't exist
        return undefined;
      }
      if (rootContextInfo?.rootSiblingPathPromise) {
        const rootSiblingPath = (await rootContextInfo.rootSiblingPathPromise)?.getPath();
        resolvedCanonicalPaths.unshift(rootSiblingPath);
        oldPaths.push(rootCurrentContext.getPath());
      }
      let siblingPath = "";
      resolvedCanonicalPaths.forEach((canonicalPath, index) => {
        if (index !== 0) {
      ...
      if (createKeepAliveContext) {
        ...
        }; // We need to use the same group ID as for the parent list binding
      } else {
      ...
      return {
        targetContext: createKeepAliveContext ? model.getKeepAliveContext(siblingPath, !!messagePath, parameters) : model.bindContext(siblingPath, undefined, parameters).getBoundContext(),
   ...
   */
  async function createDraftFromActiveDocument(oContext, oAppComponent, mParameters, groupId, messageHandler, transactionDetails) {
    ...
    try {
      ...
      if (oResponse.status === 409 || oResponse.status === 423) {
        ...
        const siblingInfo = await draft.computeSiblingInformation(oContext, oContext, undefined, undefined, bindingParameters, oAppComponent._isFclEnabled());
   ...
   * @returns Promise resolves with the {@link sap.ui.model.odata.v4.Context context} of the new draft document
   */
  async function activateDocument(oContext, oAppComponent, messageHandler, mParameters, isStandardSave) {
    const mParam = mParameters || {};
    if (!oContext) {
    ...
    if (!draft.hasPrepareAction(oContext)) {
      oActiveDocumentContext = await draft.executeDraftActivationAction(oContext, oAppComponent, messageHandler, sBatchGroup, undefined, mParameters.resourceModel);
    } else {
      ...
      const ignoreEtag = oContext.getModel().hasPendingChanges(sBatchGroup) ? true : false;
      ...
      const oActivatePromise = draft.executeDraftActivationAction(oContext, oAppComponent, messageHandler, sBatchGroup, oPreparePromise, mParameters.resourceModel, isStandardSave);
  ...
  async function deleteDraft(oContext, messageHandler, oAppComponent, bEnableStrictHandling) {
    ...
    if (bIsActiveEntity || !bIsActiveEntity && !sDiscardAction) {
      ...
      if (oContext.hasPendingChanges() && !oContext.isInactive()) {
        ...
        const isInlineEditGroup = oContext.getUpdateGroupId() === CommonUtils.INLINEEDIT_UPDATEGROUPID;
        const metaModel = oContext.getModel().getMetaModel();
        const targetEntitySet = getInvolvedDataModelObjects(metaModel.getMetaContext(oContext.getPath()))?.targetEntitySet;
        const useModelDelete = !oAppComponent._isFclEnabled() && !oContext.getProperty("IsActiveEntity") && ModelHelper.isDraftRoot(targetEntitySet) && oContext.getProperty("HasActiveEntity");
        if (useModelDelete) {
          return oContext.getModel().delete(oContext);
        }
        if (isInlineEditGroup) {
  ...
  /**
   * Terminates the draft session on the server side.
   * @param contextOrModel The context or model to be terminated
   */
  function terminateDraftSession(contextOrModel) {
    // Extract the model from context if a context was passed, otherwise use the model directly
    const model = contextOrModel?.isA("sap.ui.model.odata.v4.Context") ? contextOrModel.getModel() : contextOrModel;

    // Only clean up ABAP sessions for draft programming model
    if (model?.getHttpHeaders(true)?.["SAP-ContextId"]) {
      try {
        // Access the private oRequestor property with proper typing, this has been aligned with the model colleagues
        // they might need to offer a dedicated restricted method for sap.fe in the future, but until then we can use
        // it like this.
        model.oRequestor.sendRequest("HEAD", "", {
          "sap-terminate": "session"
        });
      } catch (error) {
        // Log error but don't block the operation
        Log.info("Failed to terminate ABAP session during draft cleanup", error);
      }
    }
  }
  const draft = {
    ...
    terminateDraftSession: terminateDraftSession
  };
  return draft;
}, false);
