...
): Promise<Context | null | undefined> {
 ...
 */
export async function getDraftOrActiveContext(context: Context, fetchProperties: string[] = []): Promise<Context | null> {
 ...
 */
export async function resolvePath(pathToResolve: string, model: ODataModel, routingService: RoutingService): Promise<string | null> {
	const metaModel = model.getMetaModel();
