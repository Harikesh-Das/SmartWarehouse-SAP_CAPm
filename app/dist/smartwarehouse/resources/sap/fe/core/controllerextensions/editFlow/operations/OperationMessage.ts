...
export default class OperationMessage {
	...
	private filterApdAndContextMessages(
		...
		const isOperationDialogOpen = this.isParameterDialogOpened;
		const parameterDialogMessages = this.getParameterDialogMessages(messages);
		// Check if any APD-targeted message is a bound transition (non-empty target AND persistent)
		const hasBoundTransitionInAPD = parameterDialogMessages.some(
			(message: Message) => message.getTargets()?.[0] !== "" && message.getPersistent() === true
		);
		...
		const apdHasBoundTransition = isOperationDialogOpen && errorTargetsInAPD && hasBoundTransitionInAPD;
		if (!(this.table && CommonUtils.getIsEditable(this.table))) {
			if (isOperationDialogOpen && errorTargetsInAPD) {
				if (apdHasBoundTransition) {
					// Set to true to enable focus on APD error fields via isMessageOutOfParameterDialog
					containsBoundTransition = true;
					showMessageDialog = false;
				} else if (unboundMessages.length === 0) {
