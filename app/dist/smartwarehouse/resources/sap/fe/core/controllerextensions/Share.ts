...
type ShareOptionsActions = {
	press: string;
};

export type ShareOptions = {
	teams?: {
		showMsTeamsOptions?: boolean | string;
		asCard?: boolean | string;
	};
	showSendEmail?: boolean | string;
	showCollaborationManager?: boolean | string;
	showSaveAsTile?: boolean | string;
	/**
	 * Toggle visibility of the "Share in SAP Build Work Zone" option.
	 * It may be shown as "Share in SAP Jam" or "Share in SAP Build Work Zone"
	 * depending on deployment. This setting applies to both labels. Set false to hide.
	 */
	showShareInWorkzone?: boolean | string;
	print?: ShareOptionsActions;
	exportToCSV?: ShareOptionsActions;
	exportToSpreadsheet?: ShareOptionsActions;
	exportToPDF?: ShareOptionsActions;
};
...
class ShareUtils extends BaseControllerExtension {
	...
	onInit(): void {
		...
		this.base.getOwnerComponent().setModel(shareInfoModel, "shareInfo");
	}
	...
	async _openShareSheetImpl(by: Control & { shareSheet?: ActionSheet }): Promise<void> {
		...
		try {
			...
			if (by.shareSheet) {
				...
				const oPopoverFragment = await XMLTemplateProcessor.loadTemplatePromise(sFragmentName, "fragment");
