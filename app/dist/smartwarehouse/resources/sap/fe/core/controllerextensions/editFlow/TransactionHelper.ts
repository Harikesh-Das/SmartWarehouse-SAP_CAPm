...
import ControlBehavior from "sap/ui/core/ControlBehavior";
...
class TransactionHelper {
	...
	getProgrammingModel(source: ODataV4Context | Binding): typeof ProgrammingModel {
		...
		if (source?.isA<ODataV4Context>("sap.ui.model.odata.v4.Context")) {
			...
			path = ((source as Binding)?.isRelative() ? (source as Binding)?.getResolvedPath() : source?.getPath()) ?? "";
		}
	 ...
	 */
	showAndAnnounceSuccessMessage(resourceModel: ResourceModel, isNewObject: boolean, entitySetName: string | undefined): void {
		const text = resourceModel.getText(
			isNewObject ? "C_TRANSACTION_HELPER_OBJECT_CREATED" : "C_TRANSACTION_HELPER_OBJECT_SAVED",
			undefined,
			entitySetName
		);
		...
		return this.withBusyLock(
			...
			async () => {
				try {
					...
					if (
						newDocumentContext?.getBinding() !== undefined &&
						parameters.requestAtLeast &&
						parameters.requestAtLeast.length > 0
					) {
				...
				if (mParameters) {
					...
					if (!mParameters.numberOfSelectedContexts) {
						...
						if (draftEnabled) {
							...
							await deleteHelper.updateDraftOptionsForDeletableTexts(
								mParameters,
								contextsToDelete,
								totalDeletable,
								resourceModel,
	...
	prepareSiblingContext(context: ODataV4Context, groupId?: string): ODataV4Context {
		...
		if (semanticKeys && semanticKeys.length) {
			...
			return model
				.bindContext(`${contextPath}/SiblingEntity`, undefined, { $select: semanticKeyPaths, $$groupId: resolvedGroupId })
				.getBoundContext();
		}
		const entityPath = metaModel.getMetaPath(contextPath);
		const technicalKeys = metaModel.getObject(`${entityPath}/SiblingEntity/`)["$Key"];
		return model
			.bindContext(`${contextPath}/SiblingEntity`, undefined, { $select: technicalKeys, $$groupId: resolvedGroupId })
			.getBoundContext();
	}
	 ...
	 */
	async getSiblingContextData(
		context: ODataV4Context | undefined
	): Promise<{ siblingContextCanonicalPath: string; siblingData: Record<string, unknown> } | undefined> {
		if (context) {
			...
			if (sProgrammingModel === ProgrammingModel.Draft) {
				...
				try {
					const [siblingContextCanonicalPath, siblingData] = await Promise.all([
						siblingContext?.requestCanonicalPath(),
						siblingContext?.requestObject()
					]); // ensure both promises are resolved before returning the data
					return {
					...
					const errorToRemove = Messaging.getMessageModel()
						.getData()
						.filter((error: Message) => error.getMessage().includes("404 Not Found")); // in case the sibling context doesn't exist, a 404 error message is added to the message model, we need to remove it to later error handling to work properly
			...
			try {
				...
				if (sProgrammingModel === ProgrammingModel.Draft && !isObjectModified) {
					...
					[siblingContextCanonicalPath, draftAdminData, siblingData] = await Promise.all([
						isNewObject ? undefined : siblingContextData?.siblingContextCanonicalPath ?? siblingContext?.requestCanonicalPath(),
						draftDataContext.requestObject(),
						isNewObject ? undefined : siblingContextData?.siblingData ?? siblingContext?.requestObject()
					]);
					if (draftAdminData) {
						isObjectModified =
					...
					if (isNewObject) {
						...
						try {
							...
							[siblingContextCanonicalPath, siblingData] = await Promise.all([
								siblingContextData?.siblingContextCanonicalPath ?? siblingContext?.requestCanonicalPath(),
								siblingContextData?.siblingData ?? siblingContext?.requestObject()
							]);
	...
	async callAction(
		...
		const finalDisableStrictHandling =
			mParameters.disableStrictHandling !== undefined
				? mParameters.disableStrictHandling // Action-level setting takes precedence (if defined)
	...
	async _handleActionResponse(
		...
		result?: PromiseSettledResult<OperationResult>[]
	): Promise<unknown> {
