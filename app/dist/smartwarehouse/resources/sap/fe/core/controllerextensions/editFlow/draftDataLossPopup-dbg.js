...
sap.ui.define(["sap/base/Log", "sap/fe/core/CommonUtils", "sap/fe/core/helpers/EditState", "sap/fe/core/helpers/ModelHelper", "../../controls/DataLossOrDraftDiscard/DraftDataLossDialog", "../../controls/Recommendations/ConfirmRecommendationDialog"], function (Log, CommonUtils, EditState, ModelHelper, DraftDataLossDialog, ConfirmRecommendationDialog) {
  ...
  async function processFclMode(draftAdminData, fnCancelFunction, oController, processFunctionForDrafts, bSkipBindingToView, context, hiddenDraft) {
    ...
    if (draftAdminData.CreationDateTime === draftAdminData.LastChangeDateTime && !oController.editFlow.isDocumentModified()) {
      ...
      return dataLossDialog.open(oController, focusOnCancel).then(selectedKey => draftDataLossPopup.handleDialogSelection(selectedKey, processFunctionForDrafts, fnCancelFunction, oController, bSkipBindingToView, context, hiddenDraft));
    }
  }
   ...
   * @returns Nothing
   */
  async function processNoActiveEntityMode(draftAdminData, fnCancelFunction, oController, processFunctionForDrafts, navigationType, bSilentlyKeepDraftOnForwardNavigation, bSkipBindingToView, context, hiddenDraft, skipBackNavigation) {
    ...
    if (EditState.isEditStateDirty()) {
      if (draftAdminData.CreationDateTime === draftAdminData.LastChangeDateTime && !oController.editFlow.isDocumentModified() && navigationType === NavigationType.BackNavigation) {
        ...
        return dataLossDialog.open(oController).then(selectedKey => draftDataLossPopup.handleDialogSelection(selectedKey, processFunctionForDrafts, fnCancelFunction, oController, bSkipBindingToView, context, hiddenDraft, skipBackNavigation));
      }
    } else {
  ...
  async function processDraftAdminData(draftAdminData, fnProcessFunction, fnCancelFunction, context, oController, bSkipBindingToView) {
    ...
    let skipBackNavigation = arguments.length > 9 ? arguments[9] : undefined;
    ...
    if (draftAdminData) {
      if (oController.getAppComponent().getRootViewController().isFclEnabled()) {
        ...
        processNoActiveEntityMode(draftAdminData, fnCancelFunction, oController, processFunctionForDrafts, navigationType, bSilentlyKeepDraftOnForwardNavigation, bSkipBindingToView, undefined, hiddenDraft, skipBackNavigation);
   ...
   * @param skipBackNavigation Boolean value which mentions if back navigation should be skipped.
   * @param breadcrumbsBackNavigation Boolean value representing whether the funcion is called from ariba breadcrumbs link which would imply to show the draft data loss popup.
   */
  async function processDataLossOrDraftDiscardConfirmation(fnProcessFunction, fnCancelFunction, oContext, oController, bSkipBindingToView) {
    ...
    let skipBackNavigation = arguments.length > 7 ? arguments[7] : undefined;
    let breadcrumbsBackNavigation = arguments.length > 8 ? arguments[8] : undefined;
    ...
    if ((originalContext.isDeleted() || !backNavShowsDialog && navigationType === NavigationType.BackNavigation || !bIsEditable) && !breadcrumbsBackNavigation) {
      fnProcessFunction(originalContext);
    } else {
      try {
        ...
        await processDraftAdminData(draftAdminData, fnProcessFunction, fnCancelFunction, hiddenDraft ? originalContext : oContext, oController, bSkipBindingToView, navigationType, oContext.getProperty("HasActiveEntity"), hiddenDraft, skipBackNavigation);
      } catch (oError) {
   ...
   */
  async function discardDraft(controller, skipBindingToView, context, hiddenDraft, skipBackNavigation) {
    context = context ?? controller.getView().getBindingContext();
    let skipBackNavigationValue = skipBackNavigation;
   ...
   * @param skipBackNavigation Boolean value which mentions if back navigation should be skipped
   */
  function handleDialogSelection(selectedKey, processFunctionForDrafts, fnCancelFunction, controller, skipBindingToView, context, hiddenDraft, skipBackNavigation) {
    switch (selectedKey) {
      ...
      case DraftDataLossOptions.Keep:
        {
          const ctx = context ?? controller.getView().getBindingContext();
          if (ctx) {
            controller.editFlow.terminateDraftSession(ctx);
          }
          processFunctionForDrafts();
          dataLossDialog.close();
          break;
      ...
      case DraftDataLossOptions.Discard:
        draftDataLossPopup.discardDraft(controller, skipBindingToView, context, hiddenDraft, skipBackNavigation).then(discardedDraft => processFunctionForDrafts(discardedDraft)).catch(function (error) {
