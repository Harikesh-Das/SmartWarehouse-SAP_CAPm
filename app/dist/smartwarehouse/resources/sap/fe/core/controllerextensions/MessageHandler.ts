...
export type NavigationToErrorPageResult = {
	...
	showMessagePageOnCreateFailure?: boolean;
};
...
class MessageHandler extends ControllerExtension {
	...
	getShowBoundMessagesInMessageDialog(): boolean {
		const internalContext = this.base.getView().getBindingContext("internal") as InternalModelContext | undefined;
		const uiModel = this.base.getView().getModel("ui");
		return (
			!uiModel?.getProperty("/isEditable") ||
			internalContext?.getProperty("isOperationDialogOpen") ||
			internalContext?.getProperty("getBoundMessagesForMassEdit") ||
			// In edit mode: only suppress the dialog when a MessageButton BB is present on this page
			this.base.getView().findAggregatedObjects(true, (element) => element.isA("sap.fe.macros.MessageButton")).length === 0
		);
	}
	...
	async beforeShowMessageButton(_messageDetails: MessageDetail[]): Promise<void> {
		if (!this.base.isViewVisible()) {
			// do not show message if the view is not visible
