 ...
 */
sap.ui.define(["sap/base/Log", "sap/base/i18n/Localization", "sap/fe/base/ClassSupport", "sap/fe/core/formatters/ValueFormatter", "sap/m/Popover", "sap/ui/core/Lib", "sap/ui/core/Locale", "sap/ui/core/format/DateFormat", "sap/ui/core/format/NumberFormat", "sap/ui/core/message/MessageType", "sap/ui/core/mvc/ControllerExtension", "sap/ui/model/Filter", "sap/ui/model/FilterOperator", "sap/ui/model/Sorter", "sap/ui/model/json/JSONModel", "../helpers/ModelHelper"], function (Log, Localization, ClassSupport, valueFormatters, Popover, Library, Locale, DateFormat, NumberFormat, MessageType, ControllerExtension, Filter, FilterOperator, Sorter, JSONModel, ModelHelper) {
  "use strict";
  ...
  let KPIManagementControllerExtension = (_dec = defineUI5Class("sap.fe.core.controllerextensions.KPIManagement"), _dec2 = methodOverride(), _dec3 = methodOverride(), _dec(_class = (_class2 = /*#__PURE__*/function (_ControllerExtension) {
    ...
    _proto.onInit = function onInit() {
      ...
      if (this.aKPIDefinitions && this.aKPIDefinitions.length) {
        ...
        this.base.getOwnerComponent().setModel(oKPIModel, "kpiModel");
    ...
    /**
     * Returns the overall filters to be applied to a KPI, including SelectionVariant filters and the IsActiveEntity filter if draft is supported.
     * @param kpiDefinition
     * @param metaModel
     * @returns An array of sap.ui.model.Filter objects
     */;
    _proto.getKPIFilters = function getKPIFilters(kpiDefinition, metaModel) {
      let filters = [];
      if (kpiDefinition.selectionVariantFilterDefinitions?.length) {
        filters = kpiDefinition.selectionVariantFilterDefinitions.map(createFilterFromDefinition).filter(filter => {
          return filter !== undefined;
        });
      }
      if (ModelHelper.isDraftSupported(metaModel, kpiDefinition.contextPath)) {
    ...
    _proto.loadKPITagData = async function loadKPITagData(kpiDefinition, oMainModel, oKPIModel, loadFull) {
      ...
      // Filters
      const filters = this.getKPIFilters(kpiDefinition, oMainModel.getMetaModel());
      if (filters.length > 0) {
