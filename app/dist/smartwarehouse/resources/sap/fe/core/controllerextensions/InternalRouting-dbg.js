...
sap.ui.define(["sap/base/Log", "sap/fe/base/ClassSupport", "sap/fe/core/CommonUtils", "sap/fe/core/controllerextensions/BusyLocker", "sap/fe/core/controllerextensions/editFlow/draft", "sap/fe/core/controllerextensions/routing/ContextPathHelper", "sap/fe/core/controllerextensions/routing/NavigationReason", "sap/fe/core/helpers/EditState", "sap/fe/core/helpers/ModelHelper", "sap/ui/core/Component", "sap/ui/core/Element", "sap/ui/core/Lib", "sap/ui/core/mvc/ControllerExtension", "sap/ui/core/mvc/OverrideExecution", "../converters/MetaModelConverter", "../helpers/RoutingHelper", "./editFlow/editFlowConstants", "./messageHandler/messageHandling"], function (Log, ClassSupport, CommonUtils, BusyLocker, draft, ContextPathHelper, NavigationReason, EditState, ModelHelper, Component, UI5Element, Library, ControllerExtension, OverrideExecution, MetaModelConverter, RoutingHelpser, UiModelConstants, messageHandling) {
  ...
  let InternalRouting = (_dec = defineUI5Class("sap.fe.core.controllerextensions.InternalRouting"), _dec2 = methodOverride(), _dec3 = methodOverride(), _dec4 = publicExtension(), _dec5 = extensible(OverrideExecution.After), _dec6 = publicExtension(), _dec7 = extensible(OverrideExecution.After), _dec8 = publicExtension(), _dec9 = extensible(OverrideExecution.After), _dec10 = publicExtension(), _dec11 = extensible(OverrideExecution.After), _dec12 = publicExtension(), _dec13 = extensible(OverrideExecution.After), _dec14 = publicExtension(), _dec15 = publicExtension(), _dec16 = publicExtension(), _dec17 = finalExtension(), _dec18 = publicExtension(), _dec19 = finalExtension(), _dec20 = publicExtension(), _dec21 = publicExtension(), _dec22 = finalExtension(), _dec23 = publicExtension(), _dec24 = extensible(OverrideExecution.Before), _dec(_class = (_class2 = /*#__PURE__*/function (_ControllerExtension) {
    ...
    /**
     * Triggered every time this controller is NOT a navigation target.
     */;
    _proto.onRouteNotMatched = function onRouteNotMatched() {
      /**/
    };
    ...
    _proto.beforeSetBindingContext = function beforeSetBindingContext(bindingContext, mParameters) {
      this.onBeforeBinding(bindingContext, mParameters);
      const view = this.base.getView();
      const oRouting = view.getController().routing;
      if (oRouting && oRouting.onBeforeBinding) {
        oRouting.onBeforeBinding(bindingContext);
      }
      if (bindingContext && view.getBindingContext() !== bindingContext) {
        // If we change the binding context, we disable that dataStateIndicators for all tables in this page
        // to clear their filters, as they don't make sense anymore
        view.getBindingContext("pageInternal")?.setProperty("enableTableDataStateIndicator", false);
      }
      const requestOnBinding = mParameters?.requestOnBinding;
      if (bindingContext && requestOnBinding && requestOnBinding.length > 0) {
        bindingContext.requestProperty(requestOnBinding).catch(e => {
          Log.error("Error when requesting properties before binding", e);
        });
      }
    };
    _proto.onAfterBinding = function onAfterBinding(bindingContext, _mParameters) {
      ...
      if (this._oAppComponent.hideViewDuringCreation && this._oPageComponent?.isA("sap.fe.templates.ObjectPage.Component")) {
    ...
    _proto.afterSetBindingContext = function afterSetBindingContext(bindingContext, _mParameters) {
      ...
      view.getBindingContext("pageInternal")?.setProperty("enableTableDataStateIndicator", true); // Restore dataStateIndicators for all tables in this page
    }
    ...
    _proto.navigateToContext = async function navigateToContext(context) {
      ...
      if (context.isA("sap.ui.model.odata.v4.ODataListBinding")) {
        if (parameters.createOnNavigateParameters?.mode === "Async") {
          ...
          try {
            const focusControl = UI5Element.getActiveElement();
            const createdContext = await parameters.createOnNavigateParameters.createContextPromise;
            // once the context is returned we navigate into it
            return await this.navigateToContext(createdContext, {
              ...
              requestOnBinding: parameters.requestOnBinding,
        ...
        const overrideNav = await this.base.getView().getController().routing.onBeforeNavigation({
          ...
          openInNewTab: parameters?.navMode === "openInNewTab"
        });
      ...
      if (parameters.reason !== NavigationReason.EditFlowAction) {
        this.removeTransitionMessagesForPreviousContext();
      }
      let skipHashGuardCheck;
      const hiddenDraftEnabled = this._oAppComponent?.getEnvironmentCapabilities()?.getCapabilities()?.HiddenDraft?.enabled;
      const isDocumentModified = this._oAppComponent?.getModel("ui")?.getProperty(UiModelConstants.DocumentModified) ?? true;
      if (this._oAppComponent.getRootViewController().isFclEnabled() && this.getFCLLevel() === 0 && hiddenDraftEnabled && !isDocumentModified) {
        skipHashGuardCheck = true;
      }
      return this._oRoutingService.navigateToContext(context, parameters, this._oView.getViewData(), this._oTargetInformation, skipHashGuardCheck);
    }
    ...
    _proto.getContextForNavigationFromTreeOrAnalyticalTable = async function getContextForNavigationFromTreeOrAnalyticalTable(originalContext) {
      ...
      if (originalContext.getProperty("IsActiveEntity") === true && originalContext.getProperty("HasDraftEntity") === true) {
        ...
        const draftOrActiveContext = await getDraftOrActiveContext(originalContext, ["HasActiveEntity"]); // We need "HasActiveEntity" to determine if the draft is new or not in RoutingService
    ...
    _proto.navigateToMessagePage = async function navigateToMessagePage(sErrorMessage, mParameters) {
      ...
      const navToCreateHash = mParameters.showMessagePageOnCreateFailure !== true && (this._oRouterProxy.getHash().includes("i-action=create") || this._oRouterProxy.getHash().includes("i-action=autoCreate"));
      if (navToCreateHash) {
      ...
      if (!aTargets || !aTargets.includes(this._oTargetInformation.targetName)) {
        ...
        if ((this._oTargetInformation.viewLevel ?? 0) >= (routeInformation?.routeLevel ?? 0)) {
          this._oAppComponent.getRoutingService().addBindingCleanupPromise(new Promise(resolve => {
            setTimeout(() => {
              if (this._oAppComponent.isDestroyStarted()) {
                resolve();
                return;
              }
    ...
    _proto._onRouteMatched = function _onRouteMatched(oEvent) {
      ...
      if (this._pendingTargetControlIdForEdit) {
        oNavigationParameters.targetControlId ??= this._pendingTargetControlIdForEdit;
        this._pendingTargetControlIdForEdit = undefined;
      }
      const {
        path,
        deferred
    ...
    _proto._bindDeferred = async function _bindDeferred(targetPath, navigationParameters) {
      ...
      if (!navigationParameters.createOnNavigateParameters || navigationParameters.createOnNavigateParameters.mode === "Deferred") {
        ...
        if (this._oPageComponent && this._oPageComponent.createDeferredContext) {
          this._oPageComponent.createDeferredContext(targetPath, navigationParameters.createOnNavigateParameters ?? {}, !!navigationParameters.bActionCreate);
        }
      }
    ...
    _proto._bindPageForEdit = async function _bindPageForEdit(targetPath, model, navigationParameters) {
      ...
      try {
        const technicalPath = await resolvePath(targetPath, model, this._oRoutingService);
        if (technicalPath === null) {
          await this.displayErrorPageForWrongSemanticURL();
          return;
        }
        const metaModel = model.getMetaModel();
        ...
        if (technicalPath.includes("IsActiveEntity=true") || isSticky && !isStickyEditMode) {
          ...
          // Store targetControlId so _onRouteMatched can inject it after editDocument navigates to the draft
          this._pendingTargetControlIdForEdit = navigationParameters.targetControlId;
          try {
    ...
    /**
     * Displays an error page when the semantic URL is wrong (i.e. key values do not correspond to an existing instance).
     */;
    _proto.displayErrorPageForWrongSemanticURL = async function displayErrorPageForWrongSemanticURL() {
      const resourceBundle = Library.getResourceBundleFor("sap.fe.core");
      await this.navigateToMessagePage(resourceBundle.getText("C_COMMON_SAPFE_DATA_RECEIVED_ERROR"), {
        title: resourceBundle.getText("C_COMMON_SAPFE_ERROR"),
        description: resourceBundle.getText("C_COMMON_SAPFE_DATA_RECEIVED_ERROR_DESCRIPTION"),
        handleShellBack: false,
    ...
    _proto._bindPageToPath = function _bindPageToPath(sTargetPath, oModel, oNavigationParameters) {
      ...
      if (oUseContext && oUseContext.getPath() === sTargetPath) {
        if (oNavigationParameters.reason === NavigationReason.EditFlowAction) {
          this._bindPageToContext(oUseContext, oModel, oNavigationParameters);
        } else if (oUseContext !== oCurrentContext) {
          ...
          if (shouldRefreshContext) {
            this._oAppComponent.getRoutingService().addBindingCleanupPromise(this._oAppComponent.getSideEffectsService().requestSideEffects([{
    ...
    _proto._bindPageToContext = function _bindPageToContext(oContext, oModel, oNavigationParameters) {
      ...
      // In FCL, a single routeMatched event broadcasts navigationInfo to all visible columns.
      // When the targetControlId belongs to a different column (e.g. sub-OP), this column is
      // not the navigation destination. Preserve its section state by setting bPersistOPScroll.
      const targetControlId = oNavigationParameters.targetControlId;
      const isFCL = oRootViewController.isFclEnabled();
      const isTargetControlInThisView = !isFCL || !targetControlId || !!this._oView.byId(targetControlId);
      const persistScroll = oNavigationParameters.bPersistOPScroll || isFCL && !isTargetControlInThisView;
      ...
      this.beforeSetBindingContext(oContext, {
        ...
        bPersistOPScroll: persistScroll,
        reason: oNavigationParameters.reason,
        showPlaceholder: oNavigationParameters.bShowPlaceholder,
        requestOnBinding: oNavigationParameters.requestOnBinding,
        targetControlId: isTargetControlInThisView ? targetControlId : undefined
      });
    ...
    _proto._createContext = function _createContext(sPath, oModel) {
      ...
      if (oRootViewController.isFclEnabled()) {
        ...
        if (!oContext) {
          ...
          const propertiesToRequest = ["HasActiveEntity", "HasDraftEntity", "IsActiveEntity"].concat(semanticKeys).concat(alternateAndSecondaryKeys);
        ...
        if (isHiddenDraftEnabled && oDraftRoot) {
          propertiesToSelect.push("DraftAdministrativeData/CreationDateTime", "DraftAdministrativeData/LastChangeDateTime");
        }
     ...
     */;
    _proto._setBindingContext = function _setBindingContext(context) {
      let previousContext, targetControl;
      if (this._oPageComponent) {
        previousContext = this._oPageComponent.getBindingContext();
        targetControl = this._oPageComponent;
      } else {
        previousContext = this._oView.getBindingContext();
        targetControl = this._oView;
      }
      targetControl.setBindingContext(context);
      if (previousContext?.isKeepAlive() && previousContext !== context) {
        const rootViewController = this._oAppComponent.getRootViewController();
        /* Before deleting the bindingContext, we ensure it's not also used in another page.*/
        if (!rootViewController.getInstancedViews().some(pageView => pageView.getBindingContext?.() === previousContext)) {
          this._setKeepAlive(previousContext, false);
        }
      }
    }
    ...
    _proto.closeColumn = async function closeColumn() {
      ...
      const isDraftSupported = ModelHelper.isDraftSupported(oMetaModel, oContext.getPath());
      const isDocumentModified = isDraftSupported && (!!this.getView()?.getModel("ui")?.getProperty(UiModelConstants.DocumentModified) || !oContext.getProperty("HasActiveEntity"));
      const isHiddenDraftEnabled = (this._oAppComponent.getEnvironmentCapabilities()?.getCapabilities()?.HiddenDraft).enabled;
      if (!isDocumentModified && isHiddenDraftEnabled && oViewData?.viewLevel === 1) {
        const mParameters = {
      ...
      } else if (oViewData?.viewLevel === 1 && isDraftSupported) {
  ...
  }(ControllerExtension), _applyDecoratedDescriptor(_class2.prototype, "onExit", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "onExit"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "onInit", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "onInit"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "onRouteMatched", [_dec4, _dec5], Object.getOwnPropertyDescriptor(_class2.prototype, "onRouteMatched"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "onRouteNotMatched", [_dec6, _dec7], Object.getOwnPropertyDescriptor(_class2.prototype, "onRouteNotMatched"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "onRouteMatchedFinished", [_dec8, _dec9], Object.getOwnPropertyDescriptor(_class2.prototype, "onRouteMatchedFinished"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "onBeforeBinding", [_dec10, _dec11], Object.getOwnPropertyDescriptor(_class2.prototype, "onBeforeBinding"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "onAfterBinding", [_dec12, _dec13], Object.getOwnPropertyDescriptor(_class2.prototype, "onAfterBinding"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "navigateToTarget", [_dec14], Object.getOwnPropertyDescriptor(_class2.prototype, "navigateToTarget"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "navigateToRoute", [_dec15], Object.getOwnPropertyDescriptor(_class2.prototype, "navigateToRoute"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "navigateBackFromTransientState", [_dec16, _dec17], Object.getOwnPropertyDescriptor(_class2.prototype, "navigateBackFromTransientState"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "isCurrentStateImpactedBy", [_dec18, _dec19], Object.getOwnPropertyDescriptor(_class2.prototype, "isCurrentStateImpactedBy"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "onDataReceived", [_dec20], Object.getOwnPropertyDescriptor(_class2.prototype, "onDataReceived"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "switchFullScreen", [_dec21, _dec22], Object.getOwnPropertyDescriptor(_class2.prototype, "switchFullScreen"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "closeColumn", [_dec23, _dec24], Object.getOwnPropertyDescriptor(_class2.prototype, "closeColumn"), _class2.prototype), _class2)) || _class);
  return InternalRouting;
}, false);
