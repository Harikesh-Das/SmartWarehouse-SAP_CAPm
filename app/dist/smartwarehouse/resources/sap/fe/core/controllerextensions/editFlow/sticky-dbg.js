...
sap.ui.define(["sap/base/Log", "sap/base/util/isEmptyObject", "sap/fe/core/CommonUtils", "sap/fe/core/helpers/ResourceModelHelper", "sap/fe/core/library", "sap/m/MessageBox", "sap/ui/core/Lib", "../../converters/MetaModelConverter", "./operations/Operation"], function (Log, isEmptyObject, CommonUtils, ResourceModelHelper, FELibrary, MessageBox, Library, MetaModelConverter, Operation) {
    ...
    if (convertedAction) {
      const result = await new Operation(appComponent, context.getModel(), convertedAction, {
        ...
        oDataProperties: {
          ...
          ignoreETag: isEmptyObject(context.getObject()),
   ...
   */
  async function activateDocument(context, appComponent, messageHandler, mParameters, isStandardSave) {
    const operation = "SaveAction";
    const model = context.getModel(),
      metaModel = model.getMetaModel(),
      metaPath = metaModel.getMetaPath(context.getPath());
    const convertedAction = getConvertedAction(context, operation);
    const label = isStandardSave ? mParameters?.resourceModel?.getText("T_OP_OBJECT_PAGE_SAVE") : mParameters?.resourceModel?.getText("C_OP_OBJECT_PAGE_SAVE");
    if (convertedAction) {
      const result = await new Operation(appComponent, context.getModel(), convertedAction, {
        messageHandler: messageHandler,
        contexts: [context],
   ...
   * @param fnCancel Optional function to execute when user cancels
   * @param view Current view
   * @param programmingModel Programming Model of the current page
   */
  function processDataLossConfirmation(fnProcess, fnCancel, view, programmingModel) {
