...
class AppStateHandler extends BaseObject {
	...
	// targetControlId from xAppState, consumed once by the ObjectPage
	private pendingTargetControlIdFromXAppState?: string;
	 ...
	 */
	consumePendingTargetControlId(): string | undefined {
	 ...
	 */
	async applyAppState(view?: View): Promise<void | {}> {
		...
		return toES6Promise<unknown[]>(oNavigationService.parseNavigation())
			...
			.then(async (aResults: unknown[]) => {
				...
				const applyViewState = await oRootController.viewState.applyViewState(
					viewId ? appStateData : this._mCurrentAppState,
				...
				const opState = oAppData?.appState?.["fe::ObjectPage"] as
