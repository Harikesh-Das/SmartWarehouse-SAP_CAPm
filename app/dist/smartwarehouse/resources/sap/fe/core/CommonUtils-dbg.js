 ...
 */
sap.ui.define(["sap/base/Log", "sap/base/util/merge", "sap/fe/core/converters/ConverterContext", "sap/fe/core/converters/MetaModelConverter", "sap/fe/core/helpers/ModelHelper", "sap/fe/core/helpers/StableIdHelper", "sap/fe/core/helpers/TypeGuards", "sap/ui/Device", "sap/ui/core/Component", "sap/ui/core/Fragment", "sap/ui/core/XMLTemplateProcessor", "sap/ui/core/util/XMLPreprocessor", "sap/ui/model/Filter", "./controls/AnyElement", "./converters/helpers/SelectionVariantHelper"], function (Log, mergeObjects, ConverterContext, MetaModelConverter, ModelHelper, StableIdHelper, TypeGuards, Device, Component, Fragment, XMLTemplateProcessor, XMLPreprocessor, Filter, AnyElement, SelectionVariantHelper) {
  "use strict";
  ...
  function normalizeSearchTerm(sSearchTerm) {
    ...
    // .trim() must run before the empty-string guard to handle whitespace-only inputs.
    // Moving trim() after the guard would cause "   " to pass through and be sent to the backend as "",
    // which RAP rejects with "Invalid value for system query option '$search'"
    const convertedSearchTerm = sSearchTerm.replace(/["();]/g, " ").trim(); // these are the specific characters that fail on both, CAP and RAP: "();
  ...
  async function updateRelateAppsModel(oBindingContext, oEntry, oObjectPageLayout, aSemKeys, oMetaModel, oMetaPath, appComponent) {
    ...
    try {
      ...
      // Remove self-navigation entries from manifest SO items (current app's own SO+action must not appear as a related app)
      aManifestSOItems.splice(0, aManifestSOItems.length, ...aManifestSOItems.filter(item => !(item.targetSemObject === sCurrentSemObj && item.targetAction === sCurrentAction)));
      if (aLinks) {
        if (aLinks.length > 0) {
          ...
          // remove all actions from current hash application if manifest contains allowedActions or unavailableActions
  ...
  function getTargetView(oControl) {
    ...
    if (oControl) {
      const templateComponent = oControl && oControl.isA("sap.fe.core.TemplateComponent") ? oControl : Component.getOwnerComponentFor(oControl);
      if (templateComponent && templateComponent.isA("sap.fe.core.TemplateComponent")) {
        return templateComponent.getRootControl();
      }
    }
  ...
  async function internalRequestProperty(sAction, oSelectedContext, sProperty) {
    ...
    return oContext.requestProperty(sProperty);
  }
  async function requestProperty(oSelectedContext, sAction, sProperty, sDynamicActionEnabledPath) {
    const oPromise = sProperty && sProperty.indexOf("/") === 0 ? requestSingletonProperty(sProperty, oSelectedContext.getModel()) : internalRequestProperty(sAction, oSelectedContext, sProperty);
  ...
  function getAdditionalParamsForCreate(oStartupParameters, oInboundParameters, metaModel, entitySetPath) {
    ...
    for (const element of aCreateParameters) {
      ...
      if (aValues && aValues.length === 1) {
        ...
        let value = aValues[0];
        if (metaModel && entitySetPath) {
          const propType = metaModel.getObject(`${entitySetPath}/${sCreateParameter}/$Type`);
          if (propType === "Edm.Date" && typeof value === "string") {
            const match = value.match(/^(\d{4})-(\d{1,2})-(\d{1,2})/);
            value = match ? match[0] : value;
          } else if (propType === "Edm.Boolean" && typeof value === "string") {
            value = value.trim().toLowerCase() === "true";
          }
        }
        oRet[sCreateParameter] = value;
      }
    }
    return oRet;
  }
  ...
  async function templateControlFragment(sFragmentName, oPreprocessorSettings, oInOptions, oModifier) {
    ...
    if (oModifier && typeof sFragmentName === "string") {
      ...
      const fragmentData = typeof sFragmentName === "string" ? await XMLTemplateProcessor.loadTemplatePromise(sFragmentName, "fragment") : sFragmentName;
   ...
   */
  async function createRootContext(programmingModel, view, appComponent, bindingParameters, context) {
    const result = findOrCreateRootContext(view.getBindingContext() ?? context, programmingModel, view, appComponent, bindingParameters, true);
