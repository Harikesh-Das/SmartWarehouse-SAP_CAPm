  ...
  let HideFormGroupAutomatically = (_dec = defineUI5Class("sap.fe.core.controls.HideFormGroupAutomatically"), _dec(_class = /*#__PURE__*/function (_EventDelegateHook) {
    ...
    _proto.hideFormContainers = function hideFormContainers(formContainers) {
      ...
      for (const formContainer of formContainers) {
        ...
        const isHiddenByFlex = this.checkIfIsHiddenByFlex(formContainer);
         ...
         */
        if (this.appComponent?.isAdaptationMode() === true || isHiddenByFlex || isUsingPartOfPreviewAnnotation || isUsingBindingOnVisibility || formContainer.data("UiHiddenPresent") === "true") {
          continue;
        }
    ...
    _proto.checkIfIsHiddenByFlex = function checkIfIsHiddenByFlex(formContainer) {
      const regex = /^sap\.ui\.fl\.appliedChanges\.\w+_hideControl$/i;
      const formContainerCustomData = formContainer.getCustomData();
      return formContainerCustomData?.some(customData => regex.test(customData.getKey()) && customData.getValue() === "true") ?? false;
    };
