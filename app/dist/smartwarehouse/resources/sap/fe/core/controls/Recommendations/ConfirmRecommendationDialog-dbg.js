...
sap.ui.define(["sap/base/Log", "sap/fe/base/ClassSupport", "sap/fe/core/converters/MetaModelConverter", "sap/fe/core/helpers/ResourceModelHelper", "sap/fe/core/library", "sap/m/Button", "sap/m/CustomListItem", "sap/m/Dialog", "sap/m/FlexItemData", "sap/m/HBox", "sap/m/List", "sap/m/Text", "sap/m/Toolbar", "sap/m/ToolbarSpacer", "sap/ui/base/ManagedObject", "sap/ui/core/library", "sap/ui/model/Sorter", "sap/ui/model/json/JSONModel", "../../formatters/ValueFormatter", "../../helpers/StandardRecommendationHelper", "sap/fe/base/jsx-runtime/jsx", "sap/fe/base/jsx-runtime/Fragment", "sap/fe/base/jsx-runtime/jsxs"], function (Log, ClassSupport, MetaModelConverter, ResourceModelHelper, FELibrary, Button, CustomListItem, Dialog, FlexItemData, HBox, List, Text, Toolbar, ToolbarSpacer, ManagedObject, library, Sorter, JSONModel, valueFormatters, StandardRecommendationHelper, _jsx, _Fragment, _jsxs) {
  ...
  let ConfirmRecommendationDialog = (_dec = defineUI5Class("sap.fe.core.controls.Recommendations.ConfirmRecommendationDialog"), _dec2 = defineReference(), _dec3 = property({
    type: "any"
  }), _dec4 = property({
    type: "string"
  }), _dec(_class = (_class2 = /*#__PURE__*/function (_ManagedObject) {
    function ConfirmRecommendationDialog(props) {
      ...
      _initializerDefineProperty(_this, "programmingModel", _descriptor3, _this);
    ...
    _proto.getOpenRecommendations = function getOpenRecommendations() {
      ...
      if (openRecommendations.length > 0) {
        openRecommendations.forEach(recommendation => {
          ...
          const metaModel = fallbackContext?.getModel().getMetaModel();
          let isPropertyValidOnFallbackContext = false;
          if (metaModel) {
            const metaPath = `${metaModel.getMetaPath(fallbackContext.getPath())}/${propertyPath}`;
            isPropertyValidOnFallbackContext = !!metaModel.getObject(metaPath);
          }
          ...
          if (isEmpty(primaryContext, propertyPath) && (!isPropertyValidOnFallbackContext || isEmpty(fallbackContext, propertyPath))) {
    ...
    _proto.open = async function open(isSave) {
      ...
      // Filter recommendationData to only include active recommendations
      if (this.acceptAllParams.recommendationData) {
        this.acceptAllParams.recommendationData = this._filterActiveRecommendations(this.acceptAllParams.recommendationData);
      }
    ...
    /**
     * Filters recommendation data to only include recommendations that are currently active.
     * @param recommendationData Array of recommendation data to filter
     * @returns Filtered array containing only active recommendations
     */;
    _proto._filterActiveRecommendations = function _filterActiveRecommendations(recommendationData) {
      const localAnnotationModel = this.view.getModel().getLocalAnnotationModel();
      const activeRecommendations = localAnnotationModel.getProperty("/activeRecommendations") ?? {};
      return recommendationData.filter(recommendation => {
        const contextPath = recommendation.context?.getPath();
        if (!contextPath || !recommendation.propertyPath) {
          return false;
        }
        const activePathsForContext = activeRecommendations[contextPath];
        if (!activePathsForContext || activePathsForContext.length === 0) {
          return false;
        }

        // Check if the recommendation's propertyPath is in the active recommendations list
        return activePathsForContext.includes(recommendation.propertyPath);
      });
    }
    ...
    _proto._filterRecommendationKeys = function _filterRecommendationKeys(annotationObject) {
      ...
      for (const key of Object.keys(annotationObject)) {
        if (key.includes("ui5.fe.recommendations")) {
          const value = annotationObject[key];
          rejectedContextData[key] = Array.isArray(value) ? [...value] : value;
        }
      }
    ...
    writable: true,
    initializer: null
  }), _descriptor3 = _applyDecoratedDescriptor(_class2.prototype, "programmingModel", [_dec4], {
    configurable: true,
    enumerable: true,
    writable: true,
    initializer: null
