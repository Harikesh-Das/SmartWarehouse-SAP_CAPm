...
export class SideEffectsService extends Service<SideEffectsSettings> {
	...
	initializeWebSocketConnection(): void {
		...
		if (url && channel) {
			this.webSocket = createWebSocket(ChannelType.SideEffectsEvents, this.appComponent);
	...
	async onSideEffectsEventReceived(event: WebSocket$MessageEvent & Event<{ pcpFields?: WebSocketMessage }>): Promise<void> {
		...
		// Call the app hook on every visible PageController right after message validation.
		// A rejected promise stops all further processing.
		const hookParameters = {
			sideEffectEventName: message.sideEffectEventName,
			sideEffectSource: message.sideEffectSource
		};
		let interactionTypeOverride: SideEffectsEventsInteractionType | undefined;
		const visiblePages = this.appComponent.getRootViewController().getVisibleViews();
		for (const page of visiblePages) {
			const pageController = page.getController();
			if (!pageController.isA<PageController>("sap.fe.core.PageController")) {
				continue;
			}
			let hookResult;
			try {
				hookResult = await pageController.sideEffects.onEventDrivenSideEffectsReceived(hookParameters);
			} catch {
				// A rejected promise from the hook stops all further processing.
				return;
			}
			if (hookResult.interactionType !== undefined) {
				interactionTypeOverride = hookResult.interactionType;
			}
		}
		...
		for (const page of visiblePages) {
			...
			if (
				...
				pageController.sideEffects.isDataPathRelevant(message.sideEffectSource, message.sideEffectEventName)
			) {
		...
		const sideEffectsControllerExtension = relevantPages[relevantPages.length - 1].sideEffects;
		const onlyRecommendations = this.isOnlyRecommendations(dataModel, relevantSideEffects);
		const interactionType =
			interactionTypeOverride ?? this.getSideEffectsInteractionType(message.sideEffectEventName, onlyRecommendations);
		...
		for (const page of relevantPages) {
			await page.sideEffects.requestSideEffectsForEvent(message.sideEffectEventName, message.sideEffectSource);
		}
	}
	 ...
	 * @param options.strictHandling The function to trigger the strict handling validation, if strict handling must be triggered after executing an action
	...
	public async executeAction(
		...
		options?: {
			groupId?: string;
			submitBatch?: boolean;
			strictHandling?: (messages: Message[]) => Promise<boolean>;
		}
	): Promise<void> {
		...
		await action.invoke(groupId, true, executionOptions.strictHandling);
	}
	 ...
	 * @returns Entity Type
	 */
	public getEntityTypeFromContext(context: Context | undefined | null): string | undefined {
		if (!context) {
			return undefined;
		}
	...
	private mapFieldAnnotations(source: EntityType): void {
		...
		source.entityProperties.forEach((property) => {
			const roleType = (property.annotations.Common as PropertyAnnotationsWithRecommendation)?.RecommendationRole;
			if (roleType) {
	...
	private getSideEffectsAnnotationFromSource(source: EntityType | Action): CommonSideEffectTypeWithQualifier[] {
		...
		const commonAnnotation = source.annotations?.Common as unknown as
			| undefined
			| (Record<string, CommonSideEffectTypeWithQualifier> & { _keys?: string[] });
		const existingKeys = commonAnnotation?._keys?.filter((key) => key.startsWith("SideEffects")) ?? [];
