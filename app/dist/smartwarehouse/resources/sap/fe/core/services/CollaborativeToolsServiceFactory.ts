...
import type { default as CollaborationManagerService } from "sap/suite/ui/commons/collaboration/CollaborationManagerService";
import type {
...
export class CollaborativeToolsService extends Service<CollaborativeToolsServiceSettings> {
	...
	private async initializeMSTeams(): Promise<void> {
		try {
			...
			this.collaborationService.isContactsCollaborationSupported =
				...
				typeof collaborationService.oTeamsHelperService.isContactsCollaborationSupported === "function" &&
				collaborationService.oTeamsHelperService.isContactsCollaborationSupported();
	...
	public async getTeamContactOptions(): Promise<TeamsContactOption[] | undefined> {
		...
		if (!this.collaborationService.contactOptions) {
			try {
				this.collaborationService.contactOptions =
					await this.collaborationService.teamsHelperService?.getTeamsContactCollabOptions();
			} catch {
				return undefined;
			}
		}
	...
	public async getCollaborationServices(): Promise<CollaborationService> {
		...
		return (ServiceContainerClass as unknown as { getCollaborationServices: () => CollaborationService }).getCollaborationServices();
	}
}
