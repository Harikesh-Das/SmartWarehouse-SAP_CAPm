...
export class EnvironmentCapabilitiesService extends Service<EnvironmentCapabilities> {
	...
	async initialize(): Promise<void> {
		...
		this.environmentCapabilities.enableSaveAndLeave = oContext.scopeObject.getManifestEntry("sap.fe")?.app?.enableSaveAndLeave;
	...
	static _loadedLibraries: Record<string, Promise<boolean>> = {};

	static async resolveLibrary(libraryName: string): Promise<boolean> {
		if (EnvironmentCapabilitiesService._loadedLibraries[libraryName] === undefined) {
			EnvironmentCapabilitiesService._loadedLibraries[libraryName] = new Promise(function (resolve) {
		...
		return EnvironmentCapabilitiesService._loadedLibraries[libraryName];
	}
