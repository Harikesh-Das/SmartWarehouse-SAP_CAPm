...
export class ShellServices extends Service<Required<ShellServicesSettings>> implements IShellServices {
	...
	/**
	 * Returns the configured home intent from the ushell config, falling back to "Shell-home".
	 * @returns The home intent string (without leading "#")
	 */
	getHomeIntent(): string {
		const shellConfig = this.getShellConfig() as unknown as Record<string, unknown> | undefined;
		const renderer = shellConfig?.defaultRenderer as string | undefined;
		if (renderer === undefined) {
			return "Shell-home";
		}
		const rendererConfig = (shellConfig?.renderers as Record<string, unknown> | undefined)?.[renderer] as
			| Record<string, unknown>
			| undefined;
		const componentDataConfig = (rendererConfig?.componentData as Record<string, unknown> | undefined)?.config as
			| Record<string, unknown>
			| undefined;
		return (componentDataConfig?.rootIntent as string | undefined) ?? "Shell-home";
	}
...
class ShellServicesFactory extends ServiceFactory<ShellServicesSettings> {
	...
	async createInstance(serviceContext: ServiceContext<ShellServicesSettings>): Promise<IShellServices> {
		...
		if (internalModel) {
			// Store FLP detection state for accessibility landmarks
			(internalModel as JSONModel).setProperty("/hasUShell", shellService.hasUShell());
