...
import { getRouteTargetNames, getRouteTargets } from "sap/fe/core/helpers/ManifestHelper";
...
import type { ManifestOutboundEntry, RouteTarget, RoutingConfiguration, RoutingTarget } from "sap/ui/core/Manifest";
...
export type CreateOnNavigateParameters =
	...
	| { mode: "Deferred"; listBinding: ODataListBinding; parentContext?: Context; data?: object; requestAtLeast?: string[] };
...
export class RoutingService extends Service<RoutingServiceSettings> {
	...
	_parseRoutingConfiguration(oRoutingConfig: RoutingConfiguration): void {
		...
		if (oRoutingConfig?.routes) {
			for (const oRouteManifestInfo of oRoutingConfig.routes) {
				const routeTargets = getRouteTargets(oRouteManifestInfo.target);
			...
			.forEach((target: RoutingTarget) => {
				...
				if (target.options) {
					...
					if (!settings.fullContextPath && sContextPath) {
						settings.fullContextPath = sContextPath.endsWith("/") ? sContextPath : `${sContextPath}/`;
					}
					...
					Object.keys(settings.navigation || {}).forEach((sNavName: string) => {
						...
						if (["sap.fe.templates.ListReport", "sap.fe.ariba.ListReport"].includes(target.name)) {
							...
							targetRoute.targets.forEach((sTargetName: string) => {
								if (
									...
									if (this._mTargets[sTargetName].options.settings.contextPath) {
										const contextPath = this._mTargets[sTargetName].options.settings.contextPath;
										this._mTargets[sTargetName].options.settings.fullContextPath = contextPath.endsWith("/")
											? contextPath
											: `${contextPath}/`;
	...
	async navigateTo(
		...
		const isHiddenDraftEnabled =
			(this.oAppComponent.getEnvironmentCapabilities().getCapabilities().HiddenDraft as HiddenDraft)?.enabled || false;
		const navigateFunction = (): void => {
			this.oRouterProxy.navToHash(targetUrl!, bPreserveHistory, false, false, bIsStickyMode || isHiddenDraftEnabled);
		};
	...
	async navigateToContext(
		...
		const oNavigationInfo: RoutingNavigationInfo = {
			...
			targetControlId: parameters?.targetControlId
		};
		...
		if (parameters.navMode === "openInNewTab") {
			...
			sap.ui.require(["sap/ushell/Container"], async (Container: Container) => {
				...
				let appStateKey: string | undefined;
				if (parameters.targetControlId) {
					// Encode targetControlId in xAppState using the same "fe::ObjectPage" convention key as cross-app navigation.
					// xAppState survives FLP's hash normalization (unlike sap-iapp-state which is stripped from the app-visible hash),
					// so parseNavigation() in the new tab will detect sap-xapp-state and return NavType.xAppState with the section data.
					try {
						const targetControlId = parameters.targetControlId;
						// applyAppState reads: selectedSubSection ?? selectedSection — set only the field we have.
						const sectionState: { selectedSection?: string; selectedSubSection?: string } = targetControlId.includes(
							"SubSection"
						)
							? { selectedSubSection: targetControlId }
							: { selectedSection: targetControlId };
						const appState = await navigationService.createEmptyAppState(this.oAppComponent);
						appState.setData({ appState: { "fe::ObjectPage": sectionState } });
						await appState.save();
						appStateKey = appState.getKey();
					} catch (e) {
						Log.warning(
							"Open in New Tab: failed to create xAppState for targetControlId, navigating without section restore",
			...
			if (currentTargetInfo?.name && ["sap.fe.templates.ListReport", "sap.fe.ariba.ListReport"].includes(currentTargetInfo.name)) {
			...
			if (targetRoute && routeParametersPromise) {
				return routeParametersPromise.then((oRouteParameters: object) => {
					Object.assign(oRouteParameters, { bIsStickyMode: isStickyMode, isHiddenDraftEnabled });
			...
			return this.oRouterProxy
				.navToHash(
					...
					isStickyMode || isHiddenDraftEnabled,
					skipHashGuardCheck,
					parameters?.focusControl
				)
				.then((bNavigated) => {
					if (!bNavigated) {
	 ...
	 * @param oRouteParameters.isHiddenDraftEnabled
	 * @returns Promise that is resolved when the navigation is finalized
	 * @final
	 */
	async navigateToRoute(
		sTargetRouteName: string,
		oRouteParameters?: { bIsStickyMode?: boolean; preserveHistory?: boolean; isHiddenDraftEnabled?: boolean }
	): Promise<boolean> {
		if (!this.enabled) {
			return Promise.resolve(false);
		}
		...
		return this.oRouterProxy.navToHash(
			...
			undefined,
			undefined,
			(oRouteParameters?.bIsStickyMode ?? false) || (oRouteParameters?.isHiddenDraftEnabled ?? false)
		);
	}
