...
sap.ui.define(["sap/base/Log", "sap/ui/core/service/Service", "sap/ui/core/service/ServiceFactory"], function (Log, Service, ServiceFactory) {
  ...
  let ShellServiceMock = /*#__PURE__*/function (_Service) {
    ...
    _proto.getHomeIntent = function getHomeIntent() {
      return "Shell-home";
    };
  ...
  let ShellServices = /*#__PURE__*/function (_Service2) {
     ...
     */;
    _proto2.getHomeIntent = function getHomeIntent() {
      const shellConfig = this.getShellConfig();
      const renderer = shellConfig?.defaultRenderer;
      if (renderer === undefined) {
        return "Shell-home";
      }
      const rendererConfig = shellConfig?.renderers?.[renderer];
      const componentDataConfig = rendererConfig?.componentData?.config;
      return componentDataConfig?.rootIntent ?? "Shell-home";
    }
  ...
  let ShellServicesFactory = /*#__PURE__*/function (_ServiceFactory) {
    ...
    _proto3.createInstance = async function createInstance(serviceContext) {
      ...
      if (internalModel) {
        ...
        internalModel.setProperty("/hasUShell", shellService.hasUShell());
        let semanticObjects = [];
        try {
