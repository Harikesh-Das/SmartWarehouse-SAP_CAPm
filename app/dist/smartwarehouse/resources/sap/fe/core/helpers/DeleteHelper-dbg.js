...
sap.ui.define(["sap/base/Log", "sap/fe/core/controllerextensions/editFlow/draft", "sap/m/CheckBox", "sap/m/Text", "sap/m/MessageToast", "sap/ui/core/message/Message", "sap/ui/core/message/MessageType", "sap/ui/core/Messaging"], function (Log, draft, CheckBox, Text, MessageToast, Message, MessageType, Messaging) {
    ...
    if (contexts?.length) {
      contexts.forEach(context => {
        ...
        if (contextPath && context.getModel()) {
          if (typeof context?.getModel()?.getLocalAnnotationModel === "function") {
  ...
  async function getUnSavedContextUser(unSavedContext) {
    const adminDataPre = unSavedContext.getObject("DraftAdministrativeData");
    if (!adminDataPre?.LastChangedByUserDescription && !adminDataPre?.LastChangedByUser) {
      await unSavedContext.requestProperty(["DraftAdministrativeData/LastChangedByUserDescription", "DraftAdministrativeData/LastChangedByUser"]);
    }
    const adminData = unSavedContext.getObject("DraftAdministrativeData");
    return adminData?.LastChangedByUserDescription ?? adminData?.LastChangedByUser ?? "";
  }
  async function getUnsavedContextsText(resourceModel, numberOfSelectedContexts, unSavedContexts, totalDeletable) {
    let infoTxt = "",
      optionTxt = "",
      optionWithoutTxt = false;
    if (numberOfSelectedContexts === 1 && unSavedContexts.length === 1) {
      //only one unsaved object are selected
      const lastChangedByUser = await getUnSavedContextUser(unSavedContexts[0]);
  ...
  async function updateDraftOptionsForDeletableTexts(mParameters, vContexts, totalDeletable, resourceModel, items, options) {
    let {
      numberOfSelectedContexts,
    ...
    if (unSavedContexts.length > 0) {
      const unsavedChangesTxts = (await deleteHelper.getUnsavedContextsText(resourceModel, numberOfSelectedContexts, unSavedContexts, totalDeletable)) || {};
    ...
    try {
      ...
      if (contexts.length) {
        ...
        await Promise.all(contextsToDelete.map(async function (context) {
          ...
          if (!draftEnabled || mParameters.draftsWithDeletableActive?.find(draftWithDeletableActive => draftWithDeletableActive.siblingInfo.targetContext === context)) {
