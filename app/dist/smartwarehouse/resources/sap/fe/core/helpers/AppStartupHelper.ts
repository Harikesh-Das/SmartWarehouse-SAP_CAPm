...
const AppStartupHelper = {
	 ...
	 */
	_getContextsFromPages: async function (startupPages: PageInfo[], model: ODataModel): Promise<Context[][]> {
		return Promise.all(
			startupPages.map(async (pageInfo) => {
			...
			if (isActiveKey) {
				for (const pageInfo of aStartupPages) {
					if (pageInfo.draftMode && pageInfo.semanticKeys?.some((key) => key.name === "IsActiveEntity") === false) {
						pageInfo.semanticKeys.push({ name: "IsActiveEntity", value: isActiveKey.value });
					}
				}
			}
		}
			...
			for (const pageInfo of aStartupPages) {
				if (pageInfo.draftMode) {
					...
					if (isActiveKey) {
						...
						keys!.push({ name: "IsActiveEntity", value: "true" });
					}
				}
			}
			...
			return reverseTargets.find((targetID) => {
				...
				return (
					...
					[
						...
						"sap.fe.ariba.ListReport",
						"sap.fe.templates.ObjectPage",
						"sap.fe.ariba.ObjectPage",
						"sap.fe.core.fpm"
					].includes(currentTarget.name)
				);
			});
		} else {
			return route.target;
	...
	_getStartupPagesFromStartupParams: function (
		...
		function findRecursive(level: number): void {
			...
			if (current.length > result.length) {
				...
				const matchesTargetResolution = parameterValueForTargetResolution
					? currentPage.targetResolutionValue === parameterValueForTargetResolution
					: currentPage.targetResolutionValue === undefined;
