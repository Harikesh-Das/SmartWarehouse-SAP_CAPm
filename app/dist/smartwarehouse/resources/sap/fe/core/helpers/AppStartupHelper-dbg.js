...
sap.ui.define(["sap/ui/model/Filter", "sap/ui/model/FilterOperator", "../converters/MetaModelConverter", "./ModelHelper", "./SemanticKeyHelper"], function (Filter, FilterOperator, MetaModelConverter, ModelHelper, SemanticKeyHelper) {
  ...
  const AppStartupHelper = {
          ...
          case "Edm.Guid":
            {
              ...
              if (matchFormatted) {
                ...
                key.value = matchUnformatted[0].replace(/^([0-9a-f]{8})([0-9a-f]{4})([0-9a-f]{4})([0-9a-f]{4})([0-9a-f]{12})$/i, "$1-$2-$3-$4-$5");
              }
              break;
            }
          default:
    ...
    /**
     * Load contexts for a list of Page infos, based on their respective keys.
     * @param startupPages
     * @param model
     * @returns A Promise for all contexts
     */
    _getContextsFromPages: async function (startupPages, model) {
      return Promise.all(startupPages.map(async pageInfo => {
        const entityType = model.getMetaModel().getObject(`${pageInfo.contextPath}/`);
        if (pageInfo.technicalKeys) {
          ...
          const context = model.bindContext(path).getBoundContext();
          return Promise.resolve([context]);
        } else {
          const keys = pageInfo.semanticKeys ?? [];
          this._sanitizeKeys(keys, entityType);
          const filter = this._createFilterFromKeys(keys, pageInfo.draftMode, model.getMetaModel());

          // only request a minimum of fields to boost backend performance since this is only used to check if an object exists
          // IsActiveEntity is needed to distinguish between draft and active entities
          const selectedFields = keys.map(key => key.name);
          if (pageInfo.draftMode && !selectedFields.includes("IsActiveEntity")) {
            selectedFields.push("IsActiveEntity");
          }
          const oListBind = model.bindList(pageInfo.contextPath, undefined, undefined, filter, {
    ...
    _requestObjectsFromParameters: async function (aStartupPages, oModel) {
      // We don't want to mix active and draft in the returned contexts,
      // So if the first page uses technical keys + draft + contains IsActiveEntity, add IsActiveEntity everywhere.
      if (aStartupPages[0].draftMode && aStartupPages[0].technicalKeys !== undefined) {
        const isActiveKey = aStartupPages[0].technicalKeys?.find(key => key.name === "IsActiveEntity");
        if (isActiveKey) {
          for (const pageInfo of aStartupPages) {
            if (pageInfo.draftMode && pageInfo.semanticKeys?.some(key => key.name === "IsActiveEntity") === false) {
              pageInfo.semanticKeys.push({
                name: "IsActiveEntity",
                value: isActiveKey.value
              });
            }
          }
        }
      }
      let contexts = await this._getContextsFromPages(aStartupPages, oModel);
      if (contexts.some(ctx => ctx.length !== 1)) {
        return contexts; // failed to load some contexts, return as is
      }

      // We don't want to mix active and draft in the returned contexts.
      // -->  Check if all contexts are either all drafts or all active entities. If not, rerun the requests by
      // forcing IsActiveEntity = true (this case can happen if a draft sub-object has been deleted but exists
      // in the active object graph). We cannot load draft + active in a single request (CAP limitation)
      const containsActive = contexts.some(ctx => ctx[0].getObject()?.["IsActiveEntity"] === true);
      const containsDraft = contexts.some(ctx => ctx[0].getObject()?.["IsActiveEntity"] === false);
      if (containsActive && containsDraft) {
        for (const pageInfo of aStartupPages) {
          if (pageInfo.draftMode) {
            const keys = pageInfo.semanticKeys ?? pageInfo.technicalKeys;
            const isActiveKey = keys.find(key => key.name === "IsActiveEntity");
            if (isActiveKey) {
        ...
        contexts = await this._getContextsFromPages(aStartupPages, oModel);
      }
      return contexts;
    },
    /**
    ...
    _getRightMostTargetName(route, manifestRouting) {
      if (Array.isArray(route.target)) {
        ...
        return reverseTargets.find(targetID => {
          ...
          return currentTarget?.name && ["sap.fe.templates.ListReport", "sap.fe.ariba.ListReport", "sap.fe.templates.ObjectPage", "sap.fe.ariba.ObjectPage", "sap.fe.core.fpm"].includes(currentTarget.name);
        });
      } else {
        return route.target;
      }
    ...
    _getReachablePageInfoFromRoute: function (oRoute, oManifestRouting, oStartupParameters, oMetaModel) {
      ...
      return {
        ...
        target: sTargetName,
        targetResolutionValue: oTarget?.options?.settings?.targetResolutionValue,
        pageLevel
      };
    },
    /**
     ...
     */
    _getStartupPagesFromStartupParams: function (oManifestRouting, oStartupParameters, oMetaModel, inboundParameterForTargetResolution) {
      const parameterValueForTargetResolution = inboundParameterForTargetResolution && oStartupParameters && oStartupParameters[inboundParameterForTargetResolution] && oStartupParameters[inboundParameterForTargetResolution][0];
      ...
      function findRecursive(level) {
        ...
        if (current.length > result.length) {
          const currentPage = current[current.length - 1];
          const matchesTargetResolution = parameterValueForTargetResolution ? currentPage.targetResolutionValue === parameterValueForTargetResolution : currentPage.targetResolutionValue === undefined;
          if (matchesTargetResolution) {
     ...
     * @returns An object containing either a hash or a context to navigate to, or an empty object if no deep link was found
     */
    getDeepLinkStartupHash: async function (oManifestRouting, oStartupParameters, oModel, targetResolutionParameter) {
      ...
      const startupPages = this._getStartupPagesFromStartupParams(oManifestRouting, oStartupParameters, oModel.getMetaModel(), targetResolutionParameter);
      if (startupPages.length === 0) {
        // No deeplink possible
        return {};
      }
