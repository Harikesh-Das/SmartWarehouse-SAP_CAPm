...
const FPMHelper = {
	loadModuleAndCallMethod: async function (
		...
		return new Promise(function (resolve: (value: unknown) => void) {
			...
			if (module.startsWith("/extension/")) {
				...
				const fnTarget = module
					? ObjectPath.get(module.replace(/\//g, ".").substring(1), (view as FEView).getController())
					: (view as FEView).getController();
				resolve(fnTarget[method](...args));
			} else {
	...
	preloadModule: async function (moduleName: string): Promise<void> {
