...
import { and, compileExpression, constant, ifElse, not, or, resolveBindingString } from "sap/fe/base/BindingToolkit";
import { UI } from "sap/fe/core/helpers/BindingHelper";
import { ButtonType, OverflowToolbarPriority } from "sap/m/library";
import { ActionType } from "../converters/ManifestSettings";
import type { BaseAction, CustomAction } from "../converters/controls/Common/Action";

type HeaderEmphasisResult = {
	emphasizedHeaderActions?: BaseAction[];
	/**
	 * Non-compiled expression indicating whether header emphasis is effectively active for any emphasized action.
	 * This is true when at least one emphasized action is visible, the footer is not shown, and no criticality action is visible.
	 */
	emphasisActiveExpr?: BindingToolkitExpression<boolean>;
};

export const EMPHASIZED_FIRST_SUFFIX = "::EmphasizedFirst";
export const SAVE_ACTION_KEY = "SaveAction";
export const APPLY_ACTION_KEY = "ApplyAction";
export const EDIT_ACTION_KEY = "EditAction";
 ...
 */
export const LR_EMPHASIS_SUPPRESSION_CONDITION =
	"{= !${viewData>/hideFilterBar} && !${viewData>/useHiddenFilterBar} && !${viewData>/liveMode} }";
 ...
 */
function buildEmphasizedFirstKey(key: string): string {
 ...
 */
function buildEmphasizedFirstId(id: string): string {
...
const ActionUtilities = {
	...
	ensurePrimaryActionNeverOverflows(actions: BaseAction[]): BaseAction[] {
		return actions.map((action) =>
			!this.isPrimaryActionType(action) ? action : { ...action, priority: OverflowToolbarPriority.NeverOverflow }
		);
	},
	 ...
	 */
	isPrimaryActionType(action: BaseAction): boolean {
	 ...
	 */
	isDraftAction(action: BaseAction): boolean {
	 ...
	 */
	isManifestAction(action: BaseAction): boolean {
		return !("annotationPath" in action) || !(action as CustomAction).annotationPath;
	},
	 ...
	 */
	isEmphasizedFirstClone(action: BaseAction): boolean {
	 ...
	 */
	getActionVisibleExpression(action: BaseAction, fallback = false): BindingToolkitExpression<boolean> {
		...
		if (action.visible !== undefined) {
			return resolveBindingString(action.visible, "boolean") as BindingToolkitExpression<boolean>;
		}
		return resolveBindingString(fallback, "boolean") as BindingToolkitExpression<boolean>;
	},
	 ...
	 */
	buildAnyActionVisibleExpression(actions: BaseAction[], include?: (action: BaseAction) => boolean): BindingToolkitExpression<boolean> {
		const exprs: BindingToolkitExpression<boolean>[] = [];
	 ...
	 */
	buildCriticalityVisibleExpr(headerActions: BaseAction[], footerActions: BaseAction[]): BindingToolkitExpression<boolean> {
	 ...
	 */
	resetEmphasis(actions: BaseAction | BaseAction[]): void {
	 ...
	 */
	findPrimaryActionCandidateFromManifestRecord(manifestActions: Record<string, CustomAction> | undefined): {
		key?: string;
		action?: CustomAction;
	} {
		if (!manifestActions) {
	 ...
	 */
	findPrimaryActionCandidateFromActionsArray(actions: BaseAction[] | undefined): BaseAction | undefined {
	 ...
	 */
	applyManifestPrimaryActionEmphasis(pageActions: BaseAction[], manifestActions?: Record<string, CustomAction>): void {
		// Extract ALL primary action keys from the manifest or page actions
		let primaryKeysFromManifest: string[];
		if (manifestActions) {
			// ObjectPage: extract primary keys from manifest record
			primaryKeysFromManifest = Object.values(manifestActions)
				.filter((action) => action.isPrimaryAction === true)
		...
		const targetActions: BaseAction[] = pageActions.filter((action) => primaryKeysFromManifest.includes(action.key));
		...
		if (targetActions.length === 0) {
			...
			Log.error(
				`Manifest action(s) with isPrimaryAction=true not found in page actions: ${missingKeys.join(", ")}. ` +
					`Verify that the action keys match between manifest configuration and rendered actions.`
			);
			return;
		}
		...
		const validTargetActions = targetActions.filter((action) => {
			...
			if (isMenuCandidate && !(action as CustomAction)?.defaultAction) {
				Log.error(
					`Action "${action.key}" is marked as isPrimaryAction but cannot be emphasized because it is a menu without a defaultAction. Only menus with a defaultAction can be emphasized.`
				);
				return false;
			}
			return true;
	 ...
	 */
	prioritizeManifestEmphasizedActions(actions: BaseAction[]): BaseAction[] {
		const manifestEmphasized: BaseAction[] = [];
		const annotationEmphasized: BaseAction[] = [];
		const nonEmphasized: BaseAction[] = [];
	 ...
	 */
	insertEmphasizedFirstClone(
		actions: BaseAction[],
		emphasizedAction: BaseAction,
		emphasisActiveExpr: BindingToolkitExpression<boolean>
	): void {
		...
		const emphasizedFirstClone: BaseAction = {
			...emphasizedAction,
	 ...
	 */
	buildHigherPriorityActionsVisibleExpr(
		higherPriorityActionsHiddenExprs: BindingToolkitExpression<boolean>[]
	): BindingToolkitExpression<boolean> {
		return higherPriorityActionsHiddenExprs.length > 0
			? not(and(...higherPriorityActionsHiddenExprs)) // At least one higher-priority action is visible
	 ...
	 * @param emphasisActiveExpr Expression that evaluates to true when emphasis should be active
	 */
	insertMultipleEmphasizedFirstClones(
		actions: BaseAction[],
		emphasizedActions: BaseAction[],
		emphasisActiveExpr: BindingToolkitExpression<boolean>
	): void {
		...
		const higherPriorityActionsHiddenExprs: BindingToolkitExpression<boolean>[] = [];
		const clonesToInsert: BaseAction[] = [];
		...
		for (let i = 0; i < emphasizedActions.length; i++) {
			...
			const showCloneExpr =
				higherPriorityActionsHiddenExprs.length === 0
					? and(emphasizedActionVisibleExpr, emphasisActiveExpr)
					: and(emphasizedActionVisibleExpr, and(...higherPriorityActionsHiddenExprs), emphasisActiveExpr);
	 ...
	 */
	getListReportHeaderEmphasisActiveExpr(): BindingToolkitExpression<boolean> {
		...
		const suppressHeaderEmphasisExpr = resolveBindingString(
			LR_EMPHASIS_SUPPRESSION_CONDITION,
			"boolean"
	 ...
	 */
	getEmphasizedAction(actions: BaseAction[]): BaseAction | undefined {
	 ...
	 */
	getAllEmphasizedActions(actions: BaseAction[]): BaseAction[] {
	 ...
	 */
	suppressFooterEmphasisWhenCriticalityVisible(
		footerActions: BaseAction[],
		criticalityVisibleExpr: BindingToolkitExpression<boolean>
	): void {
		for (const action of footerActions) {
			if (action.buttonType === ButtonType.Emphasized) {
				action.buttonType = compileExpression(
					ifElse(not(criticalityVisibleExpr), constant(ButtonType.Emphasized), constant(ButtonType.Ghost))
				);
			}
		}
	},
	 ...
	 * @param footerActions Footer actions to update
	 */
	ghostDraftActionsIfAnotherFooterActionEmphasized(footerActions: BaseAction[]): void {
	 ...
	 */
	updateEditActionEmphasis(headerActions: BaseAction[], suppressEditEmphasisExpr: BindingToolkitExpression<boolean>): void {
		const editAction = headerActions.find((headerAction) => headerAction.key === EDIT_ACTION_KEY);
		if (!editAction) {
			return;
		}
		editAction.buttonType = compileExpression(
			ifElse(suppressEditEmphasisExpr, constant(ButtonType.Ghost), constant(ButtonType.Emphasized))
		);
	},
	 ...
	 */
	applyObjectPageEmphasisRules(headerActions: BaseAction[], footerActions: BaseAction[]): BaseAction[] {
		// Determine if any action with criticality is visible at runtime (dynamic @UI.Hidden etc.).
		const hasAnyCriticalityAction =
			headerActions.some((action) => action.hasCriticality === true) ||
			footerActions.some((action) => action.hasCriticality === true);
		const criticalityVisibleExpr = hasAnyCriticalityAction
			? ActionUtilities.buildCriticalityVisibleExpr(headerActions, footerActions)
			: constant(false);
		...
		const { emphasizedHeaderActions: emphasizedHeaderActionsRef, emphasisActiveExpr: headerEmphasisActiveExpr } =
		...
		const footerEmphasisActiveExpr = this.buildAnyActionVisibleExpression(footerActions, (action) =>
			this.isEmphasizedFirstClone(action)
		);
		...
		const suppressEditEmphasisExpr = or(
			footerVisibleExpr,
			headerEmphasisActiveExpr ?? constant(false),
			footerEmphasisActiveExpr,
			criticalityVisibleExpr
		);
	 ...
	 */
	prepareHeaderEmphasis(
		headerActions: BaseAction[],
		footerVisibleExpr: BindingToolkitExpression<boolean>,
		criticalityVisibleExpr: BindingToolkitExpression<boolean>
	): HeaderEmphasisResult {
		...
		const originalVisibilityExprs = new Map<BaseAction, BindingToolkitExpression<boolean>>();
		...
		for (let i = 0; i < validEmphasizedActions.length; i++) {
			...
			const showActiveEmphasizedHeaderInstanceExpr =
				higherPriorityActionsHiddenExprs.length === 0
					? and(emphasizedActionVisibleExpr, emphasisActiveCondition)
					: and(emphasizedActionVisibleExpr, and(...higherPriorityActionsHiddenExprs), emphasisActiveCondition);
			...
			const showInactiveHeaderInstanceExpr = and(
				emphasizedActionVisibleExpr,
				or(higherPriorityActionsVisibleExpr, emphasisInactiveCondition)
			);
			...
			this.setNeverOverflowPriority(
				emphasizedAction,
				showActiveEmphasizedHeaderInstanceExpr,
				emphasizedAction.priority ?? OverflowToolbarPriority.Low
			);
		...
		const anyEmphasizedCloneVisibleExprs: BindingToolkitExpression<boolean>[] = [];
		for (const action of validEmphasizedActions) {
