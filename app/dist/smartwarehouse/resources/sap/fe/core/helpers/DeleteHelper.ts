...
async function getUnSavedContextUser(unSavedContext: ODataV4Context): Promise<string> {
	const adminDataPre = unSavedContext.getObject("DraftAdministrativeData") as DraftAdministrativeDataType | undefined;
	if (!adminDataPre?.LastChangedByUserDescription && !adminDataPre?.LastChangedByUser) {
		await unSavedContext.requestProperty([
			"DraftAdministrativeData/LastChangedByUserDescription",
			"DraftAdministrativeData/LastChangedByUser"
		]);
	}
	const adminData = unSavedContext.getObject("DraftAdministrativeData") as DraftAdministrativeDataType | undefined;
	return adminData?.LastChangedByUserDescription ?? adminData?.LastChangedByUser ?? "";
}

async function getUnsavedContextsText(
	resourceModel: ResourceModel,
	numberOfSelectedContexts: number,
	unSavedContexts: ODataV4Context[],
	totalDeletable: number
): Promise<DeleteTextInfo> {
	let infoTxt = "",
		optionTxt = "",
...
async function updateDraftOptionsForDeletableTexts(
	mParameters: DeleteParameters,
	...
	if (unSavedContexts.length > 0) {
		const unsavedChangesTxts =
			(await deleteHelper.getUnsavedContextsText(resourceModel, numberOfSelectedContexts, unSavedContexts, totalDeletable)) || {};
...
async function deleteConfirmHandler(
	...
	try {
		...
		await messageHandler.showMessages({ unHoldKey: messageHandlingKey, control: mParameters.parentControl });
