...
export function getCreatePopupFields(metaModel: ODataMetaModel, path: string, appComponent?: AppComponent): string[] {
	...
	for (const prop of entityProperties) {
		const annotations = prop.annotations,
			...
			isNonGUIDKey = isKey && prop.type !== "Edm.Guid",
			isGUIDKeyWithText = isKey && prop.type === "Edm.Guid" ? !!annotations.Common?.Text : false;

		// If it's computed we don't show it
		// if it's immutable we show it if it's visible
		// If it's a key with type guid AND ComputedDefaultValue and Text annotation we show it
		// Steampunk case : We have a key of type Guid with Text but not with ComputedDefault -> We need to show them
		// CAP case : We have keys of type Guid with no Text but with ComputedDefaulValue -> We don't show them
		// Other case : GUID Key, with no text and no ComputedDefaultValue -> We should show them from the OData spec but not from fiori guidelines

		if ((isNonGUIDKey || isGUIDKeyWithText) && isNonComputed && isVisible) {
