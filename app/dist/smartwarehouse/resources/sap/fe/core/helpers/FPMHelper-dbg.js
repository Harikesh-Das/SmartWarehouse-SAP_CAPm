  ...
  const FPMHelper = {
    loadModuleAndCallMethod: async function (module, method, source) {
      ...
      return new Promise(function (resolve) {
        ...
        if (view.getControllerName() === "sap.fe.templates.ObjectPage.ObjectPageController" || view.getControllerName() === "sap.fe.ariba.ObjectPage.ObjectPageController" || view.getControllerName() === "sap.fe.templates.ListReport.ListReportController" || view.getControllerName() === "sap.fe.ariba.ListReport.ListReportController") {
        ...
        if (module.startsWith("/extension/")) {
          ...
          const fnTarget = module ? ObjectPath.get(module.replace(/\//g, ".").substring(1), view.getController()) : view.getController();
          resolve(fnTarget[method](...args));
        } else {
    ...
    preloadModule: async function (moduleName) {
      if (!moduleName || moduleName.startsWith("/")) {
        return Promise.resolve();
      } else {
        return new Promise((resolve, reject) => {
          sap.ui.require([moduleName], () => {
            resolve();
          }, () => {
