...
sap.ui.define(["sap/fe/base/BindingToolkit", "sap/fe/core/helpers/TypeGuards"], function (BindingToolkit, TypeGuards) {
  ...
  /**
   * Modifies the selection variant and URL parameters based on the provided local property and semantic object property.
   * This function ensures that the selection variant and URL parameters are consistent with the semantic object property.
   * @param selectionVariantToBeModified The selection variant object to be modified.
   * @param localProperty The local property name used to identify the relevant select option or parameter.
   * @param urlParametersFromSelectionVariant A record of URL parameters extracted from the selection variant.
   * @param semanticObjectProperty The semantic object property name to align with the local property.
   */
  _exports.manageSemanticObjectsForCurrentUser = manageSemanticObjectsForCurrentUser;
  const computeSelectionVariantAndURLParameters = function (selectionVariantToBeModified, localProperty, urlParametersFromSelectionVariant, semanticObjectProperty) {
    ...
    let localPropertySelectOptionName = selectionVariantToBeModified.getSelectOptionsPropertyNames().find(property => {
      return property.includes(localProperty.replace("/", "."));
    });
    //2. there is no property named the same on the contextEntity so the selectOption is named property
    localPropertySelectOptionName = localPropertySelectOptionName ? localPropertySelectOptionName : localProperty.split("/").slice(-1)[0];
    if (!urlParametersFromSelectionVariant[localPropertySelectOptionName]) {
      // The navigation property has no value
      delete urlParametersFromSelectionVariant[localPropertySelectOptionName];
      selectionVariantToBeModified.removeParameter(localPropertySelectOptionName);
      selectionVariantToBeModified.removeSelectOption(localPropertySelectOptionName);
    } else if (localPropertySelectOptionName !== semanticObjectProperty) {
      // The navigation property has a value and properties names are different
      selectionVariantToBeModified.removeParameter(semanticObjectProperty);
      selectionVariantToBeModified.removeSelectOption(semanticObjectProperty);
      selectionVariantToBeModified.renameParameter(localPropertySelectOptionName, semanticObjectProperty);
      selectionVariantToBeModified.renameSelectOption(localPropertySelectOptionName, semanticObjectProperty);
      urlParametersFromSelectionVariant[semanticObjectProperty] = urlParametersFromSelectionVariant[localPropertySelectOptionName];
      delete urlParametersFromSelectionVariant[localPropertySelectOptionName];
    }
  };
  _exports.computeSelectionVariantAndURLParameters = computeSelectionVariantAndURLParameters;
  return _exports;
}, false);
