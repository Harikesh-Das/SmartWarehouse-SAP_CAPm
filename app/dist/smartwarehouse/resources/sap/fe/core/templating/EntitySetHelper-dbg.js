...
sap.ui.define(["sap/fe/core/helpers/TypeGuards", "./DataModelPathHelper"], function (TypeGuards, DataModelPathHelper) {
   ...
   */
  const getPropertiesRestrictionsBasedOnEntities = function (entitySet, targetEntitySet, parentNavigationPath) {
    ...
    return {
      nonSortableProperties: Array.from(new Set([...restrictionsFromNavigationRestrictions.nonSortableProperties, ...directRestrictions.nonSortableProperties])),
      nonFilterableProperties: Array.from(new Set([...restrictionsFromNavigationRestrictions.nonFilterableProperties, ...directRestrictions.nonFilterableProperties]))
    };
  };
    ...
    entitySet.annotations.Capabilities?.NavigationRestrictions?.RestrictedProperties?.forEach(navigationRestriction => {
      ...
      if (navigationRestriction?.SortRestrictions?.Sortable === false) {
        ...
        if (navigationProperty) {
          ...
          sortRestrictionsFromNavigationRestrictions.push(...getAllPropertiesOfNavigationProperty(navigationProperty, navigationRestriction, parentNavigationPath));
        }
      } else {
    ...
    entitySet.annotations.Capabilities?.NavigationRestrictions?.RestrictedProperties?.forEach(navigationRestriction => {
      ...
      if (navigationRestriction?.FilterRestrictions?.Filterable === false) {
        ...
        if (navigationProperty) {
          ...
          filterRestrictionsFromNavigationRestrictions.push(...getAllPropertiesOfNavigationProperty(navigationProperty, navigationRestriction, parentNavigationPath));
        }
      } else {
   ...
   */
  function getAllPropertiesOfNavigationProperty(navigationProperty, navigationRestriction, parentNavigationPath) {
   ...
   */
  function getRestrictedPropertyRelativePath(property, targetEntitySet, parentNavigationPath) {
    if (parentNavigationPath && property.value.includes(parentNavigationPath)) {
