...
export const getEditableExpression = function (
	...
	let editableExpression: BindingToolkitExpression<boolean>;
	if (oProperty !== undefined) {
		const baseEditableExpression = ifElse(
			or(
...
export const getEditMode = function (
	...
	let resultExpression: BindingToolkitExpression<string> = ifElse(
		isTotal,
		constant(FieldEditMode.ReadOnly),
		constant(FieldEditMode.Editable)
	);
	if (unitProperty) {
		...
		resultExpression = ifElse(
			or(
				and(not(isUnitPathUpdatableExpression), not(UI.IsTransientBinding)),
				isUnitReadOnly,
				isComputed(unitProperty),
				and(isImmutable(unitProperty), not(UI.IsTransientBinding)),
				measureReadOnly
			),
			ifElse(
				!isConstant(isUnitReadOnly) && isUnitReadOnly,
				ifElse(isTotal, FieldEditMode.ReadOnly, FieldEditMode.EditableReadOnly),
				ifElse(isTotal, FieldEditMode.ReadOnlyDisplay, FieldEditMode.EditableDisplay)
			),
			ifElse(isTotal, constant(FieldEditMode.ReadOnly), constant(FieldEditMode.Editable))
		);
	}
		...
		ifElse(
			...
			ifElse(
				...
				ifElse(
					...
					unitProperty ? constant(FieldEditMode.ReadOnlyDisplay) : constant(FieldEditMode.ReadOnly),
			...
			ifElse(
				...
				unitProperty ? constant(FieldEditMode.DisabledDisplay) : constant(FieldEditMode.Disabled),
				FieldEditMode.Display
			)
		),
...
export const isTotalOutdated = function (): CompiledBindingToolkitExpression {
...
export const getBindingWithUnitOrCurrency = function (
	...
	const { computeDecimalPadding, ...typeFormatOptions } = formatOptions ?? {};
	...
	let effectiveFormatOptions: typeof typeFormatOptions | undefined = formatOptions ? typeFormatOptions : undefined;
	if (forDisplayMode) {
		effectiveFormatOptions = { ...effectiveFormatOptions, emptyString: "" };
	}
...
export const getRequiredExpressionForFieldGroup = function (
	dataFieldObjectPath: DataModelObjectPath<DataFieldForAnnotation>
): CompiledBindingToolkitExpression {
	return compileExpression(and(UI.IsEditable, isRequiredExpression(dataFieldObjectPath.targetObject?.Target?.$target as FieldGroup)));
};
 ...
 */
export const getTooltipValue = function (
	dataModelPath: DataModelObjectPath<DataField | DataPointType> | undefined,
	propertyDataModelPath?: DataModelObjectPath<Property> | undefined
): BindingToolkitExpression<string> | undefined {
	let result = dataModelPath?.targetObject?.annotations?.Common?.QuickInfo
		? getExpressionFromAnnotation(dataModelPath.targetObject?.annotations?.Common?.QuickInfo, getRelativePaths(dataModelPath))
		: undefined;
	if (result == undefined) {
		result = propertyDataModelPath?.targetObject?.annotations?.Common?.QuickInfo
			? getExpressionFromAnnotation(
					propertyDataModelPath.targetObject?.annotations?.Common?.QuickInfo,
					getRelativePaths(propertyDataModelPath)
			  )
			: undefined;
	}
	return result;
