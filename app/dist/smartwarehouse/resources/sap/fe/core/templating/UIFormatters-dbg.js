...
sap.ui.define(["sap/fe/base/BindingToolkit", "sap/fe/core/converters/MetaModelConverter", "sap/fe/core/helpers/BindingHelper", "sap/fe/core/helpers/TypeGuards", "sap/fe/core/templating/DataModelPathHelper", "sap/fe/core/templating/DisplayModeFormatter", "sap/fe/core/templating/FieldControlHelper", "sap/fe/core/templating/PropertyHelper"], function (BindingToolkit, MetaModelConverter, BindingHelper, TypeGuards, DataModelPathHelper, DisplayModeFormatter, FieldControlHelper, PropertyHelper) {
  ...
  const FieldEditMode = {
    ...
    /**
     * {@link sap.ui.mdc.Field Field}, {@link sap.ui.mdc.FilterField FilterField} or {@link sap.ui.mdc.MultiValueField MultiValueField }
     * is rendered in read-only mode, with the additional parts in display mode
     */
    ReadOnlyDisplay: "ReadOnlyDisplay",
    /**
     * {@link sap.ui.mdc.Field Field}, {@link sap.ui.mdc.FilterField FilterField} or {@link sap.ui.mdc.MultiValueField MultiValueField }
     * is rendered in disabled mode, with the additional parts in display mode
     */
    DisabledDisplay: "DisabledDisplay"
  };
  _exports.FieldEditMode = FieldEditMode;
  const getDisplayMode = function (oDataModelObjectPath) {
    return DisplayModeFormatter.getDisplayMode(oDataModelObjectPath.targetObject, oDataModelObjectPath.targetEntityType);
  };
  ...
  const getEditableExpressionAsObject = function (oPropertyPath, oDataFieldConverted, oDataModelObjectPath) {
    ...
    let forAnalyticalTable = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : false;
    return getEditableExpression(oPropertyPath, oDataFieldConverted, oDataModelObjectPath, true, isEditable, considerUpdateRestrictions, forAnalyticalTable);
  };
   ...
   * @param forAnalyticalTable Whether this is for an analytical table
  ...
  const getEditableExpression = function (oPropertyPath) {
    ...
    let forAnalyticalTable = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : false;
    ...
    if (oProperty !== undefined) {
      const baseEditableExpression = ifElse(or(and(not(isPathUpdatableExpression), considerUpdateRestrictions), isComputed(oProperty), isKey(oProperty), isImmutable(oProperty), isNonEditableExpression(oProperty, relativePath)), ifElse(or(isComputed(oProperty), isNonEditableExpression(oProperty, relativePath)), false, UI.IsTransientBinding), isEditable);
      editableExpression = forAnalyticalTable ? ifElse(UI.IsTotal, constant(false), baseEditableExpression) : baseEditableExpression;
    } else {
      editableExpression = unresolvableExpression;
    }
    if (bAsObject) {
   ...
   * @param isEditable Whether or not UI.IsEditable be considered.
   * @param forAnalyticalTable Whether or not this is for an analytical table.
  ...
  const getEditMode = function (propertyPath, dataModelObjectPath) {
    ...
    const unitPropertyPath = getAssociatedCurrencyPropertyPath(property) || getAssociatedUnitPropertyPath(property);
    const isTotal = forAnalyticalTable ? UI.IsTotal : constant(false);
    let resultExpression = ifElse(isTotal, constant(FieldEditMode.ReadOnly), constant(FieldEditMode.Editable));
    if (unitProperty) {
      const isUnitReadOnly = isReadOnlyExpression(unitProperty, relativePath);
      const isUnitPathUpdatableExpression = isPathUpdatable(dataModelObjectPath, {
        propertyPath: unitPropertyPath,
        pathVisitor: (path, navigationPaths) => singletonPathVisitor(path, dataModelObjectPath.convertedTypes, navigationPaths)
      });
      resultExpression = ifElse(or(and(not(isUnitPathUpdatableExpression), not(UI.IsTransientBinding)), isUnitReadOnly, isComputed(unitProperty), and(isImmutable(unitProperty), not(UI.IsTransientBinding)), measureReadOnly), ifElse(!isConstant(isUnitReadOnly) && isUnitReadOnly, ifElse(isTotal, FieldEditMode.ReadOnly, FieldEditMode.EditableReadOnly), ifElse(isTotal, FieldEditMode.ReadOnlyDisplay, FieldEditMode.EditableDisplay)), ifElse(isTotal, constant(FieldEditMode.ReadOnly), constant(FieldEditMode.Editable)));
    }
    if (!unitProperty && (property?.annotations?.Measures?.ISOCurrency || property?.annotations?.Measures?.Unit)) {
      ...
      resultExpression = ifElse(isTotal, constant(FieldEditMode.ReadOnlyDisplay), constant(FieldEditMode.EditableDisplay));
    }
    ...
    const editModeExpression = ifElse(or(isPathUpdatableExpression, UI.IsTransientBinding), ifElse(enabledExpression, ifElse(editableExpression, resultExpression, ifElse(and(!isConstant(readOnlyExpression) && readOnlyExpression, isEditable),
    // Use ReadOnlyDisplay when field has unit/currency to properly render the unit part
    unitProperty ? constant(FieldEditMode.ReadOnlyDisplay) : constant(FieldEditMode.ReadOnly), FieldEditMode.Display)), ifElse(isEditable,
    // Use DisabledDisplay when field has unit/currency to properly render the unit part
    unitProperty ? constant(FieldEditMode.DisabledDisplay) : constant(FieldEditMode.Disabled), FieldEditMode.Display)),
    /* Grand total rows in edit mode must render as ReadOnly (not Display) so the isTotalOutdated
     ConditionalWrapper is active and visual alignment matches tables where Updatable evaluates to true. */
    ifElse(and(isTotal, isEditable), constant(FieldEditMode.ReadOnly), FieldEditMode.Display));
    if (asObject) {
  ...
  const isTotalOutdated = function () {
    return compileExpression(and(UI.IsTotal, UI.IsOutdated));
  };
  _exports.isTotalOutdated = isTotalOutdated;
  ...
  const getBindingWithUnitOrCurrency = function (oPropertyDataModelPath, propertyBindingExpression, ignoreUnitConstraint, formatOptions, forDisplayMode, showOnlyUnitDecimals, preserveDecimalsForCurrency) {
    ...
    // computeDecimalPadding is consumed here to select the type — it must not reach addTypeInformation
    // or it would be serialized into the XML binding's formatOptions.
    const {
      computeDecimalPadding,
      ...typeFormatOptions
    } = formatOptions ?? {};
    let complexType;
    if (unit) {
      complexType = "sap.ui.model.odata.type.Unit";
    } else if (computeDecimalPadding) {
      complexType = "sap.fe.core.type.DynamicCurrency";
    } else {
      complexType = "sap.ui.model.odata.type.Currency";
    }
    ...
    } else if (preserveDecimalsForCurrency) {
      constraintOptions = {
        skipDecimalsValidation: true
      };
    }
    let effectiveFormatOptions = formatOptions ? typeFormatOptions : undefined;
    if (forDisplayMode) {
    ...
    return addTypeInformation([propertyBindingExpression, unitBindingExpression], complexType, undefined, effectiveFormatOptions, constraintOptions);
  };
  ...
  const getRequiredExpressionForFieldGroup = function (dataFieldObjectPath) {
    return compileExpression(and(UI.IsEditable, isRequiredExpression(dataFieldObjectPath.targetObject?.Target?.$target)));
  };
  ...
  /**
   * Get the tooltip value from the data model path or property data model path.
   * @param dataModelPath
   * @param propertyDataModelPath
   * @returns The tooltip binding expression
   */
  _exports.getVisibleExpressionForBreadcrumbs = getVisibleExpressionForBreadcrumbs;
  const getTooltipValue = function (dataModelPath, propertyDataModelPath) {
    let result = dataModelPath?.targetObject?.annotations?.Common?.QuickInfo ? getExpressionFromAnnotation(dataModelPath.targetObject?.annotations?.Common?.QuickInfo, getRelativePaths(dataModelPath)) : undefined;
    if (result == undefined) {
      result = propertyDataModelPath?.targetObject?.annotations?.Common?.QuickInfo ? getExpressionFromAnnotation(propertyDataModelPath.targetObject?.annotations?.Common?.QuickInfo, getRelativePaths(propertyDataModelPath)) : undefined;
    }
    return result;
  };
  _exports.getTooltipValue = getTooltipValue;
  return _exports;
}, false);
