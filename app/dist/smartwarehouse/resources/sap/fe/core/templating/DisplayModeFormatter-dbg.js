...
sap.ui.define(["sap/fe/core/helpers/TypeGuards"], function (TypeGuards) {
  ...
  const getDisplayMode = function (oPropertyPath, targetEntityType) {
    ...
    const oTextArrangementAnnotation = typeof oTextAnnotation !== "string" && oTextAnnotation?.annotations?.UI?.TextArrangement?.toString() || targetEntityType.annotations?.UI?.TextArrangement?.toString();
    let sDisplayValue = oTextAnnotation ? "DescriptionValue" : "Value";
    if (oTextAnnotation && oTextArrangementAnnotation || targetEntityType.annotations?.UI?.TextArrangement?.toString()) {
