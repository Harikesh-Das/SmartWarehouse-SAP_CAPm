 ...
 */
sap.ui.define(["sap/base/Log", "sap/f/FlexibleColumnLayoutSemanticHelper", "sap/f/library", "sap/fe/base/ClassSupport", "sap/fe/core/controllerextensions/ViewState", "sap/fe/core/helpers/KeepAliveHelper", "sap/fe/core/helpers/ManifestHelper", "sap/m/Button", "sap/m/FlexBox", "sap/m/IllustratedMessage", "sap/m/Page", "sap/fe/core/helpers/PromiseKeeper", "sap/ui/Device", "../CommonUtils", "./RootViewBaseController"], function (Log, FlexibleColumnLayoutSemanticHelper, fLibrary, ClassSupport, ViewState, KeepAliveHelper, ManifestHelper, Button, FlexBox, IllustratedMessage, Page, PromiseKeeper, Device, CommonUtils, BaseController) {
  "use strict";
  ...
  let FclController = (_dec = defineUI5Class("sap.fe.core.rootView.Fcl"), _dec2 = usingExtension(ViewState.override({
    ...
    _proto.onStateChanged = function onStateChanged(oEvent) {
      ...
      const rightmostView = this.getRightmostView();
      if (rightmostView) {
        if (bIsNavigationArrow === false) {
          // The state can be changed by window resizing, so we need to update the visibility of FCL buttons
          this.updateUIStateForView(rightmostView, rightmostView.getController()._routing.getFCLLevel());
        }
        this.computeTitleHierarchy(rightmostView);
      }
    }
    ...
    _proto._getPaginatorButtonVisibility = function _getPaginatorButtonVisibility(sLayout) {
      if (Device.system.phone || sLayout === "OneColumn" || sLayout === "MidColumnFullScreen" || sLayout === "EndColumnFullScreen") {
        return true;
      }
      return false;
    };
    _proto.updateUIStateForView = async function updateUIStateForView(oView, FCLLevel) {
      await this._currentPromise?.promise;
      ...
      oView.getModel("fclhelper").setProperty("/showPaginatorButton", this._getPaginatorButtonVisibility(sLayout));
    }
    ...
    _proto.onBeforeRouteMatched = async function onBeforeRouteMatched(oEvent) {
      if (oEvent) {
        ...
        if (sLayout) {
          this._currentPromise = new PromiseKeeper();
          if (this.getAppComponent().delayFcl) {
            setTimeout(() => {
    ...
    /**
     * Gets the context path of the left most view which is not null.
     * @returns The context path of the left most view, or undefined if not found
     */;
    _proto.getLeftMostViewContextPath = function getLeftMostViewContextPath() {
      const views = this.getInstancedViews();
      for (const view of views) {
        if (view.getViewName() === "sap.fe.templates.ObjectPage.ObjectPage") {
          return view.getBindingContext()?.getPath();
        }
      }
      return undefined;
    }
