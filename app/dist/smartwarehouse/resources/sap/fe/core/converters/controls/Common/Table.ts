...
type RowNavigationInfo =
	| {
			...
			targetControlId?: string; // Section to scroll to after navigation
	  }
	| {
			type: "Navigation";
			targetPath: string;
...
export type UpdatablePropertyPathInfo = {
	isPathAnnotationExpression: boolean;
	bindingExpression: CompiledBindingToolkitExpression;
	paths: string[];
};
...
export type TableAnnotationConfiguration = {
	...
	updatablePropertyPathInfo: UpdatablePropertyPathInfo | undefined;
	displayMode?: boolean;
	threshold: number;
	entityName: string;
...
export type TableVisualization = {
	...
	columns: TableColumn[];
	actions: BaseAction[];
	commandActions?: Record<string, BaseAction>;
...
export function getTableActions(
	...
	const actionOverwriteConfig: OverrideTypeAction = getActionOverrideConfig(true, true);
	...
	tableColumns.forEach((column) => {
		...
		if (!rawContextDefiningProperties && property && !!aggregationHelper.isPropertyGroupable(property)) {
			...
			const mainColumnForTextArrangement = tableColumns.find((col) => {
				return (
					(col as AnnotationTableColumn).textArrangement?.mode === "Description" &&
					(col as AnnotationTableColumn).textArrangement?.textProperty === tableColumn.name
				);
			}) as AnnotationTableColumn | undefined;
			if (
				mainColumnForTextArrangement &&
				mainColumnForTextArrangement.relativePath &&
				rawContextDefiningProperties?.includes(mainColumnForTextArrangement.relativePath) !== true
			) {
...
export function updateTableVisualizationForType(
	...
	if (visualizationPath) {
		const manifestControlConfiguration =
			converterContext.getManifestControlConfiguration<TableManifestConfiguration>(visualizationPath);
...
type fieldOrigin =
	...
	| "draftKeys"
	| "additionalPropertiesFromManifest"
	| "additionalPropertiesFromMacroBB";
...
export function createRequestedProperties(
	...
	presentationVariantAnnotation?: PresentationVariantType,
	additionalPropertiesFromManifest?: string[],
	additionalPropertiesFromMacroBuildingBlock?: string
): RequestAtLeast {
	...
	if (additionalPropertiesFromManifest) {
		pushFieldList(
			ModelHelper.getValidatedAdditionalProperties(converterContext.getDataModelObjectPath(), additionalPropertiesFromManifest),
			"additionalPropertiesFromManifest"
		);
	}
	if (additionalPropertiesFromMacroBuildingBlock) {
		pushFieldList(
			ModelHelper.getValidatedAdditionalProperties(
				converterContext.getDataModelObjectPath(),
				additionalPropertiesFromMacroBuildingBlock.split(",")
			),
			"additionalPropertiesFromMacroBB"
		);
	}
...
export function createTableVisualization(
	...
	const oVisualization: TableVisualization = {
		...
		requestAtLeast: createRequestedProperties(
			lineItemAnnotation,
			converterContext,
			operationAvailable,
			presentationVariantAnnotation,
			tableManifestConfig.additionalProperties,
			params?.additionalPropertiesFromMacroBuildingBlock
		),
 ...
 */
function getCurrentEntitySetUpdatablePathInfo(
	converterContext: ConverterContext<PageContextPathTarget>
): UpdatablePropertyPathInfo | undefined {
	...
	const paths: Set<string> = (bindingInfo.parts as PropertyBindingInfo[]).reduce((acc: Set<string>, part: PropertyBindingInfo) => {
...
function getTableAnnotationActions(
	...
	visualizationPath: string,
	converterContext: ConverterContext<PageContextPathTarget>
): { tableActions: Array<AnnotationAction | ActionGroup> } {
	const tableActions: Array<AnnotationAction | ActionGroup> = [];
...
function getDataFieldAnnotationAction(
	...
	converterContext: ConverterContext<PageContextPathTarget>
): AnnotationAction | ActionGroup | void {
	...
	if (navigationSettings && lineItemAnnotation) {
		...
		if (navigationTarget) {
			navigationInfo = {
				type: "Outbound",
				navigationTarget,
				...(targetControlId && { targetControlId }) // Only include if defined
			};
		}
 ...
 */
function convertPropertyPathsToInfoKeys(paths: PropertyPath[], columns: TableColumn[]): string[] {
...
function getTableType(
	...
	if (tableManifestSettings?.tableSettings?.type) {
		...
		if (!["ResponsiveTable", "GridTable", "TreeTable", "AnalyticalTable"].includes(tableManifestSettings.tableSettings.type)) {
			Log.error(
				`Invalid table type specified in the manifest: ${tableManifestSettings.tableSettings.type}. Fallback to ResponsiveTable.`
			);
