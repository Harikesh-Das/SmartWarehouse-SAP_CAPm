...
sap.ui.define(["sap/base/Log", "sap/fe/base/BindingToolkit", "sap/fe/core/converters/annotations/DataField", "sap/fe/core/converters/controls/Common/Action", "sap/fe/core/converters/helpers/ConfigurableObject", "sap/fe/core/converters/helpers/IssueManager", "sap/fe/core/converters/helpers/Key", "sap/fe/core/helpers/BindingHelper", "sap/fe/core/helpers/ModelHelper", "sap/fe/core/helpers/StableIdHelper", "sap/fe/core/helpers/TypeGuards", "sap/fe/core/templating/CriticalityFormatters", "sap/fe/core/templating/DataModelPathHelper", "sap/fe/core/templating/EntitySetHelper", "sap/fe/macros/formatters/TableFormatter", "sap/fe/macros/internal/helpers/ActionHelper", "sap/ui/core/Lib", "sap/ui/core/message/MessageType", "../../ManifestSettings", "../../helpers/Aggregation", "../../helpers/ID", "./Criticality", "./table/Columns", "./table/StandardActions", "sap/ui/base/BindingInfo"], function (Log, BindingToolkit, DataField, Action, ConfigurableObject, IssueManager, Key, BindingHelper, ModelHelper, StableIdHelper, TypeGuards, CriticalityFormatters, DataModelPathHelper, EntitySetHelper, tableFormatters, ActionHelper, Library, MessageType, ManifestSettings, Aggregation, ID, Criticality, Columns, StandardActions, BindingInfo) {
  ...
  var getActionOverrideConfig = Action.getActionOverrideConfig;
  ...
  var isExpressionStaticallyResolvable = BindingToolkit.isExpressionStaticallyResolvable;
  ...
  function getTableActions(lineItemAnnotation, visualizationPath, converterContext, standardActions, navigationSettings) {
    ...
    const actionOverwriteConfig = getActionOverrideConfig(true, true);
    ...
    const overflowGroupsConfig = converterContext.getManifestControlConfiguration(visualizationPath)?.overflowGroups;
  ...
  const getExtensionInfoFromEntityType = function (entityType, tableColumns, converterContext) {
    ...
    tableColumns.forEach(column => {
      ...
      if (!rawContextDefiningProperties && property && !!aggregationHelper.isPropertyGroupable(property)) {
        ...
        // If the property is used as Text property for another one, with TextArrangement "TextOnly", we need to add the main property as a context-defining property
        // to ensure it's also loaded, so that intent-based navigation can use the value of the main property and not just its text representation.
        const mainColumnForTextArrangement = tableColumns.find(col => {
          return col.textArrangement?.mode === "Description" && col.textArrangement?.textProperty === tableColumn.name;
        });
        if (mainColumnForTextArrangement && mainColumnForTextArrangement.relativePath && rawContextDefiningProperties?.includes(mainColumnForTextArrangement.relativePath) !== true) {
          rawContextDefiningProperties ??= [];
          rawContextDefiningProperties.push(mainColumnForTextArrangement.relativePath);
        }
      }
   ...
   * @param visualizationPath The visualization path to the visualization
   */
  _exports.getExtensionInfoFromEntityType = getExtensionInfoFromEntityType;
  function updateTableVisualizationForType(tableVisualization, entityType, converterContext, presentationVariantAnnotation, visualizationPath) {
    ...
    if (visualizationPath) {
      const manifestControlConfiguration = converterContext.getManifestControlConfiguration(visualizationPath);
      if (manifestControlConfiguration?.tableSettings?.disableOwnRequestOnMVF) {
        tableVisualization.disableOwnRequestOnMVF = true;
      }
    }
  }
   ...
   * @param converterContext The converter context.
   * @param operationAvailable A string containing the available operation, which takes the properties to add to the select query.
   * @param presentationVariantAnnotation The presentation variant annotation which the RequestAtLeast properties are taken from.
   * @param additionalPropertiesFromManifest Additional property paths declared in the manifest.json file to be included in the select query.
   * @param additionalPropertiesFromMacroBuildingBlock Additional property paths declared in the building block to be included in the select query.
   * @returns The 'select' query that contains the selected fields from the parameters that were passed.
   */
  function createRequestedProperties(lineItem, converterContext, operationAvailable, presentationVariantAnnotation, additionalPropertiesFromManifest, additionalPropertiesFromMacroBuildingBlock) {
    ...
    if (additionalPropertiesFromManifest) {
      pushFieldList(ModelHelper.getValidatedAdditionalProperties(converterContext.getDataModelObjectPath(), additionalPropertiesFromManifest), "additionalPropertiesFromManifest");
    }
    if (additionalPropertiesFromMacroBuildingBlock) {
      pushFieldList(ModelHelper.getValidatedAdditionalProperties(converterContext.getDataModelObjectPath(), additionalPropertiesFromMacroBuildingBlock.split(",")), "additionalPropertiesFromMacroBB");
    }
    if (capabilitiesAnnotation) {
      pushField(capabilitiesAnnotation.DeleteRestrictions?.Deletable?.path, "restriction");
    }
    const updatePropertyInfo = getCurrentEntitySetUpdatablePathInfo(converterContext);
    if (updatePropertyInfo?.paths) {
      pushFieldList(updatePropertyInfo.paths, "updateRestriction");
    }
    return selectedFields;
  }

  /**
   * This returns the property that are available from the NavigationAvailable annotation on IBN Fields.
  ...
  function createTableVisualization(lineItemAnnotation, visualizationPath, converterContext, params) {
    ...
    const oVisualization = {
      ...
      requestAtLeast: createRequestedProperties(lineItemAnnotation, converterContext, operationAvailable, presentationVariantAnnotation, tableManifestConfig.additionalProperties, params?.additionalPropertiesFromMacroBuildingBlock),
    ...
    updateTableVisualizationForType(oVisualization, converterContext.getAnnotationEntityType(lineItemAnnotation), converterContext, presentationVariantAnnotation, visualizationPath);
  ...
  /**
   * Gets updatable properties for the current entityset, if valid.
   * @param converterContext The instance of the converter context
   * @returns The updatable properties for the rows
   */
  function getCurrentEntitySetUpdatablePathInfo(converterContext) {
    ...
    if (!isOnlyDynamicOnCurrentEntity || updatableExpression === undefined || isConstant(updatableExpression)) {
      return;
    }
    const bindingExpression = compileExpression(getExpressionFromAnnotation(updatableExpression));
    if (isPathAnnotationExpression(updatableExpression)) {
      return {
        bindingExpression,
        isPathAnnotationExpression: true,
        paths: [updatableExpression.path]
      };
    }
    const bindingInfo = BindingInfo.parse(compileExpression(getExpressionFromAnnotation(updatableExpression)));
    const paths = bindingInfo.parts.reduce((acc, part) => {
      const path = part.path;
      if (part.model === undefined && path) {
        acc.add(path);
      }
      return acc;
    }, new Set());
    return {
      isPathAnnotationExpression: false,
      bindingExpression,
  ...
  function getSelectionMode(lineItemAnnotation, visualizationPath, converterContext, isEntitySet, deleteButtonVisibilityExpression) {
    let massEditVisibilityExpression = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : constant(false);
    let cutButtonVisibilityExpression = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : constant(false);
    ...
    let hiddenBindingExpressions = [],
      visibleBindingExpressions = [];
    ...
    const massEditEnabled = !isConstant(massEditVisibilityExpression) || massEditVisibilityExpression.value !== false;
    ...
    hiddenBindingExpressions = getUIHiddenExpForActionsRequiringContext(lineItemAnnotation, converterContext.getEntityType(), converterContext.getDataModelObjectPath());
    ...
    if (hiddenBindingExpressions.length === 0 && visibleBindingExpressions.length === 0 && (cutButtonVisibilityExpression || deleteButtonVisibilityExpression || massEditEnabled)) {
      if (!isEntitySet) {
        // OP case:
        if (!deletableStaticFalse || parentEntitySetDeletable !== "false" || massEditEnabled) {
          const buttonVisibilityExpression = or(cutButtonVisibilityExpression || true, deleteButtonVisibilityExpression, massEditVisibilityExpression);
        ...
        // EntitySet case:
        return compileExpression(ifElse(or(massEditEnabled, deleteButtonVisibilityExpression), constant(selectionMode), constant(SelectionMode.None)));
      }
      ...
      if (!deletableStaticFalse || parentEntitySetDeletable !== "false" || massEditEnabled) {
        ...
        const editModebuttonVisibilityExpression = ifElse(massEditEnabled && deletableStaticFalse, massEditVisibilityExpression, constant(true));
        return compileExpression(ifElse(and(UI.IsEditable, editModebuttonVisibilityExpression), constant(selectionMode), ifElse(or(...hiddenBindingExpressions.concat(visibleBindingExpressions)), constant(selectionMode), constant(SelectionMode.None))));
      } else {
        return compileExpression(ifElse(or(...hiddenBindingExpressions.concat(visibleBindingExpressions)), constant(selectionMode), constant(SelectionMode.None)));
      }
      // EntitySet
    } else {
      return compileExpression(ifElse(or(massEditEnabled, deleteButtonVisibilityExpression), constant(selectionMode), ifElse(or(...hiddenBindingExpressions.concat(visibleBindingExpressions), massEditVisibilityExpression), constant(selectionMode), constant(SelectionMode.None))));
    }
  }
  ...
  function _getCreationBehaviour(lineItemAnnotation, tableManifestConfiguration, converterContext, navigationSettings, visualizationPath) {
    ...
    if (tableManifestConfiguration.creationMode === CreationMode.NewPage && navigationDetails?.route === undefined) {
      if (converterContext.getTemplateType() === TemplateType.ListReport) {
        if (originalTableSettings.creationMode?.name === CreationMode.NewPage) {
          // We log an error only if the creation mode was explicitly set to 'NewPage'
  ...
  const _getRowConfigurationProperty = function (lineItemAnnotation, converterContext, navigationSettings, targetPath, tableType, aggregationOnLeafLevelenabled) {
    ...
    if (navigationSettings && lineItemAnnotation) {
      const targetControlId = navigationSettings.detail?.targetControlId;
    ...
    if (presentationVariantAnnotation?.SortOrder) {
      presentationVariantAnnotation.SortOrder.forEach(condition => {
        ...
        if (conditionProperty?.$target !== undefined && !nonSortableProperties.includes(conditionProperty.$target.name)) {
          const infoKey = convertPropertyPathsToInfoKeys([conditionProperty], columns)[0];
          if (infoKey) {
  ...
  /**
   * Converts an array of propertyPath to an array of propertyInfo keys.
   * @param paths The array to be converted
   * @param columns The array of propertyInfos
   * @returns An array of propertyInfo keys
   */
  function convertPropertyPathsToInfoKeys(paths, columns) {
    const infoKeys = [];
    ...
    paths.forEach(currentPath => {
      if (currentPath?.value) {
        ...
        if (propertyInfo) {
          infoKeys.push(propertyInfo.name);
        }
      }
    });
    return infoKeys;
  ...
  function getGroupConditions(presentationVariantAnnotation, columns, tableType) {
    ...
    if (presentationVariantAnnotation?.GroupBy) {
      ...
      groupConditions.groupLevels = convertPropertyPathsToInfoKeys(aGroupBy, columns).map(infoKey => {
        return {
          key: infoKey
        };
      });
  ...
  function getAggregateConditions(presentationVariantAnnotation, columns, extensionInfoMap) {
    ...
    if (presentationVariantAnnotation?.Total) {
      ...
      convertPropertyPathsToInfoKeys(aTotals, columns).forEach(infoKey => {
        aggregateConditions[infoKey] = {};
      });
    } else {
  ...
  function getTableAnnotationConfiguration(lineItemAnnotation, visualizationPath, converterContext, tableManifestConfiguration, columns, navigationSettings, standardActionsConfiguration, presentationVariantAnnotation) {
    ...
    const navigationTargetPath = getNavigationTargetPath(converterContext, navigationPropertyPath);
    const selectionMode = getSelectionMode(lineItemAnnotation, visualizationPath, converterContext, hasAbsolutePath, standardActionsConfiguration.deleteButtonVisibilityExpression, standardActionsConfiguration.massEditButtonVisibilityExpression, standardActionsConfiguration.cutButtonVisibilityExpression);
    ...
    let searchable;
    if (tableManifestSettings.tableSettings?.isSearchable === false) {
      searchable = false;
    } else if (tableManifestConfiguration.type === "AnalyticalTable" || tableManifestConfiguration.type === "TreeTable") {
      // For analytical and tree tables, we look at the allowed transformations
      ...
      searchable = allowedTransformations ? allowedTransformations.includes("search") : true;
    } else {
      // For flat tables, we look at the SearchRestrictions annottion
      const isSearchable = isPathSearchable(converterContext.getDataModelObjectPath());
      searchable = !(isConstant(isSearchable) && isSearchable.value === false);
    }
    return {
      id: id,
      ...
      updatablePropertyPathInfo: getCurrentEntitySetUpdatablePathInfo(converterContext),
   ...
   * @param lineItemAnnotation Collection of data fields used for representation in a table or list
   */
  function updateTreeTableManifestConfiguration(tableConfiguration, tableSettings, converterContext, lineItemAnnotation) {
  ...
  function getHierarchyQualifier(entityType, tableSettings) {
    if (tableSettings.hierarchyQualifier) {
      return tableSettings.hierarchyQualifier;
    }
    const aggregationAnnotation = entityType.annotations?.Aggregation;
    const hierarchyAnnotation = entityType.annotations?.Hierarchy;
    ...
    const qualifiedName = hierarchyAnnotation?._keys?.find(key => key.includes("RecursiveHierarchy#") && aggregationAnnotation?._keys?.includes(key));
    if (qualifiedName) {
      return hierarchyAnnotation[qualifiedName]?.qualifier;
    }
  }
  ...
  function getTableManifestConfiguration(lineItemAnnotation, visualizationPath, converterContext) {
    ...
    const isCondensedTableLayoutCompliant = checkCondensedLayout && _manifestWrapper.isCondensedLayoutCompliant(tableType);
    ...
    // If aggregated data is displayed on leaf level in an AnalyticalTable, editing is not possible
    const allowAnalyticalEdit = tableSettings.analyticalConfiguration?.aggregationOnLeafLevel !== true;
    const oConfiguration = {
      ...
      readOnly: tableSettings.readOnly || (tableType === "AnalyticalTable" && !allowAnalyticalEdit ? true : undefined),
      ...
      additionalProperties: tableSettings.additionalProperties
    };
    ...
    if (tableType === "AnalyticalTable") {
      tableConfiguration.enableAnalyticalEdit = allowAnalyticalEdit;
    }
  ...
  function getTableType(converterContext, tableManifestSettings, lineItemAnnotation) {
    if (tableManifestSettings?.tableSettings?.type) {
      // in case the application specified a table type we will use this one
      if (!["ResponsiveTable", "GridTable", "TreeTable", "AnalyticalTable"].includes(tableManifestSettings.tableSettings.type)) {
        Log.error(`Invalid table type specified in the manifest: ${tableManifestSettings.tableSettings.type}. Fallback to ResponsiveTable.`);
    ...
    if (aggregationHelper.isAnalyticsSupported() && requiredAllowedTransformations.every(transformation => aggregationHelper.getAllowedTransformations()?.includes(transformation))) {
