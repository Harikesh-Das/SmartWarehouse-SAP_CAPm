...
sap.ui.define(["sap/fe/base/BindingToolkit", "sap/fe/core/converters/controls/Common/Action", "sap/fe/core/converters/helpers/ConfigurableObject", "sap/fe/core/converters/helpers/Key", "sap/fe/core/helpers/ModelHelper", "sap/fe/core/helpers/TypeGuards", "sap/fe/core/templating/DataModelPathHelper", "sap/fe/core/templating/UIFormatters", "sap/ui/core/Lib", "../../helpers/BindingHelper", "../ManifestSettings"], function (BindingToolkit, Action, ConfigurableObject, Key, ModelHelper, TypeGuards, DataModelPathHelper, UIFormatters, Library, BindingHelper, ManifestSettings) {
  ...
  function getEditButtonVisibility(converterContext, rootUpdateHidden, rootConverterContext, updateHidden, updateRestrictions, viewLevel, capabilities) {
    ...
    const updateRestrictionsAlwaysFalse = updateRestrictions !== undefined && updateRestrictions.valueOf() === false;
    ...
    const resultantExpression = ifElse(viewLevel > 1 || hiddenDraft, ifElse(updateHidden === undefined || updateHidden.valueOf() === false, and(notEditableExpression, !updateRestrictionsAlwaysFalse, equal(pathInModel("rootEditVisible", "internal"), true), ifElse(isFCLEnabled, equal(pathInModel("/showEditButton", "fclhelper"), true), true)), and(notEditableExpression, equal(updateHiddenExpression, false), equal(pathInModel("rootEditVisible", "internal"), true), ifElse(isFCLEnabled, equal(pathInModel("/showEditButton", "fclhelper"), true), true))), ifElse(rootUpdateHidden === undefined || rootUpdateHidden.valueOf() === false, notEditableExpression, and(notEditableExpression, equal(rootUpdateHiddenExpression, false))));
  ...
  function getHeaderDefaultActions(converterContext, capabilities) {
    ...
    if ((draftRoot?.EditAction || stickySessionSupported?.EditAction) && updateHidden !== true) {
      let visible = getEditButtonVisibility(converterContext, rootUpdateHidden, rootConverterContext, ModelHelper.isUpdateHidden(entitySet, entityType), updatablePropertyPath, viewLevel, capabilities);
    ...
    headerDataFieldForIBNActions.filter(dataField => {
      ...
      const isEmphasized = dataField.Criticality === undefined && dataField.annotations?.UI?.Emphasized?.valueOf() === true;
      ...
      headerActions.push({
        ...
        buttonType: isEmphasized ? ButtonType.Emphasized : ButtonType.Ghost,
        ...
        hasCriticality: dataField.Criticality !== undefined,
        isNavigable: true,
  ...
  function getFooterDefaultActions(viewLevel, converterContext, capabilities) {
    ...
    const stickySessionSupported = ModelHelper.getStickySession(entitySet),
      ...
      isDraftSupported = ModelHelper.isObjectPathDraftSupported(converterContext.getDataModelObjectPath()),
      conditionSave = Boolean(entitySetDraftRoot === "com.sap.vocabularies.Common.v1.DraftRoot" || capabilities?.HiddenDraft?.enabled && isDraftSupported || stickySessionSupported && stickySessionSupported?.SaveAction),
      conditionApply = viewLevel > 1 && !capabilities?.HiddenDraft?.enabled,
      conditionCancel = Boolean(entitySetDraftRoot === "com.sap.vocabularies.Common.v1.DraftRoot" || capabilities?.HiddenDraft?.enabled && isDraftSupported || stickySessionSupported && stickySessionSupported?.DiscardAction),
      conditionCreateNext = viewLevel > 1 && capabilities?.HiddenDraft?.enabled && isDraftSupported && !capabilities?.HiddenDraft?.hideCreateNext;
    ...
    if (conditionCreateNext) {
      const ifFCLEnabled = converterContext.getManifestWrapper().isFclEnabled();
      footerActions.push({
        type: ActionType.CreateNext,
        key: "CreateNextAction",
        visible: compileExpression(getCreateNextButtonVisibility(!!conditionCreateNext, ifFCLEnabled))
      });
    }
  ...
  /**
   * Creates an annotation-based action model entry for a DataFieldForAction.
   * This applies the UI.Emphasized annotation when allowed and when the action has no criticality.
   * @param dataField The DataFieldForAction annotation
   * @param converterContext The converter context
   * @param allowEmphasis Whether emphasis should be applied for this action.
   * Set to false for actions inside action groups (menu items) to prevent nested emphasis.
   * @returns The created annotation action definition
   */
  _exports.getFooterDefaultActions = getFooterDefaultActions;
  function getDataFieldAnnotationAction(dataField, converterContext) {
    let allowEmphasis = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;
    ...
    // If a Criticality is defined on the same action, we must not force an "Emphasized" type.
    // Otherwise the XML templating will pick the explicit buttonType (e.g. Ghost) and the Criticality
    // formatter fallback will never be applied.
    const hasCriticality = dataField.Criticality !== undefined;
    const hasUiEmphasis = dataField.annotations?.UI?.Emphasized?.valueOf() === true;
    const isEmphasized = allowEmphasis && !hasCriticality && hasUiEmphasis;
    return {
      type: ActionType.DataFieldForAction,
      annotationPath: converterContext.getEntitySetBasedAnnotationPath(dataField.fullyQualifiedName),
      key: KeyHelper.generateKeyFromDataField(dataField),
      visible: compileExpression(isVisibleExp),
      visibleExpression: isVisibleExp,
      hasCriticality: dataField.Criticality !== undefined,
      ...
      buttonType: isEmphasized ? ButtonType.Emphasized : undefined,
      isPrimaryAction: isEmphasized ? true : undefined,
      priority: undefined
    };
  }

  /**
   * Checks whether an action group defines criticality.
   * This is used to suppress emphasis when any criticality action is visible.
   * @param dataField The data field or action group to inspect
   * @returns True if criticality is defined on one of the actions of the group
   */
  function hasCriticalityDefined(dataField) {
    return (dataField.Actions || []).some(action => action.Criticality !== undefined);
  }
  ...
  function computeActionsAndActionGroups(headerActions, prioritizeCriticality, headerDataFieldForActionsOrGroups, converterContext) {
    headerDataFieldForActionsOrGroups.filter(dataField => {
      ...
      if (dataField.$Type === "com.sap.vocabularies.UI.v1.DataFieldForAction") {
        const action = getDataFieldAnnotationAction(dataField, converterContext);
        ...
        headerActions.push({
          ...
          visible: compileExpression(isVisibleExp),
          visibleExpression: isVisibleExp,
          hasCriticality: hasCriticalityDefined(dataField),
          menu: dataField.Actions.map(action => getDataFieldAnnotationAction(action, converterContext, false)),
