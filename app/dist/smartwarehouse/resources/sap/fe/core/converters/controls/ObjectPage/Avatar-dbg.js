...
sap.ui.define(["sap/fe/base/BindingToolkit", "sap/m/AvatarShape"], function (BindingToolkit, AvatarShape) {
  ...
  const getLightBoxTitle = converterContext => {
    const headerInfo = converterContext.getEntityType().annotations?.UI?.HeaderInfo;
    if (!headerInfo?.Title) {
      return undefined;
    }
    // Use the title value from HeaderInfo
    return compileExpression(getExpressionFromAnnotation(headerInfo.Title.Value));
  };
  const getLightBoxSubtitle = converterContext => {
    const headerInfo = converterContext.getEntityType().annotations?.UI?.HeaderInfo;
    if (!headerInfo?.Description) {
      return undefined;
    }
    // Use the description value from HeaderInfo
    return compileExpression(getExpressionFromAnnotation(headerInfo.Description.Value));
  };
  const getAvatar = converterContext => {
    ...
    return {
      ...
      ...(manifestImageFitType !== undefined && {
        imageFitType: compileExpression(manifestImageFitType)
      })
    };
  };
