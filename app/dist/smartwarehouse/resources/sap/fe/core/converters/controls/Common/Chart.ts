...
import type { ChartManifestConfiguration, ManifestAction, OverflowGroupsConfiguration } from "../../ManifestSettings";
...
): Array<AnnotationAction | ActionGroup> {
	const chartActions: Array<AnnotationAction | ActionGroup> = [];
...
export function getChartActions(
	...
	let chartActions = insertCustomElements<BaseAction>(aAnnotationActions, manifestActions.actions, actionOverwriteConfig);

	// Add separators based on overflowGroups configuration from manifest
	const chartManifestConfig = converterContext.getManifestControlConfiguration<ChartManifestConfiguration>(visualizationPath);
	const overflowGroupsConfig = chartManifestConfig.overflowGroups;
	if (overflowGroupsConfig) {
		chartActions = addSeparators(chartActions, overflowGroupsConfig);
	}
