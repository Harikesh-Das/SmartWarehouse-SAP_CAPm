...
sap.ui.define(["sap/fe/base/BindingToolkit", "sap/fe/core/converters/ManifestSettings", "sap/fe/core/converters/annotations/DataField", "sap/fe/core/converters/helpers/Aggregation", "sap/fe/core/converters/helpers/ConfigurableObject", "sap/fe/core/converters/helpers/DataFieldHelper", "sap/fe/core/converters/helpers/IssueManager", "sap/fe/core/converters/helpers/Key", "sap/fe/core/helpers/BindingHelper", "sap/fe/core/helpers/ModelHelper", "sap/fe/core/helpers/StableIdHelper", "sap/fe/core/helpers/TypeGuards", "sap/fe/core/templating/DataModelPathHelper", "sap/fe/core/templating/DisplayModeFormatter", "sap/fe/core/templating/EntitySetHelper", "sap/fe/core/templating/FieldControlHelper", "sap/fe/core/templating/PropertyHelper", "sap/fe/core/templating/UIFormatters", "sap/fe/macros/formatters/TableFormatter"], function (BindingToolkit, ManifestSettings, DataField, Aggregation, ConfigurableObject, DataFieldHelper, IssueManager, Key, BindingHelper, ModelHelper, StableIdHelper, TypeGuards, DataModelPathHelper, DisplayModeFormatter, EntitySetHelper, FieldControlHelper, PropertyHelper, UIFormatters, tableFormatters) {
  ...
  var hasConnectedFieldsTarget = DataField.hasConnectedFieldsTarget;
  ...
  /**
   * Determines whether a data field is eligible for cell merging in responsive tables.
   * Columns with interactive content, such as links, navigation, actions cannot be merged,
   * nor can columns with a semantic object (which would produce a QuickView link).
   * @param dataField The data field annotation to check
   * @param semanticObjectPath The semantic object path for the column, if any
   * @returns True if the column can be merged.
   */
  _exports.ColumnType = ColumnType;
  function isMergeableColumn(dataField, semanticObjectPath) {
    if (semanticObjectPath !== undefined) {
      return false;
    }
    switch (dataField.$Type) {
      ...
      case "com.sap.vocabularies.UI.v1.DataFieldForAnnotation":
        return hasDataPointTarget(dataField);
      default:
        return false;
    }
  }
   ...
   * The columns are sorted. Some properties can be overwritten in the manifest.json file.
   * @param lineItemAnnotation Collection of data fields for representation in a table or list
   * @param tableType The type of the table
   * @param visualizationPath
   * @param converterContext
   * @returns Returns all table columns that should be available, regardless of templating, personalization, or their origin
   */
  function getTableColumns(lineItemAnnotation, tableType, visualizationPath, converterContext) {
    ...
    const manifestColumns = getColumnsFromManifest(converterContext.getManifestControlConfiguration(visualizationPath).columns ?? {}, annotationColumns, converterContext, converterContext.getAnnotationEntityType(lineItemAnnotation), tableType);
    const tableColumns = insertCustomElements(annotationColumns, manifestColumns, {
      aiNotice: OverrideType.overwrite,
      ...
      mergeCells: OverrideType.overwrite,
      mergeComparisonProperties: OverrideType.overwrite
    });
  ...
  function addTextArrangentInfoToProperty(property, propertyDataModelObjectPath, tableColumns, tableColumn) {
    const displayMode = getDisplayMode(property, propertyDataModelObjectPath.targetEntityType),
  ...
  const getColumnsFromEntityType = function (columnsToBeCreated, entityType, annotationColumns, converterContext, tableType, tableCreationMode, displayModeOfDescriptionPropertiesMap, restrictionsOnProperties) {
    ...
    entityType.entityProperties.forEach(property => {
      ...
      if (!property.targetType && !exists && !propertiesNotToBeConsidered.includes(property.name)) {
        ...
        const columnInfo = getColumnDefinitionFromProperty(property, converterContext.getEntitySetBasedAnnotationPath(property.fullyQualifiedName), property.name, true, true, aggregationHelper, converterContext, tableType, restrictionsOnProperties, tableCreationMode, relatedPropertiesInfo);
      ...
      if (getDisplayMode(property, entityType) === "Description") {
        ...
        if (isPathAnnotationExpression(property?.annotations?.Common?.Text)) {
          annotationColumns.push(getColumnDefinitionFromProperty(property, converterContext.getEntitySetBasedAnnotationPath(property.fullyQualifiedName), property.name, false, false, aggregationHelper, converterContext, tableType, restrictionsOnProperties, tableCreationMode));
        }
      }
    });
  ...
  const getColumnDefinitionFromProperty = function (property, fullPropertyPath, relativePath, useDataFieldPrefix, availableForAdaptation, aggregationHelper, converterContext, tableType, restrictionsOnProperties, tableCreationMode, relatedPropertiesInfo, relativePathForMultiValue) {
    ...
    const sortable = !isHidden && !restrictionsOnProperties?.nonSortableProperties.includes(relativePath);
    const isAnalyticalTable = aggregationHelper.isAnalyticsSupported() && tableType === "AnalyticalTable";
    const filterable = !restrictionsOnProperties?.nonFilterableProperties.includes(relativePath);
    // groupable properties on an analytical table should consider hidden properties and text only annotations as other table types do
    const groupable = isAnalyticalTable ? !!aggregationHelper.isPropertyGroupable(property) && !isHidden : sortable;
    const typeConfig = {
    ...
    // Add precision for DateTimeOffset properties to support filter transformation to milliseconds
    const possibleTypes = [dataType, property.type].filter(Boolean); // Remove any undefined/null values
    const isDateTimeOffsetProperty = possibleTypes.some(type => typeof type === "string" && type.includes("DateTimeOffset"));
    const _precision = property.precision ? property.precision : propertyTypeConfig.constraints?.precision;
    if (isDateTimeOffsetProperty && _precision) {
      if (!typeConfig.constraints) {
        typeConfig.constraints = {};
      }
      typeConfig.constraints.precision = _precision;
    }
    ...
    if (isAnalyticalTable) {
      setTechnicalExtension(column, property, aggregationHelper, converterContext, relativePath);
    }
    return column;
  };

  /**
   * Sets the technical extension for analytics scenarios.
   * @param column The table column
   * @param property The property linked to the column
   * @param aggregationHelper The aggregation helper
   * @param converterContext The converter context
   * @param relativePath The relative path of the property
   */
  const setTechnicalExtension = function (column, property, aggregationHelper, converterContext, relativePath) {
    const isDraftSupported = ModelHelper.isObjectPathDraftSupported(converterContext.getDataModelObjectPath());
    const draftProperties = ["HasActiveEntity", "HasDraftEntity", "IsActiveEntity"];
    const isDraftProperty = draftProperties.includes(relativePath);
    const isGroupableProperty = !!aggregationHelper.isPropertyGroupable(property);
    if (isDraftSupported && isDraftProperty ||
    // column.isGroupable can be false when the property is hidden but it could be technically groupable
    isGroupableProperty && !column.isGroupable) {
      column.extension = {
  ...
  const createColumnExportSettings = function (column, relatedPropertiesInfo) {
    ...
    const isConnectedFields = isAnnotationOfType(column, "com.sap.vocabularies.UI.v1.DataFieldForAnnotation") && isAnnotationOfTerm(column.Target.$target, "com.sap.vocabularies.UI.v1.ConnectedFields");
    const dataType = isConnectedFields ? "String" : getDataFieldDataType(column);
  ...
  /**
   * Returns hidden binding expressions for a field group or connected fields.
   * @param dataField DataField defined in the annotations
   * @returns Compile binding of field group/connected fields expressions.
   */
  const _getHiddenExpressions = function (dataField) {
    const collectionFieldsHiddenExpression = [];
    if (isAnnotationOfType(dataField, "com.sap.vocabularies.UI.v1.DataFieldForAnnotation") && (isAnnotationOfTerm(dataField.Target.$target, "com.sap.vocabularies.UI.v1.ConnectedFields") || isAnnotationOfTerm(dataField.Target.$target, "com.sap.vocabularies.UI.v1.FieldGroup"))) {
      if (dataField?.annotations?.UI?.Hidden) {
        return compileExpression(not(equal(getExpressionFromAnnotation(dataField.annotations.UI.Hidden), true)));
      } else {
        const collectionData = hasFieldGroupTarget(dataField) ? dataField.Target.$target.Data : getConnectedFieldsData(dataField.Target.$target);
        collectionData?.forEach(innerDataField => {
          collectionFieldsHiddenExpression.push(_getVisibleExpression({
            targetObject: innerDataField
          }));
        });
        return compileExpression(ifElse(or(...collectionFieldsHiddenExpression), constant(true), constant(false)));
      }
    } else {
      return undefined;
    }
  ...
  const getLabel = function (property) {
    ...
    if (isProperty(property) || isNavigationProperty(property)) {
      ...
      if (property.Label?.toString()) {
        return property.Label?.toString();
      }
      const target = property.Target.$target;
      switch (target?.$Type) {
        // if it is a DataFieldForAnnotation pointing to a DataPoint we look at the dataPoint's label
        case "com.sap.vocabularies.UI.v1.DataPointType":
          if (target?.Value?.$target) {
            return compileExpression(getExpressionFromAnnotation(target.Value?.$target?.annotations?.Common?.Label?.valueOf()));
          }
          break;
        // if it is a DataFieldForAnnotation pointing to a FieldGroup or a ConnectedFields we look at the dataFieldAbstract's label
        case "com.sap.vocabularies.UI.v1.FieldGroupType":
        case "com.sap.vocabularies.UI.v1.ConnectedFieldsType":
  ...
  const _createRelatedColumns = function (columnsToBeCreated, existingColumns, converterContext, entityType, displayModeOfDescriptionPropertiesMap, tableType, tableCreationMode, restrictionsOnProperties) {
    ...
    Object.keys(columnsToBeCreated).forEach(name => {
      ...
      if (isMultiValueField(propertyObjectPath)) {
        ...
        const column = getColumnDefinitionFromProperty(property, annotationPath, name, true, false, aggregationHelper, converterContext, tableType, restrictionsOnProperties, tableCreationMode, undefined, newNameRelativeTargetPath);
        ...
        const column = getColumnDefinitionFromProperty(property, annotationPath, name, true, false, aggregationHelper, converterContext, tableType, restrictionsOnProperties, tableCreationMode);
        computeHiddenOnRelatedColumns(existingColumns, name, column);
        relatedColumns.push(column);
        ...
        if (!existingColumns.some(column => column.name === newName)) {
          ...
          const column = getColumnDefinitionFromProperty(property, annotationPath, name, false, false, aggregationHelper, converterContext, tableType, restrictionsOnProperties, tableCreationMode);
          computeHiddenOnRelatedColumns(existingColumns, name, column);
          relatedColumns.push(column);
  ...
  function getImportance(converterContext, dataField) {
    ...
    if (isAnnotationOfType(dataField, "com.sap.vocabularies.UI.v1.DataFieldForAnnotation")) {
      ...
      if (isAnnotationOfType(dataFieldTarget, "com.sap.vocabularies.UI.v1.FieldGroupType")) {
        ...
        const hasHighKey = fieldGroupData.some(fieldGroupDataField => {
      ...
      if (isAnnotationOfType(dataFieldTarget, "com.sap.vocabularies.UI.v1.ConnectedFieldsType")) {
        const connectedFieldsData = getConnectedFieldsData(dataFieldTarget);
        // If the connectedFields contains a required property, importance set to High
        const requiredKeys = requiredProperties.map(p => p.$target?.fullyQualifiedName);
        const hasRequiredHighKey = connectedFieldsData.some(connectedDataField => {
          return isDataFieldTypes(connectedDataField) && requiredKeys.includes(connectedDataField.Value?.$target?.fullyQualifiedName);
        });
        if (hasRequiredHighKey) {
          return Importance.High;
        }
        //If the DataFieldForAnnotation has an Importance we take it
        if (dataField?.annotations?.UI?.Importance) {
  ...
  /**
   * Gets the disableStrictHandling setting for an action from manifest configuration.
   * @param dataField The DataFieldForAction
   * @param key The action key
   * @param tableManifestSettings The table manifest configuration
   * @returns DisableStrictHandling setting or undefined
   */
  _exports.getImportance = getImportance;
  function getEnableStrictHandlingFromManifest(dataField, key, tableManifestSettings) {
    if (dataField.$Type === "com.sap.vocabularies.UI.v1.DataFieldForAction") {
      return tableManifestSettings.actions?.[key]?.disableStrictHandling;
    }
    return undefined;
  }
  ...
  const getColumnsFromAnnotations = function (lineItemAnnotation, tableType, visualizationPath, converterContext) {
    ...
    if (lineItemAnnotation) {
      ...
      lineItemAnnotation.forEach(dataField => {
        ...
        const collectionFieldsHiddenExpression = _getHiddenExpressions(dataField);
        ...
        const columnKey = KeyHelper.generateKeyFromDataField(dataField);
        const column = {
          key: columnKey,
          //KeyHelper.generateKeyFromDataField(dataField),
          type: ColumnType.Annotation,
          label: label ?? name,
          groupLabel: isGroup ? getLabel(dataField) : undefined,
          group: isGroup ? groupPath : undefined,
          collectionFieldsHiddenExpression: collectionFieldsHiddenExpression,
        ...
        if (isMergeableColumn(dataField, semanticObjectAnnotationPath) && dataField.annotations?.HTML5?.RowSpanForDuplicateValues !== undefined) {
          column.mergeCells = true;
        }
        const strictHandlingSettingForColumn = getEnableStrictHandlingFromManifest(dataField, columnKey, tableManifestSettings);
        if (strictHandlingSettingForColumn !== undefined) {
          column.disableStrictHandling = strictHandlingSettingForColumn;
        }
  ...
  /**
   * Determines if the dataField has to be flagged as required.
   * @param converterContext The converter context
   * @param dataField The dataField being processed
   * @param tableCreationMode The creation mode of the underlying table
   * @returns True if the dataField is required.
   */
  const isRequired = function (converterContext, dataField, tableCreationMode) {
    let dataFieldData;
    if (isAnnotationOfTerm(dataField, "com.sap.vocabularies.UI.v1.ConnectedFields")) {
      dataFieldData = getConnectedFieldsData(dataField);
    } else {
      dataFieldData = dataField.Data;
    }
    return dataFieldData.some(function (item) {
  ...
  const isRequiredDataFieldForAnnotation = function (converterContext, dataFieldForAnnotation, tableCreationMode) {
    ...
    // Check if the DataFieldForAnnotation points to a FieldGroup or a connectedFields
    if (hasFieldGroupTarget(dataFieldForAnnotation) || hasConnectedFieldsTarget(dataFieldForAnnotation)) {
      if (isRequired(converterContext, dataFieldTarget, tableCreationMode)) {
        return true;
      }
      const collectionData = hasFieldGroupTarget(dataFieldForAnnotation) ? dataFieldTarget.Data : getConnectedFieldsData(dataFieldTarget);
      return collectionData.some(innerDataField => {
   ...
   */
  const getColumnsFromManifest = function (columns, annotationColumns, converterContext, entityType, tableType) {
      ...
      propertyInfos.forEach(property => {
        annotationTableColumns.forEach(prop => {
          if (prop.name === property) {
            const entityPropertyName = property.replace("Property::", "");
            prop.sortable = !restrictionsOnProperties.nonSortableProperties.includes(entityPropertyName);
            prop.isGroupable = aggregationHelper.isAnalyticsSupported() && tableType === "AnalyticalTable" ? !!aggregationHelper.isPropertyNameGroupable(entityPropertyName) : prop.sortable;
            prop.filterable = !restrictionsOnProperties.nonFilterableProperties.includes(entityPropertyName);
    ...
    for (const key in columns) {
      ...
      const baseTableColumn = {
        ...
        aiNotice: manifestColumn.aiNotice,
        position: {
      ...
      if (isColumnOverride(manifestColumn, key)) {
        const rawMergeProps = manifestColumn.settings?.mergeComparisonProperties;
        let mergeComparisonProperties;
        if (rawMergeProps !== undefined) {
          mergeComparisonProperties = Array.isArray(rawMergeProps) ? rawMergeProps : [rawMergeProps];
        }
        let mergeCells = manifestColumn.settings?.mergeCells;
        // When mergeCells is set in the manifest without mergeComparisonProperties, guard against unsupported
        // column types (links, actions, QuickView) using the same check as the annotation-driven path.
        // mergeComparisonProperties is an explicit opt-in for QuickView columns and bypasses the check.
        if (mergeCells === true && !mergeComparisonProperties?.length) {
          const annotationColumn = annotationColumns.find(col => col.key === key);
          if (annotationColumn?.annotationPath) {
            const dataField = converterContext.getConverterContextFor(annotationColumn.annotationPath).getDataModelObjectPath().targetObject;
            if (!isMergeableColumn(dataField, annotationColumn.semanticObjectPath)) {
        ...
        if (manifestColumn.mergeCells !== undefined || manifestColumn.mergeComparisonProperties !== undefined) {
          converterContext.getDiagnostics().addIssue(IssueCategory.Manifest, IssueSeverity.Medium, `The custom column '${key}' uses 'mergeCells' or 'mergeComparisonProperties', which are not supported on custom columns. These settings are only applicable to annotation-based column overrides.`, IssueCategoryType, IssueCategoryType?.AnnotationColumns?.InvalidKey);
        }
        if (isSlotColumn(manifestColumn)) {
          ...
          const tableObjectPath = converterContext.getDataModelObjectPath();
          // Calculate the properties that are not used in a MVF (i.e. accessed through a 1:N navigation)
          // These properties will be loaded as custom data with the column in a ListReport or ObjectPage, therefore properties from 1:N nav
          // must be excluded (they can't be loaded using custom data as the path is incorrect, i.e. /MyMainObject(id=1)/myNavToN/myProperty --> the keys
          // are missing for myNavToN)
          const nonMultiValueFields = baseManifestColumn.properties?.filter(propertyPath => {
            const propertyObjectPath = enhanceDataModelPath(tableObjectPath, propertyPath);
            return !isMultiValueField(propertyObjectPath);
          });
          const customTableColumn = {
            ...baseManifestColumn,
            type: ColumnType.Default,
            loadedProperties: nonMultiValueFields
          };
          internalColumns[key] = customTableColumn;
        } else {
  ...
  function createPropertyAnnotatedTextOnly(converterContext, propertyDataModelObjectPath, lineItem, columnsToBeCreated, relativePath, restrictionsOnProperties) {
    ...
    if (isPropertyNavigated && isDataField(lineItem)) {
      ...
      lineItem.Target.$target.Data.forEach(field => {
        if (isDataField(field) && getDisplayMode(field.Value.$target, propertyDataModelObjectPath.targetEntityType) === "Description") {
  ...
  function getTypeConfig(property, dataType) {
    ...
    if (propertyTypeConfig.type !== "sap.ui.model.odata.type.Stream") {
      ...
      if (propertyTypeConfig.type === "sap.ui.model.odata.type.String") {
        formatOptions = {
          parseKeepsEmptyString: propertyTypeConfig.constraints?.nullable === false
        };
      }
    }
  ...
  function getCustomExportSettings(exportSettings, annotationColumns, converterContext, entityType, propertyInfos, columnKey) {
    ...
    if (propertyInfos?.length === 1) {
      columnFromAnnotations = annotationColumns.find(annotationColumn => annotationColumn.key === propertyInfos[0]);
    }
    ...
    const manifestOverrideExportSettings = {
      ...
      inputFormat: exportSettings?.inputFormat
    };
    // we make sure that manifest overrides takes over only if something is defined
    const customExportSettings = {
      ...columnFromAnnotations?.exportSettings,
      ...removeUndefinedFromExportSettings(manifestOverrideExportSettings)
    };
