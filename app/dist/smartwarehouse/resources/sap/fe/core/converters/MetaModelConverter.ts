...
import { constant, formatResult, getExpressionFromAnnotation, setConverterRootSymbol } from "sap/fe/base/BindingToolkit";
...
function parsePropertyValue(
	...
	if (annotationObject === null) {
		...
		if (annotationObject.length > 0) {
			collectionValue.Collection.type = identifyCollectionType(annotationObject[0]);
		}
		value = collectionValue;
	} else if (typeof annotationObject === "object") {
		const targetValue = handleDynamicAnnotationValue(annotationObject);
		if (!targetValue) {
			value = {
				type: "Record",
...
function parseAnnotationObject(
	...
	if (annotationObject === null) {
		...
		if (annotationObject.length > 0) {
			parsedAnnotationCollection.collection.type = identifyCollectionType(annotationObject[0]);
		}
	} else if (typeof annotationObject === "object") {
		const value: Expression | undefined = handleDynamicAnnotationValue(annotationObject);
		if (!value) {
...
function handleDynamicAnnotationValue(annotationObjectToParse: MetaModelAnnotationRecord): Expression | undefined {
	let value: Expression | undefined;
	if (annotationObjectToParse.$PropertyPath !== undefined) {
		value = { type: "PropertyPath", PropertyPath: annotationObjectToParse.$PropertyPath };
	} else if (annotationObjectToParse.$NavigationPropertyPath !== undefined) {
		value = { type: "NavigationPropertyPath", NavigationPropertyPath: annotationObjectToParse.$NavigationPropertyPath };
	} else if (annotationObjectToParse.$If !== undefined) {
		value = { type: "If", $If: annotationObjectToParse.$If as [Expression, Expression, Expression] };
	} else if (annotationObjectToParse.$And !== undefined) {
		value = { type: "And", $And: annotationObjectToParse.$And as [Expression, Expression] };
	} else if (annotationObjectToParse.$Or !== undefined) {
		value = { type: "Or", $Or: annotationObjectToParse.$Or as [Expression, Expression] };
	} else if (annotationObjectToParse.$Not !== undefined) {
		value = { type: "Not", $Not: annotationObjectToParse.$Not as Expression };
	} else if (annotationObjectToParse.$Eq !== undefined) {
		value = { type: "Eq", $Eq: annotationObjectToParse.$Eq as [Expression, Expression] };
	} else if (annotationObjectToParse.$Ne !== undefined) {
		value = { type: "Ne", $Ne: annotationObjectToParse.$Ne as [Expression, Expression] };
	} else if (annotationObjectToParse.$Gt !== undefined) {
		value = { type: "Gt", $Gt: annotationObjectToParse.$Gt as [Expression, Expression] };
	} else if (annotationObjectToParse.$Ge !== undefined) {
		value = { type: "Ge", $Ge: annotationObjectToParse.$Ge as [Expression, Expression] };
	} else if (annotationObjectToParse.$Lt !== undefined) {
		value = { type: "Lt", $Lt: annotationObjectToParse.$Lt as [Expression, Expression] };
	} else if (annotationObjectToParse.$Le !== undefined) {
		value = { type: "Le", $Le: annotationObjectToParse.$Le as [Expression, Expression] };
	} else if (annotationObjectToParse.$Add !== undefined) {
		value = { type: "Add", $Add: annotationObjectToParse.$Add as [Expression, Expression] };
	} else if (annotationObjectToParse.$Sub !== undefined) {
		value = { type: "Sub", $Sub: annotationObjectToParse.$Sub as [Expression, Expression] };
	} else if (annotationObjectToParse.$Mul !== undefined) {
		value = { type: "Mul", $Mul: annotationObjectToParse.$Mul as [Expression, Expression] };
	} else if (annotationObjectToParse.$Div !== undefined) {
		value = { type: "Div", $Div: annotationObjectToParse.$Div as [Expression, Expression] };
	} else if (annotationObjectToParse.$DivBy !== undefined) {
		value = { type: "DivBy", $DivBy: annotationObjectToParse.$DivBy as [Expression, Expression] };
	} else if (annotationObjectToParse.$Mod !== undefined) {
		value = { type: "Mod", $Mod: annotationObjectToParse.$Mod as [Expression, Expression] };
	} else if (annotationObjectToParse.$Neg !== undefined) {
		value = { type: "Neg", $Neg: annotationObjectToParse.$Neg as Expression };
	} else if (annotationObjectToParse.$In !== undefined) {
		value = { type: "In", $In: annotationObjectToParse.$In as [Expression, Expression] };
	} else if (annotationObjectToParse.$Has !== undefined) {
		value = { type: "Has", $Has: annotationObjectToParse.$Has as [Expression, Expression] };
		...
		value = { type: "Path", Path: annotationObjectToParse.$Path };
		...
		value = { type: "Decimal", Decimal: parseFloat(annotationObjectToParse.$Decimal) };
...
type ExpressionType =
	...
	| "Mod"
	| "Neg";
function identifyCollectionType(annotationObjectToParse: MetaModelAnnotationRecord): ExpressionType {
	let collectionType: ExpressionType;
	if (annotationObjectToParse.hasOwnProperty("$PropertyPath")) {
		collectionType = "PropertyPath";
	} else if (annotationObjectToParse.hasOwnProperty("$Path")) {
		collectionType = "Path";
	} else if (annotationObjectToParse.hasOwnProperty("$NavigationPropertyPath")) {
		collectionType = "NavigationPropertyPath";
	} else if (annotationObjectToParse.hasOwnProperty("$AnnotationPath")) {
		collectionType = "AnnotationPath";
	} else if (annotationObjectToParse.hasOwnProperty("$Type")) {
		collectionType = "Record";
	} else if (annotationObjectToParse.hasOwnProperty("$If")) {
		collectionType = "If";
	} else if (annotationObjectToParse.hasOwnProperty("$Or")) {
		collectionType = "Or";
	} else if (annotationObjectToParse.hasOwnProperty("$Eq")) {
		collectionType = "Eq";
	} else if (annotationObjectToParse.hasOwnProperty("$Ne")) {
		collectionType = "Ne";
	} else if (annotationObjectToParse.hasOwnProperty("$Not")) {
		collectionType = "Not";
	} else if (annotationObjectToParse.hasOwnProperty("$Gt")) {
		collectionType = "Gt";
	} else if (annotationObjectToParse.hasOwnProperty("$Ge")) {
		collectionType = "Ge";
	} else if (annotationObjectToParse.hasOwnProperty("$Lt")) {
		collectionType = "Lt";
	} else if (annotationObjectToParse.hasOwnProperty("$Le")) {
		collectionType = "Le";
	} else if (annotationObjectToParse.hasOwnProperty("$And")) {
		collectionType = "And";
	} else if (annotationObjectToParse.hasOwnProperty("$In")) {
		collectionType = "In";
	} else if (annotationObjectToParse.hasOwnProperty("$Has")) {
		collectionType = "Has";
	} else if (annotationObjectToParse.hasOwnProperty("$Add")) {
		collectionType = "Add";
	} else if (annotationObjectToParse.hasOwnProperty("$Sub")) {
		collectionType = "Sub";
	} else if (annotationObjectToParse.hasOwnProperty("$Mul")) {
		collectionType = "Mul";
	} else if (annotationObjectToParse.hasOwnProperty("$Div")) {
		collectionType = "Div";
	} else if (annotationObjectToParse.hasOwnProperty("$DivBy")) {
		collectionType = "DivBy";
	} else if (annotationObjectToParse.hasOwnProperty("$Mod")) {
		collectionType = "Mod";
	} else if (annotationObjectToParse.hasOwnProperty("$Neg")) {
		collectionType = "Neg";
	} else if (annotationObjectToParse.hasOwnProperty("$Apply")) {
	...
	for (const annotationKey in annotationObjects) {
		...
		if (annotationObjectToParse === null) {
			...
			if (annotationObjectToParse.length > 0) {
				parsedAnnotationObject.collection.type = identifyCollectionType(annotationObjectToParse[0]);
			}
		} else if (typeof annotationObjectToParse === "object") {
			const targetValue = handleDynamicAnnotationValue(annotationObjectToParse);
			if (!targetValue) {
				...
				parsedAnnotationObject.value = targetValue;
			}
		}
...
function prepareEnumTypes(enumTypeDefinition: MetaModelEnumType, enumTypeName: string, namespacePrefix: string): RawEnumType {
	const enumTypeObject: RawEnumType = {
		_type: "EnumType",
		name: enumTypeName.substring(namespacePrefix.length),
		fullyQualifiedName: enumTypeName,
		members: [],
		isFlags: enumTypeDefinition.$IsFlags === true,
		underlyingType: enumTypeDefinition.$UnderlyingType ?? "Edm.Int32"
	};

	Object.keys(enumTypeDefinition)
		.filter((propertyNameOrNot) => {
			return (
				propertyNameOrNot != "$Key" &&
				propertyNameOrNot != "$kind" &&
				propertyNameOrNot != "$IsFlags" &&
				propertyNameOrNot != "$UnderlyingType"
			);
		})
		.sort((a, b) => (a > b ? 1 : -1))
		.map((propertyName) => {
			enumTypeObject.members.push({
				_type: "EnumMember",
				fullyQualifiedName: enumTypeName.substring(namespacePrefix.length) + "/" + propertyName,
				name: propertyName,
				value: enumTypeDefinition[propertyName]
			});
		});

	return enumTypeObject;
}
...
function prepareAction(actionRawData: MetaModelAction, actionName: string, namespacePrefix: string): RawAction {
	...
	return {
		...
		parameters: parameters.map((param, idx) => ({
			...
			name: param.$Name,
			type: param.$Type,
			// The first action parameter of a bound action is the binding parameter and thus not nullable
			nullable: param.$Nullable ?? (idx === 0 && actionRawData.$kind === "Action" && actionRawData.$IsBound === true ? false : true),
...
function parseSchema(metaModelData: MetaModelContent, ignoreDatafieldDefault: boolean): RawSchema {
	...
	const parseMetaModelElement = (name: string, value: MetaModelObject): void => {
		switch (value.$kind) {
			...
			case "EnumType":
				schema.enumTypes.push(prepareEnumTypes(value, name, namespacePrefix));
				break;
...
export function convertTypes(oMetaModel: ODataMetaModel, oCapabilities?: EnvironmentCapabilities): ConvertedMetadata {
	...
	if (!convertedMetaModelMap.hasOwnProperty(metaModelID)) {
		...
		try {
			...
			setConverterRootSymbol(AnnotationConverter.CONVERTER_ROOT);
...
export function registerVirtualProperty(
	...
	return "@$ui5.fe.virtual." + propertyName;
}
...
export function deleteModelCacheData(oMetaModel: ODataMetaModel | undefined): void {
	if (oMetaModel) {
...
export function getInvolvedDataModelObjectFromPath<T>(
	...
	const dataModelObjects: ServiceObjectAndAnnotation[] = metaModelContext.visitedObjects.filter(
