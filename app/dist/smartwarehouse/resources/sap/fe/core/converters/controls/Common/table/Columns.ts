...
import { getConnectedFieldsData, isReferencePropertyStaticallyHidden } from "sap/fe/core/converters/helpers/DataFieldHelper";
...
export type CustomBasedTableColumn = Omit<BaseTableColumn, "tooltip"> & {
	id: string;
	name: string;
...
export type AnnotationTableColumn = PropertyInfo &
	AnnotationTableColumnForOverride & {
		...
		groupLabel?: string;
		group?: string;
		collectionFieldsHiddenExpression?: CompiledBindingToolkitExpression;
 ...
 */
function isMergeableColumn(dataField: DataFieldAbstractTypes, semanticObjectPath: string | undefined): boolean {
...
export const getColumnsFromEntityType = function (
	...
	if (tableType === "AnalyticalTable" && aggregationHelper.isAnalyticsSupported() && groupableProperties) {
		propertiesNotToBeConsidered = groupableProperties
			...
			.map((target) => getAssociatedTextProperty(target))
 ...
 */
const setTechnicalExtension = function (
	...
	if (
		(isDraftSupported && isDraftProperty) ||
...
const createColumnExportSettings = function (
	...
	const isConnectedFields =
		isAnnotationOfType<DataFieldForAnnotation>(column, UIAnnotationTypes.DataFieldForAnnotation) &&
		isAnnotationOfTerm<ConnectedFields>(column.Target.$target, UIAnnotationTerms.ConnectedFields);
 ...
 */
const _getHiddenExpressions = function (dataField: DataFieldAbstractTypes): CompiledBindingToolkitExpression | undefined {
	const collectionFieldsHiddenExpression: BindingToolkitExpression<boolean>[] = [];
	...
	if (
		...
		if (dataField?.annotations?.UI?.Hidden) {
			...
			const collectionData = hasFieldGroupTarget(dataField)
				? (dataField.Target.$target as FieldGroup).Data
				: getConnectedFieldsData(dataField.Target.$target as ConnectedFields);
			collectionData?.forEach((innerDataField: DataFieldAbstractTypes | DataPointTypeTypes) => {
				collectionFieldsHiddenExpression.push(
...
const getLabel = function (property: DataFieldAbstractTypes | Property | NavigationProperty, isGroup = false): string | undefined {
	...
	if (isProperty(property) || isNavigationProperty(property)) {
		...
		switch (target?.$Type) {
			...
			case UIAnnotationTypes.ConnectedFieldsType:
...
export function getImportance(
	...
	if (isAnnotationOfType<DataFieldForAnnotation>(dataField, UIAnnotationTypes.DataFieldForAnnotation)) {
		...
		if (isAnnotationOfType<FieldGroup>(dataFieldTarget, UIAnnotationTypes.FieldGroupType)) {
			...
			const hasHighKey = fieldGroupData.some((fieldGroupDataField: DataFieldAbstractTypes): boolean => {
		...
		if (isAnnotationOfType<ConnectedFields>(dataFieldTarget, UIAnnotationTypes.ConnectedFieldsType)) {
			...
			const hasRequiredHighKey = connectedFieldsData.some((connectedDataField: DataFieldAbstractTypes): boolean => {
 ...
 */
function getEnableStrictHandlingFromManifest(
	dataField: DataFieldAbstractTypes,
	key: string,
...
const getColumnsFromAnnotations = function (
	...
	if (lineItemAnnotation) {
		...
		lineItemAnnotation.forEach((dataField) => {
			...
			const column: AnnotationTableColumn = {
				key: columnKey, //KeyHelper.generateKeyFromDataField(dataField),
				type: ColumnType.Annotation,
				label: label ?? name,
			...
			if (
				isMergeableColumn(dataField, semanticObjectAnnotationPath) &&
				dataField.annotations?.HTML5?.RowSpanForDuplicateValues !== undefined
			) {
...
const isRequired = function (
	converterContext: ConverterContext<PageContextPathTarget>,
	dataField: FieldGroup | ConnectedFields,
	tableCreationMode: CreationMode
): boolean {
	let dataFieldData: DataFieldAbstractTypes[];
	if (isAnnotationOfTerm<ConnectedFields>(dataField, UIAnnotationTerms.ConnectedFields)) {
...
const isRequiredDataFieldForAnnotation = function (
	...
	if (hasFieldGroupTarget(dataFieldForAnnotation) || hasConnectedFieldsTarget(dataFieldForAnnotation)) {
		if (isRequired(converterContext, dataFieldTarget as FieldGroup, tableCreationMode)) {
			return true;
		}
		const collectionData = hasFieldGroupTarget(dataFieldForAnnotation)
			? (dataFieldTarget as FieldGroup).Data
			: getConnectedFieldsData(dataFieldTarget as ConnectedFields);
		return collectionData.some((innerDataField: DataFieldAbstractTypes) => {
...
const getColumnsFromManifest = function (
				...
				if (prop.name === property) {
					...
					prop.isGroupable =
						aggregationHelper.isAnalyticsSupported() && tableType === "AnalyticalTable"
							? !!aggregationHelper.isPropertyNameGroupable(entityPropertyName)
							: prop.sortable;
	...
	for (const key in columns) {
		...
		if (isColumnOverride(manifestColumn, key)) {
			...
			let mergeComparisonProperties: string[] | undefined;
			...
			if (mergeCells === true && !mergeComparisonProperties?.length) {
				...
				if (annotationColumn?.annotationPath) {
					const dataField = converterContext.getConverterContextFor(annotationColumn.annotationPath).getDataModelObjectPath()
						.targetObject as DataFieldAbstractTypes;
			...
			if (
				(manifestColumn as CustomDefinedTableColumnForOverride).mergeCells !== undefined ||
				(manifestColumn as CustomDefinedTableColumnForOverride).mergeComparisonProperties !== undefined
			) {
				converterContext
					.getDiagnostics()
					.addIssue(
						...
						`The custom column '${key}' uses 'mergeCells' or 'mergeComparisonProperties', which are not supported on custom columns. These settings are only applicable to annotation-based column overrides.`,
						IssueCategoryType,
...
export function getTypeConfig(property: Property | DataFieldAbstractTypes | undefined, dataType?: string): PropertyTypeConfig {
	...
	if (propertyTypeConfig.type !== "sap.ui.model.odata.type.Stream") {
		...
		if (propertyTypeConfig.type === "sap.ui.model.odata.type.String") {
			formatOptions = { parseKeepsEmptyString: propertyTypeConfig.constraints?.nullable === false };
		}
	}
...
function getCustomExportSettings(
	...
	const manifestOverrideExportSettings: ColumnExportSettings = {
