...
export function getEditButtonVisibility(
	...
	const updateRestrictionsAlwaysFalse: boolean = updateRestrictions !== undefined && updateRestrictions.valueOf() === false;
	...
	const stickySessionSupported = ModelHelper.getStickySession(entitySet), //for sticky app
		...
		conditionSave = Boolean(
			entitySetDraftRoot === CommonAnnotationTerms.DraftRoot ||
				((capabilities?.HiddenDraft as HiddenDraft)?.enabled && isDraftSupported) ||
 ...
 */
function hasCriticalityDefined(dataField: DataFieldForActionGroupTypes): boolean {
			...
			if (dataField.$Type === UIAnnotationTypes.DataFieldForAction) {
				...
				headerActions.push({
					...
					menu: dataField.Actions.map((action) =>
						getDataFieldAnnotationAction(action as DataFieldForActionTypes, converterContext, false)
					),
