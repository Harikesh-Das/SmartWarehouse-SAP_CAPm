...
export class AggregationHelper {
	...
	/**
	 * Checks if a property with a given name is groupable.
	 * @param propertyName The name of the property to check
	 * @returns `undefined` if the entity doesn't support analytical queries, true or false otherwise
	 */
	public isPropertyNameGroupable(propertyName: string): boolean | undefined {
		const property = this._getEntityProperties()?.find((prop) => prop.name === propertyName);
		return property ? this.isPropertyGroupable(property) : undefined;
	}
	 ...
	 * This Term indicates if the AggregatedProperty or the deprecated AggregatedProperties is retrieved.
	...
	public getAllowedTransformations(): String[] | undefined {
		return (
			this.oTargetAggregationAnnotations?.ApplySupported?.Transformations ??
			this.oContainerAggregationAnnotation?.ApplySupportedDefaults?.Transformations
		);
	}
}
