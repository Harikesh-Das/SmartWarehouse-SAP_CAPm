...
sap.ui.define(["sap/base/Log", "../../helpers/TypeGuards"], function (Log, TypeGuards) {
  ...
  function isAnnotationFieldStaticallyHidden(annotationProperty) {
    ...
    switch (annotationProperty.Target.$target?.term) {
      ...
      case "com.sap.vocabularies.UI.v1.ConnectedFields":
        const connectedFieldsData = getConnectedFieldsData(annotationProperty.Target.$target);
        return connectedFieldsData.every(isReferencePropertyStaticallyHidden);
  ...
  /**
   *	Collect the dataFields within the ConnectedFields.
   * @param connectedFields The ConnectedFields to be analyzed
   * @returns An array of DataFieldAbstractTypes
   */
  _exports.isAnnotationFieldStaticallyHidden = isAnnotationFieldStaticallyHidden;
  function getConnectedFieldsData(connectedFields) {
    return Object.values(connectedFields.Data).filter(connectedField => connectedField?.hasOwnProperty("Value"));
  }

  /**
   *	Get the connectedFields separator from its template.
   * @param connectedFields The ConnectedFields to be analyzed
   * @returns The separator
   */
  _exports.getConnectedFieldsData = getConnectedFieldsData;
  function getConnectedFieldsSeparator(connectedFields) {
    const match = connectedFields.Template.match(/\{[^}]+\}(.*)\{[^}]+\}/);
    return match ? match[1] : undefined;
  }

  /**
   * Checks if header is statically hidden.
   * @param propertyDataModel The property Data Model to be analyzed
   * @returns Whether the argument has been set as hidden
   */
  _exports.getConnectedFieldsSeparator = getConnectedFieldsSeparator;
