...
sap.ui.define(["sap/fe/core/converters/controls/Common/DataVisualization", "sap/fe/core/converters/controls/Common/Table", "sap/fe/core/converters/controls/Common/table/Columns", "sap/fe/core/converters/controls/ListReport/FilterField", "sap/fe/core/converters/controls/ListReport/VisualFilters", "sap/fe/core/converters/helpers/ConfigurableObject", "sap/fe/core/converters/helpers/IssueManager", "sap/fe/core/converters/helpers/Key", "sap/fe/core/helpers/ModelHelper", "sap/fe/core/helpers/TypeGuards", "sap/fe/core/templating/DataModelPathHelper", "sap/fe/core/templating/PropertyHelper", "sap/m/library", "../Common/filter/FilterRestrictions"], function (DataVisualization, Table, Columns, FilterField, VisualFilters, ConfigurableObject, IssueManager, Key, ModelHelper, TypeGuards, DataModelPathHelper, PropertyHelper, library, fetchFilterRestrictions) {
  ...
  const getFilterBarHideBasicSearch = function (listReportTables, charts, converterContext) {
    ...
    // Check if tables support search
    const noSearchInTables = listReportTables.length === 0 || listReportTables.every(table => table.annotation.searchable === false);
  ...
  const getManifestFilterFields = function (entityType, converterContext, annotationPath) {
    ...
    const filterConfigurationPath = annotationPath ? getFilterConfigurationPath(annotationPath, converterContext.getContextPath(), settingsContextPath) : undefined;
    let fbConfig = {};
  ...
  /**
   * Returns configuration path to fetch the FilterBar configurations from manifest.
   ...
   * @returns Path to fetch the FilterBar configurations from the manifest
   */
  _exports.getManifestFilterFields = getManifestFilterFields;
  const getFilterConfigurationPath = function (relAnnoPath, filterBarContextPath, pageContextPath) {
    const metaPath = `${filterBarContextPath}${filterBarContextPath.endsWith("/") ? "" : "/"}${relAnnoPath}`;
    if (pageContextPath && metaPath?.startsWith(pageContextPath) && relAnnoPath) {
      return metaPath.replace(`${pageContextPath}${pageContextPath.endsWith("/") ? "" : "/"}`, "");
    }
    return relAnnoPath;
  };
  _exports.getFilterConfigurationPath = getFilterConfigurationPath;
  ...
  const getSearchRestrictions = function (converterContext) {
    ...
    if (isEntitySet(entitySet)) {
      return entitySet.annotations.Capabilities?.SearchRestrictions;
    }

    // Handle parameterized entities - check parent entity set
    const dataModelObjectPath = converterContext.getDataModelObjectPath();
    if (dataModelObjectPath.startingEntitySet && dataModelObjectPath.startingEntitySet._type === "EntitySet") {
      return dataModelObjectPath.startingEntitySet.annotations?.Capabilities?.SearchRestrictions;
    }
    return undefined;
  };
  ...
  const getAnnotatedSelectionFieldData = function (converterContext) {
    ...
    const filterBarConfigs = converterContext.getManifestWrapper().getFilterConfiguration(annotationPath ? annotationPath : undefined);
    const filterFields = {
      ...
      ..._getSelectionFieldsByPath(entityType, filterBarConfigs.navigationProperties ?? filterBarConfigs.navigationPropertiesForPersonalization, includeHidden, converterContext)
    };
    let aSelectOptions = [];
  ...
  const processSelectionFields = function (propertyInfoFields, converterContext, defaultValuePropertyFields) {
    ...
    propertyInfoFields.forEach(function (parameterField) {
      if (parameterField.annotationPath && !parameterField.settings?.isCustomFilter) {
    ...
    let allFilterRestrictions;
    if (sEntitySetPath) {
      allFilterRestrictions = fetchFilterRestrictions.getFilterRestrictionsByDataModel(converterContext.getDataModelObjectPath());
    }
    oRet.RequiredProperties = allFilterRestrictions ? allFilterRestrictions.RequiredProperties : [];
    oRet.NonFilterableProperties = allFilterRestrictions ? allFilterRestrictions.NonFilterableProperties : [];
  ...
  const getSelectionFields = function (converterContext) {
    ...
    const existingFieldKeys = new Set(propertyInfoFields.filter(field => field.availability !== "Hidden" && field.availability !== "Adaptation").map(field => field.key));
      ...
      filterFacetMap = filterFacets.reduce((previousValue, filterFacet) => {
        for (let i = 0; i < filterFacet?.Target?.$target?.Data?.length; i++) {
          previousValue[filterFacet?.Target?.$target?.Data[i]?.Value?.path] = {
            ...
            groupLabel: filterFacet?.Label?.toString() ?? filterFacet?.Target?.$target?.Label?.toString() ?? filterFacet?.Target?.$target?.annotations?.Common?.Label?.toString() ?? filterFacet?.Target?.$target?.qualifier
          };
        }
    ...
    // Deduplicate allFilters by key before inserting manifest elements to avoid duplicate propertyInfo entries
    const seenKeys = new Set();
    const deduplicatedFilters = allFilters.filter(field => {
    ...
    const selectionFields = insertCustomManifestElements(deduplicatedFilters, entityType, converterContext, annotationPath);

    // Set group:"" and groupLabel:"" for manifest-added and annotation SelectionFields
    // This ensures all the filter fields are in the Basic group
    selectionFields.forEach(field => {
      if (existingFieldKeys.has(field.key)) {
        // This field is a SelectionField (annotation or manifest)
        field.group = "";
        field.groupLabel = "";
      }
    });
