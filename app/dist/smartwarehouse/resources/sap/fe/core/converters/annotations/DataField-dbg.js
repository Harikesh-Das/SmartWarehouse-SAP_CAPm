...
sap.ui.define(["sap/fe/base/BindingToolkit", "sap/fe/core/converters/helpers/Aggregation", "sap/fe/core/helpers/TypeGuards", "sap/fe/core/templating/DisplayModeFormatter", "sap/fe/core/templating/PropertyHelper", "../../templating/DataModelPathHelper", "../helpers/DataFieldHelper"], function (BindingToolkit, Aggregation, TypeGuards, DisplayModeFormatter, PropertyHelper, DataModelPathHelper, DataFieldHelper) {
  ...
  /**
   * Determines if the dataFieldForAnnotation points to a connectedFields annotation.
   * @param dataFieldForAnnotation The dataFieldForAnnotation being processed
   * @returns True if the dataFieldForAnnotation points to a connectedFields annotation.
   */
  _exports.hasFieldGroupTarget = hasFieldGroupTarget;
  function hasConnectedFieldsTarget(dataFieldForAnnotation) {
    const dataFieldTarget = dataFieldForAnnotation.Target.$target;
    return isAnnotationOfType(dataFieldTarget, "com.sap.vocabularies.UI.v1.ConnectedFieldsType");
  }
   ...
   * @returns True if the dataFieldForAnnotation is a field group.
   */
  _exports.hasConnectedFieldsTarget = hasConnectedFieldsTarget;
  ...
  function collectRelatedProperties(path, property, converterContext, ignoreSelf, tableType) {
    ...
    let connectedFieldsSeparator = arguments.length > 8 && arguments[8] !== undefined ? arguments[8] : undefined;
    const isPartOfaConnectedFields = !!connectedFieldsSeparator;
    if (path && property) {
      let valueIndex;
      /* EXTERNALID */
      ...
      if (isAnnotatedAsHidden) {
        ...
        _appendTemplate(relatedProperties, isPartOfaConnectedFields && relatedProperties.exportSettings?.template ? `${connectedFieldsSeparator}{${valueIndex}}` : `{${valueIndex}}`);
        return relatedProperties;
      }
      ...
      if (relatedProperties.exportSettings?.template) {
        ...
        // ConnectedFields use-case. Need to add the separator between the Fields that stay on the same line.
        _appendTemplate(relatedProperties, isPartOfaConnectedFields ? connectedFieldsSeparator : "\n");
        if (!connectedFieldsSeparator) {
      ...
      if (textProperty && textRelativePath) {
        if (textArrangementToBeConsidered) {
          ...
          const displayMode = getDisplayMode(propertyForText, propertyDataModelObjectPath.targetEntityType);
          let descriptionIndex;
          switch (displayMode) {
            case "Description":
              descriptionIndex = _setRelatedPropertiesAndGetIndex(relatedProperties, textRelativePath, textProperty, "Description");
              _appendTemplate(relatedProperties, isPartOfaConnectedFields && relatedProperties.exportSettings?.template ? `${connectedFieldsSeparator}{${descriptionIndex}}` : `{${descriptionIndex}}`);
              break;
            case "ValueDescription":
              ...
              _appendTemplate(relatedProperties, isPartOfaConnectedFields && relatedProperties.exportSettings?.template ? `${connectedFieldsSeparator}{${valueIndex}} ({${descriptionIndex}})` : `{${valueIndex}} ({${descriptionIndex}})`);
              break;
            case "DescriptionValue":
              valueIndex = _setRelatedPropertiesAndGetIndex(relatedProperties, pathForText, propertyForText);
              descriptionIndex = _setRelatedPropertiesAndGetIndex(relatedProperties, textRelativePath, textProperty, "DescriptionValue");
              _appendTemplate(relatedProperties, isPartOfaConnectedFields && relatedProperties.exportSettings?.template ? `${connectedFieldsSeparator}{${descriptionIndex}} ({${valueIndex}})` : `{${descriptionIndex}} ({${valueIndex}})`);
              break;
            default:
              valueIndex = _setRelatedPropertiesAndGetIndex(relatedProperties, pathForText, propertyForText);
              _appendTemplate(relatedProperties, isPartOfaConnectedFields && relatedProperties.exportSettings?.template ? `${connectedFieldsSeparator}{${valueIndex}}` : `{${valueIndex}}`);
        ...
        if (currencyOrUoMProperty) {
          collectCurrencyOrUnitProperties(relatedProperties, relativePath ?? path, addUnitInTemplate, propertyDataModelObjectPath, currencyOrUoMProperty, isPartOfaConnectedFields, currencyProperty);
          ...
          if (!isPartOfaConnectedFields && currencyOrUnitAnnotation) {
   ...
   */
  function collectCurrencyOrUnitProperties(relatedProperties, relativePath, addUnitInTemplate, propertyDataModelObjectPath, currencyOrUoMProperty, isPartOfaConnectedFields, currencyProperty) {
    ...
    if (property) {
      ...
      if (currencyProperty && !isPartOfaConnectedFields) {
        relatedProperties.exportSettings.isCurrency = true;
      }
      if (!isPartOfaConnectedFields) {
  ...
  function collectRelatedPropertiesRecursively(dataField, converterContext, tableType) {
    ...
    let connectedFieldsSeparator = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : undefined;
    ...
    switch (dataField?.$Type) {
      ...
      case "com.sap.vocabularies.UI.v1.DataFieldWithAction":
        if (dataField.Value) {
          ...
          relatedProperties = collectRelatedProperties(property.path, property.$target, converterContext, false, tableType, relatedProperties, isEmbedded, isStaticallyHidden, connectedFieldsSeparator);
      ...
      case "com.sap.vocabularies.UI.v1.DataFieldForAnnotation":
        switch (dataField.Target?.$target?.$Type) {
          ...
          case "com.sap.vocabularies.UI.v1.ConnectedFieldsType":
            connectedFieldsSeparator = getConnectedFieldsSeparator(dataField.Target.$target);
            getConnectedFieldsData(dataField.Target.$target).forEach(innerDataField => {
              relatedProperties = collectRelatedPropertiesRecursively(innerDataField, converterContext, tableType, relatedProperties, true, connectedFieldsSeparator);
            });
            break;
