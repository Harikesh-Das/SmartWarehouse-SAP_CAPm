...
import fetchFilterRestrictions from "../Common/filter/FilterRestrictions";
...
export const getManifestFilterFields = function (
	...
	const filterConfigurationPath = annotationPath
		? getFilterConfigurationPath(annotationPath, converterContext.getContextPath(), settingsContextPath)
		: undefined;
 ...
 */
export const getFilterConfigurationPath = function (relAnnoPath: string, filterBarContextPath: string, pageContextPath?: string): string {
...
const getAnnotatedSelectionFieldData = function (
	...
	const filterFields: Record<string, FilterField> = {
		...
		..._getSelectionFieldsByPath(
			entityType,
			filterBarConfigs.navigationProperties ?? filterBarConfigs.navigationPropertiesForPersonalization,
			includeHidden,
			converterContext
		)
	};
...
export const getSelectionFields = function (
	...
	const existingFieldKeys = new Set(
		propertyInfoFields
			.filter((field) => field.availability !== "Hidden" && field.availability !== "Adaptation")
			.map((field) => field.key)
	);
		...
		filterFacetMap = filterFacets.reduce((previousValue: Record<string, FilterGroup>, filterFacet: ReferenceFacetTypes) => {
			for (let i = 0; i < (filterFacet?.Target?.$target as FieldGroup)?.Data?.length; i++) {
				previousValue[((filterFacet?.Target?.$target as FieldGroup)?.Data[i] as DataFieldTypes)?.Value?.path] = {
					...
					groupLabel:
						filterFacet?.Label?.toString() ??
						(filterFacet?.Target?.$target as FieldGroup)?.Label?.toString() ??
						(filterFacet?.Target?.$target as FieldGroup)?.annotations?.Common?.Label?.toString() ??
						(filterFacet?.Target?.$target as FieldGroup)?.qualifier
				};
			}
