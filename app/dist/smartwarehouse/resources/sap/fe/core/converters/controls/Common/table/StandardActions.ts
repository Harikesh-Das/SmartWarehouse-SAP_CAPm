...
import {
	...
	getStaticallyResolvableExpression,
	greaterOrEqual,
	greaterThan,
	ifElse,
	isConstant,
...
function hideCreateDeleteInAnalyticalTable(
	...
	return (
		...
		standardActionsContext.tableManifestConfiguration.enableAnalyticalEdit === false
	);
}
...
export function getCreateVisibility(
	...
	return ifElse(
		and(
			...
			ifElse(
				and(standardActionsContext?.tableManifestConfiguration?.inlineCreationRowsHiddenInEditMode === false, not(isForPaste)),
				true,
				UI.IsCreateMode
			)
		),
		...
		ifElse(
			converterContext.getTemplateType() === TemplateType.ListReport,
			and(not(isCreateHidden), isInsertable),
			and(not(isCreateHidden), isInsertable, UI.IsEditable)
		)
	);
}
...
export function getDeleteVisibility(
	...
	return ifElse(
		...
		ifElse(
			converterContext.getTemplateType() !== TemplateType.ListReport,
			and(not(isDeleteHidden), pathDeletableExpression, UI.IsEditable),
			and(not(isDeleteHidden), pathDeletableExpression)
		)
	);
}
...
export function getCreateEnablement(
	...
	if (standardActionsContext.creationMode === CreationMode.InlineCreationRows) {
		...
		const inlineConditions: BindingToolkitExpression<boolean>[] = [not(standardActionsContext.hiddenAnnotation.create), UI.IsEditable];
 ...
 */
function getNoActiveGroupingCondition(
	converterContext: ConverterContext<PageContextPathTarget>,
	standardActionsContext: StandardActionsContext
): BindingToolkitExpression<boolean> {
	return converterContext.getTemplateType() !== TemplateType.ListReport &&
		standardActionsContext.tableManifestConfiguration?.type === "AnalyticalTable" &&
		standardActionsContext.tableManifestConfiguration.enableAnalyticalEdit === true
		? not(pathInModel("hasActiveGrouping", "internal"))
		: constant(true);
}
...
export function getPasteEnablement(
	...
	let isInsertable: BindingToolkitExpression<boolean> = standardActionsContext.restrictions.isInsertable.expression;
	...
	if (hasCellSelector) {
		...
		isInsertable = or(
			...
			equal(pathInModel("cellSelectorHasSelection", "internal"), true)
		);
	}
