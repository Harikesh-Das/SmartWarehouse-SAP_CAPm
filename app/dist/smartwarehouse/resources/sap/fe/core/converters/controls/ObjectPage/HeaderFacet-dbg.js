 ...
 */
sap.ui.define(["sap/fe/base/BindingToolkit", "sap/fe/core/converters/ManifestSettings", "sap/fe/core/converters/annotations/DataField", "sap/fe/core/converters/helpers/ConfigurableObject", "sap/fe/core/converters/helpers/ID", "sap/fe/core/converters/helpers/Key", "sap/fe/core/templating/DataModelPathHelper", "sap/fe/core/templating/PropertyHelper", "sap/fe/core/templating/UIFormatters", "../../../helpers/StableIdHelper", "../../helpers/DataFieldHelper", "../Common/Form"], function (BindingToolkit, ManifestSettings, DataField, ConfigurableObject, ID, Key, DataModelPathHelper, PropertyHelper, UIFormatters, StableIdHelper, DataFieldHelper, Form) {
  "use strict";
   ...
   * @param optimizedObjectPageRequesting
   * @returns Header facets from annotations
   */
  function getHeaderFacetsFromAnnotations(converterContext) {
    let optimizedObjectPageRequesting = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    const headerFacets = [];
    converterContext.getEntityType().annotations?.UI?.HeaderFacets?.forEach(facet => {
      const headerFacet = createHeaderFacet(facet, converterContext, optimizedObjectPageRequesting);
      if (headerFacet) {
   ...
   * @param disableObjectPageRequestOptimization
  ...
  function getHeaderFacetsFromManifest(manifestCustomHeaderFacets) {
    let disableObjectPageRequestOptimization = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
    ...
    Object.keys(manifestCustomHeaderFacets).forEach(manifestHeaderFacetKey => {
      ...
      customHeaderFacets[manifestHeaderFacetKey] = createCustomHeaderFacet(customHeaderFacet, manifestHeaderFacetKey, disableObjectPageRequestOptimization);
    });
  ...
  function createReferenceHeaderFacet(facetDefinition, collectionFacetDefinition, converterContext) {
    let disableObjectPageRequestOptimization = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : true;
  ...
  function createCollectionHeaderFacet(collectionFacetDefinition, converterContext) {
    ...
    if (collectionFacetDefinition.$Type === "com.sap.vocabularies.UI.v1.CollectionFacet") {
      ...
      collectionFacetDefinition.Facets.forEach(facetDefinition => {
        const facet = createReferenceHeaderFacet(facetDefinition, collectionFacetDefinition, converterContext, disableObjectPageRequestOptimization);
        if (facet) {
          facets.push(facet);
        }
      });
  ...
  function getFormElementsFromAnnotations(facetDefinition, converterContext) {
    ...
    if (facetDefinition.$Type !== "com.sap.vocabularies.UI.v1.ReferenceURLFacet" && facetDefinition.$Type !== "com.sap.vocabularies.UI.v1.CollectionFacet") {
      ...
      const navProps = getRelativePaths(converterContext.getDataModelObjectPath());
        ...
        if (dataField.annotations?.UI?.Hidden?.valueOf() !== true) {
          ...
          if ((dataField.$Type === "com.sap.vocabularies.UI.v1.DataField" || dataField.$Type === "com.sap.vocabularies.UI.v1.DataFieldWithUrl" || dataField.$Type === "com.sap.vocabularies.UI.v1.DataFieldWithNavigationPath" || dataField.$Type === "com.sap.vocabularies.UI.v1.DataFieldWithIntentBasedNavigation" || dataField.$Type === "com.sap.vocabularies.UI.v1.DataFieldWithAction") && !isReferencePropertyStaticallyHidden(dataField)) {
            annotationBasedFormElements.push({
              ...
              visible: compileExpression(isVisible(dataField, navProps)),
  ...
  function createHeaderFacet(facetDefinition, converterContext) {
    let optimizedObjectPageRequesting = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
    let headerFacet;
    switch (facetDefinition.$Type) {
      case "com.sap.vocabularies.UI.v1.ReferenceFacet":
        headerFacet = createReferenceHeaderFacet(facetDefinition, facetDefinition, converterContext, optimizedObjectPageRequesting);
        break;
      case "com.sap.vocabularies.UI.v1.CollectionFacet":
        headerFacet = createCollectionHeaderFacet(facetDefinition, converterContext, optimizedObjectPageRequesting);
        break;
      default:
        break;
    }
   ...
   */
  function createCustomHeaderFacet(customHeaderFacetDefinition, headerFacetKey, disableObjectPageRequestOptimization) {
