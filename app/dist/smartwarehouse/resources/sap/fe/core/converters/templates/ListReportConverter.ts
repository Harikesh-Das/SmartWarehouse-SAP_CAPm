...
import { addSeparators, getActionsFromManifest } from "sap/fe/core/converters/controls/Common/Action";
...
import type { CustomElementFilterField, FilterField, ManifestFilterField } from "sap/fe/core/converters/controls/ListReport/FilterBar";
import {
...
const getDefaultSemanticDates = function (
	...
	for (const filterField in filterFields) {
		if (filterFields[filterField]?.settings?.defaultValues?.length) {
			defaultSemanticDates[filterField] = (filterFields[filterField] as CustomElement<ManifestFilterField>)?.settings?.defaultValues;
		}
	}
...
const createAlpView = function (
	...
	if (primaryVisualization && secondaryVisualization) {
		const view: CombinedViewDefinition = {
			...
			defaultPath,
			viewType: ViewDefinitionType.Combined,
			visible: isViewSettingsOfType<MultiDefaultViewSettings>(viewSettings, ViewSettingsType.Multi)
				? viewSettings.visible
				: undefined,
			contentSwitcherId: getContentSwitcherId()
		};
		return view;
	}
};
...
/**
 * Creates the ListReport header actions array.
 * This applies the `primaryAction` emphasis rules for header actions.
 * It also injects an emphasized-first clone so the action is first only when emphasis is effectively active (FilterBar visibility and liveMode).
 * @param converterContext The converter context
 * @returns The header actions array
 */
export const getHeaderActions = function (converterContext: ConverterContext<PageContextPathTarget>): BaseAction[] {
	const manifestWrapper = converterContext.getManifestWrapper();
	let headerActions = insertCustomElements<BaseAction>(
		[],
		getActionsFromManifest(manifestWrapper.getHeaderActions(), converterContext).actions
	);
	// Apply emphasis based on manifest primaryAction (ListReport rules)
	ActionUtilities.applyManifestPrimaryActionEmphasis(headerActions);
	const emphasizedHeaderActions = ActionUtilities.getAllEmphasizedActions(headerActions);
	if (emphasizedHeaderActions.length > 0) {
		const emphasisActiveExpr = ActionUtilities.getListReportHeaderEmphasisActiveExpr();
		// Ensure the primary action(s) are first ONLY when emphasis is effectively active (LR FilterBar/liveMode rules)
		// Supports multiple emphasized actions with dynamic visibility (UI.Hidden) via cascading priority
		ActionUtilities.insertMultipleEmphasizedFirstClones(headerActions, emphasizedHeaderActions, emphasisActiveExpr);
	}
...
export const convertPage = function (
	...
	const pageVariantManagementID = getPageVariantManagementID();
	...
	// Collect control-level variant management IDs for tables and charts
	// Each control has its own VM with ID pattern: {controlId}::VM
	const controlVariantIds: string[] = [];
	if (variantManagement === VariantManagementType.Control) {
		// Add table variant management IDs
		lrTableVisualizations.forEach((visualization) => {
			// Table VM ID follows pattern: {tableId}::VM
			controlVariantIds.push(generate([visualization.annotation.id, "VM"]));
		});

		// Add chart variant management IDs
		lrChartVisualizations.forEach((visualization) => {
			// Chart VM ID follows pattern: {chartId}::VM
			controlVariantIds.push(generate([visualization.id, "VM"]));
		});

		// Add filterbar variant management ID if not hidden
		if (!hideFilterBar || useHiddenFilterBar) {
			controlVariantIds.push(filterVariantManagementID);
		}
	}

	const targetControlIds = [
		...(hideFilterBar && !useHiddenFilterBar ? [] : [filterBarId]),
		...(variantManagement !== VariantManagementType.Control ? visualizationIds : []),
		...(multiViewControl ? [multiViewControl.id] : []),
		...(variantManagement !== VariantManagementType.None
			? [generate([filterBarId, "LayoutToggle", "filterTypeSwitch"]), generate(["fe", "ContentSwitcher", "filterTypeSwitch"])]
			: []),
		...(!hideFilterBar && capabilities?.MagicFiltering === true ? ["fe::EasyFilterBar"] : [])
	];
	...
	return {
		...
		variantManagement: {
			...
			pageId: pageVariantManagementID,
			targetControlIds: targetControlIds.join(","),
			controlVariantIds: controlVariantIds.length > 0 ? controlVariantIds.join(",") : undefined
		},
