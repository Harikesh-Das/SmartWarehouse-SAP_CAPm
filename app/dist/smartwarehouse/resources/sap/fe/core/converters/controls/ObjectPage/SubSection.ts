...
import type { FlexSettings } from "sap/fe/core/converters/ManifestSettings";
...
import type { CustomObjectPageHeaderFacet } from "sap/fe/core/converters/controls/ObjectPage/HeaderFacet";
import {
...
import { Placement, insertCustomElements } from "../../helpers/ConfigurableObject";
...
type BaseSubSection = {
	...
	internalVisible?: CompiledBindingToolkitExpression;
 ...
 * @param converterContext The converter context
 * @param disableObjectPageRequestOptimization Flag to disable object page request optimization
 * @returns The current subsections
 */
export function createCustomHeaderFacetSubSections(
	converterContext: ConverterContext<PageContextPathTarget>,
...
function addFormMenuActions(
	...
	const hiddenActions: BaseAction[] = getFormHiddenActions(facetDefinition, converterContext) || [],
		actionOverwriteConfig: OverrideTypeAction = getActionOverrideConfig(true),
	...
	formAllActions.forEach((action) => {
		...
		if (customAction.type === ActionType.Menu && !!customAction.defaultAction) {
			(customAction.defaultAction as CustomAction).type = ActionType.Default;
		}
	});
 ...
 */
export function getFacetActions(facetDefinition: FacetTypes, converterContext: ConverterContext<PageContextPathTarget>): CombinedAction {
	let actions: BaseAction[] = [];
	switch (facetDefinition.$Type) {
		case UIAnnotationTypes.CollectionFacet:
			actions = facetDefinition.Facets.filter((subFacetDefinition) => isReferenceFacet(subFacetDefinition)).reduce(
	...
	const combinedActions = addFormMenuActions(actions, facetDefinition, converterContext);
	const sectionsInManifest = converterContext.getManifestWrapper().getSections();
	const facetOverflowGroupsConfig = sectionsInManifest?.[facetDefinition.ID as string]?.overflowGroups as OverflowGroupsConfiguration;
	combinedActions.actions = addSeparators(combinedActions.actions, facetOverflowGroupsConfig);
...
export function createSubSection(
	...
	const internalVisible = isVisibleExpression;
	...
	const subSection: BaseSubSection = {
		...
		internalVisible: compileExpression(internalVisible),
	...
	if (isHeaderSection) {
		...
		// For regular annotation-based subsections, use controlConfiguration
		const relativeAnnotationPath = converterContext.getRelativeAnnotationPath(
				...
				case UIAnnotationTypes.DataFieldForIntentBasedNavigation:
					...
					actionReducer.push({
						...
						navigationParameters: {
							semanticObject: dataField.SemanticObject as unknown as string,
							action: dataField.Action as unknown as string,
							parameters: mNavigationParameters
						}
					} as AnnotationAction);
					break;
			...
			fn(
				...
				[
					dataField.Action,
					{
						...
						disableStrictHandling: formManifestActionsConfiguration?.[key]?.disableStrictHandling
					}
				],
				ref(".editFlow")
			)
...
export function createCustomSubSection(
	...
	let actions = Object.values(manifestActions.actions) as BaseAction[];
	actions = removeDuplicateActions(actions);
	actions = addSeparators(actions, manifestSubSection.overflowGroups);
	manifestActions.actions = actions?.reduce(
	...
	const subSectionDefinition = {
		...
		subSectionCreated: manifestSubSection.subSectionCreated,
		...
		flexSettings: manifestSubSection.flexSettings
			? {
					designtime: manifestSubSection.flexSettings.designtime ?? FlexDesignTimeType.Default
			  }
	...
	if (manifestSubSection.template || manifestSubSection.name) {
		...
		// Support for section-level isPartOfPreview
		if (manifestSubSection.isPartOfPreview === false) {
			(subSectionDefinition as unknown as XMLFragmentSubSection).isPartOfPreview = false;
		}
