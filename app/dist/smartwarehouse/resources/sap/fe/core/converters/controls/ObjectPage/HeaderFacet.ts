...
import { FlexDesignTimeType, type FlexSettings } from "sap/fe/core/converters/ManifestSettings";
 ...
 */
export function getHeaderFacetsFromAnnotations(
	converterContext: ConverterContext<PageContextPathTarget>,
	optimizedObjectPageRequesting = false
): ObjectPageHeaderFacet[] {
	const headerFacets: ObjectPageHeaderFacet[] = [];
	converterContext.getEntityType().annotations?.UI?.HeaderFacets?.forEach((facet) => {
		const headerFacet: ObjectPageHeaderFacet | undefined = createHeaderFacet(facet, converterContext, optimizedObjectPageRequesting);
		if (headerFacet) {
...
export function getHeaderFacetsFromManifest(
	...
	Object.keys(manifestCustomHeaderFacets).forEach((manifestHeaderFacetKey) => {
		...
		customHeaderFacets[manifestHeaderFacetKey] = createCustomHeaderFacet(
			customHeaderFacet,
			manifestHeaderFacetKey,
 ...
 */
function createCustomHeaderFacet(
	customHeaderFacetDefinition: ManifestHeaderFacet,
	headerFacetKey: string,
	disableObjectPageRequestOptimization: boolean
): CustomObjectPageHeaderFacet {
