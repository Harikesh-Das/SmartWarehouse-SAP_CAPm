...
import { getConnectedFieldsData, getConnectedFieldsSeparator, isReferencePropertyStaticallyHidden } from "../helpers/DataFieldHelper";
 ...
 */
export function hasConnectedFieldsTarget(dataFieldForAnnotation: DataFieldForAnnotation | ReferenceFacet): boolean {
	const dataFieldTarget = dataFieldForAnnotation.Target.$target;
	return isAnnotationOfType<ConnectedFields>(dataFieldTarget, UIAnnotationTypes.ConnectedFieldsType);
}
...
export function collectRelatedProperties(
	...
	connectedFieldsSeparator: string | undefined = undefined
): ComplexPropertyInfo {
	...
	if (path && property) {
		...
		if (isAnnotatedAsHidden) {
			...
			_appendTemplate(
				relatedProperties,
				isPartOfaConnectedFields && relatedProperties.exportSettings?.template
					? `${connectedFieldsSeparator}{${valueIndex}}`
					: `{${valueIndex}}`
			);
			return relatedProperties;
		}
				...
				switch (displayMode) {
					case "Description":
						...
						_appendTemplate(
							relatedProperties,
							isPartOfaConnectedFields && relatedProperties.exportSettings?.template
								? `${connectedFieldsSeparator}{${descriptionIndex}}`
								: `{${descriptionIndex}}`
						);
						break;
					...
					case "ValueDescription":
						...
						_appendTemplate(
							relatedProperties,
							isPartOfaConnectedFields && relatedProperties.exportSettings?.template
								? `${connectedFieldsSeparator}{${valueIndex}} ({${descriptionIndex}})`
								: `{${valueIndex}} ({${descriptionIndex}})`
						);
						break;
					...
					case "DescriptionValue":
						...
						_appendTemplate(
							relatedProperties,
							isPartOfaConnectedFields && relatedProperties.exportSettings?.template
								? `${connectedFieldsSeparator}{${descriptionIndex}} ({${valueIndex}})`
								: `{${descriptionIndex}} ({${valueIndex}})`
						);
						break;
					default:
						valueIndex = _setRelatedPropertiesAndGetIndex(relatedProperties, pathForText, propertyForText);
...
function collectCurrencyOrUnitProperties(
	...
	isPartOfaConnectedFields: boolean,
		...
		case UIAnnotationTypes.DataFieldForAnnotation:
			switch (dataField.Target?.$target?.$Type) {
				...
				case UIAnnotationTypes.ConnectedFieldsType:
					...
					getConnectedFieldsData(dataField.Target.$target).forEach((innerDataField: DataFieldAbstractTypes) => {
						relatedProperties = collectRelatedPropertiesRecursively(
							innerDataField,
							converterContext,
							tableType,
