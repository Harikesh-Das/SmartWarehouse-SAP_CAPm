...
import type { MetaModelEntityType, MetaModelEnum, MetaModelNavProperty, MetaModelProperty, MetaModelType } from "types/metamodel_types";
...
async function updateRelateAppsModel(
	...
	try {
		...
		aManifestSOItems.splice(
			0,
			aManifestSOItems.length,
			...aManifestSOItems.filter((item) => !(item.targetSemObject === sCurrentSemObj && item.targetAction === sCurrentAction))
		);
		if (aLinks) {
			if (aLinks.length > 0) {
				...
				if (
					...
					(!!oManifestData.additionalSemanticObjects[aAnnotationsSOItems[0].targetSemObject].allowedActions ||
						!!oManifestData.additionalSemanticObjects[aAnnotationsSOItems[0].targetSemObject].unavailableActions)
				) {
					isSemanticObjectHasSameTargetInManifest = true;
				}
...
function getTargetView(oControl: ManagedObject | null): FEView {
	...
	if (oControl) {
		const templateComponent =
			oControl && oControl.isA<TemplateComponent>("sap.fe.core.TemplateComponent")
				? oControl
				: Component.getOwnerComponentFor(oControl);
		if (templateComponent && templateComponent.isA<TemplateComponent>("sap.fe.core.TemplateComponent")) {
			return templateComponent.getRootControl() as unknown as FEView;
		}
	}
...
async function internalRequestProperty(sAction: string, oSelectedContext: ODataV4Context, sProperty: string): Promise<unknown> {
...
async function requestProperty(
	...
	const oPromise =
		sProperty && sProperty.indexOf("/") === 0
			...
			: internalRequestProperty(sAction, oSelectedContext, sProperty);
...
async function templateControlFragment(
	...
	if (oModifier && typeof sFragmentName === "string") {
		...
		const fragmentData =
			typeof sFragmentName === "string" ? await XMLTemplateProcessor.loadTemplatePromise(sFragmentName, "fragment") : sFragmentName;
...
async function createRootContext(
	...
	const result = findOrCreateRootContext(
		(view.getBindingContext() as ODataV4Context) ?? context,
		programmingModel,
		view,
		appComponent,
		bindingParameters,
