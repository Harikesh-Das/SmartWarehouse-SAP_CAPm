...
sap.ui.define(["sap/base/Log", "sap/base/util/deepEqual", "sap/fe/base/ClassSupport", "sap/fe/core/helpers/ToES6Promise", "sap/fe/navigation/library", "sap/ui/base/Object", "./controllerextensions/BusyLocker", "./helpers/ModelHelper"], function (Log, deepEqual, ClassSupport, toES6Promise, library, BaseObject, BusyLocker, ModelHelper) {
  ...
  let AppStateHandler = (_dec = defineUI5Class("sap.fe.core.AppStateHandler"), _dec(_class = (_AppStateHandler = /*#__PURE__*/function (_BaseObject) {
    ...
    /**
     * Resets the cached app state.
     * Used before creating a partial app state (e.g. LR-only) during FCL Create navigation,
     * to prevent stale ObjectPage state from being merged into the new iAppState.
     */;
    _proto.resetCachedAppState = function resetCachedAppState() {
      this._mCurrentAppState = {};
    }

    /**
     * Returns and clears the targetControlId stored from the xAppState during the startup process of cross-app navigation.
     * Called by the object page to retrieve the section to scroll to after cross-app navigation.
     * @returns The targetControlId or undefined if none is pending
     */;
    _proto.consumePendingTargetControlId = function consumePendingTargetControlId() {
      const value = this.pendingTargetControlIdFromXAppState;
      this.pendingTargetControlIdFromXAppState = undefined;
      return value;
    }
    ...
    _proto._addAppStateInHash = function _addAppStateInHash(appStateKey, stateIdentifier) {
      if (this.oAppComponent.isUsedAsBuildingBlockContext()) {
        return;
      }
      const appComponent = this.oAppComponent,
        ...
        isHiddenDraftEnabled = this.oAppComponent.getEnvironmentCapabilities().getCapabilities().HiddenDraft?.enabled || false,
      ...
      if (newHash && this.simultaneousCreateRequest[stateIdentifier] === 0 && newHash !== hash) {
        routerProxy.navToHash(newHash, undefined, undefined, undefined, isStickyMode || isHiddenDraftEnabled);
    ...
    _proto._createNavigationParameters = function _createNavigationParameters(oAppData, sNavType) {
      return Object.assign({}, oAppData, {
        ...
        navSelVarHasDefaultsOnly: oAppData.bNavSelVarHasDefaultsOnly
      });
    }
     ...
     * @param view The view for which the app state must be applied. If no view is provided, then app state of all the views is applied.
     * @returns A promise for async handling
     */;
    _proto.applyAppState = async function applyAppState(view) {
      ...
      // lock UI model
      BusyLocker.lock(this.oAppComponent.getModel("ui"));
      ...
      return toES6Promise(oNavigationService.parseNavigation()).catch(function (aErrorData) {
        ...
        const applyViewState = await oRootController.viewState.applyViewState(viewId ? appStateData : this._mCurrentAppState, this._createNavigationParameters(oAppData, sNavType), skipMerge);
        // Extract section state from xAppState appState (cross-app navigation).
        // Uses convention key "fe::ObjectPage" since source app doesn't know target view ID.
        // It persists across LR->OP page transitions and is consumed once via consumePendingTargetControlId().
        const opState = oAppData?.appState?.["fe::ObjectPage"];
        if (opState?.selectedSection || opState?.selectedSubSection) {
          // Prefer subsection (more specific), fallback to section
          this.pendingTargetControlIdFromXAppState = opState.selectedSubSection ?? opState.selectedSection;
        }
        if (!shouldApplyState) {
          return {};
        } else {
        ...
        // unlock UI model
        BusyLocker.unlock(this.oAppComponent.getModel("ui"));
      });
    }
