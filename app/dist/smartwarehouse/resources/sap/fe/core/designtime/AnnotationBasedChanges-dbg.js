 ...
 */
sap.ui.define(["sap/base/Log", "sap/fe/core/converters/MetaModelConverter", "sap/fe/core/helpers/MetaPath", "sap/ui/core/Component", "sap/ui/core/Lib", "sap/fe/core/CommonUtils", "sap/fe/core/templating/PropertyHelper", "sap/ui/rta/plugin/annotations/AnnotationTypes"], function (Log, MetaModelConverter, MetaPath, Component, Lib, CommonUtils, PropertyHelper, AnnotationTypes) {
  "use strict";
  ...
  function _collectProperties(entityType, properties, filterFn, visitedEntity) {
    ...
    properties.push(...entityType.entityProperties.filter(filterFn).filter(prop => {
      ...
      // Always filter out UI.AdaptationHidden = true
      // Always filter out complex type
      if (prop.annotations.UI?.Hidden?.valueOf() === true || prop.annotations.UI?.AdaptationHidden?.valueOf() === true || !!prop.targetType) {
        return false;
      }
      return true;
    }).map(prop => {
  ...
  function collectProperties(templateComponent, control, filterFn) {
    ...
    if (control.getBindingContext()) {
      const contextMetaPath = metaModel?.getMetaPath(control.getBindingContext()?.getPath() ?? "");
      if (contextMetaPath) {
    ...
    if (metaModel && contextPathToUse) {
      ...
      const visitedEntity = {};
      const segments = getTargetPropertyFromFormElement(control)?.split("/");
      if (Array.isArray(segments) && segments.length >= 2) {
        const navSegment = segments[0];
        // The FormElement contains a navigation property field — collect from the target entity type
        const navEntityType = targetEntity?.getClosestEntityType()?.navigationProperties?.find(nav => nav?.name === navSegment)?.targetType;
        if (navEntityType) {
          _collectProperties(navEntityType, properties, filterFn, visitedEntity);
        }
      } else {
      ...
      delegate: {
        getAnnotationsChangeInfo: (oControl, _annotation) => {
          ...
          if (templateComponent) {
            ...
            const targetPropertyName = targetPropertyPath?.includes("/") === true ? targetPropertyPath.split("/").pop() : targetPropertyPath;
            if (targetPropertyName) {
              targetProperty = properties?.find(prop => {
                return prop.property.name === targetPropertyName;
              })?.property;
            }
  ...
  function getTextArrangementChangeAction() {
    ...
    return {
      ...
      title: oControl => {
        // Return undefined when no applicable properties exist — the RTA AnnotationPlugin
        // skips menu items whose title is falsy, preventing the handler from being called
        // with a null result from getAnnotationsChangeInfo (which would cause a crash).
        let templateComponent = Component.getOwnerComponentFor(oControl);
        if (!templateComponent) {
          const view = CommonUtils.getTargetView(oControl);
          templateComponent = view?.getController().getOwnerComponent();
