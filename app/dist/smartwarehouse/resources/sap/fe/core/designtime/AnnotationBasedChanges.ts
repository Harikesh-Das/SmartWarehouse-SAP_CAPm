	...
	properties.push(
		...entityType.entityProperties
			...
			.filter((prop) => {
				...
				if (
					prop.annotations.UI?.Hidden?.valueOf() === true ||
					prop.annotations.UI?.AdaptationHidden?.valueOf() === true ||
					!!prop.targetType
				) {
					return false;
				}
...
function collectProperties(
	...
	if (metaModel && contextPathToUse) {
		...
		if (Array.isArray(segments) && segments.length >= 2) {
			...
			const navEntityType = targetEntity?.getClosestEntityType()?.navigationProperties?.find((nav) => nav?.name === navSegment)
				?.targetType;
			if (navEntityType) {
			...
			getAnnotationsChangeInfo: (oControl: ManagedObject, _annotation: string): AnnotationChangeReturnType | null => {
				...
				if (templateComponent) {
					...
					const targetPropertyName =
						targetPropertyPath?.includes("/") === true ? targetPropertyPath.split("/").pop() : targetPropertyPath;
...
export function getTextArrangementChangeAction(key = "RTA_CONTEXT_ACTIONMENU_TEXTARRANGE"): {
	changeType: string;
	title: (oControl: ManagedObject) => string | undefined;
	type: string;
	delegate: { getAnnotationsChangeInfo: (oControl: ManagedObject, _annotation: string) => AnnotationChangeReturnType | null };
} {
	return {
