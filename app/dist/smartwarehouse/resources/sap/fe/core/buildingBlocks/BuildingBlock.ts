...
class BuildingBlock<T extends UI5Element = Control, K extends object = {}> extends BuildingBlockBase<T, K> {
	...
	protected _applyingSettings?: boolean;

	protected _visibilityCheck = false;

	private static _bbobserverMap: Map<string, ObserveBuildingBlock[]> = new Map();

	private _registeredHooks = false;

	/**
	 * Flag to indicate if the metadata check should be delayed until the modelContextChange event is fired.
	 * This is useful to avoid creating too much performance overhead when the building block is created
	 */
	protected _doNotCreateOnNullContext?: boolean;
	...
	_getOwner(failOnNullContext = false): TemplateComponent | undefined {
		if (this._visibilityCheck && !this.getVisible()) {
			return undefined;
		}
		...
		if (owner?.isA<TemplateComponent>("sap.fe.core.TemplateComponent")) {
			if (failOnNullContext && owner.getAppComponent().delayMetadataCheck && !this.getBindingContext()) {
				return undefined;
			}
			return owner;
		}
	...
	_checkIfMetadataReady(): void {
		const owner = this._getOwner(this._doNotCreateOnNullContext);
		if (!this._registeredHooks && this.getPageController() !== undefined) {
			if (
	...
	/**
	 * Checks whether the telemetry service is enabled for this session.
	 * @returns A promise that resolves to true if telemetry is enabled, false otherwise.
	 */
	async isTelemetryEnabled(): Promise<boolean> {
		return this.getAppComponent()?.getTelemetryService()?.isEnabled() ?? Promise.resolve(false);
	}
	 ...
	 * @param textID The text ID to check
	 * @returns True if the resource key exists, false otherwise
	 */
	checkIfResourceKeyExists(textID: string): boolean {
		return !!((this.getModel("sap.fe.i18n") ?? this._getOwner()?.getModel("sap.fe.i18n")) as ResourceModel)?.checkIfResourceKeyExists(
			textID
		);
	}
	...
	getInternalModel(): JSONModel | undefined {
		return (this.getModel("internal") ?? this._getOwner()?.getModel("internal")) as JSONModel | undefined;
	}
}
