...
sap.ui.define([
...
], function (library, Control, IconPool, Device, encodeXML, Localization) {
	"use strict";
	...
	SvgBase.prototype._renderText = function (mAttributes) {
		...
		if (!oRm) {
			...
			this._renderControl("text", mAttributes.attributes, false,oRm);
			if (!!mAttributes.text) {
				oRm.text(mAttributes.text);
			}
			if (mAttributes.close !== false) {
				oRm.close("text");
			}
	...
	SvgBase.prototype._renderSpanText = function (mAttributes, sText, iHeight) {
		...
		if (!oRm) {
			var sHtml = this._renderControl("text", mAttributes, false);
			sHtml += this._renderControl("tspan", {}, false);
			sHtml += encodeXML(sText);
			sHtml += "</tspan></text>";
			return sHtml;
		} else {
			this._renderControl("text",mAttributes,false,oRm);
			this._renderControl("tspan",{},false,oRm);
			oRm.text(sText);
			oRm.close("tspan");
			oRm.close("text");
		}
	...
	SvgBase.prototype._renderControl = function (sName, mAttributes, bClose,oRm) {
		if (!oRm) {
			var sHtml = "<" + sName + " ";
			mAttributes = mAttributes || {};
			Object.keys(mAttributes).forEach(function (sAttribute) {
			...
			//If oRm is passed in attributes remove the reference from the object
			delete mAttributes["renderManager"];
			//If element is a void element in that case use void start
			var aVoidTags = ["img","br","input","area","hr","col","link"];
			var bVoidTag = (aVoidTags.indexOf(sName) !== -1);
			oRm = (bVoidTag ? oRm.voidStart(sName) : oRm.openStart(sName));
			if (mAttributes) {
				Object.keys(mAttributes).forEach(function (sAttribute) {
					...
					if (typeof sValue !== "undefined" && sValue !== "") {
						switch (sAttribute) {
							case "id" :
								...
								break;
							case "class" :
								//Split the value on space and append to ORM
			...
			oRm = (bVoidTag ? oRm.voidEnd() : oRm.openEnd());
			bClose = (bClose !== false);
			if (bClose && !bVoidTag) {
				oRm.close(sName);
			}
	...
	SvgBase.prototype.getStyleObject = function(sStyle) {
		var result = {};
		if (sStyle) {
	...
	SvgBase.prototype.applyStyles = function(oRm, oStyle) {
	...
	SvgBase.prototype._renderHtmlIcon = function (sIcon, sClass, sId, sAttr, sTitle, oRm) {
		...
		if (!oRm) {
			...
			if (oIconInfo) {
				...
				this.applyStyles(oRm, this.getStyleObject(sAttr));
				oRm.openEnd();
