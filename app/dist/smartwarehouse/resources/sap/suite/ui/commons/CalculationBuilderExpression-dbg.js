...
sap.ui.define([
	...
	CalculationBuilderExpression.prototype.init = function () {
		...
		//Map of Operator Tooltips
		this._operatorTooltip = Object.freeze({
	...
	CalculationBuilderExpression.prototype._createPopoverLayout = function (oPopoverItems) {
		var fnCreateButton = function (sLabel) {
			return new Button({
				text: sLabel,
				tooltip: this._operatorTooltip[sLabel] ? oResourceBundle.getText(this._operatorTooltip[sLabel]) : this._operatorTooltip[sLabel],
			...
			Object.keys(oOperators).forEach(function (sKey) {
				...
				if (oParent._isTokenAllowed(sKey)) {
					...
					var oButton = new Button({
						text: sText,
						tooltip: this._operatorTooltip[sText] ? oResourceBundle.getText(this._operatorTooltip[sText]) : sText,
						press: this._updateOrCreateItem.bind(this, {
							type: sType,
							key: sItemKey
						})
